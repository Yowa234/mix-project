...
function censusMonth(period) {
    return `${period.slice(0, 4)}-${period.slice(4, 6)}`;
}
function validateQuery(query, credentialBaseUrl) {
...
function requestUrl(query, apiKey) {
    ...
    url.searchParams.set("time", `from ${censusMonth(sortedPeriods[0])} to ${censusMonth(sortedPeriods.at(-1))}`);
...
function parseResponse(value, query) {
    ...
    for (const rawRow of value.slice(1)) {
        ...
        const cell = (field) => stringCell(rawRow, indexes.get(field));
        ...
        if (amount === 0)
            statusFlags.push("VALUE_ZERO_REPORTED");
        observations.push({
            reporterCountry: "USA",
...
async function fetchTrade(query, apiKey, baseUrl, fetcher) {
    ...
    if (!response.ok)
        throw providerHttpStatusError(response, "美国 Census");
