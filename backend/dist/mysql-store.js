...
import { z } from "zod";
import { isProspectRunBridgeJob, recoverInterruptedAgentJobs } from "./agent-jobs.js";
import { validateAgentJobSecurity } from "./agent-job-security.js";
import { hashPassword, verifyPassword } from "./auth.js";
import { canonicalJsonStringify } from "./canonical-json.js";
import { normalizeProspectCampaignSnapshot, prospectCampaignSnapshotHash } from "./prospect-campaigns.js";
import { ensureOrganizationIdentityTeamCache, ensureOrganizationIdentitySchema, loadOrganizationIdentityState, resolveOrganizationStrongIdentityMysql } from "./organization-strong-identity-mysql.js";
import { ensureOrganizationIdentityConflictReviewSchema, ensureOrganizationIdentityConflictReviewTeamCache, loadOrganizationIdentityConflictReviewState, reviewOrganizationIdentityConflictMysql } from "./organization-identity-conflict-review-mysql.js";
import { ensureOrganizationRelationSchema, ensureOrganizationRelationTeamCache, loadOrganizationRelationState, recordOrganizationAliasMysql, recordOrganizationRelationMysql } from "./organization-relations-mysql.js";
import { ensureProspectCoverageSchema, ensureProspectCoverageTeamCache, convertProspectToCustomerMysql, convertProspectToLeadMysql, loadProspectCoverageState, recordProspectCoverageMysql, setTenantProspectDispositionMysql } from "./prospect-coverage-memory-mysql.js";
import { applyProspectQualificationCommandMysql, ensureProspectQualificationSchema, ensureProspectQualificationTeamCache, loadProspectQualificationState } from "./prospect-qualification-mysql.js";
import { normalizeProspectStrategyProviderPlan, normalizeProspectStrategyQuery, prospectStrategyFingerprint, resolveProspectStrategyQuery } from "./prospect-strategies.js";
import { prospectRunExecutionSnapshotHash } from "./prospect-runs.js";
import { prospectStrategySourcePositionIdentityHash } from "./prospect-strategy-source-position.js";
import { ensureProspectVerificationReport } from "./prospect-verification.js";
import { PROSPECT_PROVIDER_REQUEST_LEDGER_CONTRACT, hasValidProspectProviderDispatchStatusFacts, isProspectProviderDispatchTransitionAllowed, isProspectProviderRequestTransitionAllowed, prospectProviderAccountingEvidenceHash, prospectProviderAccountingEvidenceRef, prospectProviderDispatchConfirmationRef, prospectProviderResponseEvidenceRef, prospectProviderResponseHash, prospectProviderSettlementHash, sha256CanonicalJson } from "./prospect-provider-request-ledger.js";
...
import { createCredentialRef, decryptAiModelApiKey, encryptAiModelApiKey, encryptProviderConfiguration, isEncryptedAiModelApiKey, validateProviderCredentialSecurity } from "./credential-security.js";
import { agentJobIdempotencyAliases, agentJobs, aiModelConfigs, aiSiteBuilderProjects, aiSiteBuilderSettings, caseStudies, commissionCalculations, commissionExports, commissionItems, commissionProducts, commissionRules, competitors, customerActivities, customerIntelligenceSuggestions, customers, dealEvents, deals, examAttempts, examQuestionLinks, examQuestions, exams, importExportJobs, knowledgeAssets, leadActivities, leadSourceConfigs, leadSourceEvents, leads, marketOpportunityBatches, marketOpportunityCalculationEvents, marketOpportunitySnapshots, marketTradeObservations, memos, monthlySalesRecords, ocrJobs, planTasks, planTemplates, problems, prospectCampaignEvents, prospectCampaigns, prospectCampaignVersions, prospectRunQueueChildBindings, prospectRunQueueParentBindings, providerCatalog as defaultProviderCatalog, providerConnections, providerRequestLogs, providerResponseCache, reminders, salesRecordAudits, todos, tradeDocuments, users, wecomMessages, websiteOpportunities, whatsappBindings, whatsappMessages } from "./data.js";
const defaultUrl = "mysql://goodjob:change_me@127.0.0.1:3306/goodjob_crm";
async function acquireProductionInstanceLock(pool, processRole) {
    const connection = await pool.getConnection();
    const lockName = `goodjob_crm_backend_${processRole}_single_instance`;
    const [rows] = await connection.query("SELECT GET_LOCK(?, 0) AS acquired, CONNECTION_ID() AS connection_id", [lockName]);
    const result = rows[0];
    if (Number(result?.acquired || 0) !== 1) {
        connection.release();
        throw new Error(`同一数据库已有 GoodJob CRM ${processRole} 实例运行，拒绝重复启动`);
    }
    const ownerConnectionId = Number(result.connection_id);
    const timer = setInterval(async () => {
        try {
            const [checkRows] = await connection.query("SELECT IS_USED_LOCK(?) AS owner_id, CONNECTION_ID() AS connection_id", [lockName]);
...
export function createSerializedPersistence(snapshot, operation) {
...
function persistenceSnapshot(store) {
    const snapshot = Object.fromEntries(Object.entries(store).map(([key, value]) => [
        key,
...
function replaceProspectExecutionCache(store, loaded) {
    ...
    store.prospectSearchRuns.splice(0, store.prospectSearchRuns.length, ...loaded.runState.runs);
    store.prospectRunShards.splice(0, store.prospectRunShards.length, ...loaded.runState.shards);
    store.prospectRunEvents.splice(0, store.prospectRunEvents.length, ...loaded.runState.events);
    store.agentJobs.splice(0, store.agentJobs.length, ...loaded.jobs);
    store.agentJobIdempotencyAliases.splice(0, store.agentJobIdempotencyAliases.length, ...loaded.jobAliases);
    store.prospectRunQueueParentBindings.splice(0, store.prospectRunQueueParentBindings.length, ...loaded.runState.parentBindings);
    store.prospectRunQueueChildBindings.splice(0, store.prospectRunQueueChildBindings.length, ...loaded.runState.childBindings);
    store.prospectExecutionKernelStates.splice(0, store.prospectExecutionKernelStates.length, ...loaded.executionState.kernelStates);
    store.prospectExecutionCheckpoints.splice(0, store.prospectExecutionCheckpoints.length, ...loaded.executionState.checkpoints);
    store.prospectStrategySourcePositions.splice(0, store.prospectStrategySourcePositions.length, ...loaded.executionState.sourcePositions);
    store.prospectExecutionLeases.splice(0, store.prospectExecutionLeases.length, ...loaded.executionState.leases);
    store.prospectExecutionAttempts.splice(0, store.prospectExecutionAttempts.length, ...loaded.executionState.attempts);
    store.prospectProviderRequestLedgers.splice(0, store.prospectProviderRequestLedgers.length, ...loaded.executionState.providerRequestLedgers);
    store.prospectProviderRequestDispatches.splice(0, store.prospectProviderRequestDispatches.length, ...loaded.executionState.providerRequestDispatches);
    store.prospectProviderRequestAttemptBindings.splice(0, store.prospectProviderRequestAttemptBindings.length, ...loaded.executionState.providerRequestAttemptBindings);
    store.prospectProviderRequestEvents.splice(0, store.prospectProviderRequestEvents.length, ...loaded.executionState.providerRequestEvents);
    store.prospectProviderRequestAccountingEvidence.splice(0, store.prospectProviderRequestAccountingEvidence.length, ...loaded.executionState.providerRequestAccountingEvidence);
    store.prospectSourceRawBatches.splice(0, store.prospectSourceRawBatches.length, ...loaded.executionState.sourceRawBatches);
    store.prospectSourceRawRecords.splice(0, store.prospectSourceRawRecords.length, ...loaded.executionState.sourceRawRecords);
    store.prospectSourceRawHits.splice(0, store.prospectSourceRawHits.length, ...loaded.executionState.sourceRawHits);
    store.prospectExecutionPages.splice(0, store.prospectExecutionPages.length, ...loaded.executionState.pages);
    store.prospectExecutionEvents.splice(0, store.prospectExecutionEvents.length, ...loaded.executionState.events);
    store.prospectExecutionThrottleBuckets.splice(0, store.prospectExecutionThrottleBuckets.length, ...loaded.executionState.throttleBuckets);
}
async function persistProspectExecutionState(connection, store) {
    validateProspectRunPersistence(store);
    await syncProspectSearchRunRows(connection, store.prospectSearchRuns);
    ...
    await syncAgentJobRows(connection, store.agentJobs, isProspectRunBridgeJob);
    await syncAgentJobIdempotencyAliasRows(connection, store.agentJobIdempotencyAliases, isProspectRunBridgeJob);
    await appendProspectRunQueueBindingRows(connection, store);
    await syncProspectExecutionKernelState(connection, store.prospectExecutionKernelStates);
    await syncProspectExecutionCheckpoints(connection, store.prospectExecutionCheckpoints);
    await syncProspectExecutionLeases(connection, store.prospectExecutionLeases);
    await syncProspectExecutionAttempts(connection, store.prospectExecutionAttempts);
    await syncProspectProviderRequestLedgers(connection, store.prospectProviderRequestLedgers);
    await syncProspectProviderRequestDispatches(connection, store.prospectProviderRequestDispatches);
    ...
    await syncProspectStrategySourcePositions(connection, store.prospectStrategySourcePositions);
    await syncProspectExecutionThrottles(connection, store.prospectExecutionThrottleBuckets);
...
export async function createMysqlStore(options = {}) {
    ...
    const prospectExecutionLockName = `goodjob_prospect_exec_${createHash("sha256").update(databaseUrl).digest("hex").slice(0, 24)}`;
    const prospectCandidateLockName = `goodjob_prospect_candidate_${createHash("sha256").update(databaseUrl).digest("hex").slice(0, 24)}`;
    const configuredProspectExecutionLockTimeoutMs = Number(process.env.PROSPECT_EXECUTION_DB_LOCK_TIMEOUT_MS || 5_000);
    const prospectExecutionLockTimeoutSeconds = Math.max(1, Math.ceil((Number.isFinite(configuredProspectExecutionLockTimeoutMs)
        && configuredProspectExecutionLockTimeoutMs > 0
        ? configuredProspectExecutionLockTimeoutMs
        : 5_000) / 1_000));
    const configuredProspectCandidateLockTimeoutMs = Number(process.env.PROSPECT_CANDIDATE_DB_LOCK_TIMEOUT_MS || 5_000);
    const prospectCandidateLockTimeoutSeconds = Math.max(1, Math.ceil((Number.isFinite(configuredProspectCandidateLockTimeoutMs)
        && configuredProspectCandidateLockTimeoutMs > 0
        ? configuredProspectCandidateLockTimeoutMs
        : 5_000) / 1_000));
    const releaseProductionInstanceLock = process.env.NODE_ENV === "production"
    ...
    const persist = () => enqueuePersistence(() => persistAll(pool, persistenceSnapshot(store)));
    const persistMutation = (mutation) => enqueuePersistence(async () => {
    ...
    const mutateCustomerOwnership = (input) => enqueuePersistence(async () => {
        const connection = await pool.getConnection();
        let committed = false;
        try {
            await connection.beginTransaction();
            const [customerRows] = await connection.query("SELECT * FROM customers WHERE id = ? FOR UPDATE", [input.customerId]);
            const customerRow = customerRows[0];
            if (!customerRow) {
                throw new CustomerOwnershipError("CUSTOMER_NOT_FOUND", "客户不存在", 404);
            }
            ...
            if (typeof input.expectedVersion === "number"
                ...
                throw new CustomerOwnershipError("CUSTOMER_POOL_VERSION_CONFLICT", "客户归属状态已变化，请刷新后重试", 409);
            }
            ...
            let cancelledTodoIds = [];
            if (input.action === "release") {
                if (customer.poolStatus === "public") {
                    throw new CustomerOwnershipError("CUSTOMER_POOL_ALREADY_PUBLIC", "该客户已在团队公池中", 409);
                }
                const canRelease = input.actorRole === "sales"
                    ...
                    : (input.actorRole === "manager" || input.actorRole === "admin")
                        && customer.teamId === input.actorTeamId;
                if (!canRelease) {
                    throw new CustomerOwnershipError("CUSTOMER_POOL_FORBIDDEN", "只能释放本人或本团队负责的客户", 403);
                }
                if (!reason) {
                    throw new CustomerOwnershipError("CUSTOMER_POOL_FORBIDDEN", "请填写释放原因", 400);
                }
                const [activeDealRows] = await connection.query(`SELECT COUNT(*) AS count
           FROM deals
           WHERE customer_id = ?
             AND archived_at IS NULL
             AND stage NOT IN ('成交', '丢单')`, [customer.id]);
                if (Number(activeDealRows[0]?.count || 0) > 0) {
                    throw new CustomerOwnershipError("CUSTOMER_POOL_ACTIVE_DEAL", "该客户仍有活跃商机，请先完成、关闭或移交商机后再释放", 409);
                }
                const [todoRows] = await connection.query(`SELECT id
           FROM todos
           WHERE customer_id = ?
             AND done = FALSE
             AND cancelled_at IS NULL
           FOR UPDATE`, [customer.id]);
                cancelledTodoIds = todoRows
                    .map((row) => row.id);
                const [updateResult] = await connection.query(`UPDATE customers
           SET owner_id = '',
               pool_status = 'public',
               previous_owner_id = ?,
               released_by = ?,
           ...
           WHERE id = ?
             ...
             AND ownership_version = ?`, [
                ...
                if (cancelledTodoIds.length) {
                    await connection.query(`UPDATE todos
             SET done = TRUE,
                 status = 'pending',
                 history_at = ?,
                 cancelled_at = ?,
             ...
             WHERE customer_id = ?
               ...
               AND cancelled_at IS NULL`, [
                ...
                if (customer.teamId !== input.actorTeamId
                    ...
                    throw new CustomerOwnershipError("CUSTOMER_POOL_FORBIDDEN", "不能领取其他团队的公池客户", 403);
                }
                if (customer.poolStatus !== "public") {
                    throw new CustomerOwnershipError("CUSTOMER_POOL_ALREADY_CLAIMED", "该客户已被其他同事领取", 409);
                }
                const [updateResult] = await connection.query(`UPDATE customers
           SET owner_id = ?,
               pool_status = 'owned',
           ...
           WHERE id = ?
            ...
            await connection.query(`INSERT INTO customer_ownership_events
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            ...
            const [updatedRows] = await connection.query("SELECT * FROM customers WHERE id = ?", [customer.id]);
            const updatedCustomer = customerFromRow(updatedRows[0]);
            await connection.commit();
            committed = true;
            const customerIndex = store.customers.findIndex((item) => item.id === updatedCustomer.id);
            if (customerIndex >= 0)
            ...
            if (cancelledTodoIds.length) {
                ...
                for (const todo of store.todos) {
                    if (!cancelledTodoIdSet.has(todo.id))
                        continue;
                    todo.done = true;
                    todo.status = "pending";
    ...
    const persistProspectExecutionMutation = (mutation) => enqueuePersistence(async () => {
        const connection = await pool.getConnection();
        ...
        try {
            ...
            lockAcquired = Number(lockRows[0]?.acquired
                || 0) === 1;
            if (!lockAcquired) {
            ...
            if (lockAcquired) {
                try {
                    await connection.query("SELECT RELEASE_LOCK(?) AS released", [prospectExecutionLockName]);
                }
                catch {
    ...
    const persistProspectCandidateMutation = (mutation) => enqueuePersistence(async () => {
        const connection = await pool.getConnection();
        let lockAcquired = false;
        let transactionStarted = false;
        let applied;
        try {
            ...
            const authoritativeProcessing = await loadProspectCandidateProcessingStates(connection);
            store.websiteOpportunities.splice(0, store.websiteOpportunities.length, ...authoritative);
            const processingStates = store.prospectCandidateProcessingStates ||= [];
            processingStates.splice(0, processingStates.length, ...authoritativeProcessing);
            applied = mutation();
            await persistWebsiteOpportunityRows(connection, store.websiteOpportunities);
            await persistProspectCandidateProcessingStates(connection, processingStates);
            await connection.commit();
            transactionStarted = false;
            return applied.value;
        }
            ...
            if (lockAcquired) {
                try {
                    await connection.query("SELECT RELEASE_LOCK(?) AS released", [prospectCandidateLockName]);
                }
                catch {
                    // Releasing the connection also releases its advisory lock.
                }
    ...
    const persistProspectCandidates = (candidateIds) => {
        const ids = new Set(candidateIds);
        const desired = structuredClone(store.websiteOpportunities.filter((item) => ids.has(item.id)));
        return enqueuePersistence(async () => {
            ...
            try {
                ...
                for (const candidate of desired) {
                    const index = authoritative.findIndex((item) => item.id === candidate.id);
                    if (index >= 0)
                        authoritative[index] = candidate;
                    else
                        authoritative.unshift(candidate);
                }
    ...
    const reloadProspectCandidates = () => enqueuePersistence(async () => {
        ...
        store.websiteOpportunities.splice(0, store.websiteOpportunities.length, ...candidates);
        const currentProcessingStates = store.prospectCandidateProcessingStates ||= [];
        currentProcessingStates.splice(0, currentProcessingStates.length, ...processingStates);
    });
    let reloadProspectRuns;
    ...
    const loadedOrganizationIdentityState = await loadOrganizationIdentityState(pool, loadedProspectExecutionState.sourceRawRecords);
    const loadedOrganizationIdentityConflictReviewState = await loadOrganizationIdentityConflictReviewState(pool);
    const loadedOrganizationRelationState = await loadOrganizationRelationState(pool);
    const loadedProspectCoverageState = await loadProspectCoverageState(pool);
    const loadedProspectQualificationState = await loadProspectQualificationState(pool);
    const resolveOrganizationStrongIdentity = (input) => resolveOrganizationStrongIdentityMysql(pool, store, input);
    const reloadOrganizationIdentityTeam = (teamId) => enqueuePersistence(async () => {
        await ensureOrganizationIdentityTeamCache(pool, store, teamId);
    });
    const reviewOrganizationIdentityConflict = (input) => enqueuePersistence(async () => {
        await ensureOrganizationIdentityTeamCache(pool, store, input.user.teamId);
        await ensureOrganizationIdentityConflictReviewTeamCache(pool, store, input.user.teamId);
        return reviewOrganizationIdentityConflictMysql(pool, store, input);
    });
    const reloadOrganizationIdentityConflictReviewTeam = (teamId) => enqueuePersistence(async () => {
        await ensureOrganizationIdentityConflictReviewTeamCache(pool, store, teamId);
    });
    const recordOrganizationAlias = (input) => enqueuePersistence(async () => {
        await ensureOrganizationIdentityTeamCache(pool, store, input.user.teamId);
        await ensureOrganizationIdentityConflictReviewTeamCache(pool, store, input.user.teamId);
        await ensureOrganizationRelationTeamCache(pool, store, input.user.teamId);
        return recordOrganizationAliasMysql(pool, store, input);
    });
    const recordOrganizationRelation = (input) => enqueuePersistence(async () => {
        await ensureOrganizationIdentityTeamCache(pool, store, input.user.teamId);
        await ensureOrganizationIdentityConflictReviewTeamCache(pool, store, input.user.teamId);
        await ensureOrganizationRelationTeamCache(pool, store, input.user.teamId);
        return recordOrganizationRelationMysql(pool, store, input);
    });
    const reloadOrganizationRelationsTeam = (teamId) => enqueuePersistence(async () => {
        await ensureOrganizationRelationTeamCache(pool, store, teamId);
    });
    const recordProspectCoverage = (input) => recordProspectCoverageMysql(pool, store, input);
    const setTenantProspectDisposition = (input) => setTenantProspectDispositionMysql(pool, store, input);
    const convertProspectToLead = (input) => enqueuePersistence(() => convertProspectToLeadMysql(pool, store, input));
    const convertProspectToCustomer = (input) => enqueuePersistence(() => convertProspectToCustomerMysql(pool, store, input));
    const reloadProspectCoverageTeam = (teamId) => enqueuePersistence(async () => {
        await ensureProspectCoverageTeamCache(pool, store, teamId);
    });
    const applyProspectQualification = (input) => applyProspectQualificationCommandMysql(pool, store, input);
    const reloadProspectQualificationTeam = (teamId) => enqueuePersistence(async () => {
    ...
    reloadProspectRuns = () => enqueuePersistence(async () => {
        ...
        try {
            ...
            const loadedProviderConnections = await loadProviderConnections(connection);
            const candidate = persistenceSnapshot(store);
            ...
            validateProspectRunPersistence(candidate);
            await connection.commit();
            store.prospectSearchRuns.splice(0, store.prospectSearchRuns.length, ...loaded.runs);
            store.prospectRunShards.splice(0, store.prospectRunShards.length, ...loaded.shards);
            store.prospectRunEvents.splice(0, store.prospectRunEvents.length, ...loaded.events);
            store.agentJobs.splice(0, store.agentJobs.length, ...loadedJobs);
            store.agentJobIdempotencyAliases.splice(0, store.agentJobIdempotencyAliases.length, ...loadedJobAliases);
            store.prospectRunQueueParentBindings.splice(0, store.prospectRunQueueParentBindings.length, ...loaded.parentBindings);
            store.prospectRunQueueChildBindings.splice(0, store.prospectRunQueueChildBindings.length, ...loaded.childBindings);
            store.prospectExecutionKernelStates.splice(0, store.prospectExecutionKernelStates.length, ...loadedExecution.kernelStates);
            store.prospectExecutionCheckpoints.splice(0, store.prospectExecutionCheckpoints.length, ...loadedExecution.checkpoints);
            store.prospectStrategySourcePositions.splice(0, store.prospectStrategySourcePositions.length, ...loadedExecution.sourcePositions);
            store.prospectExecutionLeases.splice(0, store.prospectExecutionLeases.length, ...loadedExecution.leases);
            store.prospectExecutionAttempts.splice(0, store.prospectExecutionAttempts.length, ...loadedExecution.attempts);
            store.prospectProviderRequestLedgers.splice(0, store.prospectProviderRequestLedgers.length, ...loadedExecution.providerRequestLedgers);
            store.prospectProviderRequestDispatches.splice(0, store.prospectProviderRequestDispatches.length, ...loadedExecution.providerRequestDispatches);
            store.prospectProviderRequestAttemptBindings.splice(0, store.prospectProviderRequestAttemptBindings.length, ...loadedExecution.providerRequestAttemptBindings);
            store.prospectProviderRequestEvents.splice(0, store.prospectProviderRequestEvents.length, ...loadedExecution.providerRequestEvents);
            store.prospectProviderRequestAccountingEvidence.splice(0, store.prospectProviderRequestAccountingEvidence.length, ...loadedExecution.providerRequestAccountingEvidence);
            store.prospectSourceRawBatches.splice(0, store.prospectSourceRawBatches.length, ...loadedExecution.sourceRawBatches);
            store.prospectSourceRawRecords.splice(0, store.prospectSourceRawRecords.length, ...loadedExecution.sourceRawRecords);
            store.prospectSourceRawHits.splice(0, store.prospectSourceRawHits.length, ...loadedExecution.sourceRawHits);
            store.prospectExecutionPages.splice(0, store.prospectExecutionPages.length, ...loadedExecution.pages);
            store.prospectExecutionEvents.splice(0, store.prospectExecutionEvents.length, ...loadedExecution.events);
            store.prospectExecutionThrottleBuckets.splice(0, store.prospectExecutionThrottleBuckets.length, ...loadedExecution.throttleBuckets);
            store.aiModelConfigs.splice(0, store.aiModelConfigs.length, ...loadedAiModelConfigs);
            store.providerCatalog.splice(0, store.providerCatalog.length, ...loadedProviderCatalog);
            store.providerConnections.splice(0, store.providerConnections.length, ...loadedProviderConnections);
        }
        catch (error) {
            await connection.rollback();
            throw error;
    ...
    store = {
        ...
        customerAcquisitionSourceEvents: await loadCustomerAcquisitionSourceEvents(pool),
        customerIntelligenceSuggestions: await loadCustomerIntelligenceSuggestions(pool),
        ...
        prospectCandidateProcessingStates: await loadProspectCandidateProcessingStates(pool),
        aiModelConfigs: await loadAiModelConfigs(pool),
        aiSiteBuilderProjects: [...aiSiteBuilderProjects],
        aiSiteBuilderSettings: [...aiSiteBuilderSettings],
        ...
        prospectExecutionKernelStates: loadedProspectExecutionState.kernelStates,
        prospectExecutionCheckpoints: loadedProspectExecutionState.checkpoints,
        prospectStrategySourcePositions: loadedProspectExecutionState.sourcePositions,
        ...
        prospectProviderRequestLedgers: loadedProspectExecutionState.providerRequestLedgers,
        prospectProviderRequestDispatches: loadedProspectExecutionState.providerRequestDispatches,
        prospectProviderRequestAttemptBindings: loadedProspectExecutionState.providerRequestAttemptBindings,
        prospectProviderRequestEvents: loadedProspectExecutionState.providerRequestEvents,
        prospectProviderRequestAccountingEvidence: loadedProspectExecutionState.providerRequestAccountingEvidence,
        prospectSourceRawBatches: loadedProspectExecutionState.sourceRawBatches,
        prospectSourceRawRecords: loadedProspectExecutionState.sourceRawRecords,
        ...
        organizationIdentityClaims: loadedOrganizationIdentityState.organizationIdentityClaims,
        organizationAcceptedIdentifiers: loadedOrganizationIdentityState.organizationAcceptedIdentifiers,
        organizationIdentityResolutions: loadedOrganizationIdentityState.organizationIdentityResolutions,
        organizationSourceBindings: loadedOrganizationIdentityState.organizationSourceBindings,
        organizationIdentityConflicts: loadedOrganizationIdentityState.organizationIdentityConflicts,
        organizationIdentityConflictReviews: loadedOrganizationIdentityConflictReviewState
            .organizationIdentityConflictReviews,
        organizationCanonicalMappings: loadedOrganizationIdentityConflictReviewState
            .organizationCanonicalMappings,
        organizationAliasFacts: loadedOrganizationRelationState.organizationAliasFacts,
        organizationRelationFacts: loadedOrganizationRelationState.organizationRelationFacts,
        organizationIdentityEvents: loadedOrganizationIdentityState.organizationIdentityEvents,
        tenantProspects: loadedProspectCoverageState.tenantProspects,
        prospectCoverageEvents: loadedProspectCoverageState.prospectCoverageEvents,
        prospectEvidence: loadedProspectQualificationState.prospectEvidence,
        companyVerificationSnapshots: loadedProspectQualificationState.companyVerificationSnapshots,
        prospectIcpPolicySnapshots: loadedProspectQualificationState.prospectIcpPolicySnapshots,
        prospectIcpAssessmentSnapshots: loadedProspectQualificationState.prospectIcpAssessmentSnapshots,
        prospectContacts: loadedProspectQualificationState.prospectContacts,
        prospectContactChannels: loadedProspectQualificationState.prospectContactChannels,
        prospectContactVerificationSnapshots: loadedProspectQualificationState
            .prospectContactVerificationSnapshots,
        prospectSuppressionEvents: loadedProspectQualificationState.prospectSuppressionEvents,
        prospectContactabilityDecisions: loadedProspectQualificationState.prospectContactabilityDecisions,
        ...
        acquisitionOutcomeFeedback: await loadAcquisitionOutcomeFeedback(pool),
        prospectStrategySuggestions: await loadProspectStrategySuggestions(pool),
        prospectExecutionPages: loadedProspectExecutionState.pages,
        prospectExecutionEvents: loadedProspectExecutionState.events,
        prospectExecutionThrottleBuckets: loadedProspectExecutionState.throttleBuckets,
    ...
    if (missingDefaultProviders.length)
        store.providerCatalog.push(...missingDefaultProviders);
    ...
    store.agentJobIdempotencyAliases.splice(0, store.agentJobIdempotencyAliases.length, ...store.agentJobIdempotencyAliases.filter((item) => loadedJobIds.has(item.jobId)));
    if (!store.users.length) {
        if (!seedDevelopmentData) {
            ...
            store.customerIntelligenceSuggestions.push(...customerIntelligenceSuggestions);
            ...
            store.aiSiteBuilderProjects.push(...aiSiteBuilderProjects);
            store.aiSiteBuilderSettings.push(...aiSiteBuilderSettings);
    ...
    if (seedDevelopmentData) {
        let seedUsersChanged = false;
        for (const seedUser of users) {
            const existingUser = store.users.find((user) => user.id === seedUser.id || user.email === seedUser.email);
            if (!existingUser) {
                store.users.push({ ...seedUser });
                seedUsersChanged = true;
                continue;
            }
            const passwordCheck = await verifyPassword(existingUser.password, seedUser.password);
            if (!passwordCheck.valid) {
                existingUser.password = seedUser.password;
                existingUser.authVersion = Number(existingUser.authVersion || 1) + 1;
                seedUsersChanged = true;
            }
            if (existingUser.status !== "active"
                || existingUser.email !== seedUser.email
                || existingUser.role !== seedUser.role
                || existingUser.teamId !== seedUser.teamId) {
                existingUser.email = seedUser.email;
                existingUser.role = seedUser.role;
                existingUser.teamId = seedUser.teamId;
                existingUser.status = "active";
                existingUser.name ||= seedUser.name;
                existingUser.avatar ||= seedUser.avatar;
                existingUser.authVersion = Number(existingUser.authVersion || 1) + 1;
                seedUsersChanged = true;
            }
        }
    ...
    if (migratedConnections)
        await store.persist();
    const missingVerificationReports = store.websiteOpportunities.filter((item) => !item.verificationReport);
    if (missingVerificationReports.length) {
        missingVerificationReports.forEach(ensureProspectVerificationReport);
        await store.persistProspectCandidates?.(missingVerificationReports.map((item) => item.id));
    }
    validateProspectCampaignPersistence(store);
    validateProspectRunPersistence(store);
    validateFormalCampaignNamespace(store);
    validateMarketOpportunityPersistence(store);
    const recoveredMarketAnalysisJobs = recoverInterruptedAgentJobs(store, "prospect.market_analysis");
    if (recoveredMarketAnalysisJobs)
        await store.persist();
    return store;
}
async function ensureSchema(pool) {
  ...
  `);
    ...
    await ensureDatetimePrecision(pool, "provider_request_logs", "requested_at", false);
    ...
    await ensureUniqueIndex(pool, "prospect_campaigns", "uk_prospect_campaign_team_id", ["team_id", "id"]);
    ...
    await ensureUniqueIndex(pool, "prospect_campaign_versions", "uk_prospect_campaign_version", ["team_id", "campaign_id", "version_no"]);
    ...
    await ensureUniqueIndex(pool, "prospect_strategies", "uk_prospect_strategy_team_id", ["team_id", "id"]);
    await ensureUniqueIndex(pool, "prospect_strategies", "uk_prospect_strategy_run_ref", ["team_id", "campaign_id", "campaign_version", "id"]);
    ...
    await ensureUniqueIndex(pool, "prospect_schedules", "uk_prospect_schedule_team_id", ["team_id", "id"]);
    ...
    await ensureColumn(pool, "prospect_search_runs", "queue_bridge_version", "VARCHAR(10) NULL");
    await ensureColumn(pool, "prospect_search_runs", "execution_epoch", "INT NOT NULL DEFAULT 1");
    await replaceCheckConstraint(pool, "prospect_search_runs", "chk_prospect_run_status", `status IN (
      'queued','running','pause_requested','paused','cancel_requested',
      'cancelled','succeeded','succeeded_empty','partial_success','failed'
    )`);
    ...
    await ensureCheckConstraint(pool, "prospect_search_runs", "chk_prospect_run_queue_bridge_version", "queue_bridge_version IS NULL OR queue_bridge_version = 'v1'");
    await ensureUniqueIndex(pool, "prospect_search_runs", "uk_prospect_run_team_id", ["team_id", "id"]);
    await ensureUniqueIndex(pool, "prospect_search_runs", "uk_prospect_run_idempotency", ["team_id", "created_by", "operation_code", "idempotency_key_hash"]);
    await ensureUniqueIndex(pool, "prospect_search_runs", "uk_prospect_run_active_fingerprint", ["active_team_id", "active_owner_id", "active_query_fingerprint"]);
    ...
    await replaceCheckConstraint(pool, "prospect_run_shards", "chk_prospect_run_shard_status", `status IN (
      'queued','running','retry_scheduled','pause_requested','paused',
      'cancel_requested','cancelled','succeeded','succeeded_empty',
      'partial_success','failed'
    )`);
    await ensureUniqueIndex(pool, "prospect_run_shards", "uk_prospect_run_shard_team_run_id", ["team_id", "run_id", "id"]);
    ...
    await replaceCheckConstraint(pool, "prospect_run_events", "chk_prospect_run_event_type", `event_type IN (
    ...
    await ensureColumn(pool, "market_opportunity_batches", "first_trigger_job_id", "VARCHAR(80) NULL");
    await ensureColumn(pool, "market_opportunity_batches", "observation_cutoff_at", "DATETIME NULL");
    await ensureColumn(pool, "market_opportunity_calculation_events", "sequence_no", "BIGINT NULL");
    await pool.query(`
  ...
  `);
    await pool.query("ALTER TABLE market_opportunity_batches MODIFY COLUMN first_trigger_job_id VARCHAR(80) NOT NULL");
    await pool.query("ALTER TABLE market_opportunity_batches MODIFY COLUMN observation_cutoff_at DATETIME(3) NULL");
    await pool.query("ALTER TABLE market_opportunity_batches MODIFY COLUMN created_at DATETIME(3) NOT NULL");
    await pool.query("ALTER TABLE market_opportunity_snapshots MODIFY COLUMN created_at DATETIME(3) NOT NULL");
    await pool.query("ALTER TABLE market_opportunity_calculation_events MODIFY COLUMN sequence_no BIGINT NOT NULL");
    await pool.query("ALTER TABLE market_opportunity_calculation_events MODIFY COLUMN calculated_at DATETIME(3) NOT NULL");
    await ensureUniqueIndex(pool, "market_opportunity_batches", "uk_market_opportunity_batch", [
        "team_id",
        "owner_id",
        "campaign_id",
        "provider_id",
    ...
    await ensureUniqueIndex(pool, "market_opportunity_snapshots", "uk_market_opportunity_snapshot", ["batch_id", "reporter_code", "classification", "commodity_code"]);
    await ensureUniqueIndex(pool, "market_opportunity_calculation_events", "uk_market_opportunity_event_job", ["trigger_job_id"]);
    await ensureUniqueIndex(pool, "market_opportunity_calculation_events", "uk_market_opportunity_event_sequence", ["sequence_no"]);
    ...
    await ensureUniqueIndex(pool, "agent_jobs", "uk_agent_job_queue_bridge_ref", ["team_id", "owner_id", "id", "job_type", "parent_job_id"]);
    await ensureUniqueIndex(pool, "agent_jobs", "uk_agent_job_execution_ref", ["team_id", "owner_id", "id"]);
    ...
    await ensureUniqueIndex(pool, "agent_job_idempotency_aliases", "uk_agent_job_alias_idempotency", ["team_id", "job_type", "idempotency_key"]);
    ...
    await ensureUniqueIndex(pool, "prospect_execution_pages", "uk_prospect_execution_page_id", ["team_id", "id"]);
    ...
    await ensureUniqueIndex(pool, "prospect_strategy_source_positions", "uk_prospect_strategy_source_position_scope", ["team_id", "owner_id", "identity_hash"]);
    await ensureUniqueIndex(pool, "prospect_strategy_source_positions", "uk_prospect_strategy_source_position_id", ["team_id", "id"]);
...
async function rows(pool, sql) {
    const [result] = await pool.query(sql);
    return result;
}
async function ensureColumn(pool, table, column, definition) {
    const [result] = await pool.query("SELECT COUNT(*) AS count FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = ? AND column_name = ?", [table, column]);
    const exists = Number(result[0]?.count || 0) > 0;
    if (!exists)
        await pool.query(`ALTER TABLE ${table} ADD COLUMN ${column} ${definition}`);
}
async function ensureDatetimePrecision(pool, table, column, nullable) {
    const [result] = await pool.query(`SELECT data_type AS dataType,
...
async function ensureUniqueIndex(pool, table, index, columns) {
    const [result] = await pool.query("SELECT column_name AS columnName, non_unique AS nonUnique FROM information_schema.statistics WHERE table_schema = DATABASE() AND table_name = ? AND index_name = ? ORDER BY seq_in_index", [table, index]);
    const indexRows = result;
    ...
    if (isUnique
        ...
        && current.every((column, position) => column === columns[position]))
        return;
    if (current.length)
...
async function ensureCheckConstraint(pool, table, constraint, expression) {
...
async function replaceCheckConstraint(pool, table, constraint, expression) {
    const [result] = await pool.query(`SELECT COUNT(*) AS count
     FROM information_schema.table_constraints
     WHERE table_schema = DATABASE()
       AND table_name = ?
    ...
    const exists = Number(result[0]?.count || 0) > 0;
    if (exists) {
        await pool.query(`ALTER TABLE \`${table}\` DROP CHECK \`${constraint}\``);
    }
    await pool.query(`ALTER TABLE \`${table}\`
     ADD CONSTRAINT \`${constraint}\` CHECK (${expression})`);
}
async function repairAgentJobIdempotencyIntegrity(pool) {
    await pool.query(`DELETE alias
     FROM agent_job_idempotency_aliases alias
     LEFT JOIN agent_jobs job ON job.id = alias.job_id
     WHERE job.id IS NULL`);
    const scopeMismatch = await rows(pool, `SELECT COUNT(*) AS count
     FROM agent_job_idempotency_aliases alias
    ...
    const duplicatePrimary = await rows(pool, `SELECT COUNT(*) AS count
     FROM (
    ...
    const duplicateAlias = await rows(pool, `SELECT COUNT(*) AS count
     FROM (
       SELECT team_id, job_type, idempotency_key
    ...
    const crossTableConflict = await rows(pool, `SELECT COUNT(*) AS count
     FROM agent_job_idempotency_aliases alias
     INNER JOIN agent_jobs job
...
async function loadUsers(pool) {
    return (await rows(pool, "SELECT * FROM users")).map((row) => ({
        id: row.id,
        name: row.name,
        email: row.email,
...
async function loadCompanyProfiles(pool) {
    return (await rows(pool, "SELECT * FROM company_profiles")).map((row) => ({
        teamId: row.team_id,
...
async function loadLeadSourceEvents(pool) {
    return (await rows(pool, "SELECT * FROM lead_source_events ORDER BY received_at DESC")).map((row) => ({
        id: row.id,
        leadId: row.lead_id,
...
async function loadCustomers(pool) {
    return (await rows(pool, "SELECT * FROM customers ORDER BY created_at DESC")).map(customerFromRow);
}
...
async function loadCustomerOwnershipEvents(pool) {
    return (await rows(pool, `SELECT * FROM customer_ownership_events
...
async function loadCustomerActivities(pool) {
    return (await rows(pool, "SELECT * FROM customer_activities ORDER BY created_at DESC")).map((row) => ({
        id: row.id,
        customerId: row.customer_id,
        type: row.type || "note",
        content: row.content || "",
...
async function loadCustomerAcquisitionSourceEvents(pool) {
    return (await rows(pool, `SELECT * FROM customer_acquisition_source_events
     ORDER BY created_at DESC`)).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
...
async function loadCustomerIntelligenceSuggestions(pool) {
    const jsonArray = (value) => value
        ? (typeof value === "string" ? JSON.parse(value) : value)
        : [];
    return (await rows(pool, `SELECT * FROM customer_intelligence_suggestions
     ORDER BY created_at DESC`)).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
...
async function loadTodos(pool) {
    return (await rows(pool, "SELECT * FROM todos ORDER BY done ASC, created_at DESC")).map((row) => ({
...
async function loadProspectTouchpoints(pool) {
    return (await rows(pool, "SELECT * FROM prospect_touchpoints ORDER BY occurred_at DESC, created_at DESC")).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        prospectCandidateId: row.prospect_candidate_id,
...
async function loadProcurementSignals(pool) {
    return (await rows(pool, "SELECT * FROM procurement_signals ORDER BY observed_at DESC, created_at DESC")).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        prospectCandidateId: row.prospect_candidate_id,
...
async function loadDealRecommendations(pool) {
    const jsonArray = (value) => value
        ? (typeof value === "string" ? JSON.parse(value) : value)
        : [];
    return (await rows(pool, "SELECT * FROM deal_recommendations ORDER BY created_at DESC")).map((row) => ({
        id: row.id,
...
async function loadAcquisitionOutcomeFeedback(pool) {
    const jsonArray = (value) => value
        ? (typeof value === "string" ? JSON.parse(value) : value)
        : [];
    return (await rows(pool, "SELECT * FROM acquisition_outcome_feedback ORDER BY closed_at DESC")).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        dealId: row.deal_id,
        ...
        attributionReasonCodes: jsonArray(row.attribution_reason_codes_json),
        payloadHash: row.payload_hash,
        createdAt: row.created_at instanceof Date
            ? row.created_at.toISOString()
            : row.created_at
...
async function loadProspectStrategySuggestions(pool) {
    const jsonObject = (value) => value
        ? (typeof value === "string" ? JSON.parse(value) : value)
        : {};
    const jsonArray = (value) => value
        ? (typeof value === "string" ? JSON.parse(value) : value)
    ...
    const iso = (value) => value instanceof Date
        ? value.toISOString()
        : String(value || "");
    return (await rows(pool, `SELECT * FROM prospect_strategy_suggestions
     ORDER BY created_at DESC`)).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
...
async function loadPlanTasks(pool) {
    return (await rows(pool, "SELECT * FROM plan_tasks ORDER BY status = 'done' ASC, updated_at DESC, created_at DESC")).map((row) => ({
        id: row.id,
        title: row.title,
...
async function loadPlanTemplates(pool) {
    return (await rows(pool, "SELECT * FROM plan_templates ORDER BY sort_order ASC, updated_at DESC")).map((row) => ({
        id: row.id,
...
async function loadDeals(pool) {
    return (await rows(pool, "SELECT * FROM deals")).map((row) => ({
        id: row.id,
        customerId: row.customer_id,
        title: row.title,
        stage: row.stage,
...
async function loadDealEvents(pool) {
    return (await rows(pool, "SELECT * FROM deal_events ORDER BY created_at DESC")).map((row) => ({
        id: row.id,
        dealId: row.deal_id,
        type: row.event_type,
        content: row.content || "",
...
async function loadReminders(pool) {
    return (await rows(pool, "SELECT * FROM reminders")).map((row) => ({
...
async function loadKnowledgeAssets(pool) {
    return (await rows(pool, "SELECT * FROM knowledge_assets ORDER BY created_at DESC")).map((row) => ({
...
async function loadExams(pool) {
    return (await rows(pool, "SELECT * FROM exams ORDER BY updated_at DESC, id DESC")).map((row) => ({
        id: row.id,
        title: row.title,
...
async function loadExamQuestions(pool) {
    return (await rows(pool, "SELECT * FROM exam_questions ORDER BY updated_at DESC, id DESC")).map((row) => ({
        id: row.id,
        examId: row.exam_id,
        category: row.category,
        stem: row.stem,
...
async function loadExamQuestionLinks(pool) {
    return (await rows(pool, "SELECT * FROM exam_question_links ORDER BY exam_id ASC, sort_order ASC")).map((row) => ({
        examId: row.exam_id,
        questionId: row.question_id,
        sortOrder: Number(row.sort_order || 0)
    }));
}
async function loadExamAttempts(pool) {
    return (await rows(pool, "SELECT * FROM exam_attempts ORDER BY submitted_at DESC")).map((row) => ({
        id: row.id,
        examId: row.exam_id,
        userId: row.user_id,
...
async function loadImportExportJobs(pool) {
    return (await rows(pool, "SELECT * FROM import_export_jobs")).map((row) => ({
...
async function loadTradeDocuments(pool) {
    return (await rows(pool, "SELECT * FROM trade_documents ORDER BY updated_at DESC")).map((row) => ({
        id: row.id,
        customerId: row.customer_id || "",
        dealId: row.deal_id || "",
...
async function loadWecomMessages(pool) {
    return (await rows(pool, "SELECT * FROM wecom_messages")).map((row) => ({
        id: row.id, customerId: row.customer_id, summary: row.summary, ownerId: row.owner_id, teamId: row.team_id, status: row.status
    }));
}
async function loadOcrJobs(pool) {
    return (await rows(pool, "SELECT * FROM ocr_jobs")).map((row) => ({
        id: row.id,
        status: row.status,
...
async function loadWebsiteOpportunities(pool) {
    return (await rows(pool, "SELECT * FROM website_opportunities ORDER BY created_at DESC")).map((row) => ({
        id: row.id,
        company: row.company,
        business: row.business,
        country: row.country,
...
async function loadProspectCandidateProcessingStates(pool) {
    return (await rows(pool, `SELECT * FROM prospect_candidate_processing
     ORDER BY processed_at, hit_id`)).map((row) => ({
        hitId: row.hit_id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        runId: row.run_id,
...
async function loadAiModelConfigs(pool) {
    const storedRows = await rows(pool, "SELECT * FROM ai_model_configs ORDER BY updated_at DESC");
    const configs = [];
    for (const row of storedRows) {
        ...
        if (storedApiKey && !isEncryptedAiModelApiKey(storedApiKey)) {
            await pool.query("UPDATE ai_model_configs SET api_key = ? WHERE id = ?", [encryptAiModelApiKey(context, storedApiKey), row.id]);
        }
        configs.push({
            id: row.id,
            provider,
...
async function loadLeadSourceConfigs(pool) {
    return (await rows(pool, "SELECT * FROM lead_source_configs ORDER BY updated_at DESC")).map((row) => ({
        id: row.id,
...
async function loadProviderCatalog(pool) {
    return (await rows(pool, "SELECT * FROM provider_catalog ORDER BY created_at ASC, code ASC")).map((row) => ({
        id: row.id,
        code: row.code,
        name: row.name,
        category: row.category,
...
async function loadProviderConnections(pool) {
    return (await rows(pool, "SELECT * FROM provider_connections ORDER BY updated_at DESC")).map((row) => ({
        id: row.id,
...
async function loadProviderRequestLogs(pool) {
    return (await rows(pool, "SELECT * FROM provider_request_logs ORDER BY requested_at DESC")).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        providerId: row.provider_id,
...
async function loadProviderResponseCache(pool) {
    return (await rows(pool, "SELECT * FROM provider_response_cache ORDER BY fetched_at DESC")).map((row) => ({
        id: row.id,
        providerId: row.provider_id,
...
async function loadMarketTradeObservations(pool) {
    return (await rows(pool, "SELECT * FROM market_trade_observations ORDER BY observed_at DESC")).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        campaignId: row.campaign_id,
...
const prospectCampaignIdPersistenceSchema = z.string().regex(new RegExp(`^pc_${uuidV4Pattern}$`, "i"));
...
const prospectCampaignVersionPersistenceSchema = z.object({
    ...
    snapshot: z.object({
        ...
        sourceProviderIds: z.array(z.string().min(1).max(80).regex(/^[A-Za-z0-9._-]+$/)).max(100)
    }).strict(),
...
const prospectRunExecutionSnapshotPersistenceSchema = z.object({
    ...
    providerPlan: z.array(prospectRunProviderSnapshotPersistenceSchema).min(1).max(30)
}).strict();
...
const prospectStrategySourcePositionPersistenceSchema = z.object({
    ...
    if (position.status === "continuable"
        && !/^v1\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/.test(position.encryptedCursor)) {
        context.addIssue({
            code: z.ZodIssueCode.custom,
...
function parseProspectCampaignPersistence(schema, value, label) {
...
async function loadProspectCampaigns(pool) {
    return (await rows(pool, "SELECT * FROM prospect_campaigns ORDER BY updated_at DESC")).map((row) => parseProspectCampaignPersistence(prospectCampaignPersistenceSchema, {
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        name: row.name,
...
async function loadProspectCampaignVersions(pool) {
    return (await rows(pool, "SELECT * FROM prospect_campaign_versions ORDER BY version_no DESC")).map((row) => {
        const snapshot = normalizeProspectCampaignSnapshot(mysqlJson(row.snapshot_json));
        return parseProspectCampaignPersistence(prospectCampaignVersionPersistenceSchema, {
            id: row.id,
            teamId: row.team_id,
            campaignId: row.campaign_id,
...
async function loadProspectCampaignEvents(pool) {
    return (await rows(pool, "SELECT * FROM prospect_campaign_events ORDER BY created_at DESC, id DESC")).map((row) => parseProspectCampaignPersistence(prospectCampaignEventPersistenceSchema, {
        id: row.id,
        teamId: row.team_id,
        campaignId: row.campaign_id,
...
async function loadProspectStrategies(pool) {
    return (await rows(pool, "SELECT * FROM prospect_strategies ORDER BY updated_at DESC, id DESC")).map((row) => {
        const query = normalizeProspectStrategyQuery(mysqlJson(row.query_json));
        const providerPlan = normalizeProspectStrategyProviderPlan(mysqlJson(row.provider_plan_json));
        return parseProspectCampaignPersistence(prospectStrategyPersistenceSchema, {
            id: row.id,
            teamId: row.team_id,
            campaignId: row.campaign_id,
...
async function loadProspectStrategyEvents(pool) {
    return (await rows(pool, "SELECT * FROM prospect_strategy_events ORDER BY created_at DESC, id DESC")).map((row) => parseProspectCampaignPersistence(prospectStrategyEventPersistenceSchema, {
        id: row.id,
        teamId: row.team_id,
        campaignId: row.campaign_id,
        strategyId: row.strategy_id,
...
async function loadProspectSchedules(pool) {
    return (await rows(pool, "SELECT * FROM prospect_schedules ORDER BY next_run_at ASC, id ASC")).map((row) => parseProspectCampaignPersistence(prospectSchedulePersistenceSchema, {
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        campaignId: row.campaign_id,
...
async function loadProspectSearchRuns(pool) {
    return (await rows(pool, `SELECT *
     FROM prospect_search_runs
     ORDER BY created_at DESC, id DESC`)).map((row) => parseProspectCampaignPersistence(prospectSearchRunPersistenceSchema, {
        id: row.id,
        teamId: row.team_id,
        campaignId: row.campaign_id,
        campaignVersion: Number(row.campaign_version),
...
async function loadProspectRunShards(pool) {
    return (await rows(pool, `SELECT *
     FROM prospect_run_shards
     ORDER BY run_id, position_no, id`)).map((row) => parseProspectCampaignPersistence(prospectRunShardPersistenceSchema, {
        id: row.id,
        teamId: row.team_id,
        runId: row.run_id,
...
async function loadProspectRunEvents(pool) {
    return (await rows(pool, `SELECT *
     FROM prospect_run_events
     ORDER BY run_id, sequence_no, id`)).map((row) => parseProspectCampaignPersistence(prospectRunEventPersistenceSchema, {
        id: row.id,
        teamId: row.team_id,
        runId: row.run_id,
...
async function loadProspectRunQueueParentBindings(pool) {
    return (await rows(pool, `SELECT *
     FROM prospect_run_queue_parent_bindings
     ORDER BY created_at, id`)).map((row) => parseProspectCampaignPersistence(prospectRunQueueParentBindingPersistenceSchema, {
        id: row.id,
        teamId: row.team_id,
        runId: row.run_id,
        ownerId: row.owner_id,
...
async function loadProspectRunQueueChildBindings(pool) {
    return (await rows(pool, `SELECT *
     FROM prospect_run_queue_child_bindings
     ORDER BY created_at, id`)).map((row) => parseProspectCampaignPersistence(prospectRunQueueChildBindingPersistenceSchema, {
        id: row.id,
        teamId: row.team_id,
        runId: row.run_id,
...
async function loadProspectRunState(pool) {
...
async function loadProspectExecutionState(pool) {
    const kernelStates = (await rows(pool, `SELECT * FROM search_execution_kernel_state ORDER BY id`)).map((row) => ({
    ...
    const checkpoints = (await rows(pool, `SELECT * FROM prospect_execution_checkpoints
     ORDER BY team_id, run_id, shard_id`)).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        runId: row.run_id,
    ...
    const leases = (await rows(pool, `SELECT * FROM prospect_execution_leases
     ORDER BY team_id, run_id, claimed_at, id`)).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        runId: row.run_id,
    ...
    const attempts = (await rows(pool, `SELECT * FROM prospect_execution_attempts
     ORDER BY team_id, run_id, created_at, id`)).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        runId: row.run_id,
    ...
    const providerRequestLedgers = (await rows(pool, `SELECT * FROM prospect_provider_request_ledgers
     ORDER BY team_id, run_id, shard_id, checkpoint_no, logical_request_no`)).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        runId: row.run_id,
        ...
        responseAccountingEvidenceHash: row.response_accounting_evidence_hash || "",
    ...
    const providerRequestDispatches = (await rows(pool, `SELECT * FROM prospect_provider_request_dispatches
     ORDER BY team_id, ledger_id, dispatch_no`)).map((row) => ({
        id: row.id,
        ledgerId: row.ledger_id,
        teamId: row.team_id,
        ownerId: row.owner_id,
    ...
    const providerRequestAttemptBindings = (await rows(pool, `SELECT * FROM prospect_provider_request_attempt_bindings
       ORDER BY team_id, ledger_id, binding_no`)).map((row) => ({
        id: row.id,
        ledgerId: row.ledger_id,
        attemptId: row.attempt_id,
        teamId: row.team_id,
    ...
    const providerRequestEvents = (await rows(pool, `SELECT * FROM prospect_provider_request_events
     ORDER BY team_id, ledger_id, sequence_no`)).map((row) => ({
        id: row.id,
        ledgerId: row.ledger_id,
    ...
    const providerRequestAccountingEvidence = (await rows(pool, `SELECT * FROM prospect_provider_request_accounting_evidence
       ORDER BY team_id, ledger_id, sequence_no`)).map((row) => ({
        id: row.id,
        ledgerId: row.ledger_id,
        teamId: row.team_id,
    ...
    const pages = (await rows(pool, `SELECT * FROM prospect_execution_pages
     ORDER BY team_id, run_id, shard_id, page_sequence`)).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        runId: row.run_id,
    ...
    const sourcePositions = (await rows(pool, `SELECT * FROM prospect_strategy_source_positions
     ORDER BY team_id, owner_id, campaign_id, strategy_id, provider_code`)).map((row) => ({
        id: row.id,
    ...
    const sourceRawRecords = (await rows(pool, `SELECT * FROM prospect_source_raw_records
     ORDER BY team_id, owner_id, provider_code, connection_id,
       endpoint_code, first_observed_at, id`)).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        providerCode: row.provider_code,
    ...
    const sourceRawBatches = (await rows(pool, `SELECT * FROM prospect_source_raw_batches
     ORDER BY team_id, run_id, shard_id, created_at, id`)).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        runId: row.run_id,
    ...
    const sourceRawHits = (await rows(pool, `SELECT * FROM prospect_source_raw_hits
     ORDER BY team_id, batch_id, ordinal, id`)).map((row) => ({
        id: row.id,
    ...
    const events = (await rows(pool, `SELECT * FROM prospect_execution_events
     ORDER BY team_id, run_id, created_at, id`)).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
    ...
    const throttleBuckets = (await rows(pool, `SELECT * FROM prospect_execution_throttles
     ORDER BY team_id, provider_code, connection_id`)).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        providerCode: row.provider_code,
        connectionId: row.connection_id,
...
export function validateProspectCampaignPersistence(store) {
    ...
    const strategyEventIds = new Set();
    const strategyEventsByStrategy = new Map();
    for (const campaign of store.prospectCampaigns) {
    ...
    for (const auditEvent of store.prospectCampaignEvents) {
        if (eventIds.has(auditEvent.id))
            throw new Error("获客项目审计事件主键重复");
    ...
    for (const strategy of store.prospectStrategies) {
        ...
        const version = store.prospectCampaignVersions.find((item) => item.teamId === strategy.teamId
        ...
        const normalizedProviderPlan = normalizeProspectStrategyProviderPlan(strategy.providerPlan);
...
function isValidPersistedRunTransition(fromStatus, toStatus) {
    const transitions = {
        queued: new Set([
            "running",
            "paused",
...
function isValidPersistedShardTransition(fromStatus, toStatus) {
    const transitions = {
        queued: new Set([
            "running",
            "paused",
...
function expectedPersistedRunEventType(fromStatus, toStatus) {
    if (fromStatus === "queued" && toStatus === "running")
        return "started";
    if (toStatus === "pause_requested")
        return "pause_requested";
    if (toStatus === "paused")
        return "paused";
    if (fromStatus === "paused" && toStatus === "queued")
        return "resumed";
...
export function validateProspectRunPersistence(store) {
    ...
    const campaigns = new Map(store.prospectCampaigns.map((item) => [
        `${item.teamId}|${item.id}`,
        item
    ]));
    const versions = new Map(store.prospectCampaignVersions.map((item) => [
        `${item.teamId}|${item.campaignId}|${item.version}`,
        item
    ]));
    const strategies = new Map(store.prospectStrategies.map((item) => [
    ...
    for (const run of store.prospectSearchRuns) {
        ...
        const version = versions.get(`${run.teamId}|${run.campaignId}|${run.campaignVersion}`);
        const strategy = strategies.get(`${run.teamId}|${run.campaignId}|${run.campaignVersion}|${run.strategyId}`);
        ...
        const resolvedQuery = resolveProspectStrategyQuery(snapshot.strategy.query, version);
        ...
        snapshot.providerPlan.forEach((provider, index) => {
            ...
            if (!strategyProvider
                ...
                || !isDeepStrictEqual(provider.capabilities, [...provider.capabilities].sort())) {
    ...
    for (const run of store.prospectSearchRuns) {
        ...
        const shardProviderCodes = new Set();
        ...
        events.forEach((event, index) => {
            ...
            if (index > 0) {
                const expectedType = expectedPersistedRunEventType(previousStatus, event.toStatus);
...
function validateProspectExecutionPersistence(store) {
    const hashPattern = /^[a-f0-9]{64}$/;
    const executionFactsCount = store.prospectExecutionCheckpoints.length
    ...
    const runs = new Map(store.prospectSearchRuns.map((item) => [item.id, item]));
    const shards = new Map(store.prospectRunShards.map((item) => [item.id, item]));
    const jobs = new Map(store.agentJobs.map((item) => [item.id, item]));
    const parentBindingsByRun = new Map(store.prospectRunQueueParentBindings.map((item) => [item.runId, item]));
    const childBindingsByShard = new Map(store.prospectRunQueueChildBindings.map((item) => [item.shardId, item]));
    const checkpointsByShard = new Map();
    ...
    const activeRunScopes = new Set();
    const activeJobScopes = new Set();
    const fenceScopes = new Set();
    ...
    for (const lease of store.prospectExecutionLeases) {
        ...
        if (lease.status === "active") {
            ...
            if (activeRunScopes.has(runScope)
                ...
                || !["running", "pause_requested", "cancel_requested"].includes(shard.status)
                || job.status !== "running"
                || lease.releasedAt
                || lease.releaseReason) {
    ...
    const attemptLeaseIds = new Set();
    const sentCallScopes = new Set();
    ...
    if (store.prospectExecutionLeases.some((lease) => !attemptLeaseIds.has(lease.id))) {
        throw new Error("搜索执行租约缺少唯一的尝试事实");
    }
    const pageIds = new Set();
    const pageAttemptIds = new Set();
    const pagesByShard = new Map();
    ...
    const sourcePositionIds = new Set();
    const sourcePositionScopes = new Set();
    for (const position of store.prospectStrategySourcePositions) {
        ...
        const page = store.prospectExecutionPages.find((item) => item.id === position.sourcePageId);
        const attempt = page ? attemptsById.get(page.attemptId) : null;
        const ledger = attempt
            ? store.prospectProviderRequestLedgers.find((item) => item.originAttemptId === attempt.id)
            : null;
        ...
        const allowedConnectionIds = new Set([
            ...
            ...store.providerConnections
                .filter((item) => item.providerId === position.providerCode
    ...
    for (const checkpoint of store.prospectExecutionCheckpoints) {
        ...
        const sentAttempts = attempts.filter((item) => item.checkpointCallNo > 0);
        const currentCalls = sentAttempts
            .filter((item) => item.checkpointNo === checkpoint.checkpointNo)
            .sort((left, right) => left.checkpointCallNo - right.checkpointCallNo);
        ...
        if (checkpoint.totalCallCount !== sentAttempts.length
            ...
            || currentCalls.some((attempt, index) => attempt.checkpointCallNo !== index + 1)
    ...
    const throttleIds = new Set();
    const throttleScopes = new Set();
...
function validateProspectProviderRequestPersistence(store) {
    const hashPattern = /^[a-f0-9]{64}$/;
    const evidenceRefPattern = /^sha256:[a-f0-9]{64}$/;
    const encryptedResponsePattern = /^provider-response-v1\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/;
    const runs = new Map(store.prospectSearchRuns.map((item) => [item.id, item]));
    const shards = new Map(store.prospectRunShards.map((item) => [item.id, item]));
    const jobs = new Map(store.agentJobs.map((item) => [item.id, item]));
    const attempts = new Map(store.prospectExecutionAttempts.map((item) => [item.id, item]));
    const leases = new Map(store.prospectExecutionLeases.map((item) => [item.id, item]));
    ...
    for (const ledger of store.prospectProviderRequestLedgers) {
        ...
        const hasAllResponseHashes = responseHashes.every((item) => hashPattern.test(item));
        const hasAnyResponseHash = responseHashes.some(Boolean);
        const hasAnyResponseFact = Boolean(hasAnyResponseHash
        ...
        const hasAllResponseEvidence = Boolean(hasAllResponseHashes
            && ledger.externalRequestId
        ...
        const expectedResponseHash = hasAllResponseHashes
            ...
            ? prospectProviderResponseHash({
                ...
                accountingEvidenceHash: ledger.responseAccountingEvidenceHash
            })
            : "";
        const hasDispatchConfirmation = Boolean(ledger.externalRequestId || ledger.dispatchConfirmationRef);
        const responseLeaseFactsValid = ledger.status !== "response_received"
            || (attempt?.status === "request_started"
            ...
            || (attempt?.status === "request_outcome_unknown"
                ...
                && Boolean(lease?.releasedAt)
                && lease.releasedAt <= ledger.responseReceivedAt);
        const invalidStatusFacts = ledger.status === "prepared"
        ...
        if (externalScope)
            externalScopes.add(externalScope);
    }
    const bindingsByLedger = new Map();
    const bindingIds = new Set();
    const bindingScopes = new Set();
    ...
    const dispatchesByLedger = new Map();
    const dispatchScopes = new Set();
    ...
    for (const dispatches of dispatchesByLedger.values()) {
        ...
        if (dispatches.some((item, index) => item.dispatchNo !== index + 1)) {
            throw new Error("Provider 请求派发序号不连续");
        }
    }
    for (const ledger of store.prospectProviderRequestLedgers) {
        const dispatches = dispatchesByLedger.get(ledger.id) || [];
        const responseDispatch = dispatches.find((item) => item.status === "response_received");
        ...
        if (responsePersisted) {
            if (!responseDispatch
                || dispatches.filter((item) => item.status === "response_received").length !== 1
    ...
    for (const ledger of store.prospectProviderRequestLedgers) {
        const accounting = accountingByLedger.get(ledger.id) || [];
        const responseDispatches = (dispatchesByLedger.get(ledger.id) || []).filter((item) => item.status === "response_received");
        ...
        const responsePages = responseAttempt
            ? store.prospectExecutionPages.filter((item) => item.attemptId === responseAttempt.id
        ...
        const hasResponseSettlement = (ledger.status === "settled"
        ...
        if (!hasResponseSettlement) {
            ...
            if (ledger.status === "response_received") {
                if (responseDispatches.length !== 1
                    || !responseAttempt
                    || !["request_started", "request_outcome_unknown"].includes(responseAttempt.status)
        ...
        const expectedEstimationVersion = responseAttempt.costKind === "estimated"
...
function hasForbiddenExecutionFactKey(value) {
    if (!value || typeof value !== "object")
        return false;
    const forbidden = /(authorization|credential|api.?key|secret|header|cookie|candidate|customer|lead|opportunit|raw.?response|raw.?payload)/i;
    return Object.entries(value).some(([key, nested]) => forbidden.test(key)
        || (typeof nested === "object"
            && hasForbiddenExecutionFactKey(nested)));
}
function validateFormalCampaignNamespace(store) {
    const ambiguous = store.agentJobs.some((item) => item.aggregateType === "prospect_campaign_ref_compat_v1"
        && item.aggregateId.startsWith("pc_")) || store.marketTradeObservations.some((item) => item.campaignId.startsWith("pc_")) || store.marketOpportunityBatches.some((item) => item.campaignId.startsWith("pc_")) || store.marketOpportunitySnapshots.some((item) => item.campaignId.startsWith("pc_")) || store.marketOpportunityCalculationEvents.some((item) => item.campaignId.startsWith("pc_"));
    if (ambiguous) {
        throw new Error("检测到 pc_ 正式命名空间中的旧兼容市场数据，请先完成人工迁移");
    }
}
function parseMarketOpportunityPersistence(schema, value, label) {
    const parsed = schema.safeParse(value);
    if (!parsed.success) {
        throw new Error(`${label}持久化校验失败`, { cause: parsed.error });
    }
...
async function loadMarketOpportunityBatches(pool) {
    return (await rows(pool, "SELECT * FROM market_opportunity_batches ORDER BY created_at DESC")).map((row) => parseMarketOpportunityPersistence(marketOpportunityBatchPersistenceSchema, {
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        campaignId: row.campaign_id,
...
async function loadMarketOpportunitySnapshots(pool) {
    return (await rows(pool, "SELECT * FROM market_opportunity_snapshots ORDER BY created_at DESC, id DESC")).map((row) => parseMarketOpportunityPersistence(marketOpportunitySnapshotPersistenceSchema, {
        id: row.id,
        batchId: row.batch_id,
        teamId: row.team_id,
        ownerId: row.owner_id,
...
async function loadMarketOpportunityCalculationEvents(pool) {
    return (await rows(pool, "SELECT * FROM market_opportunity_calculation_events ORDER BY sequence_no DESC")).map((row) => parseMarketOpportunityPersistence(marketOpportunityCalculationPersistenceSchema, {
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        campaignId: row.campaign_id,
...
function validateMarketOpportunityPersistence(store) {
    ...
    for (const snapshot of store.marketOpportunitySnapshots) {
        ...
        const validReadySeries = series.length === 3
            ...
            && series.every((point) => point.evidence.providerId === snapshot.providerId
    ...
    for (const batch of store.marketOpportunityBatches) {
        ...
        const readyCount = snapshots.filter((item) => item.snapshotStatus === "metrics_ready").length;
...
async function loadAgentJobs(pool) {
    return (await rows(pool, "SELECT * FROM agent_jobs ORDER BY created_at DESC")).map((row) => ({
        id: row.id,
        teamId: row.team_id,
        ownerId: row.owner_id,
        jobType: row.job_type,
...
async function loadAgentJobIdempotencyAliases(pool) {
    return (await rows(pool, `SELECT alias.*
     FROM agent_job_idempotency_aliases alias
     INNER JOIN agent_jobs job ON job.id = alias.job_id
     ORDER BY alias.created_at DESC`)).map((row) => ({
        id: row.id,
        jobId: row.job_id,
        teamId: row.team_id,
        jobType: row.job_type,
...
async function loadProblems(pool) {
    return (await rows(pool, "SELECT * FROM problems ORDER BY status = 'resolved' ASC, created_at DESC")).map((row) => ({
        id: row.id,
        title: row.title,
        category: row.category,
...
async function loadMemos(pool) {
    return (await rows(pool, "SELECT * FROM memos ORDER BY pinned DESC, archived ASC, updated_at DESC")).map((row) => ({
        id: row.id,
        title: row.title,
        content: row.content,
        category: row.category,
...
async function loadCompetitors(pool) {
    return (await rows(pool, "SELECT * FROM competitors ORDER BY threat_level = 'high' DESC, updated_at DESC")).map((row) => ({
        id: row.id,
        company: row.company,
        country: row.country,
        segment: row.segment,
...
async function loadCaseStudies(pool) {
    return (await rows(pool, "SELECT * FROM case_studies ORDER BY status = 'published' DESC, updated_at DESC")).map((row) => ({
        id: row.id,
        title: row.title,
...
async function loadCommissionProducts(pool) {
    return (await rows(pool, "SELECT * FROM commission_products ORDER BY status = 'active' DESC, updated_at DESC")).map((row) => ({
        id: row.id,
        name: row.name,
        category: row.category || "",
        model: row.model || "",
...
async function loadCommissionRules(pool) {
    return (await rows(pool, "SELECT * FROM commission_rules ORDER BY enabled DESC, created_at DESC")).map((row) => ({
        id: row.id,
...
async function loadMonthlySalesRecords(pool) {
    return (await rows(pool, "SELECT * FROM monthly_sales_records ORDER BY month_value DESC, updated_at DESC")).map((row) => ({
        id: row.id,
...
async function loadSalesRecordAudits(pool) {
    return (await rows(pool, "SELECT * FROM sales_record_audits ORDER BY created_at DESC")).map((row) => ({
        id: row.id,
        recordId: row.record_id,
...
async function loadCommissionCalculations(pool) {
    return (await rows(pool, "SELECT * FROM commission_calculations ORDER BY month_value DESC, calculated_at DESC")).map((row) => ({
        id: row.id,
        month: row.month_value,
        ownerId: row.owner_id,
        teamId: row.team_id,
...
async function loadCommissionItems(pool) {
    return (await rows(pool, "SELECT * FROM commission_items ORDER BY created_at DESC")).map((row) => ({
        id: row.id,
...
async function loadCommissionExports(pool) {
    return (await rows(pool, "SELECT * FROM commission_exports ORDER BY created_at DESC")).map((row) => ({
        id: row.id,
        month: row.month_value,
...
async function loadWhatsAppBindings(pool) {
    return (await rows(pool, "SELECT * FROM whatsapp_bindings ORDER BY last_message_at DESC")).map((row) => ({
        id: row.id,
        customerId: row.customer_id,
...
async function loadWhatsAppMessages(pool) {
    return (await rows(pool, "SELECT * FROM whatsapp_messages ORDER BY created_at ASC")).map((row) => ({
        id: row.id,
        customerId: row.customer_id,
        direction: row.direction,
        content: row.content || "",
...
async function loadDailyReports(pool) {
    return (await rows(pool, "SELECT * FROM daily_reports ORDER BY report_date DESC, submitted_at DESC")).map((row) => ({
        id: row.id,
...
async function loadDailyReportComments(pool) {
    return (await rows(pool, "SELECT * FROM daily_report_comments ORDER BY created_at ASC")).map((row) => ({
        id: row.id,
...
async function loadInternalMessages(pool) {
    return (await rows(pool, "SELECT * FROM internal_messages ORDER BY created_at DESC")).map((row) => ({
        id: row.id,
...
async function persistAll(pool, store) {
    validateProspectCampaignPersistence(store);
    validateProspectRunPersistence(store);
    validateFormalCampaignNamespace(store);
    validateMarketOpportunityPersistence(store);
    ...
    try {
        ...
        await syncAgentJobRows(connection, store.agentJobs, (item) => !isProspectRunBridgeJob(item));
        await syncAgentJobIdempotencyAliasRows(connection, store.agentJobIdempotencyAliases, (item) => !isProspectRunBridgeJob(item));
...
async function replaceRows(connection, table, items, values, columns) {
...
async function persistWebsiteOpportunityRows(connection, items) {
    await replaceRows(connection, "website_opportunities", items, (item) => [
        item.id,
        item.company,
        item.business,
        item.country,
...
async function persistProspectCandidateProcessingStates(connection, desired) {
    const persisted = await loadProspectCandidateProcessingStates(connection);
    const desiredByHitId = new Map(desired.map((item) => [item.hitId, item]));
    ...
    const persistedHitIds = new Set(persisted.map((item) => item.hitId));
    try {
        await insertRows(connection, "prospect_candidate_processing", desired.filter((item) => !persistedHitIds.has(item.hitId)), (item) => [
            item.hitId,
            item.teamId,
            item.ownerId,
            item.runId,
...
function verifyImmutableRows(persisted, desired, label) {
...
function isMysqlDuplicateKeyError(error) {
    if (!error || typeof error !== "object")
        return false;
    const value = error;
    return value.code === "ER_DUP_ENTRY" || value.errno === 1062;
}
async function insertRows(connection, table, items, values, columns) {
    if (!items.length)
        return;
    const mapped = items.map(values);
    const placeholders = mapped.map((row) => `(${row.map(() => "?").join(",")})`).join(",");
...
async function appendProviderRequestLogRows(connection, desired) {
    const desiredById = new Map(desired.map((item) => [item.id, item]));
    if (desiredById.size !== desired.length) {
        throw new Error("数据源请求日志存在重复主键");
    }
    const persistedById = new Map((await loadProviderRequestLogs(connection)).map((item) => [item.id, item]));
    ...
    await insertRows(connection, "provider_request_logs", desired.filter((item) => !persistedById.has(item.id)), (item) => [
        item.id,
        item.teamId,
        item.ownerId,
        item.providerId,
...
async function syncProspectCampaignRows(connection, desired) {
    ...
    for (const campaign of desired) {
        ...
        if (!existing) {
            await insertRows(connection, "prospect_campaigns", [campaign], (item) => [
                item.id,
                item.teamId,
                item.ownerId,
                item.name,
        ...
        if (isDeepStrictEqual(existing, campaign))
            continue;
        ...
        const [result] = await connection.query(`UPDATE prospect_campaigns
...
async function appendProspectCampaignHistoryRows(connection, store) {
    ...
    verifyImmutableRows(persistedVersions, store.prospectCampaignVersions, "获客项目版本");
    verifyImmutableRows(persistedEvents, store.prospectCampaignEvents, "获客项目审计事件");
    ...
    await insertRows(connection, "prospect_campaign_versions", store.prospectCampaignVersions.filter((item) => !persistedVersionIds.has(item.id)), (item) => [
        item.id,
        item.teamId,
        item.campaignId,
        item.version,
    ...
    await insertRows(connection, "prospect_campaign_events", store.prospectCampaignEvents.filter((item) => !persistedEventIds.has(item.id)), (item) => [
        item.id,
        item.teamId,
        item.campaignId,
        item.eventType,
...
async function syncProspectStrategyRows(connection, desired) {
    ...
    for (const strategy of desired) {
        ...
        if (!existing) {
            await insertRows(connection, "prospect_strategies", [strategy], (item) => [
                item.id,
                item.teamId,
                item.campaignId,
        ...
        if (isDeepStrictEqual(existing, strategy))
            continue;
        ...
        const [result] = await connection.query(`UPDATE prospect_strategies
...
async function appendProspectStrategyHistoryRows(connection, store) {
    const persistedEvents = await loadProspectStrategyEvents(connection);
    verifyImmutableRows(persistedEvents, store.prospectStrategyEvents, "获客搜索策略审计事件");
    const persistedIds = new Set(persistedEvents.map((item) => item.id));
    await insertRows(connection, "prospect_strategy_events", store.prospectStrategyEvents.filter((item) => !persistedIds.has(item.id)), (item) => [
        item.id,
        item.teamId,
        item.campaignId,
        item.strategyId,
...
async function syncProspectScheduleRows(connection, desired) {
    ...
    for (const existing of persisted) {
        if (desiredById.has(existing.id))
            continue;
        const [result] = await connection.query(`DELETE FROM prospect_schedules
       WHERE id = ? AND team_id = ? AND revision_no = ?`, [existing.id, existing.teamId, existing.revision]);
        if (Number(result.affectedRows || 0) !== 1) {
    ...
    for (const schedule of desired) {
        ...
        if (!existing) {
            await insertRows(connection, "prospect_schedules", [schedule], (item) => [
                item.id,
                item.teamId,
                item.ownerId,
                item.campaignId,
        ...
        if (isDeepStrictEqual(existing, schedule))
            continue;
        ...
        const [result] = await connection.query(`UPDATE prospect_schedules
...
function validAgentJobTransition(existing, desired) {
...
async function syncAgentJobRows(connection, desired, owns = () => true) {
    ...
    for (const job of ownedDesired) {
        ...
        if (!existing) {
            await insertRows(connection, "agent_jobs", [job], (item) => [
                item.id,
                item.teamId,
                item.ownerId,
                item.jobType,
        ...
        const [result] = await connection.query(`UPDATE agent_jobs
...
async function syncAgentJobIdempotencyAliasRows(connection, desired, owns = () => true) {
    const persisted = (await loadAgentJobIdempotencyAliases(connection))
        .filter(owns);
    const ownedDesired = desired.filter(owns);
    verifyImmutableRows(persisted, ownedDesired, "智能任务幂等别名");
    const persistedIds = new Set(persisted.map((item) => item.id));
    await insertRows(connection, "agent_job_idempotency_aliases", ownedDesired.filter((item) => !persistedIds.has(item.id)), (item) => [
        item.id,
        item.jobId,
        item.teamId,
        item.jobType,
...
async function appendProspectRunQueueBindingRows(connection, store) {
    ...
    verifyImmutableRows(persistedParents, store.prospectRunQueueParentBindings, "搜索运行父桥接绑定");
    verifyImmutableRows(persistedChildren, store.prospectRunQueueChildBindings, "搜索运行子桥接绑定");
    const parentIds = new Set(persistedParents.map((item) => item.id));
    const childIds = new Set(persistedChildren.map((item) => item.id));
    await insertRows(connection, "prospect_run_queue_parent_bindings", store.prospectRunQueueParentBindings.filter((item) => !parentIds.has(item.id)), (item) => [
        item.id,
        item.teamId,
        item.runId,
        item.ownerId,
    ...
    await insertRows(connection, "prospect_run_queue_child_bindings", store.prospectRunQueueChildBindings.filter((item) => !childIds.has(item.id)), (item) => [
        item.id,
        item.teamId,
        item.runId,
        item.shardId,
...
async function syncProspectExecutionKernelState(connection, desired) {
    if (desired.length > 1) {
    ...
    if (!desired.length) {
        if (persisted.length)
            throw new Error("搜索执行内核状态不允许删除");
        return;
    }
    const state = desired[0];
    const existing = persisted[0];
    if (!existing) {
        await insertRows(connection, "search_execution_kernel_state", [state], (item) => [
            item.id,
            item.kernelEpoch,
            item.instanceId,
    ...
    if (isDeepStrictEqual(existing, state))
        return;
    if (state.id !== existing.id
        || state.kernelEpoch !== existing.kernelEpoch + 1) {
        throw new Error("搜索执行内核 epoch CAS 冲突");
    }
    const [result] = await connection.query(`UPDATE search_execution_kernel_state
...
function validateProspectCheckpointMutation(existing, checkpoint) {
    const epochAdvanced = checkpoint.runEpoch === existing.runEpoch + 1;
    const checkpointAdvanced = checkpoint.checkpointNo === existing.checkpointNo + 1;
    const pageAdvanced = checkpoint.pageSequence === existing.pageSequence + 1;
    const callAdvanced = checkpoint.totalCallCount === existing.totalCallCount + 1;
    const aggregatesUnchanged = checkpoint.acceptedCount === existing.acceptedCount
    ...
    if (checkpoint.checkpointCallCount < existing.checkpointCallCount
        ...
        || callAdvanced !== (checkpoint.checkpointCallCount === existing.checkpointCallCount + 1)) {
...
async function syncProspectExecutionCheckpoints(connection, desired) {
    ...
    for (const checkpoint of desired) {
        ...
        if (!existing) {
            await insertRows(connection, "prospect_execution_checkpoints", [checkpoint], (item) => [
                item.id,
                item.teamId,
                item.ownerId,
                item.runId,
        ...
        if (isDeepStrictEqual(existing, checkpoint))
            continue;
        ...
        const [result] = await connection.query(`UPDATE prospect_execution_checkpoints
...
function validateProspectLeaseMutation(existing, lease) {
...
async function syncProspectExecutionLeases(connection, desired) {
    ...
    for (const lease of desired) {
        ...
        if (!existing) {
            await insertRows(connection, "prospect_execution_leases", [lease], (item) => [
                item.id,
                item.teamId,
                item.ownerId,
                item.runId,
        ...
        if (isDeepStrictEqual(existing, lease))
            continue;
        ...
        const [result] = await connection.query(`UPDATE prospect_execution_leases
...
function validateProspectAttemptMutation(existing, attempt) {
...
async function syncProspectExecutionAttempts(connection, desired) {
    ...
    for (const attempt of desired) {
        ...
        if (!existing) {
            await insertRows(connection, "prospect_execution_attempts", [attempt], (item) => [
                item.id,
                item.teamId,
                item.ownerId,
                item.runId,
        ...
        if (isDeepStrictEqual(existing, attempt))
            continue;
        ...
        if (!immutableFieldsMatch || attempt.version !== existing.version + 1) {
            const changedFields = Object.keys(attempt).filter((field) => !isDeepStrictEqual(existing[field], attempt[field]));
            throw new Error(`搜索执行尝试 CAS 冲突或作用域被修改`
                + `（字段：${changedFields.join(",") || "无"}；`
                + `版本：${existing.version}->${attempt.version}）`);
        }
        validateProspectAttemptMutation(existing, attempt);
        const [result] = await connection.query(`UPDATE prospect_execution_attempts
...
function providerRequestLedgerImmutableFieldsMatch(existing, ledger) {
...
function providerRequestLedgerOneTimeFieldsPreserved(existing, ledger) {
    const stringFields = [
        "externalRequestId",
    ...
    if (stringFields.some((field) => Boolean(existing[field]) && ledger[field] !== existing[field])) {
        return false;
    }
    const clearsUnknownOutcomeError = existing.status === "outcome_unknown"
        && ledger.status === "response_received"
...
async function syncProspectProviderRequestLedgers(connection, desired) {
    const persisted = (await loadProspectExecutionState(connection)).providerRequestLedgers;
    const persistedById = new Map(persisted.map((item) => [item.id, item]));
    const desiredById = new Map(desired.map((item) => [item.id, item]));
    if (desiredById.size !== desired.length) {
    ...
    for (const ledger of desired) {
        ...
        if (!existing) {
            await insertRows(connection, "prospect_provider_request_ledgers", [ledger], (item) => [
                item.id,
                item.teamId,
                item.ownerId,
                item.runId,
        ...
        if (!providerRequestLedgerImmutableFieldsMatch(existing, ledger)
            ...
            || !isProspectProviderRequestTransitionAllowed(existing.status, ledger.status)) {
            throw new Error("Provider 请求账本 CAS 冲突或不可变证据被修改");
        }
        const [result] = await connection.query(`UPDATE prospect_provider_request_ledgers
...
function providerRequestDispatchImmutableFieldsMatch(existing, dispatch) {
...
async function syncProspectProviderRequestDispatches(connection, desired) {
    const persisted = (await loadProspectExecutionState(connection)).providerRequestDispatches;
    const persistedById = new Map(persisted.map((item) => [item.id, item]));
    const desiredById = new Map(desired.map((item) => [item.id, item]));
    if (desiredById.size !== desired.length) {
    ...
    for (const dispatch of desired) {
        ...
        if (!existing) {
            await insertRows(connection, "prospect_provider_request_dispatches", [dispatch], (item) => [
                item.id,
                item.ledgerId,
                item.teamId,
                item.ownerId,
        ...
        if (isDeepStrictEqual(existing, dispatch))
            continue;
        const recordsLateProviderResponse = existing.status === "outcome_unknown"
            && dispatch.status === "response_received";
        const oneTimeFieldsPreserved = (!existing.externalRequestId
        ...
        if (!providerRequestDispatchImmutableFieldsMatch(existing, dispatch)
            ...
            || !isProspectProviderDispatchTransitionAllowed(existing.status, dispatch.status)) {
            throw new Error("Provider 请求派发记录 CAS 冲突或证据被修改");
        }
        const [result] = await connection.query(`UPDATE prospect_provider_request_dispatches
...
async function appendProspectProviderRequestFacts(connection, store) {
    const persisted = await loadProspectExecutionState(connection);
    verifyImmutableRows(persisted.providerRequestAttemptBindings, store.prospectProviderRequestAttemptBindings, "Provider 请求执行尝试绑定");
    verifyImmutableRows(persisted.providerRequestEvents, store.prospectProviderRequestEvents, "Provider 请求状态事件");
    verifyImmutableRows(persisted.providerRequestAccountingEvidence, store.prospectProviderRequestAccountingEvidence, "Provider 请求成本证据");
    const bindingIds = new Set(persisted.providerRequestAttemptBindings.map((item) => item.id));
    const eventIds = new Set(persisted.providerRequestEvents.map((item) => item.id));
    const accountingIds = new Set(persisted.providerRequestAccountingEvidence.map((item) => item.id));
    await insertRows(connection, "prospect_provider_request_attempt_bindings", store.prospectProviderRequestAttemptBindings.filter((item) => !bindingIds.has(item.id)), (item) => [
        item.id,
        item.ledgerId,
        item.attemptId,
        item.teamId,
    ...
    await insertRows(connection, "prospect_provider_request_events", store.prospectProviderRequestEvents.filter((item) => !eventIds.has(item.id)), (item) => [
        item.id,
        item.ledgerId,
    ...
    await insertRows(connection, "prospect_provider_request_accounting_evidence", store.prospectProviderRequestAccountingEvidence.filter((item) => !accountingIds.has(item.id)), (item) => [
        item.id,
        item.ledgerId,
        item.teamId,
        item.ownerId,
...
async function appendProspectExecutionPagesAndEvents(connection, store) {
    const persisted = await loadProspectExecutionState(connection);
    verifyImmutableRows(persisted.pages, store.prospectExecutionPages, "搜索执行页摘要");
    verifyImmutableRows(persisted.events, store.prospectExecutionEvents, "搜索执行事件");
    const pageIds = new Set(persisted.pages.map((item) => item.id));
    const eventIds = new Set(persisted.events.map((item) => item.id));
    await insertRows(connection, "prospect_execution_pages", store.prospectExecutionPages.filter((item) => !pageIds.has(item.id)), (item) => [
        item.id,
        item.teamId,
        item.ownerId,
        item.runId,
    ...
    await insertRows(connection, "prospect_execution_events", store.prospectExecutionEvents.filter((item) => !eventIds.has(item.id)), (item) => [
        item.id,
        item.teamId,
        item.ownerId,
        item.runId,
...
async function appendProspectSourceRawRows(connection, store) {
    const persisted = await loadProspectExecutionState(connection);
    const desiredGroups = [
    ...
    for (const group of desiredGroups) {
        if (new Set(group.items.map((item) => item.id)).size
            !== group.items.length) {
            throw new ProspectSourceRawError("PROSPECT_SOURCE_RAW_CONFLICT", `${group.label}存在重复主键`);
        }
    }
    const recordIds = new Set(persisted.sourceRawRecords.map((item) => item.id));
    const batchIds = new Set(persisted.sourceRawBatches.map((item) => item.id));
    const hitIds = new Set(persisted.sourceRawHits.map((item) => item.id));
    try {
        await insertRows(connection, "prospect_source_raw_records", store.prospectSourceRawRecords.filter((item) => !recordIds.has(item.id)), (item) => [
            item.id,
            item.teamId,
            item.ownerId,
            item.providerCode,
        ...
        created_at)`);
        await insertRows(connection, "prospect_source_raw_batches", store.prospectSourceRawBatches.filter((item) => !batchIds.has(item.id)), (item) => [
            item.id,
            item.teamId,
            item.ownerId,
            item.runId,
        ...
        await insertRows(connection, "prospect_source_raw_hits", store.prospectSourceRawHits.filter((item) => !hitIds.has(item.id)), (item) => [
            item.id,
            item.batchId,
            item.recordId,
            item.teamId,
        ...
        if (isMysqlDuplicateKeyError(error)) {
            throw new ProspectSourceRawError("PROSPECT_SOURCE_RAW_CONFLICT", "Provider 原始事实数据库唯一约束或 CAS 冲突");
        }
        throw error;
    }
}
async function syncProspectStrategySourcePositions(connection, desired) {
    const persisted = (await loadProspectExecutionState(connection)).sourcePositions;
    const persistedById = new Map(persisted.map((item) => [item.id, item]));
    const desiredById = new Map(desired.map((item) => [item.id, item]));
    if (desiredById.size !== desired.length) {
    ...
    for (const position of desired) {
        ...
        if (!existing) {
            await insertRows(connection, "prospect_strategy_source_positions", [position], (item) => [
                item.id,
                item.identityHash,
                item.teamId,
                item.ownerId,
        ...
        if (isDeepStrictEqual(existing, position))
            continue;
        const immutableFieldsMatch = position.identityHash === existing.identityHash
        ...
        const [result] = await connection.query(`UPDATE prospect_strategy_source_positions
...
async function syncProspectExecutionThrottles(connection, desired) {
    const persisted = (await loadProspectExecutionState(connection)).throttleBuckets;
    const persistedById = new Map(persisted.map((item) => [item.id, item]));
    const desiredById = new Map(desired.map((item) => [item.id, item]));
    if (desiredById.size !== desired.length) {
    ...
    for (const throttle of desired) {
        ...
        if (!existing) {
            await insertRows(connection, "prospect_execution_throttles", [throttle], (item) => [
                item.id,
                item.teamId,
                item.providerCode,
                item.connectionId,
        ...
        if (isDeepStrictEqual(existing, throttle))
            continue;
        ...
        const [result] = await connection.query(`UPDATE prospect_execution_throttles
...
async function syncProspectSearchRunRows(connection, desired) {
    ...
    for (const run of desired) {
        ...
        if (!existing) {
            await insertRows(connection, "prospect_search_runs", [run], (item) => [
                item.id,
                item.teamId,
                item.campaignId,
                item.campaignVersion,
        ...
        const immutableFieldsMatch = run.teamId === existing.teamId
            ...
            && isDeepStrictEqual(run.executionSnapshot, existing.executionSnapshot)
        ...
        const [result] = await connection.query(`UPDATE prospect_search_runs
...
async function syncProspectRunShardRows(connection, desired) {
    ...
    for (const shard of desired) {
        ...
        if (!existing) {
            await insertRows(connection, "prospect_run_shards", [shard], (item) => [
                item.id,
                item.teamId,
                item.runId,
                item.providerCode,
        ...
        if (isDeepStrictEqual(existing, shard))
            continue;
        ...
        const [result] = await connection.query(`UPDATE prospect_run_shards
...
async function appendProspectRunEventRows(connection, desired) {
    ...
    try {
        await insertRows(connection, "prospect_run_events", desired.filter((item) => !persistedById.has(item.id)), (item) => [
            item.id,
            item.teamId,
            item.runId,
            item.sequence,
...
async function appendMarketOpportunityRows(connection, store) {
    ...
    verifyImmutableRows(persistedBatches, store.marketOpportunityBatches, "市场机会事实批次");
    verifyImmutableRows(persistedSnapshots, store.marketOpportunitySnapshots, "市场机会事实快照");
    verifyImmutableRows(persistedEvents, store.marketOpportunityCalculationEvents, "市场机会计算事件");
    ...
    const persistedEventIds = new Set(persistedEvents.map((item) => item.id));
    await insertRows(connection, "market_opportunity_batches", store.marketOpportunityBatches.filter((item) => !persistedBatchIds.has(item.id)), (item) => [
        item.id,
        item.teamId,
        item.ownerId,
        item.campaignId,
    ...
    await insertRows(connection, "market_opportunity_snapshots", store.marketOpportunitySnapshots.filter((item) => !persistedSnapshotIds.has(item.id)), (item) => [
        item.id,
        item.batchId,
        item.teamId,
        item.ownerId,
    ...
    await insertRows(connection, "market_opportunity_calculation_events", store.marketOpportunityCalculationEvents.filter((item) => !persistedEventIds.has(item.id)), (item) => [
        item.id,
        item.teamId,
        item.ownerId,
        item.campaignId,
...
function migrateLegacyLeadSourceConfigs(store) {
    let migrated = 0;
    for (const legacy of store.leadSourceConfigs) {
        const exists = store.providerConnections.some((item) => item.providerId === legacy.provider
