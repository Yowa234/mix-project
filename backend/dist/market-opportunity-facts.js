...
function reporterKey(item) {
    return normalized(item.reporterCode);
}
function sourceObservations(store, scope) {
    return store.marketTradeObservations.filter((item) => item.teamId === scope.teamId
...
export function currentMarketOpportunityDatasetFingerprint(store, scope) {
    return marketOpportunityDatasetFingerprint(sourceObservations(store, scope));
}
export function marketOpportunityDatasetFingerprint(observations) {
...
function matchingPartner(observations, reporter, period, partnerCode) {
    return observations
        .filter((item) => reporterKey(item) === reporterKey(reporter)
        && item.period === period
...
function selectEvidence(candidates, kind) {
...
function calculateSnapshot(input) {
    ...
    if (!periods.length) {
        ...
        for (const period of periods) {
            const selected = selectEvidence(matchingPartner(input.group.observations, input.reporter, period, WORLD_PARTNER_CODE), "world");
            if (!selected.item) {
    ...
    if (metrics.reportedImportValueSeries.length === 3) {
        ...
        const china = selectEvidence(matchingPartner(input.group.observations, input.reporter, input.comparisonPeriod, CHINA_MAINLAND_PARTNER_CODE), "china");
        if (!china.item) {
...
function buildGroups(observations) {
    ...
    return [...groups.values()].sort((left, right) => left.classification.localeCompare(right.classification)
        || left.commodityCode.localeCompare(right.commodityCode));
}
function comparisonPeriod(group) {
    const currentYear = new Date().getUTCFullYear();
    return group.observations
        .filter((item) => item.partnerCode === WORLD_PARTNER_CODE
        && isUsableValue(item)
        && Number(item.period) < currentYear)
        .map((item) => item.period)
        .sort((left, right) => right.localeCompare(left))[0] || "";
}
export function materializeMarketOpportunityFacts(store, scope) {
    ...
    const existingBatch = store.marketOpportunityBatches.find((item) => item.teamId === scope.teamId
        && item.ownerId === scope.ownerId
        && item.campaignId === scope.campaignId
        && item.providerId === PROVIDER_ID
    ...
    if (!batch) {
        ...
        const candidates = groups.flatMap((group) => {
            ...
            for (const item of group.observations) {
                if (!reporters.has(reporterKey(item)))
                    reporters.set(reporterKey(item), item);
            }
            const commonPeriod = comparisonPeriod(group);
            return [...reporters.values()]
                .sort((left, right) => left.reporterCountry.localeCompare(right.reporterCountry)
        ...
        if (!batch.emptyReason && !batch.readyCount) {
            batch.emptyReason = snapshots.some((item) => item.insufficiencyReasons.includes("non_contiguous_three_year_series"))
    ...
    const event = {
        ...
        sequence: Math.max(0, ...store.marketOpportunityCalculationEvents.map((item) => item.sequence)) + 1,
        calculatedAt
    };
