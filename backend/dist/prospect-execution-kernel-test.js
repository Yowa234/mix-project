...
import { ProspectExecutionKernel, ProspectExecutionKernelError } from "./prospect-execution-kernel.js";
import { DeterministicFakeProspectProvider } from "./prospect-fake-provider.js";
import { prospectProviderResponseComponentHashes, prospectProviderResponseHash } from "./prospect-provider-request-ledger.js";
import { registerProspectRunQueueBridge, validateProspectRunQueueBridge } from "./prospect-run-queue-bridge.js";
...
const original = {
    ...
    sourcePositions: structuredClone(store.prospectStrategySourcePositions),
    leases: structuredClone(store.prospectExecutionLeases),
    attempts: structuredClone(store.prospectExecutionAttempts),
    providerRequestLedgers: structuredClone(store.prospectProviderRequestLedgers),
    providerRequestDispatches: structuredClone(store.prospectProviderRequestDispatches),
    providerRequestEvents: structuredClone(store.prospectProviderRequestEvents),
    providerRequestAttemptBindings: structuredClone(store.prospectProviderRequestAttemptBindings),
    providerRequestAccountingEvidence: structuredClone(store.prospectProviderRequestAccountingEvidence),
...
class DelayedFakeProspectProvider extends DeterministicFakeProspectProvider {
    delayMs;
    constructor(scripts, delayMs) {
        super(scripts);
...
class InspectingFakeProspectProvider extends DeterministicFakeProspectProvider {
    inspect;
    constructor(scripts, inspect) {
        super(scripts);
...
function owner() {
    const existing = store.users.find((item) => item.status === "active"
...
function snapshot(providerCodes) {
    const query = {
...
function createRun(runOwner, providerCodes, sourceSnapshot) {
    ...
    const run = {
        ...
        executionSnapshot,
        executionSnapshotHash: prospectRunExecutionSnapshotHash(executionSnapshot),
...
function repeatedRun(runOwner, sourceRun, mutateSnapshot) {
    const executionSnapshot = structuredClone(sourceRun.executionSnapshot);
    mutateSnapshot?.(executionSnapshot);
    return createRun(runOwner, executionSnapshot.providerPlan.map((item) => item.providerCode), executionSnapshot);
}
try {
    clearExecutionState();
    const runOwner = owner();
    const run = createRun(runOwner, ["fake.alpha", "fake.beta"]);
    assert.throws(() => new ProspectExecutionKernel({
        store,
        workerId: "worker-1",
        allowedRunIds: [],
        claimSecret: "s".repeat(32)
    }), (error) => error instanceof ProspectExecutionKernelError
    ...
    assert.equal(store.prospectExecutionLeases[0]?.claimTokenHmac.includes(first.claimToken), false);
    assert.equal(await kernel.claimNext("2026-07-14T00:00:03.000Z"), null, "同一 Run 有活动租约时不能领取第二个子任务");
    ...
    await assert.rejects(kernel.heartbeat({
        leaseId: first.lease.id,
    ...
    assert.equal(store.agentJobs.find((item) => item.id === first.job.id)?.status, "queued");
    const reclaimed = await restarted.claimNext("2026-07-14T00:00:07.000Z");
    assert.ok(reclaimed);
    ...
    assert.equal((await executionKernel.executeNext(provider, "2026-07-14T00:00:09.000Z")).kind, "success");
    const alphaCheckpoint = store.prospectExecutionCheckpoints.find((item) => item.runId === run.id && item.providerCode === "fake.alpha");
    ...
    assert.equal(alphaCheckpoint.encryptedCursor.includes("secret-alpha-cursor"), false);
    assert.equal((await executionKernel.executeNext(provider, "2026-07-14T00:00:10.000Z")).kind, "success");
    assert.equal(store.prospectRunShards.find((item) => item.runId === run.id && item.providerCode === "fake.alpha")?.status, "succeeded");
    const firstBeta = await executionKernel.executeNext(provider, "2026-07-14T00:00:11.000Z");
    assert.equal(firstBeta.kind, "failure");
    assert.equal(store.prospectExecutionCheckpoints.find((item) => item.runId === run.id && item.providerCode === "fake.beta")?.checkpointCallCount, 1);
    assert.equal((await executionKernel.executeNext(provider, "2026-07-14T00:00:12.000Z")).kind, "idle", "retryAfterAt 到达前不能再次调用 Provider");
    assert.equal((await executionKernel.executeNext(provider, "2026-07-14T00:00:20.000Z")).kind, "failure");
    assert.equal((await executionKernel.executeNext(provider, "2026-07-14T00:00:30.000Z")).kind, "success");
    const betaCheckpoint = store.prospectExecutionCheckpoints.find((item) => item.runId === run.id && item.providerCode === "fake.beta");
    ...
    assert.equal(store.prospectExecutionPages.reduce((sum, item) => sum + item.acceptedCount, 0), 3);
    assert.equal(store.prospectExecutionAttempts.filter((item) => item.providerAttemptNo > 0).length, 5);
    assert.equal(JSON.stringify(store.prospectExecutionPages).includes("secret-alpha-cursor"), false);
    assert.doesNotThrow(() => validateProspectRunQueueBridge(store, run));
    ...
    const prepareClaim = await prepareKernel.claimNext("2026-07-14T00:10:01.000Z");
    assert.ok(prepareClaim);
    const prepareCheckpoint = store.prospectExecutionCheckpoints.find((item) => item.runId === prepareRun.id);
    const prepareJob = store.agentJobs.find((item) => item.id === prepareClaim.job.id);
    const throttleCountBeforePrepare = store.prospectExecutionThrottleBuckets.length;
    ...
    assert.equal(prepared.dispatchRequest.endpointCode, "company-search");
    assert.equal(store.prospectProviderRequestLedgers.filter((item) => item.runId === prepareRun.id).length, 1);
    assert.equal(store.prospectProviderRequestAttemptBindings.filter((item) => item.ledgerId === prepared.ledger.id).length, 1);
    assert.deepEqual(store.prospectProviderRequestEvents
        .filter((item) => item.ledgerId === prepared.ledger.id)
        .map((item) => [item.sequence, item.eventType]), [[1, "prepared"]]);
    assert.equal(prepareProvider.listPhysicalCalls().length, 0);
    assert.equal(prepareClaim.attempt.status, "claimed");
    assert.equal(store.prospectExecutionAttempts.find((item) => item.id === prepareClaim.attempt.id)?.status, "claimed");
    assert.equal(store.prospectExecutionLeases.find((item) => item.id === prepareClaim.lease.id)?.requestStartedAt, "");
    ...
    assert.equal(store.prospectExecutionThrottleBuckets.length, throttleCountBeforePrepare);
    assert.equal(prepared.ledger.encryptedRequestEnvelope.includes("industrial lighting"), false);
    ...
    assert.equal(store.prospectProviderRequestLedgers.filter((item) => item.runId === prepareRun.id).length, 1);
    assert.equal(store.prospectProviderRequestAttemptBindings.filter((item) => item.ledgerId === prepared.ledger.id).length, 1);
    assert.equal(store.prospectProviderRequestEvents.filter((item) => item.ledgerId === prepared.ledger.id).length, 1);
    await prepareKernel.requestPause(prepareRun.id, "2026-07-14T00:10:04.000Z");
    assert.equal(prepareRun.status, "paused");
    assert.equal(store.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id)?.status, "prepared");
    await assert.rejects(prepareKernel.dispatchPreparedProviderRequest(prepareProvider, {
        leaseId: prepareClaim.lease.id,
        claimToken: prepareClaim.claimToken,
    ...
    assert.equal(prepareProvider.listPhysicalCalls().length, 0);
    await prepareKernel.resume(prepareRun.id, "2026-07-14T00:10:05.000Z");
    const resumedPrepareClaim = await prepareKernel.claimNext("2026-07-14T00:10:06.000Z");
    ...
    assert.deepEqual(store.prospectProviderRequestAttemptBindings
        .filter((item) => item.ledgerId === prepared.ledger.id)
        .map((item) => item.bindingNo), [1, 2]);
    await prepareKernel.requestCancel(prepareRun.id, "2026-07-14T00:10:08.000Z");
    const cancelledPreparedLedger = store.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id);
    assert.equal(cancelledPreparedLedger.status, "settled");
    assert.equal(cancelledPreparedLedger.settlementKind, "cancelled_before_dispatch");
    ...
    await assert.rejects(prepareKernel.dispatchPreparedProviderRequest(prepareProvider, {
        ...
        && error.code === "EXECUTION_LEASE_NOT_ACTIVE");
    assert.equal(prepareProvider.listPhysicalCalls().length, 0);
    const cursorPrepareRun = createRun(runOwner, ["fake.prepare-cursor"]);
    ...
    assert.equal((await cursorPrepareKernel.executeNext(cursorPrepareProvider, "2026-07-14T00:20:01.000Z")).kind, "success");
    const cursorPrepareClaim = await cursorPrepareKernel.claimNext("2026-07-14T00:20:02.000Z");
    assert.ok(cursorPrepareClaim);
    const cursorCheckpoint = store.prospectExecutionCheckpoints.find((item) => item.runId === cursorPrepareRun.id);
    const cursorJob = store.agentJobs.find((item) => item.id === cursorPrepareClaim.job.id);
    ...
    const cursorPrepared = await cursorPrepareKernel.prepareProviderRequest({
        leaseId: cursorPrepareClaim.lease.id,
        claimToken: cursorPrepareClaim.claimToken,
        now: "2026-07-14T00:20:03.000Z"
    });
    assert.equal(cursorPrepared.providerRequest.cursor, "prepare-private-cursor");
    assert.equal(cursorPrepared.ledger.encryptedRequestEnvelope.includes("prepare-private-cursor"), false);
    assert.equal(cursorPrepared.ledger.encryptedRequestEnvelope.includes("industrial lighting"), false);
    assert.deepEqual({
        total: cursorCheckpoint.totalCallCount,
        checkpoint: cursorCheckpoint.checkpointCallCount,
        attempts: cursorJob.attemptCount
    ...
    await cursorPrepareKernel.requestCancel(cursorPrepareRun.id, "2026-07-14T00:20:04.000Z");
    const prepareRollbackRun = createRun(runOwner, ["fake.prepare-rollback"]);
    ...
    const prepareRollbackClaim = await prepareRollbackKernel.claimNext("2026-07-14T00:30:01.000Z");
    ...
    try {
        await assert.rejects(prepareRollbackKernel.prepareProviderRequest({
    ...
    assert.deepEqual(executionStateSnapshot(), beforePreparePersistenceFailure);
    await prepareRollbackKernel.requestCancel(prepareRollbackRun.id, "2026-07-14T00:30:03.000Z");
    const dispatchRollbackRun = createRun(runOwner, ["fake.dispatch-rollback"]);
    ...
    const dispatchRollbackClaim = await dispatchRollbackKernel.claimNext("2026-07-14T00:31:01.000Z");
    assert.ok(dispatchRollbackClaim);
    const dispatchRollbackPrepared = await dispatchRollbackKernel.prepareProviderRequest({
        leaseId: dispatchRollbackClaim.lease.id,
        claimToken: dispatchRollbackClaim.claimToken,
        now: "2026-07-14T00:31:02.000Z"
    });
    const dispatchRollbackProvider = new DeterministicFakeProspectProvider({
    ...
    try {
        await assert.rejects(dispatchRollbackKernel.dispatchPreparedProviderRequest(dispatchRollbackProvider, {
            leaseId: dispatchRollbackClaim.lease.id,
            claimToken: dispatchRollbackClaim.claimToken,
    ...
    assert.deepEqual(executionStateSnapshot(), beforeDispatchPersistenceFailure);
    assert.equal(dispatchRollbackProvider.listPhysicalCalls().length, 0);
    await dispatchRollbackKernel.requestCancel(dispatchRollbackRun.id, "2026-07-14T00:31:04.000Z");
    const dispatchRun = createRun(runOwner, ["fake.dispatch-confirmed"]);
    ...
    const dispatchClaim = await dispatchKernel.claimNext("2026-07-14T00:32:01.000Z");
    assert.ok(dispatchClaim);
    const dispatchPrepared = await dispatchKernel.prepareProviderRequest({
        leaseId: dispatchClaim.lease.id,
        claimToken: dispatchClaim.claimToken,
        now: "2026-07-14T00:32:02.000Z"
    });
    const dispatchCheckpoint = store.prospectExecutionCheckpoints.find((item) => item.runId === dispatchRun.id);
    const dispatchJob = store.agentJobs.find((item) => item.id === dispatchClaim.job.id);
    ...
    const dispatchProvider = new InspectingFakeProspectProvider({
        ...
        const persistedLedger = store.prospectProviderRequestLedgers.find((item) => item.id === dispatchPrepared.ledger.id);
        const persistedDispatch = store.prospectProviderRequestDispatches.find((item) => item.ledgerId === dispatchPrepared.ledger.id);
        const persistedAttempt = store.prospectExecutionAttempts.find((item) => item.id === dispatchClaim.attempt.id);
        const persistedLease = store.prospectExecutionLeases.find((item) => item.id === dispatchClaim.lease.id);
        ...
        assert.equal(persistedLease?.requestStartedAt, "2026-07-14T00:32:03.000Z");
        observedDurableDispatchStart = true;
    });
    const dispatchResult = await dispatchKernel.dispatchPreparedProviderRequest(dispatchProvider, {
        leaseId: dispatchClaim.lease.id,
        claimToken: dispatchClaim.claimToken,
    ...
    assert.equal(dispatchProvider.listPhysicalCalls()
        .filter((item) => item.providerExecuted).length, 1);
    const confirmedLedger = store.prospectProviderRequestLedgers.find((item) => item.id === dispatchPrepared.ledger.id);
    const confirmedDispatch = store.prospectProviderRequestDispatches.find((item) => item.ledgerId === dispatchPrepared.ledger.id);
    ...
    assert.equal(confirmedLedger.externalRequestId, confirmedDispatch.externalRequestId);
    assert.equal(confirmedLedger.responseHash, dispatchResult.response.responseHash);
    assert.equal(confirmedLedger.rawResponseHash, dispatchResult.response.rawResponseHash);
    assert.equal(confirmedLedger.normalizedResultHash, dispatchResult.response.normalizedResultHash);
    assert.equal(confirmedLedger.responseAccountingEvidenceHash, dispatchResult.response.accountingEvidenceHash);
    assert.equal(confirmedLedger.httpStatus, 200);
    assert.equal(confirmedLedger.providerOutcomeCode, "SUCCESS");
    assert.match(confirmedLedger.encryptedResponseEnvelope, /^provider-response-v1\./);
    assert.match(confirmedLedger.responseEvidenceRef, /^sha256:[a-f0-9]{64}$/);
    assert.equal(confirmedLedger.encryptedResponseEnvelope.includes("requestUnits"), false);
    assert.equal(confirmedLedger.encryptedResponseEnvelope.includes("USD"), false);
    assert.equal(confirmedDispatch.responseHash, confirmedLedger.responseHash);
    assert.ok(confirmedLedger.dispatchConfirmedAt
        < confirmedLedger.responseReceivedAt);
    assert.equal(store.prospectExecutionLeases.find((item) => item.id === dispatchClaim.lease.id)?.releaseReason, "RESPONSE_RECEIVED_PENDING_SETTLEMENT");
    const pendingAttempt = store.prospectExecutionAttempts.find((item) => item.id === dispatchClaim.attempt.id);
    ...
    assert.equal(store.prospectExecutionPages.some((item) => item.runId === dispatchRun.id), false);
    assert.equal(store.prospectProviderRequestAccountingEvidence.some((item) => item.ledgerId === confirmedLedger.id), false);
    assert.deepEqual({
        total: dispatchCheckpoint.totalCallCount,
        checkpoint: dispatchCheckpoint.checkpointCallCount,
        attempts: dispatchJob.attemptCount
    ...
    const replayedPersisted = await dispatchKernel.persistPreparedProviderResponse({
        leaseId: dispatchClaim.lease.id,
        claimToken: dispatchClaim.claimToken,
        ledgerId: dispatchPrepared.ledger.id,
    ...
    const conflictingHashes = prospectProviderResponseComponentHashes(conflictingStep);
    ...
    await assert.rejects(dispatchKernel.persistPreparedProviderResponse({
        leaseId: dispatchClaim.lease.id,
        claimToken: dispatchClaim.claimToken,
        ledgerId: dispatchPrepared.ledger.id,
        response: {
    ...
    await assert.rejects(dispatchKernel.persistPreparedProviderResponse({
        ...
        && error.code === "EXECUTION_CLAIM_TOKEN_INVALID");
    const originalResponseEnvelope = confirmedLedger.encryptedResponseEnvelope;
    confirmedLedger.encryptedResponseEnvelope =
        `${originalResponseEnvelope.slice(0, -1)}${originalResponseEnvelope.endsWith("A") ? "B" : "A"}`;
    await assert.rejects(dispatchKernel.persistPreparedProviderResponse({
        leaseId: dispatchClaim.lease.id,
        claimToken: dispatchClaim.claimToken,
        ledgerId: dispatchPrepared.ledger.id,
    ...
    const originalProviderOutcomeCode = confirmedLedger.providerOutcomeCode;
    ...
    assert.equal(dispatchProvider.listPhysicalCalls().length, 1);
    const replayedAfterRestart = await dispatchRecoveryKernel.dispatchPreparedProviderRequest(dispatchProvider, {
        leaseId: dispatchClaim.lease.id,
        claimToken: dispatchClaim.claimToken,
        ledgerId: dispatchPrepared.ledger.id,
    ...
    assert.equal(dispatchProvider.listPhysicalCalls().length, 1);
    assert.equal(await dispatchRecoveryKernel.claimNext("2026-07-14T00:32:06.000Z"), null);
    await assert.rejects(dispatchRecoveryKernel.completePage({
        leaseId: dispatchClaim.lease.id,
        claimToken: dispatchClaim.claimToken,
        ...
        && error.code === "EXECUTION_LEASE_NOT_ACTIVE");
    await assert.rejects(dispatchRecoveryKernel.settlePersistedProviderResponse({
        teamId: "other-team",
    ...
    const responseLeaseBeforeSettlement = structuredClone(store.prospectExecutionLeases.find((item) => item.id === dispatchClaim.lease.id));
    const successfulSettlement = await dispatchRecoveryKernel.settlePersistedProviderResponse({
        teamId: confirmedLedger.teamId,
        ownerId: confirmedLedger.ownerId,
        runId: confirmedLedger.runId,
        ledgerId: confirmedLedger.id,
    ...
    assert.match(successfulSettlement.ledger.settlementHash, /^[a-f0-9]{64}$/);
    assert.equal(successfulSettlement.attempt.status, "succeeded");
    assert.equal(successfulSettlement.accountingEvidence.provenance, "provider_reported");
    ...
    assert.equal(successfulSettlement.rawBatch?.batch.ledgerId, confirmedLedger.id);
    assert.equal(successfulSettlement.rawBatch?.batch.recordCount, 2);
    assert.equal(dispatchRun.status, "succeeded");
    assert.deepEqual(store.prospectExecutionLeases.find((item) => item.id === dispatchClaim.lease.id), responseLeaseBeforeSettlement);
    assert.deepEqual({
        leads: store.leads.length,
        customers: store.customers.length,
        deals: store.deals.length,
    ...
    const rawBatch = await dispatchRecoveryKernel.persistSettledProviderRawBatch({
        teamId: confirmedLedger.teamId,
        ownerId: confirmedLedger.ownerId,
        runId: confirmedLedger.runId,
        ledgerId: confirmedLedger.id,
    ...
    assert.deepEqual(rawBatch.hits.map((item) => item.ordinal), [1, 2]);
    assert.equal(rawBatch.records.some((item) => item.encryptedEnvelope.includes("Example Industrial Lighting")), false);
    assert.deepEqual({
        leads: store.leads.length,
        customers: store.customers.length,
        deals: store.deals.length,
    ...
    const replayedRawBatch = await dispatchRecoveryKernel.persistSettledProviderRawBatch({
        teamId: confirmedLedger.teamId,
        ownerId: confirmedLedger.ownerId,
        runId: confirmedLedger.runId,
        ledgerId: confirmedLedger.id,
    ...
    const storedRawBatch = store.prospectSourceRawBatches.find((item) => item.id === rawBatch.batch.id);
    ...
    await assert.rejects(dispatchRecoveryKernel.persistSettledProviderRawBatch({
        teamId: confirmedLedger.teamId,
        ownerId: confirmedLedger.ownerId,
        runId: confirmedLedger.runId,
        ledgerId: confirmedLedger.id,
    ...
    const storedRawRecord = store.prospectSourceRawRecords.find((item) => item.id === rawBatch.records[0]?.id);
    ...
    const storedRawHit = store.prospectSourceRawHits.find((item) => item.id === rawBatch.hits[0]?.id);
    ...
    const replayedSettlement = await dispatchRecoveryKernel.settlePersistedProviderResponse({
        teamId: confirmedLedger.teamId,
        ownerId: confirmedLedger.ownerId,
        runId: confirmedLedger.runId,
        ledgerId: confirmedLedger.id,
    ...
    assert.doesNotThrow(() => validateProspectRunQueueBridge(store, dispatchRun));
    const rawRollbackRun = createRun(runOwner, ["fake.dispatch-raw-rollback"]);
    ...
    const rawRollbackClaim = await rawRollbackKernel.claimNext("2026-07-14T00:32:08.200Z");
    assert.ok(rawRollbackClaim);
    const rawRollbackPrepared = await rawRollbackKernel.prepareProviderRequest({
    ...
    const rawRollbackResponse = await rawRollbackKernel.dispatchPreparedProviderRequest(rawRollbackProvider, {
        leaseId: rawRollbackClaim.lease.id,
        claimToken: rawRollbackClaim.claimToken,
    ...
    try {
        await assert.rejects(rawRollbackKernel.settlePersistedProviderResponse({
    ...
    assert.equal(store.prospectSourceRawBatches.some((item) => item.ledgerId === rawRollbackPrepared.ledger.id), false);
    const rawRollbackSettlement = await rawRollbackKernel.settlePersistedProviderResponse({
        teamId: rawRollbackPrepared.ledger.teamId,
        ownerId: rawRollbackPrepared.ledger.ownerId,
        runId: rawRollbackPrepared.ledger.runId,
        ledgerId: rawRollbackPrepared.ledger.id,
    ...
    assert.equal(store.prospectSourceRawBatches.filter((item) => item.ledgerId === rawRollbackPrepared.ledger.id).length, 1);
    const emptyRawRun = createRun(runOwner, ["fake.dispatch-empty-raw"]);
    ...
    const emptyRawClaim = await emptyRawKernel.claimNext("2026-07-14T00:32:09.100Z");
    ...
    const emptyRawResponse = await emptyRawKernel.dispatchPreparedProviderRequest(emptyRawProvider, {
        leaseId: emptyRawClaim.lease.id,
        claimToken: emptyRawClaim.claimToken,
    ...
    const emptyRawSettlement = await emptyRawKernel.settlePersistedProviderResponse({
    ...
    const missingRawPolicyRun = createRun(runOwner, ["fake.dispatch-missing-raw-policy"]);
    ...
    const missingRawPolicyClaim = await missingRawPolicyKernel.claimNext("2026-07-14T00:32:09.600Z");
    assert.ok(missingRawPolicyClaim);
    const missingRawPolicyPrepared = await missingRawPolicyKernel.prepareProviderRequest({
        leaseId: missingRawPolicyClaim.lease.id,
        claimToken: missingRawPolicyClaim.claimToken,
        now: "2026-07-14T00:32:09.700Z"
    });
    const missingRawPolicyProvider = new DeterministicFakeProspectProvider({
    ...
    const missingRawPolicyResponse = await missingRawPolicyKernel.dispatchPreparedProviderRequest(missingRawPolicyProvider, {
        leaseId: missingRawPolicyClaim.lease.id,
        claimToken: missingRawPolicyClaim.claimToken,
    ...
    await assert.rejects(missingRawPolicyKernel.settlePersistedProviderResponse({
        ...
        ledgerId: missingRawPolicyPrepared.ledger.id,
        expectedResponseHash: missingRawPolicyResponse.response.responseHash,
        now: "2026-07-14T00:32:09.900Z"
    }), (error) => error instanceof ProspectExecutionKernelError
        && error.code === "EXECUTION_PROVIDER_RAW_POLICY_REQUIRED");
    assert.deepEqual(executionStateSnapshot(), beforeMissingRawPolicySettlement);
    assert.equal(store.prospectSourceRawBatches.some((item) => item.ledgerId === missingRawPolicyPrepared.ledger.id), false);
    const failureResponseRun = createRun(runOwner, ["fake.dispatch-failure-response"]);
    ...
    const failureResponseClaim = await failureResponseKernel.claimNext("2026-07-14T00:32:11.000Z");
    assert.ok(failureResponseClaim);
    const failureResponsePrepared = await failureResponseKernel.prepareProviderRequest({
        leaseId: failureResponseClaim.lease.id,
        claimToken: failureResponseClaim.claimToken,
        now: "2026-07-14T00:32:12.000Z"
    });
    const failureResponseProvider = new DeterministicFakeProspectProvider({
    ...
    const failureResponseResult = await failureResponseKernel.dispatchPreparedProviderRequest(failureResponseProvider, {
        leaseId: failureResponseClaim.lease.id,
        claimToken: failureResponseClaim.claimToken,
    ...
    const failureResponseLedger = store.prospectProviderRequestLedgers.find((item) => item.id === failureResponsePrepared.ledger.id);
    assert.equal(failureResponseLedger.status, "response_received");
    assert.equal(failureResponseLedger.providerOutcomeCode, "PROVIDER_RATE_LIMITED");
    assert.equal(failureResponseLedger.httpStatus, 503);
    assert.match(failureResponseLedger.encryptedResponseEnvelope, /^provider-response-v1\./);
    assert.equal(store.prospectExecutionAttempts.find((item) => item.id === failureResponseClaim.attempt.id)?.status, "request_started");
    assert.equal(failureResponseRun.status, "running");
    assert.equal(store.prospectExecutionPages.some((item) => item.runId === failureResponseRun.id), false);
    assert.equal(store.prospectProviderRequestAccountingEvidence.some((item) => item.ledgerId === failureResponseLedger.id), false);
    const failureSettlement = await failureResponseKernel.settlePersistedProviderResponse({
    ...
    assert.equal(failureSettlement.retryAfterAt, "2026-07-14T00:33:00.000Z");
    ...
    assert.equal(failureSettlement.accountingEvidence.provenance, "provider_reported");
    assert.equal(store.prospectExecutionPages.some((item) => item.runId === failureResponseRun.id), false);
    assert.equal(store.prospectRunShards.find((item) => item.runId === failureResponseRun.id)?.status, "retry_scheduled");
    assert.equal(failureSettlement.rawBatch, undefined);
    assert.equal(store.prospectSourceRawBatches.some((item) => item.ledgerId === failureResponseLedger.id), false);
    const terminalFailureRun = createRun(runOwner, ["fake.dispatch-terminal-failure"]);
    ...
    const terminalFailureClaim = await terminalFailureKernel.claimNext("2026-07-14T00:32:21.000Z");
    assert.ok(terminalFailureClaim);
    const terminalFailurePrepared = await terminalFailureKernel.prepareProviderRequest({
        leaseId: terminalFailureClaim.lease.id,
        claimToken: terminalFailureClaim.claimToken,
        now: "2026-07-14T00:32:22.000Z"
    });
    const terminalFailureProvider = new DeterministicFakeProspectProvider({
    ...
    const terminalFailureResponse = await terminalFailureKernel.dispatchPreparedProviderRequest(terminalFailureProvider, {
        leaseId: terminalFailureClaim.lease.id,
        claimToken: terminalFailureClaim.claimToken,
    ...
    const terminalFailureSettlement = await terminalFailureKernel.settlePersistedProviderResponse({
    ...
    assert.equal(terminalFailureSettlement.accountingEvidence.provenance, "estimated");
    assert.equal(terminalFailureSettlement.accountingEvidence.estimationMethodVersion, "fake-provider-estimate-v1");
    const pendingPauseRun = createRun(runOwner, ["fake.dispatch-pending-pause"]);
    ...
    const pendingPauseClaim = await pendingPauseKernel.claimNext("2026-07-14T00:32:31.000Z");
    assert.ok(pendingPauseClaim);
    const pendingPausePrepared = await pendingPauseKernel.prepareProviderRequest({
        leaseId: pendingPauseClaim.lease.id,
        claimToken: pendingPauseClaim.claimToken,
        now: "2026-07-14T00:32:32.000Z"
    });
    const pendingPauseProvider = new DeterministicFakeProspectProvider({
    ...
    const pendingPauseResponse = await pendingPauseKernel.dispatchPreparedProviderRequest(pendingPauseProvider, {
        leaseId: pendingPauseClaim.lease.id,
        claimToken: pendingPauseClaim.claimToken,
    ...
    await pendingPauseKernel.requestPause(pendingPauseRun.id, "2026-07-14T00:32:34.000Z");
    assert.equal(pendingPauseRun.status, "pause_requested");
    assert.equal(store.prospectRunShards.find((item) => item.runId === pendingPauseRun.id)?.status, "pause_requested");
    const pendingPauseSettlement = await pendingPauseKernel.settlePersistedProviderResponse({
    ...
    assert.equal(store.prospectSourceRawBatches.some((item) => item.ledgerId === pendingPausePrepared.ledger.id), false);
    assert.equal(pendingPauseRun.status, "paused");
    assert.equal(store.prospectRunShards.find((item) => item.runId === pendingPauseRun.id)?.status, "paused");
    const startedRecoveryRun = createRun(runOwner, ["fake.dispatch-started-recovery"]);
    ...
    const startedRecoveryClaim = await startedRecoveryKernel.claimNext("2026-07-14T00:33:01.000Z");
    assert.ok(startedRecoveryClaim);
    const startedRecoveryPrepared = await startedRecoveryKernel.prepareProviderRequest({
        leaseId: startedRecoveryClaim.lease.id,
        claimToken: startedRecoveryClaim.claimToken,
        now: "2026-07-14T00:33:02.000Z"
    });
    const startedRecoveryProvider = new DeterministicFakeProspectProvider({
    ...
    const durableStart = await startedRecoveryKernel.startPreparedProviderDispatch({
        leaseId: startedRecoveryClaim.lease.id,
        claimToken: startedRecoveryClaim.claimToken,
    ...
    const unknownStartedLedger = store.prospectProviderRequestLedgers.find((item) => item.id === startedRecoveryPrepared.ledger.id);
    const unknownStartedDispatch = store.prospectProviderRequestDispatches.find((item) => item.ledgerId === startedRecoveryPrepared.ledger.id);
    ...
    const concurrentDispatchRun = createRun(runOwner, ["fake.dispatch-concurrent"]);
    ...
    const concurrentDispatchClaim = await concurrentDispatchKernel.claimNext("2026-07-14T00:34:01.000Z");
    assert.ok(concurrentDispatchClaim);
    const concurrentDispatchPrepared = await concurrentDispatchKernel.prepareProviderRequest({
        leaseId: concurrentDispatchClaim.lease.id,
        claimToken: concurrentDispatchClaim.claimToken,
        now: "2026-07-14T00:34:02.000Z"
    });
    const concurrentDispatchProvider = new DeterministicFakeProspectProvider({
    ...
    const concurrentDispatchResults = await Promise.allSettled([
        concurrentDispatchKernel.dispatchPreparedProviderRequest(concurrentDispatchProvider, {
            leaseId: concurrentDispatchClaim.lease.id,
            claimToken: concurrentDispatchClaim.claimToken,
    ...
    assert.equal(concurrentDispatchResults.filter((item) => item.status === "fulfilled").length, 1);
    assert.equal(concurrentDispatchResults.filter((item) => item.status === "rejected"
        && item.reason instanceof ProspectExecutionKernelError
        && item.reason.code === "EXECUTION_REQUEST_ALREADY_STARTED").length, 1);
    assert.equal(concurrentDispatchProvider.listPhysicalCalls().length, 1);
    assert.equal(concurrentDispatchProvider.listPhysicalCalls()
        .filter((item) => item.providerExecuted).length, 1);
    ...
    assert.equal(concurrentDispatchProvider.listPhysicalCalls().length, 1);
    const dispatchRaceRun = createRun(runOwner, ["fake.dispatch-recovery-race"]);
    ...
    const dispatchRaceClaim = await dispatchRaceKernel.claimNext("2026-07-14T00:34:11.000Z");
    assert.ok(dispatchRaceClaim);
    const dispatchRacePrepared = await dispatchRaceKernel.prepareProviderRequest({
        leaseId: dispatchRaceClaim.lease.id,
        claimToken: dispatchRaceClaim.claimToken,
        now: "2026-07-14T00:34:12.000Z"
    });
    const dispatchRaceProvider = new DeterministicFakeProspectProvider({
    ...
    const racingDispatch = dispatchRaceKernel.dispatchPreparedProviderRequest(dispatchRaceProvider, {
        leaseId: dispatchRaceClaim.lease.id,
        claimToken: dispatchRaceClaim.claimToken,
        ledgerId: dispatchRacePrepared.ledger.id,
        now: "2026-07-14T00:34:13.000Z"
    });
    const dispatchRacePageCount = store.prospectExecutionPages.length;
    const dispatchRaceAccountingCount = store.prospectProviderRequestAccountingEvidence.length;
    await new Promise((resolve) => setTimeout(resolve, 5));
    assert.equal(store.prospectProviderRequestLedgers.find((item) => item.id === dispatchRacePrepared.ledger.id)?.status, "dispatch_started");
    ...
    const dispatchRaceTerminalLease = structuredClone(store.prospectExecutionLeases.find((item) => item.id === dispatchRaceClaim.lease.id));
    ...
    const dispatchRaceLedger = store.prospectProviderRequestLedgers.find((item) => item.id === dispatchRacePrepared.ledger.id);
    const dispatchRaceDispatch = store.prospectProviderRequestDispatches.find((item) => item.ledgerId === dispatchRacePrepared.ledger.id);
    const dispatchRaceAttempt = store.prospectExecutionAttempts.find((item) => item.id === dispatchRaceClaim.attempt.id);
    const dispatchRaceLease = store.prospectExecutionLeases.find((item) => item.id === dispatchRaceClaim.lease.id);
    ...
    assert.ok(dispatchRaceLedger.responseReceivedAt > dispatchRaceLedger.unknownAt);
    ...
    assert.equal(store.prospectExecutionPages.length, dispatchRacePageCount);
    assert.equal(store.prospectProviderRequestAccountingEvidence.length, dispatchRaceAccountingCount);
    assert.deepEqual(store.prospectProviderRequestEvents
    ...
    const dispatchRaceReplayed = await dispatchRaceRecoveryKernel.dispatchPreparedProviderRequest(dispatchRaceProvider, {
        leaseId: dispatchRaceClaim.lease.id,
        claimToken: dispatchRaceClaim.claimToken,
        ledgerId: dispatchRacePrepared.ledger.id,
    ...
    const cancelledResponseRun = createRun(runOwner, ["fake.dispatch-cancelled-response"]);
    ...
    const cancelledResponseClaim = await cancelledResponseKernel.claimNext("2026-07-14T00:34:21.000Z");
    assert.ok(cancelledResponseClaim);
    const cancelledResponsePrepared = await cancelledResponseKernel.prepareProviderRequest({
        leaseId: cancelledResponseClaim.lease.id,
        claimToken: cancelledResponseClaim.claimToken,
        now: "2026-07-14T00:34:22.000Z"
    });
    const cancelledResponseProvider = new DeterministicFakeProspectProvider({
        "fake.dispatch-cancelled-response": [{
                kind: "success",
                acceptedCount: 2,
                rawCount: 2,
    ...
    const cancelledResponseDispatch = cancelledResponseKernel.dispatchPreparedProviderRequest(cancelledResponseProvider, {
        leaseId: cancelledResponseClaim.lease.id,
        claimToken: cancelledResponseClaim.claimToken,
    ...
    await new Promise((resolve) => setTimeout(resolve, 5));
    await cancelledResponseKernel.requestCancel(cancelledResponseRun.id, "2026-07-14T00:34:24.000Z");
    ...
    await cancelledResponseRecoveryKernel.start("2026-07-14T00:34:25.000Z");
    const cancelledResponseTerminalLease = structuredClone(store.prospectExecutionLeases.find((item) => item.id === cancelledResponseClaim.lease.id));
    const cancelledResponseResult = await cancelledResponseDispatch;
    assert.equal(cancelledResponseResult.kind, "response_received");
    const cancelledResponseLedger = store.prospectProviderRequestLedgers.find((item) => item.id === cancelledResponsePrepared.ledger.id);
    assert.equal(cancelledResponseLedger.status, "response_received");
    assert.match(cancelledResponseLedger.unknownReason, /cancelled_with_provider_request_in_flight/);
    assert.equal(cancelledResponseRun.status, "cancelled");
    assert.equal(store.prospectExecutionAttempts.find((item) => item.id === cancelledResponseClaim.attempt.id)?.status, "request_outcome_unknown");
    assert.equal(store.prospectExecutionPages.some((item) => item.runId === cancelledResponseRun.id), false);
    assert.equal(store.prospectProviderRequestAccountingEvidence.some((item) => item.ledgerId === cancelledResponseLedger.id), false);
    const cancelledResponseLease = store.prospectExecutionLeases.find((item) => item.id === cancelledResponseClaim.lease.id);
    assert.equal(cancelledResponseLease?.releaseReason, "CANCELLED_REQUEST_OUTCOME_UNKNOWN");
    assert.deepEqual(cancelledResponseLease, cancelledResponseTerminalLease);
    const cancelledLateSettlement = await cancelledResponseRecoveryKernel.settlePersistedProviderResponse({
    ...
    assert.equal(cancelledLateSettlement.ledger.settlementKind, "cancelled_late");
    assert.equal(cancelledLateSettlement.attempt.status, "cancelled_late");
    assert.equal(cancelledLateSettlement.accountingEvidence.provenance, "unknown");
    assert.equal(cancelledResponseRun.status, "cancelled");
    assert.equal(cancelledLateSettlement.rawBatch, undefined);
    assert.equal(store.prospectSourceRawBatches.some((item) => item.ledgerId === cancelledResponseLedger.id), false);
    assert.equal(store.prospectExecutionPages.some((item) => item.runId === cancelledResponseRun.id), false);
    assert.deepEqual(store.prospectExecutionLeases.find((item) => item.id === cancelledResponseClaim.lease.id), cancelledResponseTerminalLease);
    const interruptedDispatchRun = createRun(runOwner, ["fake.dispatch-interrupted"]);
    ...
    const interruptedDispatchClaim = await interruptedDispatchKernel.claimNext("2026-07-14T00:35:01.000Z");
    assert.ok(interruptedDispatchClaim);
    const interruptedDispatchPrepared = await interruptedDispatchKernel.prepareProviderRequest({
        leaseId: interruptedDispatchClaim.lease.id,
        claimToken: interruptedDispatchClaim.claimToken,
        now: "2026-07-14T00:35:02.000Z"
    });
    const interruptedDispatchProvider = new DeterministicFakeProspectProvider({
    ...
    await assert.rejects(interruptedDispatchKernel.dispatchPreparedProviderRequest(interruptedDispatchProvider, {
        leaseId: interruptedDispatchClaim.lease.id,
        claimToken: interruptedDispatchClaim.claimToken,
    ...
    const interruptedLedger = store.prospectProviderRequestLedgers.find((item) => item.id === interruptedDispatchPrepared.ledger.id);
    const interruptedDispatch = store.prospectProviderRequestDispatches.find((item) => item.ledgerId === interruptedDispatchPrepared.ledger.id);
    assert.equal(interruptedDispatchProvider.listPhysicalCalls().length, 1);
    assert.equal(interruptedDispatchProvider.listPhysicalCalls()
        .filter((item) => item.providerExecuted).length, 1);
    ...
    assert.equal(store.prospectExecutionAttempts.find((item) => item.id === interruptedDispatchClaim.attempt.id)?.status, "request_outcome_unknown");
    assert.equal(await interruptedDispatchKernel.claimNext("2026-07-14T00:35:04.000Z"), null, "结果未知的 Provider 请求不得自动重发");
    const throttledDispatchRun = createRun(runOwner, ["fake.dispatch-throttled"]);
    ...
    const throttledDispatchClaim = await throttledDispatchKernel.claimNext("2026-07-14T00:36:01.000Z");
    assert.ok(throttledDispatchClaim);
    const throttledDispatchPrepared = await throttledDispatchKernel.prepareProviderRequest({
        leaseId: throttledDispatchClaim.lease.id,
        claimToken: throttledDispatchClaim.claimToken,
        now: "2026-07-14T00:36:02.000Z"
    });
    const throttledDispatchProvider = new DeterministicFakeProspectProvider({
    ...
    const throttledDispatchResult = await throttledDispatchKernel.dispatchPreparedProviderRequest(throttledDispatchProvider, {
        leaseId: throttledDispatchClaim.lease.id,
        claimToken: throttledDispatchClaim.claimToken,
    ...
    assert.equal(store.prospectProviderRequestLedgers.find((item) => item.id === throttledDispatchPrepared.ledger.id)?.status, "prepared");
    assert.equal(store.prospectExecutionCheckpoints.find((item) => item.runId === throttledDispatchRun.id)?.totalCallCount, 0);
    assert.equal(store.agentJobs.find((item) => item.id === throttledDispatchClaim.job.id)?.attemptCount, 0);
    await throttledDispatchKernel.requestCancel(throttledDispatchRun.id, "2026-07-14T00:36:04.000Z");
    ...
    const pauseClaim = await pauseKernel.claimNext("2026-07-14T01:00:01.000Z");
    ...
    await pauseKernel.requestPause(pauseRun.id, "2026-07-14T01:00:03.000Z");
    assert.equal(pauseRun.status, "pause_requested");
    if (!pauseStarted.ready)
        throw new Error("pause request did not start");
    ...
    assert.equal(store.prospectRunShards.find((item) => item.runId === pauseRun.id)
        ?.status, "paused");
    ...
    assert.equal((await pauseKernel.executeNext(pauseProvider, "2026-07-14T01:00:06.000Z")).kind, "success");
    ...
    const pauseBeforeClaim = await pauseBeforeRequestKernel.claimNext("2026-07-14T01:10:01.000Z");
    assert.ok(pauseBeforeClaim);
    await pauseBeforeRequestKernel.requestPause(pauseBeforeRequestRun.id, "2026-07-14T01:10:02.000Z");
    assert.equal(pauseBeforeRequestRun.status, "paused");
    assert.equal(store.prospectExecutionLeases.find((item) => item.id === pauseBeforeClaim.lease.id)?.releaseReason, "PAUSED_BEFORE_REQUEST");
    assert.equal(store.prospectExecutionAttempts.find((item) => item.leaseId === pauseBeforeClaim.lease.id)?.errorCode, "PAUSED_BEFORE_REQUEST");
    await assert.rejects(pauseBeforeRequestKernel.beginRequest({
        leaseId: pauseBeforeClaim.lease.id,
        claimToken: pauseBeforeClaim.claimToken,
        now: "2026-07-14T01:10:03.000Z"
    }), (error) => error instanceof ProspectExecutionKernelError
        && error.code === "EXECUTION_LEASE_NOT_ACTIVE");
    assert.doesNotThrow(() => validateProspectRunQueueBridge(store, pauseBeforeRequestRun));
    const pauseRecoveryRun = createRun(runOwner, ["fake.pause-recovery-active", "fake.pause-recovery-waiting"]);
    ...
    const pauseRecoveryClaim = await pauseRecoveryKernel.claimNext("2026-07-14T01:20:01.000Z");
    ...
    await pauseRecoveryKernel.requestPause(pauseRecoveryRun.id, "2026-07-14T01:20:03.000Z");
    ...
    assert.equal(store.prospectRunShards.find((item) => item.id === pauseRecoveryClaim.shard.id)?.status, "failed");
    assert.equal(store.prospectRunShards.find((item) => item.runId === pauseRecoveryRun.id
        && item.id !== pauseRecoveryClaim.shard.id)?.status, "paused");
    assert.doesNotThrow(() => validateProspectRunQueueBridge(store, pauseRecoveryRun));
    ...
    const cancelClaim = await cancelKernel.claimNext("2026-07-14T02:00:01.000Z");
    ...
    if (!cancelStarted.ready)
        throw new Error("cancel request did not start");
    const pageCountBeforeCancel = store.prospectExecutionPages.length;
    await cancelKernel.requestCancel(cancelRun.id, "2026-07-14T02:00:03.000Z");
    assert.equal(cancelRun.status, "cancel_requested");
    await assert.rejects(cancelKernel.heartbeat({
        leaseId: cancelClaim.lease.id,
        claimToken: cancelClaim.claimToken,
    ...
    assert.equal(store.prospectExecutionAttempts.find((item) => item.leaseId === cancelClaim.lease.id)?.status, "cancelled_late");
    assert.doesNotThrow(() => validateProspectRunQueueBridge(store, cancelRun));
    const cancelBeforeRequestRun = createRun(runOwner, ["fake.cancel-before"]);
    ...
    const cancelBeforeClaim = await cancelBeforeRequestKernel.claimNext("2026-07-14T02:10:01.000Z");
    assert.ok(cancelBeforeClaim);
    await cancelBeforeRequestKernel.requestCancel(cancelBeforeRequestRun.id, "2026-07-14T02:10:02.000Z");
    assert.equal(cancelBeforeRequestRun.status, "cancelled");
    assert.equal(store.prospectExecutionLeases.find((item) => item.id === cancelBeforeClaim.lease.id)?.releaseReason, "CANCELLED_BEFORE_REQUEST");
    assert.equal(store.prospectExecutionCheckpoints.find((item) => item.runId === cancelBeforeRequestRun.id)?.completionReason, "CANCELLED_BY_USER");
    assert.doesNotThrow(() => validateProspectRunQueueBridge(store, cancelBeforeRequestRun));
    ...
    const unknownClaim = await unknownKernel.claimNext("2026-07-14T03:00:01.000Z");
    ...
    assert.equal(store.prospectExecutionAttempts.find((item) => item.leaseId === unknownClaim.lease.id)?.status, "request_outcome_unknown");
    assert.equal(await recoveryKernel.claimNext("2026-07-14T03:00:04.000Z"), null, "REQUEST_OUTCOME_UNKNOWN 不得自动重试");
    ...
    await partialKernel.executeNext(partialProvider, "2026-07-14T04:00:01.000Z");
    await partialKernel.executeNext(partialProvider, "2026-07-14T04:00:02.000Z");
    await partialKernel.executeNext(partialProvider, "2026-07-14T04:00:03.000Z");
    assert.equal(store.agentJobs.find((item) => item.id === store.prospectRunQueueChildBindings.find((binding) => binding.runId === partialRun.id
        && store.prospectRunShards.find((shard) => shard.id === binding.shardId)?.providerCode === "fake.fail")?.jobId)?.status, "dead_letter");
    assert.equal(partialRun.status, "running");
    await partialKernel.executeNext(partialProvider, "2026-07-14T04:00:04.000Z");
    ...
    assert.equal((await rollbackWriterKernel.executeNext(rollbackProvider, "2026-07-14T05:00:01.000Z")).kind, "success");
    ...
    const rollbackClaim = await rollbackReaderKernel.claimNext("2026-07-14T05:00:03.000Z");
    assert.ok(rollbackClaim);
    const beforeRejectedBegin = executionStateSnapshot();
    await assert.rejects(rollbackReaderKernel.beginRequest({
    ...
    assert.doesNotThrow(() => validateProspectRunQueueBridge(store, rollbackRun));
    await rollbackReaderKernel.requestCancel(rollbackRun.id, "2026-07-14T05:00:05.000Z");
    ...
    assert.equal((await shortTagWriterKernel.executeNext(shortTagProvider, "2026-07-14T05:10:01.000Z")).kind, "success");
    ...
    const shortTagClaim = await shortTagReaderKernel.claimNext("2026-07-14T05:10:03.000Z");
    assert.ok(shortTagClaim);
    const shortTagCheckpoint = store.prospectExecutionCheckpoints.find((item) => item.runId === shortTagRun.id);
    const shortTagParts = shortTagCheckpoint.encryptedCursor.split(".");
    shortTagParts[2] = Buffer.from(shortTagParts[2], "base64url").subarray(0, 15).toString("base64url");
    shortTagCheckpoint.encryptedCursor = shortTagParts.join(".");
    const beforeShortTagRejection = executionStateSnapshot();
    await assert.rejects(shortTagReaderKernel.beginRequest({
    ...
    await shortTagReaderKernel.requestCancel(shortTagRun.id, "2026-07-14T05:10:05.000Z");
    ...
    assert.equal((await cursorHashWriterKernel.executeNext(cursorHashProvider, "2026-07-14T05:20:01.000Z")).kind, "success");
    ...
    const cursorHashClaim = await cursorHashReaderKernel.claimNext("2026-07-14T05:20:03.000Z");
    assert.ok(cursorHashClaim);
    const cursorHashCheckpoint = store.prospectExecutionCheckpoints.find((item) => item.runId === cursorHashRun.id);
    cursorHashCheckpoint.cursorHash = "0".repeat(64);
    const beforeCursorHashRejection = executionStateSnapshot();
    await assert.rejects(cursorHashReaderKernel.beginRequest({
    ...
    await cursorHashReaderKernel.requestCancel(cursorHashRun.id, "2026-07-14T05:20:05.000Z");
    const crossRunSnapshot = snapshot(["fake.cross-run-position"]);
    crossRunSnapshot.providerPlan[0].pageLimit = 1;
    const crossRunSource = createRun(runOwner, ["fake.cross-run-position"], crossRunSnapshot);
    ...
    const crossRunSourceProvider = new DeterministicFakeProspectProvider({
    ...
    assert.equal((await crossRunSourceKernel.executeNext(crossRunSourceProvider, "2026-07-14T05:25:01.000Z")).kind, "success");
    const continuablePosition = store.prospectStrategySourcePositions.find((item) => item.sourceRunId === crossRunSource.id);
    ...
    assert.equal(continuablePosition.encryptedCursor.includes("cross-run-page-2"), false);
    assert.equal(store.prospectRunShards.find((item) => item.runId === crossRunSource.id)?.hasCursor, false);
    assert.equal(JSON.stringify(store.prospectExecutionEvents.filter((item) => item.runId === crossRunSource.id)).includes("cross-run-page-2"), false);
    ...
    const crossRunResumeProvider = new DeterministicFakeProspectProvider({
        "fake.cross-run-position": [{
                kind: "success",
                acceptedCount: 1,
                rawCount: 1,
    ...
    let resumedCursor = "";
    assert.equal((await crossRunResumeKernel.executeNext({
        search: async (request) => {
            resumedCursor = request.cursor;
            return crossRunResumeProvider.search(request);
        }
    }, "2026-07-14T05:25:03.000Z")).kind, "success");
    assert.equal(resumedCursor, "cross-run-page-2");
    const exhaustedPosition = store.prospectStrategySourcePositions.find((item) => item.id === continuablePosition.id);
    ...
    assert.equal((await crossRunExhaustedKernel.executeNext({
        search: async () => {
            exhaustedProviderCalls += 1;
            throw new Error("exhausted position must not call Provider");
        }
    }, "2026-07-14T05:25:05.000Z")).kind, "idle");
    assert.equal(exhaustedProviderCalls, 0);
    assert.equal(crossRunExhausted.status, "succeeded_empty");
    assert.equal(store.prospectRunShards.find((item) => item.runId === crossRunExhausted.id)?.status, "succeeded_empty");
    assert.equal(store.prospectExecutionCheckpoints.find((item) => item.runId === crossRunExhausted.id)?.completionReason, "SOURCE_POSITION_EXHAUSTED");
    const failedResumeSnapshot = snapshot(["fake.failed-resume"]);
    failedResumeSnapshot.providerPlan[0].pageLimit = 1;
    const failedResumeSource = createRun(runOwner, ["fake.failed-resume"], failedResumeSnapshot);
    ...
    const failedResumeSourceProvider = new DeterministicFakeProspectProvider({
        "fake.failed-resume": [{
                kind: "success",
                acceptedCount: 1,
                rawCount: 1,
    ...
    await failedResumeSourceKernel.executeNext(failedResumeSourceProvider, "2026-07-14T05:26:01.000Z");
    const positionBeforeFailure = structuredClone(store.prospectStrategySourcePositions.find((item) => item.sourceRunId === failedResumeSource.id));
    ...
    const failedResumeProvider = new DeterministicFakeProspectProvider({
        "fake.failed-resume": [{
                kind: "failure",
    ...
    assert.equal((await failedResumeKernel.executeNext({
        search: async (request) => {
            failedResumeCursor = request.cursor;
            return failedResumeProvider.search(request);
        }
    }, "2026-07-14T05:26:03.000Z")).kind, "failure");
    assert.equal(failedResumeCursor, "retry-this-page");
    assert.deepEqual(store.prospectStrategySourcePositions.find((item) => item.id === positionBeforeFailure.id), positionBeforeFailure);
    const changedQueryRun = repeatedRun(runOwner, failedResumeSource, (executionSnapshot) => {
    ...
    const changedQueryProvider = new DeterministicFakeProspectProvider({
        "fake.failed-resume": [{
                kind: "success",
                acceptedCount: 0,
                rawCount: 0,
    ...
    const isolatedOwnerProvider = new DeterministicFakeProspectProvider({
        "fake.failed-resume": [{
                kind: "success",
                acceptedCount: 0,
                rawCount: 0,
    ...
    await assert.rejects(slowResponseKernel.executeNext(slowResponseProvider, "2026-07-14T05:30:01.000Z"), (error) => error instanceof ProspectExecutionKernelError
        && error.code === "EXECUTION_LEASE_FENCE_INVALID");
    assert.equal(store.prospectExecutionPages.some((item) => item.runId === slowResponseRun.id), false);
    assert.equal(store.prospectExecutionAttempts.find((item) => item.runId === slowResponseRun.id)?.status, "request_started");
    assert.equal(await slowResponseKernel.recoverExpiredLeases("2026-07-14T05:30:03.000Z"), 1);
    assert.equal(slowResponseRun.status, "failed");
    assert.equal(store.prospectExecutionAttempts.find((item) => item.runId === slowResponseRun.id)?.status, "request_outcome_unknown");
    ...
    store.prospectExecutionKernelStates.splice(0, store.prospectExecutionKernelStates.length, ...original.kernelStates);
    store.prospectExecutionCheckpoints.splice(0, store.prospectExecutionCheckpoints.length, ...original.checkpoints);
    store.prospectStrategySourcePositions.splice(0, store.prospectStrategySourcePositions.length, ...original.sourcePositions);
    store.prospectExecutionLeases.splice(0, store.prospectExecutionLeases.length, ...original.leases);
    store.prospectExecutionAttempts.splice(0, store.prospectExecutionAttempts.length, ...original.attempts);
    store.prospectProviderRequestLedgers.splice(0, store.prospectProviderRequestLedgers.length, ...original.providerRequestLedgers);
    store.prospectProviderRequestDispatches.splice(0, store.prospectProviderRequestDispatches.length, ...original.providerRequestDispatches);
    store.prospectProviderRequestEvents.splice(0, store.prospectProviderRequestEvents.length, ...original.providerRequestEvents);
    store.prospectProviderRequestAttemptBindings.splice(0, store.prospectProviderRequestAttemptBindings.length, ...original.providerRequestAttemptBindings);
    store.prospectProviderRequestAccountingEvidence.splice(0, store.prospectProviderRequestAccountingEvidence.length, ...original.providerRequestAccountingEvidence);
    store.prospectSourceRawBatches.splice(0, store.prospectSourceRawBatches.length, ...original.sourceRawBatches);
    store.prospectSourceRawRecords.splice(0, store.prospectSourceRawRecords.length, ...original.sourceRawRecords);
    store.prospectSourceRawHits.splice(0, store.prospectSourceRawHits.length, ...original.sourceRawHits);
    store.prospectExecutionPages.splice(0, store.prospectExecutionPages.length, ...original.pages);
    store.prospectExecutionEvents.splice(0, store.prospectExecutionEvents.length, ...original.events);
    store.prospectExecutionThrottleBuckets.splice(0, store.prospectExecutionThrottleBuckets.length, ...original.throttles);
}
