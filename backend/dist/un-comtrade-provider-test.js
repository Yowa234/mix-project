...
function comtradeRecord(overrides = {}) {
    return {
        reporterCode: 842,
        reporterISO: "USA",
...
const previewAdapterPage = await UN_COMTRADE_PROVIDER.trade({ query: baseQuery, cursor: "" }, { apiKey: "" }, tools());
...
const limitedAdapterPage = await UN_COMTRADE_PROVIDER.trade({ query: limitedQuery, cursor: "" }, { apiKey: "" }, tools());
...
const validatedAdapterPage = await UN_COMTRADE_PROVIDER.trade({ query: validationQuery, cursor: "" }, { apiKey: "" }, tools());
...
const keyedAdapterPage = await UN_COMTRADE_PROVIDER.trade({ query: baseQuery, cursor: "" }, { apiKey: "private-test-key" }, tools());
...
await assert.rejects(UN_COMTRADE_PROVIDER.trade({ query: baseQuery, cursor: "" }, { apiKey: "" }, tools()).catch((error) => {
...
await assert.rejects(UN_COMTRADE_PROVIDER.trade({ query: baseQuery, cursor: "" }, { apiKey: "" }, tools()), (error) => error instanceof ProviderContractError
    && error.code === "PROVIDER_SCHEMA_CHANGED"
    && error.phase === "trade");
