...
async function loadOrCreateCredentials(filePath) {
    try {
        const file = await stat(filePath);
        if (!file.isFile())
            throw new Error("凭据路径不是普通文件");
...
async function provision(credentials) {
    ...
    if (!databaseUrl)
        throw new Error("缺少 DATABASE_URL 或 MYSQL_URL，无法写入 MySQL");
    ...
    try {
        ...
        for (const item of credentials) {
            const [rows] = await connection.execute("SELECT id,email,role,team_id FROM users WHERE email = ? OR (role = 'admin' AND team_id = ?) FOR UPDATE", [item.email, item.teamId]);
            if (rows.length) {
