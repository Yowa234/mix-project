...
import { defineProvider, ProviderContractError, PROVIDER_CONTRACT_VERSION } from "./provider-contract.js";
...
import { PROSPECT_COVERAGE_MEMORY_CONTRACT, recordProspectCoverage } from "./prospect-coverage-memory.js";
...
function testProviderCatalog(provider) {
...
function executionSnapshot(providerCodes) {
    const query = {
...
function createRun(owner, providerCodes) {
    ...
    async search() {
        return {
            records: [{
                    ...
                    sourceUrl: "https://search.gleif.org/#/record/5493001KJTIIGC8Y1R12",
                    recordType: "identity_evidence",
...
const originalPersistProspectExecutionMutation = store.persistProspectExecutionMutation;
const originalPersistProspectCandidateMutation = store.persistProspectCandidateMutation;
let executionMutationCount = 0;
let candidateMutationCount = 0;
store.persist = async () => undefined;
store.persistProspectExecutionMutation = async (mutation) => {
    executionMutationCount += 1;
    return mutation().value;
};
store.persistProspectCandidateMutation = async (mutation) => {
...
store.providerCatalog.splice(0, store.providerCatalog.length, ...store.providerCatalog.filter((item) => item.code !== gleifCode));
store.providerCatalog.push(testProviderCatalog(successProvider), testProviderCatalog(failureProvider), testProviderCatalog(gleifProvider));
...
const providerRawEnvelopeSecret = "worker-stage-provider-raw-secret-at-least-32-characters";
const organizationIdentitySecret = "worker-stage-identity-secret-at-least-32-characters";
const prospectCoverageSecret = "worker-stage-coverage-secret-at-least-32-characters";
...
try {
    await worker.start();
    await waitFor(() => teamARun.status === "partial_success"
        && teamBRun.status === "succeeded"
        && teamARepeatRun.status === "succeeded", "all team-scoped runs to finish");
    ...
    assert.ok(executionMutationCount > 0, "execution kernel should prefer the prospect execution transaction path");
    assert.ok(candidateMutationCount > 0, "candidate cleaning should prefer its dedicated transaction path");
    assert.ok(dispatchScopes.every((scope) => scope.startsWith(`${ownerA.teamId}:${ownerA.id}:`)
        || scope.startsWith(`${ownerB.teamId}:${ownerB.id}:`)));
    assert.equal(store.prospectProviderRequestLedgers.filter((item) => item.runId === teamARun.id || item.runId === teamBRun.id
        || item.runId === teamARepeatRun.id).length, 5);
    assert.equal(store.prospectSourceRawHits.filter((item) => item.runId === teamARun.id).length, 2);
    assert.equal(store.prospectSourceRawHits.filter((item) => item.runId === teamARepeatRun.id).length, 1);
    assert.equal(store.prospectSourceRawHits.filter((item) => item.runId === teamBRun.id).length, 1);
    for (const ledger of store.prospectProviderRequestLedgers.filter((item) => item.runId === teamARun.id
    ...
    const generated = store.websiteOpportunities.filter((item) => item.source === successCode
        && [teamARun.id, teamBRun.id].some((runId) => item.sourceEvidence?.some((evidence) => evidence.providerRecordId.includes("worker-record-")
            && store.prospectSourceRawHits.some((hit) => hit.runId === runId
                && hit.teamId === item.teamId
                && hit.ownerId === item.ownerId))));
    assert.equal(generated.length, 2);
    assert.equal(generated.filter((item) => item.teamId === ownerA.teamId && item.ownerId === ownerA.id).length, 1);
    assert.equal(generated.filter((item) => item.teamId === ownerB.teamId && item.ownerId === ownerB.id).length, 1);
    const gleifCandidate = store.websiteOpportunities.find((item) => item.teamId === ownerA.teamId
        && item.ownerId === ownerA.id
        && item.source === gleifCode
        && item.sourceEvidence?.some((evidence) => evidence.providerRecordId === "5493001KJTIIGC8Y1R12"));
    assert.ok(gleifCandidate);
    assert.equal(store.organizations.filter((item) => item.teamId === ownerA.teamId).length, 1);
    assert.equal(store.tenantProspects.filter((item) => item.teamId === ownerA.teamId).length, 1);
    assert.equal(store.prospectCoverageEvents.filter((item) => item.teamId === ownerA.teamId
        && item.sourceHitId).length, 2);
    assert.deepEqual(store.prospectCoverageEvents
        .filter((item) => item.teamId === ownerA.teamId
        && item.sourceHitId)
        .map((item) => item.classification)
        .sort(), ["duplicate", "net_new"]);
    assert.equal(store.websiteOpportunities.filter((item) => item.teamId === ownerA.teamId
        && item.ownerId === ownerA.id
        && item.organizationId === gleifCandidate.organizationId).length, 1);
    ...
    assert.equal(store.prospectCandidateProcessingStates?.filter((item) => item.teamId === ownerA.teamId
        || item.teamId === ownerB.teamId).length, 4);
    ...
    assert.equal(store.tenantProspects.find((item) => item.id === gleifCandidate.tenantProspectId)?.status, "excluded");
    const initialCoverageEvent = store.prospectCoverageEvents.find((item) => item.teamId === ownerA.teamId
        && item.organizationId === gleifCandidate.organizationId
        && item.classification === "net_new");
    assert.ok(initialCoverageEvent);
    const originalHit = store.prospectSourceRawHits.find((item) => item.id === initialCoverageEvent.sourceHitId);
    ...
    assert.equal(store.tenantProspects.find((item) => item.id === gleifCandidate.tenantProspectId)?.status, "active");
    ...
    assert.equal(store.tenantProspects.find((item) => item.id === gleifCandidate.tenantProspectId)?.lastReviewedAt, reviewedAt);
    const linkedLead = {
        id: `lead_${randomUUID()}`,
    ...
    assert.equal(store.tenantProspects.find((item) => item.id === gleifCandidate.tenantProspectId)?.leadId, linkedLead.id);
    ...
    assert.doesNotThrow(() => validateProspectRunQueueBridge(store, teamARun));
    assert.doesNotThrow(() => validateProspectRunQueueBridge(store, teamBRun));
    assert.doesNotThrow(() => validateProspectRunQueueBridge(store, teamARepeatRun));
    await new Promise((resolve) => setTimeout(resolve, 150));
    const stopStartedAt = Date.now();
    await worker.stop();
    assert.ok(Date.now() - stopStartedAt < 1_000, "idle worker should wake and stop without waiting for poll timeout");
