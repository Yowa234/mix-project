...
import { attachAgentJobIdempotencyAlias, enqueueAgentJob, findAgentJobByIdempotency, startAgentJob } from "./agent-jobs.js";
...
async function main() {
    ...
    try {
        ...
        let releaseFailedPersistence;
        let markFailedPersistenceStarted;
        const failedPersistenceGate = new Promise((resolve) => {
            releaseFailedPersistence = resolve;
        });
        const failedPersistenceStarted = new Promise((resolve) => {
        ...
        const serializedPersist = createSerializedPersistence(() => [...queuedAliases], async (snapshot) => {
            persistenceRuns += 1;
            activePersistenceRuns += 1;
            maximumConcurrentPersistenceRuns = Math.max(maximumConcurrentPersistenceRuns, activePersistenceRuns);
            try {
        ...
        await assert.rejects(failedPersistence, /simulated queued persistence failure/);
        await recoveryPersistence;
        assert.equal(maximumConcurrentPersistenceRuns, 1);
        assert.equal(persistenceRuns, 2);
        assert.deepEqual(persistedAliases, [], "失败持久化的别名快照不能在后续队列任务中复活");
        ...
        opportunity.verificationReport = buildProspectVerificationReport(opportunity, "2026-07-13T08:00:00.000Z");
       ...
       ADD UNIQUE KEY uk_provider_response_cache(provider_id, provider_version, request_fingerprint)`);
        await admin.query(`ALTER TABLE \`${databaseName}\`.provider_request_logs
       MODIFY COLUMN requested_at DATETIME NOT NULL`);
        await admin.query(`ALTER TABLE \`${databaseName}\`.market_trade_observations DROP INDEX uk_market_trade_observation`);
        await admin.query(`ALTER TABLE \`${databaseName}\`.market_trade_observations
       ...
       DROP COLUMN source_revision`);
        ...
        const interruptedAlias = attachAgentJobIdempotencyAlias(firstStart, interruptedMarketAnalysisJob, interruptedMarketAnalysisAliasKey).alias;
        ...
        const firstTradeId = tradeTeamAFirst.observations.find((item) => item.commodityCode === "940542").id;
        const firstTradeCreatedAt = tradeTeamAFirst.observations.find((item) => item.commodityCode === "940542").createdAt;
        ...
        const marketFacts = await firstStart.persistMutation(() => {
            ...
            return {
                value,
                rollback: () => {
                    firstStart.marketOpportunityBatches.splice(0, firstStart.marketOpportunityBatches.length - beforeBatchCount);
                    firstStart.marketOpportunitySnapshots.splice(0, firstStart.marketOpportunitySnapshots.length - beforeSnapshotCount);
                    firstStart.marketOpportunityCalculationEvents.splice(0, firstStart.marketOpportunityCalculationEvents.length - beforeEventCount);
                }
            };
        });
        assert.equal(marketFacts.batch.status, "metrics_ready");
        assert.equal(marketFacts.snapshots.length, 1);
        assert.equal(marketFacts.snapshots[0]?.metrics.reportedImportValueSeries[0]?.evidence.payloadHash, "1".repeat(64));
        ...
        const sourceWorld2022 = firstStart.marketTradeObservations.find((item) => item.teamId === "europe"
        ...
        await assert.rejects(() => firstStart.persistMutation(() => {
            firstStart.marketOpportunitySnapshots.unshift(invalidSnapshot);
            return {
                value: invalidSnapshot.id,
                rollback: () => {
                    const index = firstStart.marketOpportunitySnapshots.findIndex((item) => item.id === invalidSnapshot.id);
                    if (index >= 0)
                        firstStart.marketOpportunitySnapshots.splice(index, 1);
                }
            };
        }), /市场机会事实快照持久化校验失败/);
        assert.equal(firstStart.marketOpportunitySnapshots.some((item) => item.id === invalidSnapshot.id), false);
        const barrierOrder = [];
        ...
        const [encryptedRows] = await admin.query(`SELECT api_key FROM \`${databaseName}\`.ai_model_configs WHERE id = ?`, [aiConfigId]);
        assert.match(encryptedRows[0]?.api_key || "", /^ai-v1\./);
        assert.notEqual(encryptedRows[0]?.api_key, aiSecret);
        const [cacheRows] = await admin.query(`SELECT payload_encrypted FROM \`${databaseName}\`.provider_response_cache WHERE provider_id = ?`, ["un_comtrade"]);
        assert.match(cacheRows[0]?.payload_encrypted || "", /^cache-v1\./);
        assert.equal(cacheRows[0]?.payload_encrypted.includes("2061149344"), false);
        const [aliasRows] = await admin.query(`SELECT job_id, idempotency_key
       FROM \`${databaseName}\`.agent_job_idempotency_aliases
       WHERE id = ?`, [interruptedAlias.id]);
        ...
        const [aliasIndexRows] = await admin.query(`SHOW INDEX FROM \`${databaseName}\`.agent_job_idempotency_aliases
       WHERE Key_name = 'uk_agent_job_alias_idempotency'`);
        assert.deepEqual(aliasIndexRows
            .sort((left, right) => left.Seq_in_index - right.Seq_in_index)
            .map((item) => item.Column_name), ["team_id", "job_type", "idempotency_key"]);
        assert.ok(aliasIndexRows.every((item) => Number(item.Non_unique) === 0));
        const [jobIndexRows] = await admin.query(`SHOW INDEX FROM \`${databaseName}\`.agent_jobs
       WHERE Key_name = 'uk_agent_job_idempotency'`);
        assert.deepEqual(jobIndexRows
            .sort((left, right) => left.Seq_in_index - right.Seq_in_index)
            .map((item) => item.Column_name), ["team_id", "job_type", "idempotency_key"]);
        ...
        await admin.query(`INSERT INTO \`${databaseName}\`.agent_job_idempotency_aliases
       ...
       VALUES (?, ?, ?, ?, ?, NOW())`, [
        ...
        await assert.rejects(() => createMysqlStore(), /主任务与别名之间存在冲突绑定/);
        await admin.query(`DELETE FROM \`${databaseName}\`.agent_job_idempotency_aliases WHERE id = ?`, [conflictingAliasId]);
       ...
       VALUES (?, ?, ?, ?, ?, NOW())`, [
        ...
        const recoveredInterruptedJob = secondStart.agentJobs.find((item) => item.id === interruptedMarketAnalysisJob.id);
        ...
        assert.equal(findAgentJobByIdempotency(secondStart, {
            teamId: "europe",
            jobType: "prospect.market_analysis",
            idempotencyKey: interruptedMarketAnalysisAliasKey
        })?.id, interruptedMarketAnalysisJob.id);
        assert.equal(secondStart.agentJobIdempotencyAliases.find((item) => item.id === interruptedAlias.id)?.jobId, interruptedMarketAnalysisJob.id);
        const [orphanRows] = await admin.query(`SELECT COUNT(*) AS count
       FROM \`${databaseName}\`.agent_job_idempotency_aliases
       WHERE id = ?`, [orphanAliasId]);
        ...
        assert.equal(secondStart.providerRequestLogs.find((item) => item.id === millisecondRequestLog.id)?.requestedAt, millisecondRequestLog.requestedAt, "Provider request log milliseconds must survive repeated persistence");
        assert.equal(secondStart.aiModelConfigs.find((item) => item.id === aiConfigId)?.apiKey, aiSecret, "Encrypted AI API key must decrypt after a cold MySQL store start");
        ...
        const cacheHit = await readProviderResponseCache(cacheKey, new Date("2026-07-13T12:00:00.000Z"));
        assert.deepEqual(cacheHit?.payload, cachePayload);
        const reloadedTradeTeamA = secondStart.marketTradeObservations.filter((item) => item.teamId === "europe" && item.campaignId === "mysql_trade_campaign");
        const reloadedTradeTeamB = secondStart.marketTradeObservations.filter((item) => item.teamId === "trade_team_b" && item.campaignId === "mysql_trade_campaign");
        assert.equal(reloadedTradeTeamA.length, 3);
        assert.equal(reloadedTradeTeamB.length, 1);
        assert.equal(reloadedTradeTeamA.filter((item) => item.commodityCode === "940542").length, 2);
        assert.equal(reloadedTradeTeamA.find((item) => item.ownerId === teamASecondUser.id)?.tradeValueUsd, 2250000);
        assert.equal(reloadedTradeTeamA.find((item) => item.commodityCode === "940542")?.tradeValueUsd, 1500000);
        assert.deepEqual(reloadedTradeTeamA.find((item) => item.commodityCode === "940542")?.statusFlags, ["REVISED"]);
        assert.equal(reloadedTradeTeamA.find((item) => item.commodityCode === "940542")?.reporterCountry, "usa");
        assert.equal(reloadedTradeTeamA.find((item) => item.commodityCode === "940542")?.reporterCode, "842");
        assert.equal(reloadedTradeTeamA.find((item) => item.commodityCode === "940542")?.partnerCode, "0");
        assert.equal(reloadedTradeTeamA.find((item) => item.commodityCode === "940542")?.commodityDescription, "Other electric lamps and lighting fittings");
        assert.equal(reloadedTradeTeamA.find((item) => item.commodityCode === "940542")?.sourceRevision, "2026-07-12");
        assert.equal(reloadedTradeTeamA.find((item) => item.commodityCode === "940549")?.tradeValueUsd, null);
        assert.equal(reloadedTradeTeamA.find((item) => item.commodityCode === "940549")?.suppressed, true);
        const reloadedMarketBatch = secondStart.marketOpportunityBatches.find((item) => item.id === marketFacts.batch.id);
        const reloadedMarketSnapshot = secondStart.marketOpportunitySnapshots.find((item) => item.batchId === marketFacts.batch.id);
        const reloadedMarketEvent = secondStart.marketOpportunityCalculationEvents.find((item) => item.id === marketFacts.event.id);
        ...
        assert.equal(reloadedMarketBatch.observationCutoffAt, "2026-07-13T08:00:00.000Z");
        ...
        assert.equal(reloadedMarketSnapshot.metrics.reportedImportValueSeries[0]?.tradeValueUsd, 100, "Immutable opportunity evidence must not follow later source observation changes");
        assert.equal(reloadedMarketSnapshot.metrics.reportedImportValueSeries[0]?.evidence.payloadHash, "1".repeat(64), "Immutable evidence payload hash must survive cold start unchanged");
        assert.equal(secondStart.marketTradeObservations.find((item) => item.id === sourceWorld2022.id)
            ?.tradeValueUsd, 999);
        assert.equal(secondStart.marketOpportunityBatches.filter((item) => item.campaignId === marketCampaignId
            && item.teamId === "europe"
            && item.ownerId === teamASecondUser.id).length, 1);
        assert.equal(secondStart.marketOpportunityBatches.filter((item) => item.campaignId === marketCampaignId
            && item.teamId === "trade_team_b"
            && item.ownerId === teamBUser.id).length, 1);
        ...
        await assert.rejects(() => secondStart.persist(), /市场机会事实快照不可变历史被删除或修改/);
        reloadedMarketSnapshot.commodityDescription = originalCommodityDescription;
        await secondStart.persist();
        await admin.query(`UPDATE \`${databaseName}\`.market_opportunity_snapshots
       SET metrics_json = JSON_SET(
         metrics_json,
         '$.reportedImportValueSeries',
         JSON_OBJECT()
       )
       WHERE id = ?`, [reloadedMarketSnapshot.id]);
        await assert.rejects(() => createMysqlStore(), /市场机会事实快照持久化校验失败/);
        await admin.query(`UPDATE \`${databaseName}\`.market_opportunity_snapshots
       SET metrics_json = ? WHERE id = ?`, [JSON.stringify(reloadedMarketSnapshot.metrics), reloadedMarketSnapshot.id]);
        await admin.query(`UPDATE \`${databaseName}\`.market_opportunity_batches
       SET candidate_count = candidate_count + 1 WHERE id = ?`, [reloadedMarketBatch.id]);
        await assert.rejects(() => createMysqlStore(), /市场机会事实批次持久化校验失败/);
        await admin.query(`UPDATE \`${databaseName}\`.market_opportunity_batches
       SET candidate_count = ? WHERE id = ?`, [reloadedMarketBatch.candidateCount, reloadedMarketBatch.id]);
        ...
        await secondStart.persist();
        await admin.query(`UPDATE \`${databaseName}\`.market_opportunity_calculation_events
       SET trigger_job_id = ? WHERE id = ?`, [crossOwnerTriggerJob.id, reloadedMarketEvent.id]);
        await assert.rejects(() => createMysqlStore(), /市场机会计算事件持久化校验失败/);
        await admin.query(`UPDATE \`${databaseName}\`.market_opportunity_calculation_events
       SET trigger_job_id = ? WHERE id = ?`, [marketFactJob.id, reloadedMarketEvent.id]);
        const [marketScoreRows] = await admin.query(`SELECT market_score, growth_score, china_supply_score
       ...
       WHERE id = ?`, [reloadedMarketSnapshot.id]);
        ...
        await admin.query(`UPDATE \`${databaseName}\`.market_opportunity_snapshots
       SET market_score = 1 WHERE id = ?`, [reloadedMarketSnapshot.id]);
        await assert.rejects(() => createMysqlStore(), /市场机会事实快照持久化校验失败/);
        await admin.query(`UPDATE \`${databaseName}\`.market_opportunity_snapshots
       SET market_score = NULL WHERE id = ?`, [reloadedMarketSnapshot.id]);
        await admin.query(`UPDATE \`${databaseName}\`.market_opportunity_snapshots
       SET owner_id = ? WHERE id = ?`, [teamASecondUser.id, reloadedMarketSnapshot.id]);
        await assert.rejects(() => createMysqlStore(), /市场机会事实快照持久化校验失败/);
        await admin.query(`UPDATE \`${databaseName}\`.market_opportunity_snapshots
       SET owner_id = ? WHERE id = ?`, ["u_sales_shirley", reloadedMarketSnapshot.id]);
        await admin.query(`ALTER TABLE \`${databaseName}\`.market_opportunity_snapshots
       DROP INDEX uk_market_opportunity_snapshot,
       ADD INDEX uk_market_opportunity_snapshot(batch_id, reporter_country)`);
        await createMysqlStore();
        const [marketSnapshotIndexRows] = await admin.query(`SHOW INDEX FROM \`${databaseName}\`.market_opportunity_snapshots
       WHERE Key_name = 'uk_market_opportunity_snapshot'`);
        assert.deepEqual(marketSnapshotIndexRows
            .sort((left, right) => left.Seq_in_index - right.Seq_in_index)
            .map((item) => item.Column_name), ["batch_id", "reporter_code", "classification", "commodity_code"]);
        assert.ok(marketSnapshotIndexRows.every((item) => Number(item.Non_unique) === 0));
        const thirdStart = await createMysqlStore();
        setStore(thirdStart);
        assert.equal(thirdStart.providerCatalog.find((item) => item.code === "serper")?.name, "团队自定义搜索源", "A later cold start must not overwrite the administrator's Provider customization");
        assert.ok(thirdStart.providerCatalog.some((item) => item.code === "hunter"), "Restored Provider catalog item must be persisted");
        assert.deepEqual(thirdStart.websiteOpportunities.find((item) => item.id === opportunity.id)?.sourceEvidence, [evidence], "Provider evidence must remain durable across repeated cold starts");
        assert.deepEqual(thirdStart.websiteOpportunities.find((item) => item.id === opportunity.id)?.verificationReport, opportunity.verificationReport, "Verification report must remain durable across repeated cold starts");
        const legacySecret = "legacy-plaintext-ai-secret";
        await admin.query(`UPDATE \`${databaseName}\`.ai_model_configs SET api_key = ? WHERE id = ?`, [legacySecret, aiConfigId]);
        const migratedStart = await createMysqlStore();
        assert.equal(migratedStart.aiModelConfigs.find((item) => item.id === aiConfigId)?.apiKey, legacySecret, "Legacy plaintext AI API keys must remain usable during migration");
        const [migratedRows] = await admin.query(`SELECT api_key FROM \`${databaseName}\`.ai_model_configs WHERE id = ?`, [aiConfigId]);
        assert.match(migratedRows[0]?.api_key || "", /^ai-v1\./);
        assert.notEqual(migratedRows[0]?.api_key, legacySecret);
        const [columns] = await admin.query(`SHOW COLUMNS FROM \`${databaseName}\`.website_opportunities`);
        ...
        const [cacheColumns] = await admin.query(`SHOW COLUMNS FROM \`${databaseName}\`.provider_response_cache`);
        ...
        const [cacheIndexes] = await admin.query(`SHOW INDEX FROM \`${databaseName}\`.provider_response_cache WHERE Key_name = 'uk_provider_response_cache'`);
        assert.deepEqual(cacheIndexes
            .sort((left, right) => left.Seq_in_index - right.Seq_in_index)
            .map((row) => row.Column_name), ["provider_id", "provider_version", "request_fingerprint", "license_scope"]);
        const [tradeIndexes] = await admin.query(`SHOW INDEX FROM \`${databaseName}\`.market_trade_observations WHERE Key_name = 'uk_market_trade_observation'`);
        assert.deepEqual(tradeIndexes
            .sort((left, right) => left.Seq_in_index - right.Seq_in_index)
        ...
        const [tradeColumns] = await admin.query(`SHOW COLUMNS FROM \`${databaseName}\`.market_trade_observations`);
        const tradeColumnTypes = new Map(tradeColumns.map((column) => [column.Field, column.Type.toLowerCase()]));
        ...
        const [requestLogColumns] = await admin.query(`SHOW COLUMNS FROM \`${databaseName}\`.provider_request_logs`);
        assert.equal(requestLogColumns.find((column) => column.Field === "requested_at")
            ?.Type.toLowerCase(), "datetime(3)");
