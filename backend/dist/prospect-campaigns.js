...
import { z } from "zod";
import { approvedStrategiesForCurrentVersion, assertProspectStrategyOwnerConsistency, createDefaultProspectStrategyRecords, prospectStrategyActivationIssues, transferProspectStrategyOwners } from "./prospect-strategies.js";
import { activeProspectRunsForCampaign, pauseQueuedProspectRunsForCampaign } from "./prospect-run-guards.js";
...
export const prospectCampaignSnapshotSchema = z.object({
    ...
    sourceProviderIds: z.array(z.string().trim().min(1).max(80).regex(/^[A-Za-z0-9._-]+$/)).max(100).default([])
}).strict();
...
export function normalizeProspectCampaignSnapshot(value = {}) {
...
export function prospectCampaignSnapshotHash(snapshot) {
    return createHash("sha256")
        .update(JSON.stringify(normalizeProspectCampaignSnapshot(snapshot)))
        .digest("hex");
}
export function prospectCampaignEtag(campaign) {
...
function assertCampaignCrudRole(user) {
    if (user.role === "super_admin") {
        throw new ProspectCampaignRequestError(403, "CAMPAIGN_CRUD_FORBIDDEN", "超级管理员默认不能访问团队业务项目");
    }
}
...
function findVisibleCampaign(store, user, campaignId) {
    ...
    if (!campaign || !canReadCampaign(user, campaign)) {
        throw new ProspectCampaignRequestError(404, "CAMPAIGN_NOT_FOUND", "获客项目不存在或无权访问");
    }
    return campaign;
}
function assertIfMatch(campaign, ifMatch) {
    if (!ifMatch) {
        throw new ProspectCampaignRequestError(428, "PRECONDITION_REQUIRED", "修改获客项目必须提供 If-Match");
    }
    if (ifMatch.trim() !== prospectCampaignEtag(campaign)) {
        throw new ProspectCampaignRequestError(412, "CAMPAIGN_REVISION_CONFLICT", "获客项目已被其他操作更新，请刷新后重试", { revision: campaign.revision, etag: prospectCampaignEtag(campaign) });
    }
}
function resolveOwner(store, user, requestedOwnerId) {
    const ownerId = requestedOwnerId || user.id;
    if (ownerId !== user.id && user.role !== "manager" && user.role !== "admin") {
        throw new ProspectCampaignRequestError(403, "CAMPAIGN_OWNER_FORBIDDEN", "业务员不能把项目分配给其他账号");
    }
    const owner = store.users.find((item) => item.id === ownerId
        && item.teamId === user.teamId
        && item.status === "active"
        && item.role !== "super_admin");
    if (!owner) {
        throw new ProspectCampaignRequestError(404, "CAMPAIGN_OWNER_NOT_FOUND", "负责人不存在、已停用或不属于当前团队");
    }
    return owner;
}
function campaignVersions(store, campaign) {
    return store.prospectCampaignVersions
        .filter((item) => item.teamId === campaign.teamId && item.campaignId === campaign.id)
        .sort((left, right) => right.version - left.version);
}
function currentVersion(store, campaign) {
...
function campaignDetail(store, campaign) {
...
function snapshotCampaignState(store) {
    return {
        ...
        runQueueParentBindings: structuredClone(store.prospectRunQueueParentBindings),
        runQueueChildBindings: structuredClone(store.prospectRunQueueChildBindings)
    };
}
function restoreCampaignState(store, snapshot) {
    store.prospectCampaigns.splice(0, store.prospectCampaigns.length, ...snapshot.campaigns);
    store.prospectCampaignVersions.splice(0, store.prospectCampaignVersions.length, ...snapshot.versions);
    store.prospectCampaignEvents.splice(0, store.prospectCampaignEvents.length, ...snapshot.events);
    store.prospectStrategies.splice(0, store.prospectStrategies.length, ...snapshot.strategies);
    store.prospectStrategyEvents.splice(0, store.prospectStrategyEvents.length, ...snapshot.strategyEvents);
    store.prospectSearchRuns.splice(0, store.prospectSearchRuns.length, ...snapshot.runs);
    store.prospectRunShards.splice(0, store.prospectRunShards.length, ...snapshot.runShards);
    ...
    store.prospectRunQueueParentBindings.splice(0, store.prospectRunQueueParentBindings.length, ...snapshot.runQueueParentBindings);
    store.prospectRunQueueChildBindings.splice(0, store.prospectRunQueueChildBindings.length, ...snapshot.runQueueChildBindings);
}
async function persistCampaignMutation(store, mutation) {
...
export function listProspectCampaigns(store, user, includeArchived = false) {
    assertCampaignCrudRole(user);
...
export function getProspectCampaign(store, user, campaignId) {
    return campaignDetail(store, findVisibleCampaign(store, user, campaignId));
}
export async function createProspectCampaign(input) {
    ...
    return persistCampaignMutation(input.store, () => {
        ...
        const normalizedSnapshot = normalizeProspectCampaignSnapshot(input.body.snapshot);
        const campaign = {
            id: campaignId,
...
export async function updateProspectCampaign(input) {
    return persistCampaignMutation(input.store, () => {
        ...
        if (!nextOwner) {
            throw new ProspectCampaignRequestError(409, "CAMPAIGN_OWNER_INVALID", "当前负责人已不存在，请由团队管理员重新分配");
        }
        if (nextOwner.id !== campaign.ownerId
            && input.user.role !== "manager"
            && input.user.role !== "admin") {
            throw new ProspectCampaignRequestError(403, "CAMPAIGN_OWNER_FORBIDDEN", "只有团队经理或管理员可以转交负责人");
        }
        ...
        if (terminal && nameChanged) {
            throw new ProspectCampaignRequestError(409, "CAMPAIGN_READ_ONLY", "已完成或已归档项目只能转交负责人，不能修改业务内容");
        }
        ...
        if (ownerChanged) {
            if (activeProspectRunsForCampaign(input.store, campaign.teamId, campaign.id).length) {
                throw new ProspectCampaignRequestError(409, "CAMPAIGN_ACTIVE_RUNS", "项目存在活动搜索运行，请先取消后再转交负责人");
            }
            assertProspectStrategyOwnerConsistency(input.store.prospectStrategies.filter((item) => item.teamId === campaign.teamId
                && item.campaignId === campaign.id), previousOwnerId);
        }
...
export async function createProspectCampaignVersion(input) {
    return persistCampaignMutation(input.store, () => {
        ...
        assertIfMatch(campaign, input.ifMatch);
        if (activeProspectRunsForCampaign(input.store, campaign.teamId, campaign.id).length) {
            throw new ProspectCampaignRequestError(409, "CAMPAIGN_ACTIVE_RUNS", "项目存在活动搜索运行，请先取消后再发布新规则版本");
        }
        if (campaign.status !== "draft" && campaign.status !== "paused") {
            throw new ProspectCampaignRequestError(409, "CAMPAIGN_VERSION_LOCKED", "只有草稿或已暂停项目可以发布新规则版本");
        }
...
export function prospectCampaignMissingFields(snapshot) {
    const missing = [];
    if (!snapshot.goal)
        missing.push("goal");
    ...
    if (!snapshot.customerTypes.length)
        missing.push("customerTypes");
    if (!snapshot.applicationScenarios.length)
        missing.push("applicationScenarios");
    if (!snapshot.sourceProviderIds.length)
        missing.push("sourceProviderIds");
    return missing;
}
export async function activateProspectCampaign(input) {
    return persistCampaignMutation(input.store, () => {
        ...
        assertIfMatch(campaign, input.ifMatch);
        if (campaign.status !== "draft" && campaign.status !== "paused") {
            throw new ProspectCampaignRequestError(409, "CAMPAIGN_STATE_INVALID", "当前状态不能启动项目");
        }
        const missingFields = prospectCampaignMissingFields(currentVersion(input.store, campaign).snapshot);
        if (missingFields.length) {
            throw new ProspectCampaignRequestError(422, "CAMPAIGN_FIELDS_REQUIRED", "获客项目资料尚未完整", { missingFields });
        }
        const approvedStrategies = approvedStrategiesForCurrentVersion(input.store, campaign);
        if (!approvedStrategies.length) {
            throw new ProspectCampaignRequestError(409, "CAMPAIGN_STRATEGY_REQUIRED", "需先配置并校验至少一条搜索策略");
        }
        const providerIssues = prospectStrategyActivationIssues(input.store, campaign, approvedStrategies);
        if (providerIssues.length) {
            throw new ProspectCampaignRequestError(422, "CAMPAIGN_PROVIDER_NOT_READY", "当前搜索策略的数据源尚未全部就绪", { issues: providerIssues });
        }
...
export async function transitionProspectCampaign(input) {
    return persistCampaignMutation(input.store, () => {
        ...
        if (!allowed.includes(input.targetStatus)) {
            throw new ProspectCampaignRequestError(409, "CAMPAIGN_STATE_INVALID", `项目不能从 ${campaign.status} 转为 ${input.targetStatus}`);
        }
        if ((input.targetStatus === "completed"
            || input.targetStatus === "archived")
            && activeProspectRunsForCampaign(input.store, campaign.teamId, campaign.id).length) {
            throw new ProspectCampaignRequestError(409, "CAMPAIGN_ACTIVE_RUNS", "项目存在活动搜索运行，请先取消后再完成或归档项目");
        }
        const previousStatus = campaign.status;
        const now = new Date().toISOString();
        if (input.targetStatus === "paused") {
            try {
                ...
                if (error instanceof ProspectRunQueueBridgeIntegrityError) {
                    throw new ProspectCampaignRequestError(409, error.code, "搜索运行队列桥接完整性校验失败，项目不能暂停");
                }
                throw error;
            }
        }
...
export function resolveMarketCampaignReference(input) {
    if (!input.campaignId.startsWith("pc_")) {
        return {
            campaignContractMode: "compat_v1",
            campaignScope: "owner",
            campaign: null
        };
    }
    ...
    const campaign = input.store.prospectCampaigns.find((item) => item.id === input.campaignId);
    if (!campaign
    ...
    if (input.requireActive && campaign.status !== "active") {
        throw new ProspectCampaignRequestError(409, "CAMPAIGN_NOT_ACTIVE", "获客项目尚未启动");
    }
    return {
        campaignContractMode: "formal_v1",
        campaignScope: "owner",
        campaign
    };
}
