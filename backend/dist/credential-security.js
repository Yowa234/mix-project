...
function credentialSecret() {
    ...
    if (configured && configured.length >= 32)
        return configured;
    if (configured)
        throw new Error("PROVIDER_CREDENTIAL_KEY 必须至少包含 32 个字符");
...
export function encryptProviderConfiguration(context, configuration) {
...
export function decryptProviderConfiguration(context, encryptedValue) {
...
function providerResponseCacheAdditionalData(context) {
    return Buffer.from([
...
export function encryptProviderResponseCachePayload(context, payload) {
    const iv = randomBytes(12);
...
export function decryptProviderResponseCachePayload(context, encryptedValue) {
    const [version, ivValue, tagValue, payloadValue] = encryptedValue.split(".");
    if (version !== "cache-v1" || !ivValue || !tagValue || !payloadValue) {
        throw new Error("Provider 响应缓存密文格式无效");
    }
    const decipher = createDecipheriv("aes-256-gcm", providerResponseCacheEncryptionKey(), Buffer.from(ivValue, "base64url"));
...
function aiModelAdditionalData(context) {
    return Buffer.from([
        "ai-model-api-key",
        context.id,
        context.provider,
...
export function isEncryptedAiModelApiKey(value) {
    return value.startsWith("ai-v1.");
}
export function encryptAiModelApiKey(context, apiKey) {
    if (!apiKey)
        return "";
    const iv = randomBytes(12);
...
export function decryptAiModelApiKey(context, encryptedValue) {
    if (!encryptedValue)
        return "";
    const [version, ivValue, tagValue, payloadValue] = encryptedValue.split(".");
    ...
    const decipher = createDecipheriv("aes-256-gcm", aiModelEncryptionKey(), Buffer.from(ivValue, "base64url"));
