...
const CANCELLABLE_STATUSES = new Set(["queued", "running", "retry_scheduled"]);
...
export function isProspectRunBridgeJob(job) {
    return PROSPECT_RUN_BRIDGE_JOB_TYPES.has(job.jobType);
}
function assertGenericAgentJob(job) {
...
export function prospectRunBridgeIdempotencyDigest(value) {
    return idempotencyDigest(value.trim());
}
function safeFailureMessage(errorCode) {
    const messages = {
...
export function findAgentJobByIdempotency(store, input) {
    const teamId = input.teamId.trim();
    const jobType = input.jobType.trim();
    const idempotencyKey = input.idempotencyKey.trim();
    if (!teamId || !jobType || !idempotencyKey)
        return undefined;
    if (PROSPECT_RUN_BRIDGE_JOB_TYPES.has(jobType))
        return undefined;
    const digest = idempotencyDigest(idempotencyKey);
    const primary = store.agentJobs.find((item) => item.teamId === teamId
        && item.jobType === jobType
        && item.idempotencyKey === digest);
    if (primary)
        return primary;
    const alias = store.agentJobIdempotencyAliases.find((item) => item.teamId === teamId
        && item.jobType === jobType
        && item.idempotencyKey === digest);
    if (!alias)
        return undefined;
    return store.agentJobs.find((item) => item.id === alias.jobId
        && item.teamId === teamId
        && item.jobType === jobType);
}
export function attachAgentJobIdempotencyAlias(store, job, rawIdempotencyKey) {
    assertGenericAgentJob(job);
    if (!store.agentJobs.some((item) => item.id === job.id
    ...
    if (!idempotencyKey)
        throw new Error("智能获客任务别名必须提供幂等键");
    const digest = idempotencyDigest(idempotencyKey);
    if (job.idempotencyKey === digest) {
        return { alias: null, duplicate: true };
    }
    const primary = store.agentJobs.find((item) => item.teamId === job.teamId
        && item.jobType === job.jobType
        && item.idempotencyKey === digest);
    if (primary) {
    ...
    const existing = store.agentJobIdempotencyAliases.find((item) => item.teamId === job.teamId
        && item.jobType === job.jobType
        && item.idempotencyKey === digest);
    if (existing) {
        if (existing.jobId === job.id)
            return { alias: existing, duplicate: true };
        throw new Error("该幂等键已绑定其他任务");
    }
    const alias = {
        id: `ajia_${randomUUID()}`,
...
export function enqueueAgentJob(store, input) {
    const teamId = input.teamId.trim();
    const ownerId = input.ownerId.trim();
    const jobType = input.jobType.trim();
    const idempotencyKey = input.idempotencyKey.trim();
    if (!teamId || !ownerId)
        throw new Error("智能获客任务必须明确指定团队和负责人");
    if (!/^[a-z0-9._-]{3,80}$/i.test(jobType))
        throw new Error("智能获客任务类型格式无效");
    if (PROSPECT_RUN_BRIDGE_JOB_TYPES.has(jobType)) {
        throw new Error("该任务类型仅允许由搜索运行桥接器创建");
    }
    if (!idempotencyKey)
        throw new Error("智能获客任务必须提供幂等键");
    ...
    if (input.parentJobId) {
        ...
        if (!parent || parent.teamId !== teamId)
            throw new Error("父任务不存在或不属于当前团队");
    }
...
export function registerProspectRunBridgeJob(store, input) {
    const owner = store.users.find((item) => item.id === input.ownerId
        && item.teamId === input.teamId
        && item.status === "active");
    if (!owner)
        throw new Error("搜索运行桥接任务负责人无效");
    ...
    if (input.jobType === PROSPECT_RUN_PROVIDER_FETCH_JOB_TYPE) {
        ...
        if (!parent)
            throw new Error("搜索运行子桥接任务缺少同域父任务");
    }
...
export function startAgentJob(job) {
    assertGenericAgentJob(job);
    ...
    if (job.status === "retry_scheduled") {
        ...
        if (!Number.isFinite(retryAt.getTime()))
            throw new Error("任务重试时间无效");
        if (retryAt.getTime() > Date.now())
            throw new Error("任务尚未到重试时间");
    }
    job.status = "running";
    job.attemptCount += 1;
...
export function completeAgentJob(job, output = {}) {
    assertGenericAgentJob(job);
    if (job.status !== "running")
        throw new Error("只有执行中的任务可以完成");
...
export function failAgentJob(job, errorCode, nextAttemptAt = "") {
    assertGenericAgentJob(job);
    if (job.status !== "running")
        throw new Error("只有执行中的任务可以记录失败");
...
export function retryAgentJob(job) {
    assertGenericAgentJob(job);
    if (!RETRYABLE_STATUSES.has(job.status))
        throw new Error("当前任务状态不能重试");
...
export function cancelAgentJob(job) {
    assertGenericAgentJob(job);
...
export function recoverInterruptedAgentJobs(store, jobType) {
    if (PROSPECT_RUN_BRIDGE_JOB_TYPES.has(jobType)) {
        throw new Error("搜索运行桥接任务不能通过通用恢复入口处理");
    }
    const now = new Date().toISOString();
    const recovered = store.agentJobs.filter((job) => job.jobType === jobType && job.status === "running");
...
export function publicAgentJob(job) {
    assertGenericAgentJob(job);
    return {
        id: job.id,
        teamId: job.teamId,
...
export function cancelProspectRunBridgeJob(job, cancelledAt) {
