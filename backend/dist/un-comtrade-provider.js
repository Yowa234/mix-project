import { z } from "zod";
import { ProviderContractError, defineTradeProvider, providerHttpStatusError } from "./provider-contract.js";
...
function actualClassification(code) {
...
function requestedClassificationMatches(actual, requested) {
    return requested === "HS" || actual === requested;
}
function countryLabel(iso, description, code) {
    if (code === 0 || description?.trim().toLocaleLowerCase() === "world")
        return "WORLD";
    return iso?.trim().toUpperCase() || description?.trim() || String(code ?? "UNKNOWN");
}
function tradeFlow(record) {
    const flow = (record.flowCode || "").trim().toUpperCase();
    if (flow === "M" || flow === "X")
        return flow === "M" ? "IMPORT" : "EXPORT";
    return null;
}
function statusFlags(record) {
    const flags = [];
    if (record.isReported === false)
        flags.push("NOT_REPORTED");
    if (record.isOriginalClassification === false)
        flags.push("CLASSIFICATION_CONVERTED");
    if (record.primaryValue === null || record.primaryValue === undefined)
        flags.push("TRADE_VALUE_MISSING");
    if (record.netWgt === null || record.netWgt === undefined)
        flags.push("NET_WEIGHT_MISSING");
    if (record.qty === null || record.qty === undefined)
        flags.push("QUANTITY_MISSING");
    return flags;
}
function canonicalNumericCode(value) {
...
function observation(record, query) {
    ...
    const flow = tradeFlow(record);
    if (!flow)
        throw new Error("UN Comtrade flow must be validated before mapping");
...
function parseResponse(value, query) {
    let parsed;
    try {
...
function requestUrl(query, apiKey) {
...
function validateUnComtradeQuery(query) {
...
async function fetchTrade(query, apiKey, fetcher) {
    ...
    if (!response.ok)
        throw providerHttpStatusError(response, "UN Comtrade");
