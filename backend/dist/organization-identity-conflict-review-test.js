...
function requiredUser(id) {
    const user = usersById.get(id);
    if (!user)
        throw new Error(`Missing identity review test user: ${id}`);
    return user;
}
...
function organization(id, teamId, legalName) {
    return {
        id,
        teamId,
        scopeType: "team",
...
function conflict(id, teamId, ownerId, organizationIds) {
    return {
        id,
...
store.organizations.push(organization(canonicalId, "europe", "Identity Review Canonical Ltd"), organization(aliasId, "europe", "Identity Review Alias Ltd"), organization(keepLeftId, "europe", "Identity Review Independent A Ltd"), organization(keepRightId, "europe", "Identity Review Independent B Ltd"), organization(hiddenLeftId, "identity-review-hidden-team", "Hidden A Ltd"), organization(hiddenRightId, "identity-review-hidden-team", "Hidden B Ltd"));
store.organizationIdentityConflicts.push(conflict(mergeConflictId, "europe", "u_sales_shirley", [canonicalId, aliasId]), conflict(keepConflictId, "europe", "u_sales_mia", [keepLeftId, keepRightId]), conflict(hiddenConflictId, "identity-review-hidden-team", "hidden-owner", [hiddenLeftId, hiddenRightId]));
const projectedOpportunity = {
    id: opportunityId,
...
function reviewRequest(conflictId, token, ifMatch, body) {
    const headers = {};
    if (ifMatch)
        headers["if-match"] = ifMatch;
    return request(`/api/organization-identity-conflicts/${conflictId}/review`, token, {
        method: "POST",
        headers,
...
try {
    const anonymous = await request("/api/organization-identity-conflicts", null);
    assert.equal(anonymous.response.status, 401);
    const sales = await request("/api/organization-identity-conflicts", salesToken);
    assert.equal(sales.response.status, 403);
    assert.equal(sales.json.errorCode, "IDENTITY_REVIEW_FORBIDDEN");
    const superAdmin = await request("/api/organization-identity-conflicts", superAdminToken);
    assert.equal(superAdmin.response.status, 403);
    assert.equal(superAdmin.json.errorCode, "IDENTITY_REVIEW_FORBIDDEN");
    const managerList = await request("/api/organization-identity-conflicts?status=open", managerToken);
    assert.equal(managerList.response.status, 200);
    assert.equal(managerList.json.conflicts.some((item) => item.id === mergeConflictId), true);
    assert.equal(managerList.json.conflicts.some((item) => item.id === hiddenConflictId), false);
    const mergeListItem = managerList.json.conflicts.find((item) => item.id === mergeConflictId);
    assert.equal(mergeListItem.etag, `"organization-identity-conflict:${mergeConflictId}:1"`);
    const adminList = await request("/api/organization-identity-conflicts?status=open", adminToken);
    assert.equal(adminList.response.status, 200);
    const keepListItem = adminList.json.conflicts.find((item) => item.id === keepConflictId);
    assert.ok(keepListItem);
    const crossTeamReview = await reviewRequest(hiddenConflictId, managerToken, `"organization-identity-conflict:${hiddenConflictId}:1"`, {
        action: "keep_separate",
        note: "跨团队复核必须被隐藏"
    });
    assert.equal(crossTeamReview.response.status, 404);
    assert.equal(crossTeamReview.json.errorCode, "IDENTITY_CONFLICT_NOT_FOUND");
    const missingIfMatch = await reviewRequest(mergeConflictId, managerToken, null, {
        action: "merge",
    ...
    const staleIfMatch = await reviewRequest(mergeConflictId, managerToken, `"organization-identity-conflict:${mergeConflictId}:0"`, {
        action: "merge",
        canonicalOrganizationId: canonicalId,
        note: "过期并发版本"
    });
    assert.equal(staleIfMatch.response.status, 412);
    assert.equal(staleIfMatch.json.errorCode, "IDENTITY_CONFLICT_REVISION_CONFLICT");
    const invalidCanonical = await reviewRequest(mergeConflictId, managerToken, mergeListItem.etag, {
        action: "merge",
    ...
    const merged = await reviewRequest(mergeConflictId, managerToken, mergeListItem.etag, {
        action: "merge",
        canonicalOrganizationId: canonicalId,
    ...
    assert.equal(merged.response.headers.get("etag"), `"organization-identity-conflict:${mergeConflictId}:2"`);
    const firstReviewSnapshot = structuredClone(merged.json.review);
    const secondReview = await reviewRequest(mergeConflictId, adminToken, merged.json.etag, {
        action: "keep_separate",
        note: "尝试覆盖已有结论"
    });
    assert.equal(secondReview.response.status, 409);
    assert.equal(secondReview.json.errorCode, "IDENTITY_CONFLICT_ALREADY_REVIEWED");
    assert.deepEqual(store.organizationIdentityConflictReviews.find((item) => item.conflictId === mergeConflictId), firstReviewSnapshot);
    const keptSeparate = await reviewRequest(keepConflictId, adminToken, keepListItem.etag, {
        action: "keep_separate",
    ...
    const openAfterReview = await request("/api/organization-identity-conflicts?status=open", managerToken);
    assert.equal(openAfterReview.json.conflicts.some((item) => item.id === mergeConflictId), false);
    const resolvedAfterReview = await request("/api/organization-identity-conflicts?status=resolved", managerToken);
    assert.equal(resolvedAfterReview.json.conflicts.some((item) => item.id === mergeConflictId), true);
    assert.equal(store.organizationIdentityConflicts.find((item) => item.id === mergeConflictId)?.status, "open");
    const projected = await request("/api/tools/website-opportunities", managerToken);
    assert.equal(projected.response.status, 200);
    assert.equal(projected.json.opportunities.find((item) => item.id === opportunityId)?.organizationId, canonicalId);
    ...
    const reviewOperation = document.paths["/api/organization-identity-conflicts/{id}/review"].post;
    const ifMatchParameter = reviewOperation.parameters.find((item) => item.name === "If-Match" && item.in === "header");
    assert.equal(ifMatchParameter?.required, true);
    assert.deepEqual(reviewOperation.requestBody.content["application/json"].schema.required, ["action", "note"]);
    ...
    console.log("Organization identity conflict review permissions, isolation, immutable history, projection and Swagger tests passed");
}
finally {
