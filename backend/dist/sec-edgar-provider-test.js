...
import { ProviderContractError, normalizeProviderPage, normalizeProviderQuery, providerErrorFromUnknown } from "./provider-contract.js";
...
const adapterPage = await SEC_EDGAR_PROVIDER.search({ query, cursor: "" }, { apiKey: "GoodJobCRM admin@example.com" }, tools());
...
for (const credential of [
    ...
    await assert.rejects(SEC_EDGAR_PROVIDER.search({ query, cursor: "" }, credential, tools()), (error) => {
...
const noKeywordPage = await SEC_EDGAR_PROVIDER.search({
    ...
    cursor: ""
}, { apiKey: "GoodJobCRM admin@example.com" }, tools());
...
await assert.rejects(SEC_EDGAR_PROVIDER.search({ query, cursor: "" }, { apiKey: "GoodJobCRM admin@example.com" }, tools()), (error) => error instanceof ProviderContractError
