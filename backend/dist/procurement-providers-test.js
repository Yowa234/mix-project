...
import { normalizeProviderPage, normalizeProviderQuery } from "./provider-contract.js";
...
const tedPage = await EU_TED_PROVIDER.search({ query, cursor: "" }, { apiKey: "" }, tools(EU_TED_PROVIDER));
...
const worldBankPage = await WORLD_BANK_PROCUREMENT_PROVIDER.search({ query, cursor: "" }, { apiKey: "" }, tools(WORLD_BANK_PROCUREMENT_PROVIDER));
...
const contractsFinderPage = await UK_CONTRACTS_FINDER_PROVIDER.search({ query, cursor: "" }, { apiKey: "" }, tools(UK_CONTRACTS_FINDER_PROVIDER));
assert.equal(requestedUrl, "https://www.contractsfinder.service.gov.uk/api/rest/2/search_notices/json");
...
const skippedUk = await UK_CONTRACTS_FINDER_PROVIDER.search({
    ...
    cursor: ""
}, { apiKey: "" }, tools(UK_CONTRACTS_FINDER_PROVIDER));
