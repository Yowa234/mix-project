...
function masterSecret() {
    ...
    if (configured && configured.length >= 32)
        return configured;
    if (configured)
        throw new Error("智能获客任务加密主密钥必须至少包含 32 个字符");
...
function additionalData(context, payloadType) {
    return Buffer.from([
        context.id,
        context.teamId,
        context.ownerId,
...
export function encryptAgentJobPayload(context, payloadType, payload) {
...
export function decryptAgentJobPayload(context, payloadType, encryptedValue) {
    if (!encryptedValue)
        return {};
