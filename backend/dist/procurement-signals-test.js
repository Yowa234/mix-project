...
import { dismissDealRecommendation, linkProcurementContextToLead, linkRecommendationToDeal, proposeDealRecommendation, recordProcurementSignal } from "./procurement-signals.js";
...
function candidate(ownerId = "sales-a", teamId = "team-a") {
    return {
        id: "candidate-a",
        company: "Buyer A",
...
function recordStrongSignal(store) {
...
}
{
    ...
    linkRecommendationToDeal(store, proposed.recommendation, duplicate, "sales-a", "linked_existing_deal");
...
}
{
    const store = isolatedStore();
    const result = recordStrongSignal(store);
    const recommendation = proposeDealRecommendation(store, result.signal).recommendation;
    assert.throws(() => dismissDealRecommendation(recommendation, "sales-b", "not mine"), /只有建议归属业务员/u);
...
}
{
    ...
    const result = recordStrongSignal(store);
    const recommendation = proposeDealRecommendation(store, result.signal).recommendation;
    linkProcurementContextToLead(store, store.websiteOpportunities[0], "lead-a");
