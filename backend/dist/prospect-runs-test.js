...
import { attachAgentJobIdempotencyAlias, cancelAgentJob, completeAgentJob, enqueueAgentJob, failAgentJob, publicAgentJob, recoverInterruptedAgentJobs, retryAgentJob, startAgentJob } from "./agent-jobs.js";
...
import { createProspectRun, ProspectRunRequestError, transitionProspectRun, validateProspectRunSecurity } from "./prospect-runs.js";
import { validateAllProspectRunQueueBridges, validateProspectRunQueueBridge } from "./prospect-run-queue-bridge.js";
import { app } from "./server.js";
import { getStore } from "./store.js";
import { createOpenApiDocument } from "./swagger.js";
function testUser(id, teamId, role) {
    return {
        id,
        name: id,
...
function installSerializedRunMutationHarness() {
    ...
    const persistSerializedMutation = (mutation) => {
    ...
    store.persistProspectExecutionMutation = (mutation) => {
...
const tokens = Object.fromEntries([salesA, salesA2, managerA, managerB, superAdmin]
...
async function request(input) {
    const headers = {};
    if (input.user)
        headers.authorization = `Bearer ${tokens[input.user.id]}`;
...
async function createReadyCampaign(input) {
    ...
    return {
        campaign: store.prospectCampaigns.find((item) => item.id === created.campaign.id),
        strategy
    };
}
...
try {
    ...
    const unreadyStrategy = store.prospectStrategies.find((item) => item.campaignId === unready.campaign.id);
    ...
    const unreadyCodes = new Set(unreadyResult.json.issues.map((issue) => issue.code));
    ...
    const parentJob = store.agentJobs.find((item) => item.id === parentBinding.jobId);
    const childJob = store.agentJobs.find((item) => item.id === childBinding.jobId);
    ...
    const parentPayload = decryptAgentJobPayload(parentJob, "input", parentJob.inputJsonEncrypted);
    const childPayload = decryptAgentJobPayload(childJob, "input", childJob.inputJsonEncrypted);
    ...
    assert.throws(() => completeAgentJob(parentJob), /通用任务入口/);
    assert.throws(() => failAgentJob(parentJob, "PROVIDER_TIMEOUT"), /通用任务入口/);
    ...
    assert.throws(() => attachAgentJobIdempotencyAlias(store, parentJob, "forbidden-bridge-alias"), /通用任务入口/);
    assert.throws(() => recoverInterruptedAgentJobs(store, "prospect.orchestrate"), /通用恢复入口/);
    ...
    assert.equal(integrityBlockedReplay.json.errorCode, "RUN_QUEUE_BRIDGE_INTEGRITY_INVALID");
    parentJob.idempotencyKey = originalParentIdempotencyKey;
    const originalTraceId = parentJob.traceId;
    parentJob.traceId = "trace_x";
    assert.throws(() => validateProspectRunQueueBridge(store, store.prospectSearchRuns[0]), /任务字段完整性校验失败/);
    ...
    assert.equal(payloadTamperDetail.json.errorCode, "RUN_QUEUE_BRIDGE_INTEGRITY_INVALID");
    ...
    assert.equal(integrityBlockedPause.json.errorCode, "RUN_QUEUE_BRIDGE_INTEGRITY_INVALID");
    ...
    assert.equal(replayAfterPause.response.headers.get("etag"), paused.response.headers.get("etag"));
    ...
    assert.equal(shardStateBlockedCampaignPause.json.errorCode, "RUN_QUEUE_BRIDGE_INTEGRITY_INVALID");
    ...
    assert.deepEqual(cancelled.json.events.map((event) => event.sequence), [1, 2, 3, 4, 5, 6]);
    ...
    const secondRunId = secondRun.json.run.id;
    ...
    const secondPage = await request({
        path: `/api/prospect-runs?limit=1&cursor=${encodeURIComponent(firstPage.json.nextCursor)}`,
        user: salesA2
    });
    ...
    const tampered = await request({
        path: `/api/prospect-runs?limit=1&cursor=${encodeURIComponent(tamperedCursor)}`,
        user: salesA2
    });
    ...
    assert.equal(managerDuplicateRun.json.teamDuplicateAssociation.runId, secondRunId);
    ...
    const executionMutationCallsBeforeCreate = prospectExecutionMutationCalls;
    ...
    const sameStoreCreated = sameStoreRequests.find((item) => !item.idempotencyReplayed);
    const sameStoreReplayed = sameStoreRequests.find((item) => item.idempotencyReplayed);
    assert.ok(sameStoreCreated);
    assert.ok(sameStoreReplayed);
    assert.equal(sameStoreCreated.run.id, sameStoreReplayed.run.id);
    assert.equal(store.prospectSearchRuns.filter((item) => item.campaignId === sameStoreCampaign.campaign.id).length, 1);
    ...
    const differentKeyCreated = differentKeyResults.find((item) => item.status === "fulfilled");
    const differentKeyRejected = differentKeyResults.find((item) => item.status === "rejected");
    assert.ok(differentKeyCreated);
    assert.ok(differentKeyRejected);
    assert.equal(differentKeyRejected.reason instanceof ProspectRunRequestError, true);
    assert.equal(differentKeyRejected.reason.code, "ACTIVE_RUN_EXISTS");
    await transitionProspectRun({
        store,
        user: publicUser(managerB),
        runId: differentKeyCreated.value.run.id,
        ifMatch: `"${differentKeyCreated.value.run.id}:${differentKeyCreated.value.run.revision}"`,
        action: "cancel",
    ...
    assert.equal(store.prospectSearchRuns.some((item) => item.campaignId === pauseRaceCampaign.campaign.id), false);
    assert.ok(prospectExecutionMutationCalls > executionMutationCallsBeforeCreate);
    ...
    const createRunDoc = document.paths["/api/prospect-strategies/{id}/runs"]?.post;
    assert.ok(createRunDoc);
    assert.match(createRunDoc.description || "", /control_plane_only_v1/);
    assert.ok(createRunDoc.parameters?.some((item) => item.name === "If-Match" && item.required));
    assert.ok(createRunDoc.parameters?.some((item) => item.name === "Idempotency-Key" && item.required));
    ...
    assert.equal(JSON.stringify(document).includes("prospect.orchestrate"), false);
    assert.equal(JSON.stringify(document).includes("prospect.provider.fetch"), false);
    const legacyRun = {
    ...
    assert.deepEqual(validateProspectRunQueueBridge(store, legacyRun), { parentJob: null, childJobs: [] });
    assert.equal(store.prospectRunQueueParentBindings.some((item) => item.runId === legacyRun.id), false);
    assert.equal(store.prospectRunQueueChildBindings.some((item) => item.runId === legacyRun.id), false);
    store.prospectSearchRuns.pop();
    const previousNodeEnv = process.env.NODE_ENV;
    const previousIdempotencySecret = process.env.PROSPECT_RUN_IDEMPOTENCY_SECRET;
    ...
    store.agentJobIdempotencyAliases.splice(0, store.agentJobIdempotencyAliases.length, ...original.jobAliases);
