...
export const prospectStrategyProviderPlanSchema = z.array(prospectStrategyProviderPlanItemSchema).max(30).superRefine((items, context) => {
    const seen = new Set();
...
export function normalizeProspectStrategyQuery(value = {}) {
...
export function normalizeProspectStrategyProviderPlan(value = []) {
...
export function resolveProspectStrategyQuery(query, version) {
    return {
...
export function prospectStrategyEtag(strategy) {
...
function publicStrategyEvent(event) {
    const { teamId: _teamId, ...visible } = event;
    return visible;
}
function assertStrategyRole(user) {
    if (user.role === "super_admin") {
        throw new ProspectStrategyRequestError(403, "STRATEGY_CRUD_FORBIDDEN", "超级管理员默认不能访问团队获客策略");
    }
}
...
function findVisibleCampaign(store, user, campaignId) {
    ...
    if (!campaign || !canReadCampaign(user, campaign)) {
        throw new ProspectStrategyRequestError(404, "CAMPAIGN_NOT_FOUND", "获客项目不存在或无权访问");
    }
    return campaign;
}
function findCampaignVersion(store, campaign, versionNumber) {
    const version = store.prospectCampaignVersions.find((item) => item.teamId === campaign.teamId
        && item.campaignId === campaign.id
        && item.version === versionNumber);
    if (!version)
        throw new Error("获客项目策略引用的版本不存在");
    return version;
}
function findVisibleStrategy(store, user, strategyId) {
    ...
    if (!strategy) {
        throw new ProspectStrategyRequestError(404, "STRATEGY_NOT_FOUND", "搜索策略不存在或无权访问");
    }
    const campaign = store.prospectCampaigns.find((item) => item.id === strategy.campaignId
...
function assertCampaignIfMatch(campaign, ifMatch) {
    if (!ifMatch) {
        throw new ProspectStrategyRequestError(428, "PRECONDITION_REQUIRED", "创建搜索策略必须提供 Campaign If-Match");
    }
    const expected = `"${campaign.id}:${campaign.revision}"`;
    if (ifMatch.trim() !== expected) {
        throw new ProspectStrategyRequestError(412, "CAMPAIGN_REVISION_CONFLICT", "获客项目已被其他操作更新，请刷新后重试", { revision: campaign.revision, etag: expected });
    }
}
function assertStrategyIfMatch(strategy, ifMatch) {
    if (!ifMatch) {
        throw new ProspectStrategyRequestError(428, "PRECONDITION_REQUIRED", "修改搜索策略必须提供 If-Match");
    }
    if (ifMatch.trim() !== prospectStrategyEtag(strategy)) {
        throw new ProspectStrategyRequestError(412, "STRATEGY_REVISION_CONFLICT", "搜索策略已被其他操作更新，请刷新后重试", { revision: strategy.revision, etag: prospectStrategyEtag(strategy) });
    }
}
function assertCampaignAllowsStrategyContent(campaign) {
    if (campaign.status !== "draft" && campaign.status !== "paused") {
        throw new ProspectStrategyRequestError(409, "STRATEGY_CAMPAIGN_STATE_INVALID", "只有草稿或已暂停项目可以新增、编辑或审批搜索策略");
    }
}
function assertCurrentDraft(campaign, strategy) {
    if (strategy.status !== "draft") {
        throw new ProspectStrategyRequestError(409, "STRATEGY_READ_ONLY", "已审批或已禁用策略不能修改业务内容");
    }
    if (strategy.campaignVersion !== campaign.currentVersion) {
        throw new ProspectStrategyRequestError(409, "STRATEGY_VERSION_OUTDATED", "该策略属于旧项目版本，请复制为当前版本草稿后再操作");
    }
}
function strategyDetail(store, strategy) {
    return {
        strategy: publicStrategy(strategy),
        events: store.prospectStrategyEvents
            .filter((item) => item.teamId === strategy.teamId && item.strategyId === strategy.id)
            .sort((left, right) => right.createdAt.localeCompare(left.createdAt))
            .map(publicStrategyEvent)
    };
}
function snapshotStrategyState(store) {
    return {
...
function restoreStrategyState(store, snapshot) {
    store.prospectCampaigns.splice(0, store.prospectCampaigns.length, ...snapshot.campaigns);
    store.prospectCampaignEvents.splice(0, store.prospectCampaignEvents.length, ...snapshot.campaignEvents);
...
async function persistStrategyMutation(store, mutation) {
...
export function createProspectStrategyEvent(input) {
    return {
        id: `pse_${randomUUID()}`,
...
export function listProspectStrategies(store, user, campaignId, includeDisabled = false) {
    const campaign = findVisibleCampaign(store, user, campaignId);
    const strategies = store.prospectStrategies
        .filter((item) => item.teamId === campaign.teamId
        && item.campaignId === campaign.id
        && (includeDisabled || item.status !== "disabled"))
        .sort((left, right) => right.campaignVersion - left.campaignVersion
        || right.updatedAt.localeCompare(left.updatedAt))
        .map(publicStrategy);
    return { strategies, total: strategies.length };
}
export function getProspectStrategy(store, user, strategyId) {
...
export async function createProspectStrategy(input) {
    return persistStrategyMutation(input.store, () => {
        ...
        const copySource = input.body.copyFromStrategyId
            ? findVisibleStrategy(input.store, input.user, input.body.copyFromStrategyId).strategy
            : null;
        if (copySource && copySource.campaignId !== campaign.id) {
            throw new ProspectStrategyRequestError(404, "STRATEGY_NOT_FOUND", "复制来源策略不存在或无权访问");
        }
        ...
        const strategy = {
            ...
            name: input.body.name?.trim()
                || (copySource ? `${copySource.name} 副本` : `搜索策略 ${input.store.prospectStrategies.filter((item) => item.campaignId === campaign.id).length + 1}`),
            status: "draft",
            revision: 1,
            query,
            providerPlan,
...
export async function updateProspectStrategy(input) {
    return persistStrategyMutation(input.store, () => {
        ...
        const version = findCampaignVersion(input.store, campaign, strategy.campaignVersion);
...
function catalogValidationIssues(store, version, providerPlan) {
    const issues = [];
    const allowedProviders = new Set(version.snapshot.sourceProviderIds.map((item) => item.toLocaleLowerCase("en-US")));
    for (const plan of providerPlan) {
        ...
        if (!catalog.capabilities.some((item) => item === "company" || item === "ai" || item === "web")) {
            issues.push({
...
function strategyValidationIssues(store, version, query, providerPlan) {
...
function providerReadinessIssues(store, strategy) {
    const issues = [];
    for (const plan of strategy.providerPlan) {
        ...
        if (!catalog)
            continue;
        if (plan.providerId === "ai_search") {
            const ready = store.aiModelConfigs.some((item) => item.ownerId === strategy.ownerId
                && item.teamId === strategy.teamId
                && item.enabled
                && item.useLeadFinder
                && Boolean(item.apiKey));
        ...
        const connection = store.providerConnections.find((item) => item.providerId.toLocaleLowerCase("en-US") === plan.providerId
        ...
        try {
            const configuration = decryptProviderConfiguration(connection, connection.configurationEncrypted);
...
export function approvedStrategiesForCurrentVersion(store, campaign) {
...
export function prospectStrategyActivationIssues(store, campaign, strategies) {
...
export function prospectStrategyRunReadinessIssues(store, campaign, strategy) {
    const issues = [];
...
export async function approveProspectStrategy(input) {
    return persistStrategyMutation(input.store, () => {
        ...
        assertCurrentDraft(campaign, strategy);
        const version = findCampaignVersion(input.store, campaign, strategy.campaignVersion);
        const issues = strategyValidationIssues(input.store, version, strategy.query, strategy.providerPlan);
        if (issues.length) {
            throw new ProspectStrategyRequestError(422, "STRATEGY_VALIDATION_FAILED", "搜索策略尚未通过审批校验", { issues });
        }
        const duplicate = input.store.prospectStrategies.find((item) => item.id !== strategy.id
            && item.teamId === strategy.teamId
        ...
        if (duplicate) {
            throw new ProspectStrategyRequestError(409, "STRATEGY_DUPLICATE_APPROVED", "当前项目版本已存在相同搜索范围的已审批策略", { duplicateStrategyId: duplicate.id });
        }
        const previousRevision = strategy.revision;
        const now = new Date().toISOString();
...
export async function disableProspectStrategy(input) {
    return persistStrategyMutation(input.store, () => {
        ...
        if (campaign.status === "completed" || campaign.status === "archived") {
            throw new ProspectStrategyRequestError(409, "STRATEGY_CAMPAIGN_READ_ONLY", "已完成或已归档项目的策略只能随项目转交负责人");
        }
        if (strategy.status === "disabled") {
            throw new ProspectStrategyRequestError(409, "STRATEGY_STATE_INVALID", "搜索策略已经禁用");
        }
        if (activeProspectRunsForStrategy(input.store, strategy.teamId, strategy.id).length) {
            throw new ProspectStrategyRequestError(409, "STRATEGY_ACTIVE_RUNS", "搜索策略存在活动运行，请先取消后再禁用");
        }
        if (campaign.status === "active"
            && strategy.status === "approved"
            && strategy.campaignVersion === campaign.currentVersion
            && approvedStrategiesForCurrentVersion(input.store, campaign).length <= 1) {
            throw new ProspectStrategyRequestError(409, "STRATEGY_LAST_APPROVED", "活动项目必须保留至少一条已审批策略，请先暂停项目");
        }
...
export function previewProspectStrategy(input) {
    ...
    const duplicate = input.store.prospectStrategies.find((item) => item.id !== strategy.id
        ...
        && input.store.prospectCampaigns.some((candidateCampaign) => candidateCampaign.id === item.campaignId
            && candidateCampaign.teamId === item.teamId
            && canReadCampaign(input.user, candidateCampaign)));
    const validationIssues = strategyValidationIssues(input.store, version, query, providerPlan);
...
export function transferProspectStrategyOwners(input) {
    const strategies = input.store.prospectStrategies.filter((item) => item.teamId === input.campaign.teamId
...
export function assertProspectStrategyOwnerConsistency(strategies, expectedOwnerId) {
