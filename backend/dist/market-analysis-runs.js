...
import { attachAgentJobIdempotencyAlias, completeAgentJob, enqueueAgentJob, failAgentJob, findAgentJobByIdempotency, publicAgentJob, retryAgentJob, startAgentJob } from "./agent-jobs.js";
import { applyMarketTradeObservations, marketTradeObservationIdentity, } from "./market-trade-observations.js";
import { materializeMarketOpportunityFacts } from "./market-opportunity-facts.js";
import { normalizeTradeQuery, ProviderContractError, providerErrorFromUnknown } from "./provider-contract.js";
import { providerRequestFingerprint } from "./provider-request-logging.js";
import { createProviderExecutionContext, executeProviderTradeQuery } from "./provider-runtime.js";
...
const activeMarketAnalysisRuns = new Map();
...
export class MarketAnalysisRunProviderError extends Error {
    ...
    failure;
    job;
    constructor(status, failure, job) {
...
function runScope(job, storedInput) {
    return {
        jobId: job.id,
        teamId: job.teamId,
        ownerId: job.ownerId,
...
function snapshotRunState(store, scope) {
    return {
        job: structuredClone(store.agentJobs.find((item) => item.id === scope.jobId) || null),
        providerRequestLogs: structuredClone(store.providerRequestLogs.filter((item) => item.runId === scope.jobId))
    };
}
function restoreRunState(store, scope, snapshot) {
    const jobIndex = store.agentJobs.findIndex((item) => item.id === scope.jobId);
    if (jobIndex >= 0) {
        if (snapshot.job)
            store.agentJobs.splice(jobIndex, 1, snapshot.job);
        else
            store.agentJobs.splice(jobIndex, 1);
    }
    ...
    store.providerRequestLogs.splice(0, store.providerRequestLogs.length, ...snapshot.providerRequestLogs, ...unrelatedLogs);
}
function snapshotObservationState(store, scope, observations) {
    const identities = new Set(observations.map((observation) => marketTradeObservationIdentity({
        teamId: scope.teamId,
    ...
    return {
        identities,
        observations: structuredClone(store.marketTradeObservations.filter((item) => identities.has(marketTradeObservationIdentity(item))))
    };
}
function restoreObservationState(store, snapshot) {
    const unrelated = store.marketTradeObservations.filter((item) => !snapshot.identities.has(marketTradeObservationIdentity(item)));
    store.marketTradeObservations.splice(0, store.marketTradeObservations.length, ...snapshot.observations, ...unrelated);
}
function requestFingerprint(campaignId, providerId, query) {
...
function providerFailureStatus(failure) {
    ...
    if (failure.code === "PROVIDER_AUTH_FAILED"
        ...
        || failure.code === "PROVIDER_POLICY_BLOCKED")
        return 409;
    return 502;
}
...
function publicMarketAnalysisObservation(item) {
    return {
        id: item.id,
...
function completedSummary(job, fallbackQuery) {
    if (!job.outputJsonEncrypted)
        return null;
    try {
...
function storedInputForJob(job) {
    let payload;
    try {
        payload = decryptAgentJobPayload(job, "input", job.inputJsonEncrypted);
    }
    catch {
        throw new MarketAnalysisRunRequestError(409, "IDEMPOTENCY_STATE_INVALID", "历史运行状态不可读取，请更换幂等键后重试");
    }
    ...
    let query;
    try {
        query = normalizeTradeQuery(payload.query);
    }
    catch {
        throw new MarketAnalysisRunRequestError(409, "IDEMPOTENCY_STATE_INVALID", "历史市场分析参数已损坏，请更换幂等键后重试");
    }
    if (!campaignId
        || !providerId
        || !storedFingerprint
        ...
        throw new MarketAnalysisRunRequestError(409, "IDEMPOTENCY_STATE_INVALID", "历史市场分析任务与兼容项目引用不一致，请更换幂等键后重试");
    }
    return {
        campaignId,
        providerId,
...
function assertRetryDue(job) {
    ...
    if (Number.isFinite(retryAt.getTime()) && retryAt.getTime() > Date.now()) {
        ...
        throw new MarketAnalysisRunProviderError(providerFailureStatus(failure), failure, job);
    }
}
function findMatchingCooldownJob(store, input) {
    const now = Date.now();
    const candidates = store.agentJobs
        .filter((job) => job.teamId === input.user.teamId
...
async function attachAndPersistAlias(store, job, idempotencyScope) {
    try {
        await persistStoreMutation(store, () => {
            ...
            return {
                ...
                rollback: () => {
                    if (!attachedAliasId)
                        return;
                    const aliasIndex = store.agentJobIdempotencyAliases.findIndex((item) => item.id === attachedAliasId);
                    if (aliasIndex >= 0)
                        store.agentJobIdempotencyAliases.splice(aliasIndex, 1);
                }
            };
        });
    }
        ...
        if (error instanceof Error && error.message === "该幂等键已绑定其他任务") {
            throw new MarketAnalysisRunRequestError(409, "IDEMPOTENCY_KEY_REUSED", "该幂等键已用于不同的市场分析请求");
        }
        throw error;
    }
}
...
function prepareJobForExecution(job) {
...
function assertJobIdempotencyBindingAvailable(store, job) {
    const conflictingPrimary = store.agentJobs.find((item) => item.id !== job.id
        && item.teamId === job.teamId
        && item.jobType === job.jobType
        && item.idempotencyKey === job.idempotencyKey);
    const conflictingAlias = store.agentJobIdempotencyAliases.find((item) => item.jobId !== job.id
        && item.teamId === job.teamId
        && item.jobType === job.jobType
        && item.idempotencyKey === job.idempotencyKey);
...
async function executeMarketAnalysisJob(input) {
    ...
    const connection = store.providerConnections.find((item) => item.providerId === provider.id
        && item.scope === "personal"
    ...
    try {
        return await persistStoreMutation(store, () => {
            const beforeFinal = {
                ...
                calculationEventIds: new Set(store.marketOpportunityCalculationEvents.map((item) => item.id))
            };
            const createdBatchIds = new Set();
            const createdSnapshotIds = new Set();
            const createdCalculationEventIds = new Set();
            const restoreCreatedOpportunityFacts = () => {
                store.marketOpportunityBatches.splice(0, store.marketOpportunityBatches.length, ...store.marketOpportunityBatches.filter((item) => !createdBatchIds.has(item.id)));
                store.marketOpportunitySnapshots.splice(0, store.marketOpportunitySnapshots.length, ...store.marketOpportunitySnapshots.filter((item) => !createdSnapshotIds.has(item.id)));
                store.marketOpportunityCalculationEvents.splice(0, store.marketOpportunityCalculationEvents.length, ...store.marketOpportunityCalculationEvents.filter((item) => !createdCalculationEventIds.has(item.id)));
            };
            const restorePartiallyCreatedOpportunityFacts = () => {
                store.marketOpportunityBatches.splice(0, store.marketOpportunityBatches.length, ...store.marketOpportunityBatches.filter((item) => beforeFinal.batchIds.has(item.id)));
                store.marketOpportunitySnapshots.splice(0, store.marketOpportunitySnapshots.length, ...store.marketOpportunitySnapshots.filter((item) => beforeFinal.snapshotIds.has(item.id)));
                store.marketOpportunityCalculationEvents.splice(0, store.marketOpportunityCalculationEvents.length, ...store.marketOpportunityCalculationEvents.filter((item) => beforeFinal.calculationEventIds.has(item.id)));
            };
            try {
                ...
                if (!opportunity.reusedBatch)
                    createdBatchIds.add(opportunity.batch.id);
                for (const snapshot of opportunity.snapshots)
...
function responseFor(duplicate, job, result) {
    return {
        duplicate,
...
export async function createMarketAnalysisRun(input) {
    ...
    if (existingByIdempotency) {
        ...
        if (existingByIdempotency.status === "succeeded"
            ...
            return responseFor(true, existingByIdempotency, completedSummary(existingByIdempotency, storedInput.query));
        }
    ...
    if (active) {
        ...
        if (!ready) {
            ...
            throw new MarketAnalysisRunRequestError(409, "IDEMPOTENCY_STATE_CHANGED", "幂等状态已变化，请重新提交请求");
        }
        await active.persistAlias(() => attachAndPersistAlias(store, active.job, idempotencyScope));
    ...
    if (enqueuedIndex >= 0)
        store.agentJobs.splice(enqueuedIndex, 1);
    const storedInput = {
        campaignId: input.campaignId,
        providerId: provider.id,
        requestFingerprint: fingerprint,
...
export async function retryMarketAnalysisJob(store, user, job) {
    assertJobOwner(job, user);
