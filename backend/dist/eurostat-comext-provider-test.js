...
import { ProviderContractError, normalizeProviderTradePage, normalizeTradeQuery, providerErrorFromUnknown } from "./provider-contract.js";
...
const adapterPage = await EUROSTAT_COMEXT_PROVIDER.trade({ query: baseQuery, cursor: "" }, { apiKey: "" }, tools());
const requestUrl = new URL(requestedUrl);
assert.equal(requestUrl.origin, "https://ec.europa.eu");
assert.equal(requestUrl.pathname, "/eurostat/api/comext/dissemination/statistics/1.0/data/DS-045409");
...
async function expectPolicy(query, baseUrl = "") {
    await assert.rejects(EUROSTAT_COMEXT_PROVIDER.trade({ query, cursor: "" }, { apiKey: "", baseUrl }, tools()), (error) => providerErrorFromUnknown(error, "trade").code === "PROVIDER_POLICY_BLOCKED");
}
for (const override of [
    ...
    await expectPolicy({ ...baseQuery, ...override });
}
await expectPolicy(baseQuery, "https://ec.europa.eu");
setProviderHttpTestTransport(async () => new Response(JSON.stringify({ ...responsePayload(), id: ["freq"] }), { status: 200 }));
await assert.rejects(EUROSTAT_COMEXT_PROVIDER.trade({ query: baseQuery, cursor: "" }, { apiKey: "" }, tools()), (error) => error instanceof ProviderContractError
    && error.code === "PROVIDER_SCHEMA_CHANGED");
assertProviderRequestAllowed("https://ec.europa.eu/eurostat/api/comext/dissemination/statistics/1.0/data/DS-045409?freq=M", "GET", EUROSTAT_COMEXT_PROVIDER.networkPolicy);
assert.throws(() => assertProviderRequestAllowed("https://ec.europa.eu/eurostat/api/comext/dissemination/statistics/1.0/data/OTHER", "GET", EUROSTAT_COMEXT_PROVIDER.networkPolicy));
