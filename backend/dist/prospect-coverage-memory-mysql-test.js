...
import { PROSPECT_COVERAGE_MEMORY_CONTRACT, ProspectCoverageMemoryError, listOwnerProspectCoverageEvents, listTenantProspects } from "./prospect-coverage-memory.js";
...
const identityMasterSecret = "coverage-mysql-identity-master-secret-v1-".repeat(2);
const rawEnvelopeSecret = "coverage-mysql-raw-envelope-secret-v1-".repeat(2);
const rawIdentitySecret = "coverage-mysql-raw-identity-secret-v1-".repeat(2);
const coverageMasterSecret = "coverage-mysql-state-master-secret-v1-".repeat(2);
const at = (minute) => new Date(Date.UTC(2026, 6, 14, 8, minute)).toISOString();
...
async function insertCoverageFixture(connection, fixture) {
...
function coverageInput(fixture, resolutionId, coveredAt, evidenceHash) {
    return {
        teamId: fixture.teamId,
        ownerId: fixture.ownerId,
        resolutionId,
...
async function main() {
    ...
    if (!applicationUrl || !adminConnectionUrl) {
        throw new Error("Coverage MySQL test requires MYSQL_TEST_ADMIN_URL, "
            + "DATABASE_URL or MYSQL_URL");
    }
    const adminUrl = new URL(adminConnectionUrl);
    const appUrl = new URL(applicationUrl);
    const databaseName = `goodjob_coverage_test_${randomUUID().replaceAll("-", "").slice(0, 16)}`;
    ...
    try {
       ...
       CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`);
        ...
        const [ownerKeyRows] = await database.query(`SELECT constraint_name AS constraintName, column_name AS columnName
       ...
       ORDER BY constraint_name, ordinal_position`, [databaseName]);
        assert.deepEqual(ownerKeyRows.map((row) => [
        ...
        const firstIdentity = await store.resolveOrganizationStrongIdentity(identityInput(first, "COVERAGE-REG-100"));
        assert.ok(["new_entity", "exact_match"].includes(firstIdentity.resolution.result)
            && firstIdentity.resolution.organizationId
            && firstIdentity.resolution.bindingId, JSON.stringify(firstIdentity.resolution));
        const firstResult = await store.recordProspectCoverage(coverageInput(first, firstIdentity.resolution.id, at(20)));
        assert.equal(firstResult.classification, "net_new");
        assert.equal(firstResult.prospect.queueState, "pending");
        const replay = await store.recordProspectCoverage(coverageInput(first, firstIdentity.resolution.id, at(20)));
        assert.equal(replay.idempotent, true);
        stage = "cross owner duplicate and new intelligence";
        const duplicateSource = createRawFixture("coverage-team-a", "owner-b", "source-b");
        await insertCoverageFixture(database, duplicateSource);
        const duplicateIdentity = await store.resolveOrganizationStrongIdentity(identityInput(duplicateSource, "COVERAGE-REG-100"));
        assert.equal(duplicateIdentity.resolution.organizationId, firstIdentity.resolution.organizationId);
        const duplicate = await store.recordProspectCoverage(coverageInput(duplicateSource, duplicateIdentity.resolution.id, at(21)));
        ...
        const intelligenceSource = createRawFixture("coverage-team-a", "owner-b", "source-b");
        await insertCoverageFixture(database, intelligenceSource);
        const intelligenceIdentity = await store.resolveOrganizationStrongIdentity(identityInput(intelligenceSource, "COVERAGE-REG-100"));
        const intelligence = await store.recordProspectCoverage(coverageInput(intelligenceSource, intelligenceIdentity.resolution.id, at(22), hash(60_001)));
        ...
        const otherTeam = createRawFixture("coverage-team-b", "owner-a", "source-a");
        await insertCoverageFixture(database, otherTeam);
        const otherIdentity = await store.resolveOrganizationStrongIdentity(identityInput(otherTeam, "COVERAGE-REG-100"));
        const otherResult = await store.recordProspectCoverage(coverageInput(otherTeam, otherIdentity.resolution.id, at(24)));
        assert.equal(otherResult.classification, "net_new");
        assert.equal(listTenantProspects(store, { teamId: "coverage-team-a" }).length, 1);
        assert.equal(listTenantProspects(store, { teamId: "coverage-team-b" }).length, 1);
        ...
        const coldState = await loadProspectCoverageState(pool, "coverage-team-a");
        assert.equal(coldState.tenantProspects.length, 1);
        assert.equal(coldState.prospectCoverageEvents.length, 4);
        const [prospectRows] = await database.query(`SELECT id,row_mac FROM tenant_prospects
       WHERE team_id = 'coverage-team-a'`);
        const prospectRow = prospectRows[0];
        await database.query("UPDATE tenant_prospects SET row_mac = ? WHERE id = ?", ["0".repeat(64), prospectRow.id]);
        await assert.rejects(loadProspectCoverageState(pool, "coverage-team-a"), isCoverageIntegrityError);
        await database.query("UPDATE tenant_prospects SET row_mac = ? WHERE id = ?", [prospectRow.row_mac, prospectRow.id]);
        const [eventRows] = await database.query(`SELECT id,row_mac FROM prospect_coverage_events
       WHERE team_id = 'coverage-team-a'
       ORDER BY sequence_no LIMIT 1`);
        const eventRow = eventRows[0];
        await database.query("UPDATE prospect_coverage_events SET row_mac = ? WHERE id = ?", ["f".repeat(64), eventRow.id]);
        await assert.rejects(loadProspectCoverageState(pool, "coverage-team-a"), isCoverageIntegrityError);
        await database.query("UPDATE prospect_coverage_events SET row_mac = ? WHERE id = ?", [eventRow.row_mac, eventRow.id]);
        assert.deepEqual(await countBusinessRows(database), businessBefore);
        await database.end();
        await pool.end();
        console.log("Prospect coverage MySQL schema, persistence, isolation, cold load "
