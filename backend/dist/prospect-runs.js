...
import { cancelProspectRunQueueBridge, PROSPECT_RUN_QUEUE_BRIDGE_VERSION, ProspectRunQueueBridgeIntegrityError, registerProspectRunQueueBridge, validateProspectRunQueueBridge } from "./prospect-run-queue-bridge.js";
import { prospectStrategyEtag, prospectStrategyRunReadinessIssues, resolveProspectStrategyQuery } from "./prospect-strategies.js";
...
export function validateProspectRunSecurity() {
    const idempotencySecret = process.env.PROSPECT_RUN_IDEMPOTENCY_SECRET?.trim() || "";
    const cursorSecret = process.env.PROSPECT_RUN_CURSOR_SECRET?.trim() || "";
    if (process.env.NODE_ENV === "production"
        && Buffer.byteLength(idempotencySecret, "utf8") < 32) {
        throw new Error("生产环境必须配置至少 32 字节的 PROSPECT_RUN_IDEMPOTENCY_SECRET");
    }
    if (process.env.NODE_ENV === "production"
        && Buffer.byteLength(cursorSecret, "utf8") < 32) {
        throw new Error("生产环境必须配置至少 32 字节的 PROSPECT_RUN_CURSOR_SECRET");
    }
}
function configuredSecret(environmentName, fallback) {
...
export function prospectRunExecutionSnapshotHash(snapshot) {
    return stableHash(snapshot);
}
function idempotencyKeyHash(rawKey) {
    return createHmac("sha256", configuredSecret("PROSPECT_RUN_IDEMPOTENCY_SECRET", DEVELOPMENT_IDEMPOTENCY_SECRET))
        .update(rawKey)
        .digest("hex");
}
...
function assertRunRole(user) {
    if (user.role === "super_admin") {
        throw new ProspectRunRequestError(403, "RUN_ACCESS_FORBIDDEN", "超级管理员默认不能访问团队搜索运行");
    }
}
function canReadCampaign(user, campaign) {
    if (user.role === "super_admin")
        return false;
...
function visibleCampaign(store, user, campaignId, teamId) {
    assertRunRole(user);
    const campaign = store.prospectCampaigns.find((item) => item.id === campaignId && (!teamId || item.teamId === teamId));
    return campaign && canReadCampaign(user, campaign) ? campaign : null;
}
function findVisibleStrategy(store, user, strategyId) {
    assertRunRole(user);
    const strategy = store.prospectStrategies.find((item) => item.id === strategyId);
    if (!strategy) {
        throw new ProspectRunRequestError(404, "STRATEGY_NOT_FOUND", "搜索策略不存在或无权访问");
    }
    const campaign = visibleCampaign(store, user, strategy.campaignId, strategy.teamId);
    if (!campaign) {
        throw new ProspectRunRequestError(404, "STRATEGY_NOT_FOUND", "搜索策略不存在或无权访问");
    }
    return { strategy, campaign };
}
function findVisibleRun(store, user, runId) {
    assertRunRole(user);
    ...
    const campaign = run
        ...
        : null;
    if (!run || !campaign) {
        throw new ProspectRunRequestError(404, "RUN_NOT_FOUND", "搜索运行不存在或无权访问");
    }
    return { run, campaign };
}
function assertStrategyIfMatch(strategy, ifMatch) {
    if (!ifMatch) {
        throw new ProspectRunRequestError(428, "PRECONDITION_REQUIRED", "创建搜索运行必须提供 Strategy If-Match");
    }
    const expected = prospectStrategyEtag(strategy);
    if (ifMatch.trim() !== expected) {
        throw new ProspectRunRequestError(412, "STRATEGY_REVISION_CONFLICT", "搜索策略已被其他操作更新，请刷新后重试", { revision: strategy.revision, etag: expected });
    }
}
export function prospectRunEtag(run) {
    return `"${run.id}:${run.revision}"`;
}
function assertRunIfMatch(run, ifMatch) {
    if (!ifMatch) {
        throw new ProspectRunRequestError(428, "PRECONDITION_REQUIRED", "修改搜索运行必须提供 If-Match");
    }
    const expected = prospectRunEtag(run);
    if (ifMatch.trim() !== expected) {
        throw new ProspectRunRequestError(412, "RUN_REVISION_CONFLICT", "搜索运行已被其他操作更新，请刷新后重试", { revision: run.revision, etag: expected });
    }
}
function campaignVersion(store, campaign, versionNumber) {
    const version = store.prospectCampaignVersions.find((item) => item.teamId === campaign.teamId
        && item.campaignId === campaign.id
        && item.version === versionNumber);
    if (!version) {
        throw new ProspectRunRequestError(409, "RUN_SOURCE_INTEGRITY_INVALID", "搜索运行引用的项目版本不存在");
    }
    return version;
}
function runReadinessIssues(store, campaign, strategy) {
    const issues = [];
...
function providerSnapshot(store, strategy) {
    return strategy.providerPlan.map((plan, index) => {
        const catalog = store.providerCatalog.find((item) => item.code.toLocaleLowerCase("en-US") === plan.providerId);
        if (!catalog) {
            throw new ProspectRunRequestError(409, "RUN_SOURCE_INTEGRITY_INVALID", `数据源 ${plan.providerId} 不在当前目录中`);
        }
        const provider = plan.providerId === "ai_search"
            ? null
            : getProvider(plan.providerId);
        if (plan.providerId !== "ai_search" && !provider) {
            throw new ProspectRunRequestError(409, "RUN_SOURCE_INTEGRITY_INVALID", `数据源 ${plan.providerId} 缺少执行适配器`);
        }
        return {
...
function executionSnapshot(store, campaign, strategy) {
...
function publicRun(run, includeSnapshot) {
    return {
        id: run.id,
...
function snapshotRunState(store) {
    return {
...
function restoreRunState(store, snapshot) {
    ...
    store.prospectRunEvents.splice(0, store.prospectRunEvents.length, ...snapshot.events);
    store.agentJobs.splice(0, store.agentJobs.length, ...snapshot.jobs);
    store.agentJobIdempotencyAliases.splice(0, store.agentJobIdempotencyAliases.length, ...snapshot.jobAliases);
...
function idempotencyMatch(input) {
    return input.store.prospectSearchRuns.find((run) => run.teamId === input.user.teamId
...
function replayResult(input) {
    ...
    if (!run)
        return null;
    const campaign = visibleCampaign(input.store, input.user, run.campaignId, run.teamId);
    if (!campaign) {
        throw new ProspectRunRequestError(409, "IDEMPOTENCY_KEY_UNAVAILABLE", "该幂等键已用于当前不可访问的请求，请更换幂等键");
    }
    if (run.requestHash !== input.requestHash) {
        throw new ProspectRunRequestError(409, "IDEMPOTENCY_KEY_CONFLICT", "该 Idempotency-Key 已用于不同的搜索运行请求");
    }
    return {
...
function activeDuplicate(store, strategy, ownerId) {
    return store.prospectSearchRuns.find((run) => run.teamId === strategy.teamId
        && run.ownerId === ownerId
        && run.queryFingerprint === strategy.queryFingerprint
        && isActiveProspectRun(run));
}
function teamDuplicateAssociation(store, user, strategy, ownerId) {
    if (user.role !== "manager" && user.role !== "admin")
        return null;
    const duplicate = store.prospectSearchRuns.find((run) => run.teamId === strategy.teamId
        && run.ownerId !== ownerId
        && run.queryFingerprint === strategy.queryFingerprint
        && isActiveProspectRun(run));
    return duplicate ? {
...
async function recoverCreateConflict(input) {
    try {
        ...
        throw new ProspectRunRequestError(503, "RUN_CONFLICT_RECOVERY_UNAVAILABLE", "搜索运行并发冲突恢复暂不可用，请稍后重试");
    }
    findVisibleStrategy(input.store, input.user, input.strategyId);
    const replay = replayResult(input);
    if (replay)
        return replay;
    const { strategy, campaign } = findVisibleStrategy(input.store, input.user, input.strategyId);
    const duplicate = activeDuplicate(input.store, strategy, campaign.ownerId);
    if (duplicate) {
        throw new ProspectRunRequestError(409, "ACTIVE_RUN_EXISTS", "当前负责人已有相同搜索范围的活动运行", { runId: duplicate.id, campaignId: duplicate.campaignId });
    }
    throw new ProspectRunRequestError(409, "RUN_CONCURRENT_CONFLICT", "搜索运行已被并发请求更新，请刷新后重试");
}
export async function createProspectRun(input) {
    const preflight = findVisibleStrategy(input.store, input.user, input.strategyId);
    ...
    try {
        return await persistRunMutation(input.store, () => {
            ...
            if (issues.length) {
                throw new ProspectRunRequestError(422, "RUN_NOT_READY", "搜索运行尚未通过就绪校验", { issues });
            }
            const duplicate = activeDuplicate(input.store, strategy, campaign.ownerId);
            if (duplicate) {
                throw new ProspectRunRequestError(409, "ACTIVE_RUN_EXISTS", "当前负责人已有相同搜索范围的活动运行", { runId: duplicate.id, campaignId: duplicate.campaignId });
            }
            const association = teamDuplicateAssociation(input.store, input.user, strategy, campaign.ownerId);
...
export function parseProspectRunListQuery(value) {
...
function matchesFilters(run, filters) {
    if (filters.campaignId && run.campaignId !== filters.campaignId)
        return false;
    if (filters.strategyId && run.strategyId !== filters.strategyId)
        return false;
    if (filters.ownerId && run.ownerId !== filters.ownerId)
        return false;
...
function afterCursor(run, cursor) {
    return run.createdAt < cursor.createdAt
        || (run.createdAt === cursor.createdAt && run.id < cursor.id);
}
function cursorContext(user) {
    return [user.teamId, user.id, user.role].join("\u001f");
}
function cursorSignature(encoded, context) {
    return createHmac("sha256", configuredSecret("PROSPECT_RUN_CURSOR_SECRET", DEVELOPMENT_CURSOR_SECRET))
        .update(context)
        .update("\n")
        .update(encoded)
        .digest("base64url");
...
function encodeCursor(payload, context) {
...
function decodeCursor(value, context) {
    ...
    if (!encoded || !signature || rest.length
        ...
        throw new ProspectRunRequestError(400, "RUN_CURSOR_INVALID", "分页游标无效、已过期或不属于当前查询");
    }
    try {
...
export function listProspectRuns(input) {
    ...
    const matched = input.store.prospectSearchRuns
        .filter((run) => Boolean(visibleCampaign(input.store, input.user, run.campaignId, run.teamId)))
...
export function getProspectRun(store, user, runId) {
    return runDetail(store, findVisibleRun(store, user, runId).run);
}
function verifyStoredSnapshot(run) {
    if (prospectRunExecutionSnapshotHash(run.executionSnapshot)
        !== run.executionSnapshotHash) {
        throw new ProspectRunRequestError(409, "RUN_SNAPSHOT_INTEGRITY_INVALID", "搜索运行快照校验失败，不能继续变更状态");
    }
}
function verifyQueueBridge(store, run) {
    try {
        return validateProspectRunQueueBridge(store, run);
    }
    catch (error) {
        if (error instanceof ProspectRunQueueBridgeIntegrityError) {
            throw new ProspectRunRequestError(409, error.code, "搜索运行队列桥接完整性校验失败，不能继续操作");
        }
        throw error;
    }
}
function assertShardState(store, run, expectedStatus) {
    const shards = store.prospectRunShards.filter((item) => item.teamId === run.teamId && item.runId === run.id);
    if (shards.length !== run.executionSnapshot.providerPlan.length
        || shards.some((shard) => shard.status !== expectedStatus)) {
        throw new ProspectRunRequestError(409, "RUN_SHARD_STATE_INVALID", "搜索运行分片状态不一致，不能继续变更状态");
    }
    return shards;
}
function transitionDefinition(current, action) {
    if (action === "pause" && current === "queued") {
        return { status: "paused", eventType: "paused" };
    }
    if (action === "resume" && current === "paused") {
        return { status: "queued", eventType: "resumed" };
    }
    if (action === "cancel" && (current === "queued" || current === "paused")) {
        return { status: "cancelled", eventType: "cancelled" };
    }
    throw new ProspectRunRequestError(409, "RUN_STATE_INVALID", `搜索运行不能从 ${current} 执行 ${action}`);
}
export async function transitionProspectRun(input) {
    return persistRunMutation(input.store, () => {
        const before = snapshotRunState(input.store);
        const { run, campaign } = findVisibleRun(input.store, input.user, input.runId);
        ...
        if (input.action === "resume") {
            const strategy = input.store.prospectStrategies.find((item) => item.id === run.strategyId
                && item.teamId === run.teamId
                && item.campaignId === run.campaignId);
            if (!strategy || campaign.ownerId !== run.ownerId) {
                throw new ProspectRunRequestError(409, "RUN_SOURCE_INTEGRITY_INVALID", "搜索运行与当前项目或策略归属不一致");
            }
            const issues = runReadinessIssues(input.store, campaign, strategy);
            if (issues.length) {
                throw new ProspectRunRequestError(422, "RUN_NOT_READY", "搜索运行恢复前的就绪校验未通过", { issues });
            }
            const duplicate = input.store.prospectSearchRuns.find((item) => item.id !== run.id
                && item.teamId === run.teamId
        ...
        input.store.prospectRunEvents.push(createRunEvent({
            ...
            reason: input.body.reason || (input.action === "pause"
                ? "暂停搜索运行"
                : input.action === "resume"
                    ? "恢复搜索运行"
                    : "取消搜索运行")
