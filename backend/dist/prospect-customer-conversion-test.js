...
import { convertProspectToCustomer, ProspectCustomerConversionError } from "./prospect-customer-conversion.js";
import { PROSPECT_LEAD_CONVERSION_CONTRACT, PROSPECT_LEAD_SOURCE_CHANNEL } from "./prospect-lead-conversion.js";
import { getStore } from "./store.js";
const coverageSecret = "prospect-customer-conversion-test-secret-v1-".repeat(2);
const at = (minute) => new Date(Date.UTC(2026, 6, 15, 10, minute)).toISOString();
...
function customer(id, teamId, ownerId, company = "Existing Customer GmbH") {
    return {
        id,
        company,
        country: "DE",
...
function acquisitionEvent(fixture, input = {}) {
    sequence += 1;
    return {
...
function isConversionError(code) {
    return (error) => error instanceof ProspectCustomerConversionError && error.code === code;
}
...
function testCreateAndIdempotency() {
    ...
    assert.equal(result.sourceEvent.leadSourceEventId, fixture.source.id);
    assert.equal(result.sourceEvent.organizationId, fixture.organizationId);
    assert.equal(result.coverageEvent.reasonCode, "HUMAN_CONFIRMED_CUSTOMER_CONVERSION");
    ...
    assert.equal(fixture.store.customerActivities.length, 1);
    assert.equal(fixture.store.leadActivities.length, 1);
    assert.throws(() => convertProspectToCustomer(fixture.store, {
        ...fixture.input,
        company: "Changed payload"
    }), isConversionError("PROSPECT_CUSTOMER_CONVERSION_IDEMPOTENCY_CONFLICT"));
}
function testLinkExistingWithoutOverwriting() {
    const fixture = createFixture("link");
    const existing = customer("customer-current-owner", fixture.teamId, fixture.ownerId);
    ...
    assert.deepEqual(fixture.store.customers.find((item) => item.id === existing.id), masterBefore);
    assert.equal(fixture.store.customers.length, 1);
...
function testOwnerAndSourceIsolation() {
    ...
    owner.store.customers.push(customer("customer-other-owner", owner.teamId, owner.otherOwnerId));
    const ownerBefore = {
        ...
        sourceEvents: structuredClone(owner.store.customerAcquisitionSourceEvents)
    };
    assert.throws(() => convertProspectToCustomer(owner.store, {
        ...owner.input,
        mode: "link_existing",
    ...
    assert.deepEqual(owner.store.customerAcquisitionSourceEvents, ownerBefore.sourceEvents);
    ...
    assert.throws(() => convertProspectToCustomer(source.store, source.input), isConversionError("PROSPECT_CUSTOMER_CONVERSION_SOURCE_INVALID"));
...
function testOrganizationOwnershipConflict() {
    ...
    fixture.store.customerAcquisitionSourceEvents.push(acquisitionEvent(fixture, {
        teamId: fixture.teamId,
        ownerId: fixture.otherOwnerId,
        organizationId: fixture.organizationId
    }));
    assert.throws(() => convertProspectToCustomer(fixture.store, fixture.input), isConversionError("PROSPECT_CUSTOMER_CONVERSION_ORGANIZATION_OWNERSHIP_CONFLICT"));
...
console.log("Prospect customer conversion create, link, isolation, source, "
