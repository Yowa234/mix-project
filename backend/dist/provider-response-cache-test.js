...
import { providerRequestFingerprint, recordProviderCacheHit, withProviderRequestLogging } from "./provider-request-logging.js";
...
const hit = await readProviderResponseCache(key, new Date("2026-07-13T12:00:00.000Z"));
...
assert.deepEqual((await readProviderResponseCache(key, new Date("2026-07-13T12:00:00.000Z")))?.payload, payload);
assert.deepEqual((await readProviderResponseCache(restrictedKey, new Date("2026-07-13T12:00:00.000Z")))?.payload, restrictedPayload);
const expired = await readProviderResponseCache(key, new Date("2026-07-15T00:00:00.000Z"));
...
const tampered = await readProviderResponseCache(key, new Date("2026-07-13T12:00:00.000Z"));
