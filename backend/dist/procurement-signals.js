...
function productLooksSimilar(left, right) {
...
function resolveCustomerId(store, signal) {
    if (signal.customerId)
        return signal.customerId;
    const candidate = store.websiteOpportunities.find((item) => item.id === signal.prospectCandidateId
    ...
    const lead = leadId
        ? store.leads.find((item) => item.id === leadId
            && item.teamId === signal.teamId
            && item.ownerId === signal.ownerId)
        : undefined;
    ...
    return prospectId
        ? store.tenantProspects.find((item) => item.id === prospectId && item.teamId === signal.teamId)?.customerId || ""
        : "";
}
export function resolveRecommendationCustomerId(store, recommendation) {
    if (recommendation.customerId)
        return recommendation.customerId;
    const signal = store.procurementSignals.find((item) => item.id === recommendation.signalId
        && item.teamId === recommendation.teamId
        && item.ownerId === recommendation.ownerId);
    return signal ? resolveCustomerId(store, signal) : "";
}
export function activeDuplicateDeals(store, signal) {
    const customerId = resolveCustomerId(store, signal);
    if (!customerId || !signal.product)
        return [];
    return store.deals.filter((deal) => deal.teamId === signal.teamId
...
function derivedEvidence(input) {
    ...
    if (clean(input.product, 200))
        evidence.add("product_requirement");
    if (Number(input.quantity || 0) > 0)
        evidence.add("quantity");
    if (Number(input.targetPrice || 0) > 0)
        evidence.add("target_price");
    if (clean(input.deliveryRequirement))
        evidence.add("delivery");
    if (clean(input.certificationRequirement))
        evidence.add("certification");
    if (clean(input.purchaseTimeline))
        evidence.add("purchase_timeline");
    if (clean(input.projectName))
        evidence.add("project_tender");
    return [...evidence];
}
function defaultNextAction(signal) {
    if (signal.nextAction)
        return signal.nextAction;
    if (!signal.quantity)
        return "确认采购数量、规格与年度用量";
    if (!signal.targetPrice)
        return "确认报价条款并准备正式报价";
    if (!signal.purchaseTimeline)
        return "确认采购窗口和交付节点";
    return "按客户需求准备报价并确认下一次沟通时间";
}
export function assessDealRecommendation(store, signal) {
    const reasonCodes = signal.evidenceTypes
        .filter((type) => type === "product_requirement" || strongEvidence.has(type));
    const missingFields = [];
    if (!signal.product)
        missingFields.push("产品/应用");
    if (!signal.evidenceTypes.some((type) => strongEvidence.has(type))) {
        missingFields.push("真实采购动作");
    }
    if (!defaultNextAction(signal))
        missingFields.push("下一步动作");
...
export function recordProcurementSignal(store, input) {
    ...
    const existing = store.procurementSignals.find((item) => item.teamId === input.candidate.teamId
    ...
    const signal = {
        ...
        evidenceTypes,
        evidenceSummary: clean(input.evidenceSummary || input.touchpoint.content, 2000),
        product,
...
export function proposeDealRecommendation(store, signal) {
    const assessment = assessDealRecommendation(store, signal);
    const existing = store.dealRecommendations.find((item) => item.signalId === signal.id
        && item.teamId === signal.teamId
        && item.ownerId === signal.ownerId);
...
export function dismissDealRecommendation(recommendation, actorId, reason) {
...
export function linkRecommendationToDeal(store, recommendation, deal, actorId, mode) {
    if (recommendation.ownerId !== actorId
    ...
    const signal = store.procurementSignals.find((item) => item.id === recommendation.signalId);
    if (signal)
        signal.customerId = deal.customerId;
    const candidate = store.websiteOpportunities.find((item) => item.id === recommendation.prospectCandidateId);
    if (candidate) {
    ...
    const prospect = recommendation.tenantProspectId
        ? store.tenantProspects.find((item) => item.id === recommendation.tenantProspectId
            && item.teamId === recommendation.teamId)
        : undefined;
    if (prospect) {
...
export function linkProcurementContextToLead(store, candidate, leadId) {
...
export function linkProcurementContextToCustomer(store, context, customerId) {
    const candidateIds = new Set(context.prospectCandidateIds || []);
    const matches = (item) => item.teamId === context.teamId
        && item.ownerId === context.ownerId
        && (Boolean(context.leadId && item.leadId === context.leadId)
            || Boolean(context.tenantProspectId
...
export function recommendationReasonText(recommendation) {
