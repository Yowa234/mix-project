...
import { decryptAiModelApiKey, decryptProviderConfiguration, encryptAiModelApiKey } from "./credential-security.js";
...
if (!address || typeof address === "string")
    throw new Error("Cannot start security test server");
...
try {
    ...
    for (const unsafeHost of ["0.0.0.0", "::", "10.0.0.8"]) {
        try {
            ...
            if (error instanceof Error && error.message.startsWith("production backend host unexpectedly"))
                throw error;
        }
    }
    if (app.get("trust proxy") !== "loopback")
        throw new Error("Express must only trust loopback proxies");
    ...
    if (encryptedAiCredential.includes(aiCredentialSecret))
        throw new Error("AI credential ciphertext leaked plaintext");
    ...
    try {
        ...
        if (error instanceof Error && error.message === "AI credential must be bound to its tenant context")
            throw error;
    }
    ...
    if (!secureAiConfig.response.ok)
        throw new Error("secure Gemini configuration save failed");
    let capturedAiUrl = "";
    ...
    if (!csrfWrite.response.ok)
        throw new Error("cookie write with CSRF must succeed");
    ...
    const forgedClaimsToken = jwt.sign({ ver: 1, role: "super_admin", teamId: "all" }, TEST_JWT_SECRET, {
    ...
    if (knowledge.json.assets?.some((asset) => asset.id === "k2")) {
    ...
    if (jobs.json.jobs?.some((job) => job.operatorId !== "u_sales_shirley")) {
    ...
    if (!examDetail.response.ok || examDetail.json.questions?.some((question) => question.answerIndex !== -1 || question.answerIndexes.length || question.explanation)) {
    ...
    for (const target of ["http://127.0.0.1:4188", "http://localhost", "http://169.254.169.254/latest/meta-data"]) {
        ...
        if (!blocked)
            throw new Error(`SSRF target must be blocked: ${target}`);
    }
    ...
    const documentedOperations = Object.values(docsAsAdmin.json.paths || {}).reduce((total, pathItem) => total + Object.keys(pathItem).filter((method) => ["get", "post", "put", "patch", "delete"].includes(method)).length, 0);
    const routeLayers = (app._router?.stack || []);
    const registeredOperations = routeLayers.reduce((total, layer) => {
    ...
    if (!createdTenantAdmin.response.ok)
        throw new Error("tenant administrator creation failed");
    ...
    if (!createdAccount.response.ok)
        throw new Error("temporary account creation failed");
    ...
    const tenantProviderConnection = getStore().providerConnections.find((item) => item.providerId === "serper" && item.ownerId === tempUserId && item.teamId === tenantTeamId);
    if (!tenantProviderConnection
        || tenantProviderConnection.configurationEncrypted.includes(tenantProviderSecret)
        || decryptProviderConfiguration(tenantProviderConnection, tenantProviderConnection.configurationEncrypted).apiKey !== tenantProviderSecret) {
    ...
    const primaryAdminSerper = primaryAdminProviders.json.providers?.find((item) => item.id === "serper");
    ...
    const tenantSerperAfterCrossDelete = tenantProvidersAfterCrossDelete.json.providers?.find((item) => item.id === "serper");
    ...
    if (tenantSalesAgentJobs.json.total !== 2
        || tenantSalesAgentJobs.json.jobs?.some((job) => job.ownerId !== tempUserId)) {
    ...
    if (!tenantCustomer.response.ok)
        throw new Error("temporary tenant customer creation failed");
    const isolationMarker = `TENANT-B-${tenantSuffix}`;
    const cloneSeed = (items, overrides) => {
        if (!items[0])
            throw new Error("tenant isolation test requires seeded data");
    ...
    if (managerCustomers.json.customers?.some((customer) => customer.id === tenantCustomer.json.customer.id)) {
    ...
    if (adminAccounts.json.accounts?.some((account) => account.id === tempUserId || account.id === createdTenantAdmin.json.account.id)) {
    ...
    if (!superAccounts.json.accounts?.some((account) => account.id === tempUserId)
        || !superAccounts.json.accounts?.some((account) => account.id === createdTenantAdmin.json.account.id)) {
    ...
    if (!passwordChanged.response.ok)
        throw new Error("temporary password change failed");
    ...
    if (!disabled.response.ok)
        throw new Error("temporary account disable failed");
