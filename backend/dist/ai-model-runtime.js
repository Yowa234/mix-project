...
export async function callAiModel(config, prompt, maxInputChars = 12_000, fetcher) {
        ...
        if (protocol === "anthropic") {
            const response = await request(`${endpointBase}/messages`, {
                ...
                body: JSON.stringify({
                    ...
                    system: "你擅长整理授权 API、搜索服务和用户提供的结构化资料。"
            ...
            const response = await request(endpoint, {
                ...
                body: JSON.stringify({
                    ...
                    contents: [{
                            role: "user",
                            parts: [{
                                    text: "你擅长整理授权 API、搜索服务和用户提供的结构化资料。"
                                        + "不得声称访问过企业网页。"
        ...
        const response = await request(`${endpointBase}/chat/completions`, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: "你擅长整理授权 API、搜索服务和用户提供的结构化资料。"
                            + "不得声称访问过企业网页。"
                            + "输出必须可被 JSON.parse 解析。"
                    },
                    {
...
export function aiHttpErrorMessage(status) {
...
export async function aiGenerateLeads(query, config, fetcher) {
