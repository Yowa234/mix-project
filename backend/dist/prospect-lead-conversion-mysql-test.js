...
import { ORGANIZATION_STRONG_IDENTITY_CONTRACT } from "./organization-strong-identity.js";
import { PROSPECT_COVERAGE_MEMORY_CONTRACT } from "./prospect-coverage-memory.js";
import { ensureProspectCoverageSchema, loadProspectCoverageState } from "./prospect-coverage-memory-mysql.js";
...
import { ensureProspectQualificationSchema } from "./prospect-qualification-mysql.js";
...
const identityMasterSecret = "conversion-mysql-identity-master-secret-v1-".repeat(2);
const rawEnvelopeSecret = "conversion-mysql-raw-envelope-secret-v1-".repeat(2);
const rawIdentitySecret = "conversion-mysql-raw-identity-secret-v1-".repeat(2);
const coverageMasterSecret = "conversion-mysql-coverage-master-secret-v1-".repeat(2);
const qualificationMasterSecret = "conversion-mysql-qualification-master-secret-v1-".repeat(2);
const at = (minute) => new Date(Date.UTC(2026, 6, 15, 9, minute)).toISOString();
const hash = (seed) => seed.toString(16).padStart(64, "0");
let rawSequence = 0;
...
function identityClaims(registration) {
    return [
        baseClaim({
...
function createRawFixture(teamId, ownerId, connectionId) {
    rawSequence += 1;
...
async function insertRawFixture(connection, fixture) {
    ...
    try {
        await connection.query(`INSERT INTO prospect_source_raw_hits (
...
function identityInput(fixture, registration) {
...
function coverageInput(fixture, resolutionId, coveredAt) {
    return {
        teamId: fixture.teamId,
        ownerId: fixture.ownerId,
        resolutionId,
...
async function addCampaign(store, connection, teamId, ownerId, campaignId, suffix, fixture) {
    ...
    await connection.query(`INSERT INTO prospect_campaigns (
    ...
    await connection.query(`INSERT INTO prospect_campaign_versions (
    ...
    await connection.query(`INSERT INTO prospect_strategies (
    ...
    for (const event of strategyEvents) {
        await connection.query(`INSERT INTO prospect_strategy_events (
    ...
    await connection.query(`INSERT INTO prospect_search_runs (
    ...
    for (const event of runEvents) {
        await connection.query(`INSERT INTO prospect_run_events (
    ...
    await connection.query(`INSERT INTO prospect_run_shards (
...
async function qualifyProspect(store, input) {
...
async function main() {
    ...
    if (!applicationUrl || !adminConnectionUrl) {
        throw new Error("Lead conversion MySQL test requires MYSQL_TEST_ADMIN_URL, "
            + "DATABASE_URL or MYSQL_URL");
    }
    const adminUrl = new URL(adminConnectionUrl);
    const appUrl = new URL(applicationUrl);
    const databaseName = `goodjob_conversion_test_${randomUUID().replaceAll("-", "").slice(0, 16)}`;
    ...
    try {
       ...
       CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`);
        ...
        const owner = store.users.find((item) => item.id === "u_sales_shirley") || store.users.find((item) => item.role === "sales" && item.status === "active");
        ...
        await addCampaign(store, database, teamId, ownerId, campaignId, `${suffix}-1`, first);
        await insertRawFixture(database, first);
        const firstIdentity = await store.resolveOrganizationStrongIdentity(identityInput(first, `CONVERSION-REG-${suffix}-1`));
        const firstCoverage = await store.recordProspectCoverage(coverageInput(first, firstIdentity.resolution.id, at(20)));
        ...
        assert.equal(countsAfterConversion.leads, protectedBefore.leads + 1);
        assert.equal(countsAfterConversion.lead_source_events, protectedBefore.lead_source_events + 1);
        assert.equal(countsAfterConversion.lead_activities, protectedBefore.lead_activities + 1);
        assert.equal(countsAfterConversion.prospect_coverage_events, protectedBefore.prospect_coverage_events + 2);
        ...
        assert.equal(countsAfterConversion.website_opportunities, protectedBefore.website_opportunities);
        ...
        const customerConverted = await store.convertProspectToCustomer(customerConversionInput);
        ...
        assert.equal(customerConverted.sourceEvent.leadSourceEventId, converted.sourceEvent.id);
        const countsAfterCustomerConversion = await countRows(database);
        assert.equal(countsAfterCustomerConversion.customers, countsAfterConversion.customers + 1);
        assert.equal(countsAfterCustomerConversion.customer_activities, countsAfterConversion.customer_activities + 1);
        assert.equal(countsAfterCustomerConversion.customer_acquisition_source_events, countsAfterConversion.customer_acquisition_source_events + 1);
        assert.equal(countsAfterCustomerConversion.lead_activities, countsAfterConversion.lead_activities + 1);
        assert.equal(countsAfterCustomerConversion.prospect_coverage_events, countsAfterConversion.prospect_coverage_events + 1);
        assert.equal(countsAfterCustomerConversion.deals, countsAfterConversion.deals);
        assert.equal(countsAfterCustomerConversion.deal_events, countsAfterConversion.deal_events);
        assert.equal(countsAfterCustomerConversion.todos, countsAfterConversion.todos);
        assert.equal(countsAfterCustomerConversion.website_opportunities, countsAfterConversion.website_opportunities);
        const customerReplay = await store.convertProspectToCustomer(customerConversionInput);
        assert.equal(customerReplay.replayed, true);
        assert.equal(customerReplay.customer.id, customerConverted.customer.id);
        assert.deepEqual(await countRows(database), countsAfterCustomerConversion);
        stage = "customer conversion cold load";
        const coldCoverage = await loadProspectCoverageState(pool, teamId);
        const coldProspect = coldCoverage.tenantProspects.find((item) => item.id === firstCoverage.prospect.id);
        assert.equal(coldProspect?.status, "converted");
        assert.equal(coldProspect?.leadId, converted.lead.id);
        assert.equal(coldProspect?.customerId, customerConverted.customer.id);
        const coldStore = await createMysqlStore();
        assert.equal(coldStore.customers.some((item) => item.id === customerConverted.customer.id
            && item.teamId === teamId
            && item.ownerId === ownerId), true);
        assert.equal(coldStore.customerAcquisitionSourceEvents.some((item) => item.id === customerConverted.sourceEvent.id
            && item.customerId === customerConverted.customer.id
            && item.leadId === converted.lead.id
            && item.prospectId === firstCoverage.prospect.id), true);
        const [leadRows] = await database.query(`SELECT id,owner_id,team_id,source_channel,external_id,
              converted_customer_id,converted_deal_id
       FROM leads WHERE id = ?`, [converted.lead.id]);
        ...
        assert.equal(String(leadRows[0]?.external_id), firstCoverage.prospect.id);
        assert.equal(String(leadRows[0]?.converted_customer_id), customerConverted.customer.id);
        assert.equal(String(leadRows[0]?.converted_deal_id || ""), "");
        stage = "transaction rollback";
        const secondCampaignId = `pc_${randomUUID()}`;
        const second = createRawFixture(teamId, ownerId, "conversion-source-rollback");
        await addCampaign(store, database, teamId, ownerId, secondCampaignId, `${suffix}-rollback`, second);
        await insertRawFixture(database, second);
        const secondIdentity = await store.resolveOrganizationStrongIdentity(identityInput(second, `CONVERSION-REG-${suffix}-2`));
        const secondCoverage = await store.recordProspectCoverage(coverageInput(second, secondIdentity.resolution.id, at(50)));
        ...
        await admin.query(`CREATE TRIGGER fail_human_conversion_event
       ...
       SET MESSAGE_TEXT = 'forced conversion rollback'`);
        try {
            ...
            await admin.query(`DROP TRIGGER IF EXISTS \`${databaseName}\`.fail_human_conversion_event`);
        }
        ...
        const rollbackProspect = rollbackCoverage.tenantProspects.find((item) => item.id === secondCoverage.prospect.id);
        ...
        await admin.query(`CREATE TRIGGER fail_customer_conversion_coverage_event
       BEFORE INSERT ON prospect_coverage_events
       FOR EACH ROW
       SIGNAL SQLSTATE '45000'
       SET MESSAGE_TEXT = 'forced customer conversion rollback'`);
        try {
            await assert.rejects(store.convertProspectToCustomer({
                ...
                prospectId: secondCoverage.prospect.id,
                idempotencyKey: `mysql-customer-conversion-${suffix}-rollback`,
                convertedAt: at(66)
            }));
        }
        finally {
            await admin.query(`DROP TRIGGER IF EXISTS \`${databaseName}\`.${"fail_customer_conversion_coverage_event"}`);
        }
        assert.deepEqual(await countRows(database), beforeCustomerRollback);
        const customerRollbackCoverage = await loadProspectCoverageState(pool, teamId);
        const customerRollbackProspect = customerRollbackCoverage.tenantProspects.find((item) => item.id === secondCoverage.prospect.id);
        assert.equal(customerRollbackProspect?.status, "converted");
        assert.equal(customerRollbackProspect?.leadId, secondConverted.lead.id);
        assert.equal(customerRollbackProspect?.customerId, "");
        const [secondLeadRows] = await database.query(`SELECT converted_customer_id,converted_deal_id
       FROM leads WHERE id = ?`, [secondConverted.lead.id]);
        assert.equal(String(secondLeadRows[0]?.converted_customer_id || ""), "");
        assert.equal(String(secondLeadRows[0]?.converted_deal_id || ""), "");
        console.log("Prospect lead/customer conversion MySQL transaction, replay, "
