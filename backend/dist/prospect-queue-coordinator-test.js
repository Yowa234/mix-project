...
class FakeQueueBackend {
    ...
    async emit(signal, kind = "execution") {
...
const ready = backend.enqueued.find((item) => item.signal.agentJobId === "job_ready");
const delayed = backend.enqueued.find((item) => item.signal.agentJobId === "job_delayed");
const dead = backend.enqueued.find((item) => item.signal.agentJobId === "job_dead");
assert.ok(ready);
assert.ok(delayed);
assert.ok(dead);
...
const firstSignalIds = backend.enqueued.map((item) => item.options.signalId);
await coordinator.synchronize();
assert.deepEqual(backend.enqueued.slice(3).map((item) => item.options.signalId), firstSignalIds);
...
assert.equal(coordinator.status().lastErrorCode, "TEST_QUEUE_UNAVAILABLE");
backend.failAgentJobId = "";
await coordinator.synchronize();
assert.equal(coordinator.status().degraded, false);
assert.equal(coordinator.status().lastErrorCode, "");
assert.equal(validateProspectRedisUrl("redis://127.0.0.1:6379/0"), "redis://127.0.0.1:6379/0");
assert.equal(validateProspectRedisUrl("rediss://user:password@example.test:6380/2"), "rediss://user:password@example.test:6380/2");
assert.throws(() => validateProspectRedisUrl("https://example.test"), /redis/);
assert.throws(() => validateProspectRedisUrl("redis://127.0.0.1/1/2"), /REDIS_URL/);
