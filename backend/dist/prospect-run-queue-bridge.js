...
import { cancelProspectRunBridgeJob, isProspectRunBridgeJob, PROSPECT_RUN_ORCHESTRATE_JOB_TYPE, PROSPECT_RUN_PROVIDER_FETCH_JOB_TYPE, prospectRunBridgeIdempotencyDigest, registerProspectRunBridgeJob } from "./agent-jobs.js";
...
const TRACE_ID_PATTERN = /^trace_[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
export class ProspectRunQueueBridgeIntegrityError extends Error {
    code = "RUN_QUEUE_BRIDGE_INTEGRITY_INVALID";
    runId;
...
function exactPayload(runId, actual, expected) {
...
function decryptedPayload(run, job) {
    try {
...
function validateJobBase(run, binding, job, expectedIdempotencyKey) {
...
function expectedParentJobStatus(run) {
...
function allowedChildJobStatuses(shard) {
    switch (shard.status) {
        case "queued":
...
function validateRunShardState(run, shards) {
    const statuses = new Set(shards.map((item) => item.status));
    const allIn = (...allowed) => [...statuses].every((status) => allowed.includes(status));
    const valid = run.status === "queued"
        ? allIn("queued", "retry_scheduled", "succeeded", "succeeded_empty", "partial_success", "failed") && (statuses.has("queued") || statuses.has("retry_scheduled"))
        : run.status === "running"
            ? allIn("queued", "running", "retry_scheduled", "succeeded", "succeeded_empty", "partial_success", "failed")
            : run.status === "pause_requested"
                ? allIn("pause_requested", "paused", "succeeded", "succeeded_empty", "partial_success", "failed")
                : run.status === "paused"
                    ? allIn("paused", "succeeded", "succeeded_empty", "partial_success", "failed") && statuses.has("paused")
                    : run.status === "cancel_requested"
                        ? allIn("cancel_requested", "cancelled", "succeeded", "succeeded_empty", "partial_success", "failed")
                        : run.status === "cancelled"
                            ...
                            : run.status === "succeeded"
                                ...
                                : run.status === "succeeded_empty"
                                    ...
                                    : run.status === "partial_success"
                                        ? allIn("succeeded", "succeeded_empty", "partial_success", "failed")
                                            && (statuses.has("failed")
...
function validateParent(store, run, binding) {
    ...
    if (!job)
        fail(run.id, "搜索运行父桥接任务不存在");
    validateJobBase(run, binding, job, prospectRunBridgeIdempotencyDigest(`queue-bridge:v1:${run.teamId}:${run.id}:parent`));
    ...
    exactPayload(run.id, decryptedPayload(run, job), {
        runId: run.id,
        executionSnapshotHash: run.executionSnapshotHash,
        bridgeVersion: PROSPECT_RUN_QUEUE_BRIDGE_VERSION
    });
...
function validateChild(store, run, shard, parent, binding) {
    const expectedHash = bindingHash({
        teamId: run.teamId,
        ownerId: run.ownerId,
        runId: run.id,
    ...
    const job = store.agentJobs.find((item) => item.id === binding.jobId);
    if (!job)
        fail(run.id, "搜索运行子桥接任务不存在");
    validateJobBase(run, binding, job, prospectRunBridgeIdempotencyDigest(`queue-bridge:v1:${run.teamId}:${run.id}:shard:${shard.id}`));
...
export function registerProspectRunQueueBridge(store, run) {
    ...
    if (store.prospectRunQueueParentBindings.some((item) => item.teamId === run.teamId && item.runId === run.id) || store.prospectRunQueueChildBindings.some((item) => item.teamId === run.teamId && item.runId === run.id)) {
...
export function validateProspectRunQueueBridge(store, run) {
    const parents = store.prospectRunQueueParentBindings.filter((item) => item.teamId === run.teamId && item.runId === run.id);
    const children = store.prospectRunQueueChildBindings.filter((item) => item.teamId === run.teamId && item.runId === run.id);
...
export function validateAllProspectRunQueueBridges(store) {
    ...
    const parentBindingIds = new Set();
    ...
    for (const job of store.agentJobs.filter(isProspectRunBridgeJob)) {
        ...
        if (store.agentJobIdempotencyAliases.some((item) => item.jobId === job.id)) {
...
export function cancelProspectRunQueueBridge(store, run, cancelledAt) {
