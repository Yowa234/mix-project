...
function appendContactInfo(current, contact, contactInfo) {
    const parts = [clean(contact), clean(contactInfo)].filter(meaningful);
    const incoming = [...new Set(parts)].filter((part) => !normalized(current).includes(normalized(part))).join(" / ");
    if (!incoming)
        return "";
    if (!current)
        return incoming;
...
function suggestedField(key, currentValue, suggestedValue, evidenceSummary) {
...
export function generateCustomerIntelligenceSuggestion(store, input) {
    ...
    const evidenceSummary = clean(candidate.description
        || candidate.sourceEvidence?.at(-1)?.evidenceSummary
        || "智能获客再次发现该客户的新资料");
    const documentContact = appendContactInfo(customer.documentContact, candidate.contact, candidate.contactInfo);
    const suggestedFields = [
        suggestedField("company", customer.company, candidate.company, evidenceSummary),
        suggestedField("country", customer.country, candidate.country, evidenceSummary),
        suggestedField("contact", customer.contact, candidate.contact, evidenceSummary),
        suggestedField("documentContact", customer.documentContact, documentContact, evidenceSummary)
    ].filter(Boolean);
    ...
    const existing = store.customerIntelligenceSuggestions.find((item) => item.teamId === customer.teamId
...
function ownedSuggestion(store, suggestionId, teamId, ownerId) {
    const suggestion = store.customerIntelligenceSuggestions.find((item) => item.id === suggestionId
        && item.teamId === teamId
        && item.ownerId === ownerId);
    if (!suggestion)
        throw new Error("客户情报不存在或无权访问");
    const customer = store.customers.find((item) => item.id === suggestion.customerId
        && item.teamId === teamId
        && item.ownerId === ownerId);
    if (!customer)
...
export function acceptCustomerIntelligence(store, input) {
    const { suggestion, customer } = ownedSuggestion(store, input.suggestionId, input.teamId, input.ownerId);
    if (suggestion.status !== "pending") {
        throw new Error("该客户情报已经处理");
    }
    const selected = [...new Set(input.selectedFields)];
    const fields = suggestion.suggestedFields.filter((item) => selected.includes(item.key));
...
export function rejectCustomerIntelligence(store, input) {
    const { suggestion, customer } = ownedSuggestion(store, input.suggestionId, input.teamId, input.ownerId);
    if (suggestion.status !== "pending") {
        throw new Error("该客户情报已经处理");
    }
