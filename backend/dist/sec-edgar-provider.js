...
function providerError(message, code, phase = "search") {
...
function validateConnection(userAgent, baseUrl, phase) {
    if (baseUrl.trim()) {
    ...
    if (normalized.length < 8 || normalized.length > 200
        ...
        throw providerError("SEC Fair Access 要求填写可识别的 User-Agent，例如 GoodJobCRM admin@example.com", "PROVIDER_CONNECTION_INVALID", phase);
    }
    return normalized;
}
...
function queryTerms(query) {
    ...
    return [...new Set(values
            ...
            .filter((value) => value.length >= 2 && !QUERY_STOP_WORDS.has(value)))].slice(0, 40);
}
function excluded(title, query) {
    const normalized = normalizeText(title);
    return query.excludeKeywords.some((keyword) => normalized.includes(normalizeText(keyword)));
}
function matchScore(title, ticker, terms) {
    const normalizedTitle = normalizeText(title);
    const normalizedTicker = normalizeText(ticker);
    if (terms.includes(normalizedTicker))
        return 100;
    if (terms.includes(normalizedTitle))
        return 98;
    if (terms.some((term) => normalizedTitle.startsWith(term)))
        return 92;
    const matches = terms.filter((term) => normalizedTitle.includes(term) || normalizedTicker.includes(term)).length;
    if (!matches)
        return 0;
...
function parseResponse(value) {
    if (!value || typeof value !== "object" || Array.isArray(value)
        || Object.keys(value).length > 25_000) {
...
function toLead(record, confidence) {
...
async function fetchTickers(userAgent, fetcher) {
    ...
    if (!response.ok)
        throw providerHttpStatusError(response, "SEC EDGAR");
