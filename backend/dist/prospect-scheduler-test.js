...
import { createProspectSchedule, deleteProspectSchedule, listProspectSchedules, nextProspectScheduleOccurrence, ProspectScheduleRequestError, prospectScheduleEtag, transitionProspectSchedule } from "./prospect-schedules.js";
import { approveProspectStrategy, createProspectStrategy, prospectStrategyEtag, updateProspectStrategy } from "./prospect-strategies.js";
...
async function expectScheduleError(action, status, code) {
    try {
        await action;
...
const otherManager = testUser("scheduler_other_manager", "scheduler_team_b", "manager");
...
async function createReadyCampaign(input) {
    ...
    for (let index = 2; index <= (input.strategyCount || 1); index += 1) {
        const currentCampaign = store.prospectCampaigns.find((item) => item.id === created.campaign.id);
        ...
        const strategyItem = store.prospectStrategies.find((item) => item.id === createdStrategy.strategy.id);
        await approveProspectStrategy({
            store,
            user: publicUser(owner),
...
async function createSchedule(input) {
    const strategy = store.prospectStrategies.find((item) => item.id === input.strategyId);
    ...
    return store.prospectSchedules.find((item) => item.id === result.schedule.id);
}
function exhaustRunStrategy(run, strategy) {
...
try {
    assert.equal(nextProspectScheduleOccurrence("2026-01-01T10:15:00.000Z", "daily", "UTC"), "2026-01-02T10:15:00.000Z");
    assert.equal(nextProspectScheduleOccurrence("2026-01-01T10:15:00.000Z", "weekly", "UTC"), "2026-01-08T10:15:00.000Z");
    assert.equal(nextProspectScheduleOccurrence("2026-01-31T10:15:00.000Z", "monthly", "UTC"), "2026-02-28T10:15:00.000Z");
    ...
    assert.equal(listProspectSchedules(store, publicUser(otherManager)).total, 0);
    assert.throws(() => listProspectSchedules(store, publicUser(superAdmin)), (error) => error instanceof ProspectScheduleRequestError
        && error.status === 403
        && error.code === "SCHEDULE_ACCESS_FORBIDDEN");
    await expectScheduleError(createProspectSchedule({
        store,
    ...
    await expectScheduleError(transitionProspectSchedule({
        ...
        ifMatch: lifecycleEtag1,
        action: "resume"
    }), 412, "SCHEDULE_REVISION_CONFLICT");
    await transitionProspectSchedule({
        store,
        user: publicUser(owner),
        scheduleId: lifecycleSchedule.id,
    ...
    assert.equal(store.prospectSchedules.some((item) => item.id === lifecycleSchedule.id), false);
    let schedulerNow = new Date("2026-01-02T09:00:01.000Z");
    let genericMutationCalls = 0;
    let executionMutationCalls = 0;
    const applyMutation = async (mutation) => mutation().value;
    ...
    assert.ok(new Date(overdueSchedule.nextRunAt).getTime() > schedulerNow.getTime());
    ...
    store.prospectCampaigns.find((item) => item.id === invalid.campaign.id).status = "paused";
    ...
    assert.equal(invalidSchedule.lastFailureCode, "SCHEDULE_REFERENCE_CHANGED");
    ...
    }), 422, "RECURRING_COST_APPROVAL_REQUIRED");
    ...
    const firstRotatingRun = store.prospectSearchRuns.find((item) => item.id === rotatingSchedule.lastRunId);
    assert.equal(firstRotatingRun.strategyId, rotating.strategies[0].id);
    firstRotatingRun.status = "succeeded";
    schedulerNow = new Date(new Date(rotatingSchedule.nextRunAt).getTime() + 1_000);
    await rotatingScheduler.runOnce();
    const secondRotatingRun = store.prospectSearchRuns.find((item) => item.id === rotatingSchedule.lastRunId);
    ...
    assert.equal(rotatingSchedule.lastFailureCode, "SCHEDULE_ALL_STRATEGIES_EXHAUSTED");
    ...
    assert.ok(store.prospectRunEvents.some((item) => item.runId === reviewRunId && item.reason.includes("review:")));
    await reviewScheduler.runOnce();
    assert.equal(store.prospectSearchRuns.length, beforeReviewRunCount + 1);
    const listedReviewSchedule = listProspectSchedules(store, publicUser(owner)).schedules.find((item) => item.id === reviewSchedule.id);
    ...
    const createScheduleDoc = document.paths["/api/prospect-strategies/{id}/schedules"]?.post;
    assert.ok(createScheduleDoc?.parameters?.some((item) => item.name === "If-Match" && item.required));
    ...
    store.prospectSchedules.splice(0, store.prospectSchedules.length, ...original.schedules);
    ...
    store.tenantProspects.splice(0, store.tenantProspects.length, ...original.tenantProspects);
    store.prospectCoverageEvents.splice(0, store.prospectCoverageEvents.length, ...original.coverageEvents);
