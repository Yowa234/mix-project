...
const optionalFilter = (max, pattern) => {
...
const querySchema = z.object({
    ...
    const periods = [value.period, value.periodFrom, value.periodTo].filter((item) => Boolean(item));
...
export function tradeObservationListMetadata(campaignContractMode = "compat_v1") {
    return {
...
function publicPeriodType(value) {
    return value.includes("-") ? "monthly" : "annual";
}
function internalPeriod(value) {
    return value.replace("-", "");
}
function storedPeriodType(value) {
    if (/^\d{4}$/.test(value))
        return "annual";
...
function datasetFingerprint(observations) {
    return stableFingerprint(observations
        .map((item) => ({
        id: item.id,
...
function scopeContext(user, campaignId) {
    return [user.teamId, user.id, campaignId].join("\u001f");
}
function cursorSignature(encodedPayload, context) {
...
function decodeCursor(value, context) {
    ...
    if (!encodedPayload || !signature || rest.length
        ...
        throw new TradeObservationListRequestError("TRADE_OBSERVATION_CURSOR_INVALID", "分页游标无效、已过期或不属于当前查询");
    }
    try {
        return cursorPayloadSchema.parse(JSON.parse(Buffer.from(encodedPayload, "base64url").toString("utf8")));
    }
    catch {
        throw new TradeObservationListRequestError("TRADE_OBSERVATION_CURSOR_INVALID", "分页游标无效、已过期或不属于当前查询");
    }
...
function matchesFilters(item, filters) {
    ...
    if (filters.providerId && item.providerId.toLocaleLowerCase() !== filters.providerId)
        return false;
    if (filters.reporterCode && item.reporterCode !== filters.reporterCode)
        return false;
    if (filters.partnerCode && item.partnerCode !== filters.partnerCode)
        return false;
    if (filters.flow && item.tradeFlow !== (filters.flow === "import" ? "IMPORT" : "EXPORT"))
        return false;
    if (filters.classification && item.classification.toUpperCase() !== filters.classification)
        return false;
    if (filters.commodityCode && item.commodityCode.toUpperCase() !== filters.commodityCode)
        return false;
    if (filters.periodType && itemPeriodType !== filters.periodType)
        return false;
    if (filters.period && item.period !== internalPeriod(filters.period))
        return false;
    if (filters.periodFrom && item.period < internalPeriod(filters.periodFrom))
        return false;
    if (filters.periodTo && item.period > internalPeriod(filters.periodTo))
        return false;
    return true;
}
...
function tradeValueState(item) {
    if (item.suppressed)
        return "suppressed";
    if (item.tradeValueUsd === 0)
        return "reported_zero";
    if (typeof item.tradeValueUsd === "number")
        return "reported";
    if (item.statusFlags.includes("TRADE_VALUE_MISSING"))
        return "unavailable";
    return "unknown";
}
function measurementState(value) {
    if (value === 0)
        return "reported_zero";
...
export function parseTradeObservationListQuery(value) {
...
export function listTradeObservations(input) {
    ...
    const matched = input.store.marketTradeObservations
        .filter((item) => item.teamId === input.user.teamId
