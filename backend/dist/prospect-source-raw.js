...
export const PROSPECT_SOURCE_RAW_SCHEMA_VERSION = "fake-provider-source-records-v1";
...
function assertJsonValue(value, depth = 0, budget = { nodes: 0 }) {
    budget.nodes += 1;
...
export function normalizeProspectProviderSourceRecords(value, expectedRawCount) {
    if (!Array.isArray(value)
    ...
    return value.map((candidate, index) => {
        ...
        const normalized = {
            ...
            fetchedAt: normalizedIso(record.fetchedAt, `Provider 第 ${index + 1} 条原始记录 fetchedAt`),
...
export function prospectProviderRawArtifactHash(sourceRecords) {
...
function rawEnvelopeKey(secret) {
    return createHash("sha256")
        .update("goodjob-provider-raw-envelope-key-v1\u001f")
        .update(secret)
        .digest();
}
function rawRecordAad(record) {
...
function encryptRawRecord(record, value, secret) {
    const iv = randomBytes(12);
    const cipher = createCipheriv("aes-256-gcm", rawEnvelopeKey(secret), iv);
...
function decryptRawRecord(record, secret) {
    const [version, ivText, tagText, payloadText, ...rest] = record.encryptedEnvelope.split(".");
    if (rest.length
        ...
        || !tagText
        || !payloadText) {
        throw new ProspectSourceRawError("PROSPECT_SOURCE_RAW_ENVELOPE_INVALID", "Provider 原始记录信封格式无效");
    }
    try {
        const decode = (text) => {
        ...
        const decipher = createDecipheriv("aes-256-gcm", rawEnvelopeKey(secret), iv, { authTagLength: 16 });
        ...
        throw new ProspectSourceRawError("PROSPECT_SOURCE_RAW_ENVELOPE_INVALID", "Provider 原始记录信封完整性校验失败");
    }
}
function sourceIdentityHash(input, source) {
...
function recordId(input, identitySecret) {
...
function hitId(batchIdValue, ordinal) {
    return `psrh_${sha256({
        contract: PROSPECT_SOURCE_RAW_CONTRACT,
...
function rawBatchHash(batch, hits) {
    return sha256({
        contract: PROSPECT_SOURCE_RAW_CONTRACT,
...
function verifyRecord(record, expected, input) {
...
export function readProspectSourceRawRecord(store, input) {
    requireSecret(input.envelopeSecret, "Provider 原始记录加密密钥");
    const records = store.prospectSourceRawRecords.filter((item) => item.id === input.recordId
    ...
    if (record.envelopeHash !== createHash("sha256")
        ...
        .digest("hex")
        || record.recordHash !== rawRecordHash(recordWithoutHash(record))) {
        throw new ProspectSourceRawError("PROSPECT_SOURCE_RAW_ENVELOPE_INVALID", "Provider 原始记录完整性校验失败");
    }
    return {
...
function replayExistingBatch(store, batch, normalizedRecords, input) {
    ...
    for (let index = 0; index < normalizedRecords.length; index += 1) {
        ...
        const recordRows = store.prospectSourceRawRecords.filter((item) => item.id === hit.recordId);
...
export function appendProspectSourceRawBatch(store, rawInput) {
    ...
    const sourceRecords = normalizeProspectProviderSourceRecords(rawInput.sourceRecords);
    const input = {
        ...rawInput,
    ...
    const batchesForLedger = store.prospectSourceRawBatches.filter((item) => item.ledgerId === input.ledgerId);
    if (batchesForLedger.length > 1) {
        conflict("同一 Provider 账本存在多个原始批次");
    }
    if (batchesForLedger.length === 1) {
        return replayExistingBatch(store, batchesForLedger[0], sourceRecords, input);
    }
    ...
    const retentionUntil = new Date(new Date(createdAt).getTime()
        + input.policy.retentionDays * 86_400_000).toISOString();
    const records = [];
    const hits = [];
    const newRecords = [];
    const seenRecordIds = new Set();
    ...
    const versionKey = (record) => canonical({
        teamId: record.teamId,
        ownerId: record.ownerId,
        providerCode: record.providerCode,
        connectionId: record.connectionId,
