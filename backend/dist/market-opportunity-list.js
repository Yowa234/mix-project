...
const optionalCode = (max) => z.string().trim().min(1).max(max).regex(/^[A-Za-z0-9._-]+$/).optional();
...
function scopeContext(user, campaignId, batchId) {
...
function decodeCursor(value, context) {
    ...
    if (!encoded || !signature || rest.length
        ...
        throw new MarketOpportunityListRequestError(400, "MARKET_OPPORTUNITY_CURSOR_INVALID", "分页游标无效、已过期或不属于当前查询");
    }
    try {
...
function matchesFilters(item, filters) {
    if (filters.countryCode && item.reporterCode !== filters.countryCode)
        return false;
    if (filters.classification
        && item.classification.toUpperCase() !== filters.classification)
        return false;
    if (filters.commodityCode
        && item.commodityCode.toUpperCase() !== filters.commodityCode)
        return false;
    if (filters.snapshotStatus && item.snapshotStatus !== filters.snapshotStatus)
        return false;
    return true;
}
...
function publicSnapshot(item) {
    return {
        ...
        metrics: {
            ...
            chinaMainlandSupplyShareReason: item.metrics.chinaMainlandSupplyShareReason || null,
...
function publicCalculation(item) {
    return item ? {
        eventId: item.id,
...
function latestCalculationFirst(left, right) {
    return right.sequence - left.sequence
        || right.calculatedAt.localeCompare(left.calculatedAt)
        || right.id.localeCompare(left.id);
}
export function parseMarketOpportunityListQuery(value) {
    return querySchema.parse(value);
}
export function listMarketOpportunities(input) {
    const inScope = (item) => item.teamId === input.user.teamId
    ...
    const selectedBatch = input.query.batchId
        ? input.store.marketOpportunityBatches.find((item) => item.id === input.query.batchId && inScope(item))
        : latestCalculation
            ? input.store.marketOpportunityBatches.find((item) => item.id === latestCalculation.batchId && inScope(item))
            : undefined;
    if (input.query.batchId && !selectedBatch) {
        throw new MarketOpportunityListRequestError(404, "MARKET_OPPORTUNITY_BATCH_NOT_FOUND", "市场机会事实批次不存在或无权访问");
    }
    ...
    if (!selectedBatch) {
        return {
            ...
            fallbackReason: null,
            interpretation: "报告进口额是海关统计事实，不等同于消费市场规模、真实需求或采购意向；转口、保税贸易、本地产量及统计差异会影响解读。",
            sort: CURSOR_SORT,
            filters,
