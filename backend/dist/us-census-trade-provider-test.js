...
import { normalizeProviderTradePage, normalizeTradeQuery, providerErrorFromUnknown } from "./provider-contract.js";
import { assertProviderRequestAllowed, createProviderHttpClient, setProviderHttpTestTransport } from "./provider-http-client.js";
...
function censusRow(flow, overrides = {}) {
    const values = {
        value: "1234567",
        commodity: "9405",
...
function consecutiveMonths(startYear, startMonth, count) {
...
async function runTrade(query = baseQuery, credential = { apiKey: " census-test-key " }) {
    return await US_CENSUS_TRADE_PROVIDER.trade({ query, cursor: "" }, credential, tools());
}
async function expectProviderCode(operation, code, httpStatus) {
...
for (const commodityCode of ["94", "9405", "940542"]) {
    setProviderHttpTestTransport(async () => new Response(JSON.stringify([censusHeader("import")]), { status: 200 }));
...
await expectProviderCode(runTrade(baseQuery, { apiKey: "", baseUrl: "" }), "PROVIDER_CONNECTION_INVALID");
await expectProviderCode(runTrade(baseQuery, { apiKey: "key", baseUrl: "https://api.census.gov" }), "PROVIDER_POLICY_BLOCKED");
...
for (const payload of [
    ...
    setProviderHttpTestTransport(async () => new Response(JSON.stringify(payload), { status: 200 }));
...
for (const [status, expectedCode] of [
    ...
    setProviderHttpTestTransport(async () => new Response("provider failure", {
        status,
        headers: status === 429 ? { "retry-after": "60" } : {}
    }));
    await expectProviderCode(runTrade(), expectedCode, status);
}
assertProviderRequestAllowed("https://api.census.gov/data/timeseries/intltrade/imports/hs?get=GEN_VAL_MO", "GET", US_CENSUS_TRADE_PROVIDER.networkPolicy);
assertProviderRequestAllowed("https://api.census.gov/data/timeseries/intltrade/exports/hs?get=ALL_VAL_MO", "GET", US_CENSUS_TRADE_PROVIDER.networkPolicy);
for (const blockedUrl of [
    ...
    assert.throws(() => assertProviderRequestAllowed(blockedUrl, "GET", US_CENSUS_TRADE_PROVIDER.networkPolicy));
}
assert.throws(() => assertProviderRequestAllowed("https://api.census.gov/data/timeseries/intltrade/imports/hs", "POST", US_CENSUS_TRADE_PROVIDER.networkPolicy));
