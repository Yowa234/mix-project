...
import { ProviderContractError, defineProvider, providerHttpStatusError } from "./provider-contract.js";
import { assertProviderBaseUrlAllowed, assertProviderRedirectAllowed, assertProviderRequestAllowed, createProviderHttpClient, setProviderHttpTestTransport } from "./provider-http-client.js";
import { createProviderExecutionContext, executeProviderEnrich, executeProviderHealth, executeProviderSearch } from "./provider-runtime.js";
import { providerRequestFingerprint, recordProviderHttpResult, withProviderRequestLogging } from "./provider-request-logging.js";
const query = {
...
function catalog(provider, overrides = {}) {
...
function connection(providerId, overrides = {}) {
    const now = "2026-07-13T00:00:00.000Z";
    const identity = {
...
function provider(id, search, options = {}) {
...
async function expectCode(operation, code) {
    await assert.rejects(operation, (error) => error instanceof ProviderContractError && error.code === code);
}
...
for (const [status, expectedCode] of [[401, "PROVIDER_AUTH_FAILED"], [429, "PROVIDER_RATE_LIMITED"]]) {
...
await assert.rejects(executeProviderSearch({
    ...
    query,
    onLogs() { }
}), (error) => error instanceof ProviderContractError
...
assert.throws(() => assertProviderRedirectAllowed("https://api.example.com/v1/search", "https://unknown.example/v1/result", networkPolicy));
assert.doesNotThrow(() => assertProviderRedirectAllowed("https://api.example.com/v1/search", "https://download.example.com/v1/result", { ...networkPolicy, allowedPathPrefixes: ["/v1/"] }));
assert.throws(() => assertProviderRedirectAllowed("https://api.example.com/v1/search", "https://download.example.com/v1/result", networkPolicy, true));
