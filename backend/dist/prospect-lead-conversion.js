...
import { currentContactabilityDecision } from "./prospect-qualification.js";
export const PROSPECT_LEAD_CONVERSION_CONTRACT = "prospect-to-lead-human-confirmation-v1";
...
function canonical(value) {
    ...
    if (typeof result !== "string") {
        fail("PROSPECT_LEAD_CONVERSION_DATA_INTEGRITY", "线索转换请求无法规范化", 500);
    }
    return result;
}
...
function requiredText(value, label, max) {
    ...
    if (!text || text.length > max) {
        fail("PROSPECT_LEAD_CONVERSION_INVALID", `${label}不能为空且不能超过 ${max} 个字符`, 400);
    }
    return text;
}
function normalizedIso(value) {
    ...
    if (!Number.isFinite(date.getTime())) {
        fail("PROSPECT_LEAD_CONVERSION_INVALID", "转换时间无效", 400);
    }
...
export function prospectLeadConversionIds(raw) {
...
function sourcePayload(event) {
    try {
        ...
        fail("PROSPECT_LEAD_CONVERSION_DATA_INTEGRITY", "线索来源审计载荷损坏", 500);
    }
}
function findProspect(store, input) {
    ...
    if (matches.length !== 1) {
        fail("PROSPECT_LEAD_CONVERSION_NOT_FOUND", "候选客户不存在或无权访问", 404);
    }
    return matches[0];
}
function findApprovedDecision(store, input) {
    const selected = store.prospectContactabilityDecisions.find((item) => item.id === input.decisionId
    ...
    if (!selected || selected.status !== "approved_contactable"
        ...
        fail("PROSPECT_LEAD_CONVERSION_NOT_APPROVED", "当前业务员没有可用于转换的人工批准记录");
    }
    ...
    if (!current || current.id !== selected.id
        ...
        fail("PROSPECT_LEAD_CONVERSION_STALE", "候选资格或联系方式已变化，请重新审核后再转线索");
    }
    return selected;
}
function findApprovedChannel(store, input, decision) {
    const channel = store.prospectContactChannels.find((item) => item.id === decision.channelId
        && item.teamId === input.teamId
        && item.ownerId === input.ownerId
        && item.prospectId === input.prospectId);
    const contact = channel
        ? store.prospectContacts.find((item) => item.id === channel.contactId
            && item.teamId === input.teamId
            && item.ownerId === input.ownerId
            && item.prospectId === input.prospectId)
        : undefined;
    if (!channel || !contact) {
        fail("PROSPECT_LEAD_CONVERSION_STALE", "人工批准的联系人或联系方式已不可用");
    }
    return { channel, contact };
}
function sourceUrl(store, input, channel) {
    return store.prospectEvidence.find((item) => item.id === channel.sourceEvidenceId
        && item.teamId === input.teamId
        && item.ownerId === input.ownerId
        && item.prospectId === input.prospectId)?.sourceRef.slice(0, 500) || "";
}
function leadContactName(input, contact) {
    return input.contact || contact.name || contact.department || contact.title;
}
function conversionLead(input, leadId, company, contact, channel, decision, sourceRef) {
...
function commitLocalState(store, local) {
    ...
    store.leadSourceEvents.splice(0, store.leadSourceEvents.length, ...local.leadSourceEvents);
...
function convertOnLocalStore(local, input) {
    ...
    const priorSource = local.leadSourceEvents.find((item) => item.id === ids.sourceEventId);
    const otherSource = local.leadSourceEvents.find((item) => item.teamId === input.teamId
        && item.ownerId === input.ownerId
        && item.channel === PROSPECT_LEAD_SOURCE_CHANNEL
        && item.externalId === input.prospectId
        && item.id !== ids.sourceEventId);
    if (otherSource) {
        fail("PROSPECT_LEAD_CONVERSION_ALREADY_COMPLETED", "该候选客户已经使用其他确认请求转为线索");
    }
    if (priorSource) {
        const payload = sourcePayload(priorSource);
        if (!sameHash(String(payload.conversionRequestHash || ""), conversionRequestHash)) {
            fail("PROSPECT_LEAD_CONVERSION_IDEMPOTENCY_CONFLICT", "该 Idempotency-Key 已用于不同的线索转换请求");
        }
        const priorLead = local.leads.find((item) => item.id === priorSource.leadId
            && item.teamId === input.teamId
            && item.ownerId === input.ownerId);
        const priorActivity = local.leadActivities.find((item) => item.id === ids.activityId && item.leadId === priorSource.leadId);
        if (!priorLead || !priorActivity
            || prospect.status !== "converted"
            || prospect.leadId !== priorLead.id) {
            fail("PROSPECT_LEAD_CONVERSION_DATA_INTEGRITY", "线索转换审计链与候选状态不一致", 500);
        }
        ...
        if (!disposition.idempotent) {
            fail("PROSPECT_LEAD_CONVERSION_DATA_INTEGRITY", "线索转换幂等事件缺失", 500);
        }
        return {
            replayed: true,
    ...
    if (prospect.status === "converted" || prospect.leadId) {
        fail("PROSPECT_LEAD_CONVERSION_ALREADY_COMPLETED", "该候选客户已经转为线索");
    }
    if (prospect.status !== "active"
        || prospect.exclusionMode !== "none"
        || prospect.queueState === "suppressed") {
        fail("PROSPECT_LEAD_CONVERSION_NOT_ELIGIBLE", "该候选客户当前处于排除、禁止联系或不可转换状态");
    }
    const decision = findApprovedDecision(local, input);
    const { channel, contact } = findApprovedChannel(local, input, decision);
    const organization = local.organizations.find((item) => item.id === prospect.organizationId
        && item.teamId === input.teamId
        && item.status === "active");
    if (!organization) {
        fail("PROSPECT_LEAD_CONVERSION_NOT_ELIGIBLE", "候选客户的企业身份不存在或已失效");
    }
    const created = input.mode === "create_new";
    let lead;
    if (created) {
        if (local.leads.some((item) => item.id === ids.leadId)) {
            fail("PROSPECT_LEAD_CONVERSION_DATA_INTEGRITY", "线索转换标识发生冲突", 500);
        }
        lead = conversionLead(input, ids.leadId, input.company || organization.legalName, contact, channel, decision, sourceUrl(local, input, channel));
        local.leads.unshift(lead);
    }
    else {
        const existing = local.leads.find((item) => item.id === input.existingLeadId
            && item.teamId === input.teamId
            && item.ownerId === input.ownerId
            && !item.deletedAt);
        if (!existing) {
            fail("PROSPECT_LEAD_CONVERSION_NOT_FOUND", "已有线索不存在或无权访问", 404);
        }
        lead = existing;
    }
    ...
    try {
        ...
        if (error instanceof ProspectCoverageMemoryError
            ...
            fail("PROSPECT_LEAD_CONVERSION_IDEMPOTENCY_CONFLICT", "该 Idempotency-Key 已用于不同的候选处置请求");
        }
        throw error;
    }
    return {
...
export function convertProspectToLead(store, raw) {
    if (store.mode === "mysql") {
        fail("PROSPECT_LEAD_CONVERSION_MYSQL_TRANSACTION_REQUIRED", "MySQL 线索转换必须通过专用事务提交", 500);
    }
    const input = normalizeInput(raw);
    const local = {
        ...store,
