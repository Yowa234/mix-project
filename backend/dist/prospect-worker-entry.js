import { validateAuthSecurity } from "./auth.js";
import { validateAgentJobSecurity } from "./agent-job-security.js";
import { validateProviderCredentialSecurity } from "./credential-security.js";
...
import { validateProspectRunSecurity } from "./prospect-runs.js";
...
async function startProspectWorker() {
    ...
    const shutdown = async (signal) => {
        if (shuttingDown)
            return;
        shuttingDown = true;
        console.log(`GoodJob CRM prospect worker received ${signal}, shutting down`);
        try {
            await service.stop();
            await store.close?.();
            process.exit(0);
            ...
            console.error("GoodJob CRM prospect worker shutdown failed: "
...
if (process.env.NODE_ENV !== "test") {
    void startProspectWorker().catch((error) => {
        console.error("GoodJob CRM prospect worker startup failed: "
            + (error instanceof Error ? error.message : String(error)));
        process.exit(1);
    });
}
