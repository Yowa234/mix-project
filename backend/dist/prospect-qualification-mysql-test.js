...
import { listOwnerProspectQualification, ProspectQualificationError } from "./prospect-qualification.js";
import { applyProspectQualificationCommandMysql, ensureProspectQualificationSchema, loadProspectQualificationState } from "./prospect-qualification-mysql.js";
import { getStore } from "./store.js";
const masterSecret = "prospect-qualification-mysql-master-secret-v1-".repeat(2);
...
function qualificationStore(teamId, ownerId, prospectId, organizationId) {
    const base = getStore();
    const organization = {
        id: organizationId,
        teamId,
...
async function main() {
    ...
    if (!applicationUrl || !adminConnectionUrl) {
        throw new Error("Qualification MySQL test requires MYSQL_TEST_ADMIN_URL, "
            + "DATABASE_URL or MYSQL_URL");
    }
    const adminUrl = new URL(adminConnectionUrl);
    const appUrl = new URL(applicationUrl);
    const databaseName = `goodjob_qualification_test_${randomUUID().replaceAll("-", "").slice(0, 16)}`;
    ...
    try {
        ...
        const store = qualificationStore(teamId, ownerId, prospectId, organizationId);
        ...
        const inserted = await applyProspectQualificationCommandMysql(pool, store, command);
        assert.equal(inserted.replayed, false);
        assert.equal((await applyProspectQualificationCommandMysql(pool, store, command)).replayed, true);
        const coldState = await loadProspectQualificationState(pool, teamId);
        assert.equal(coldState.prospectEvidence.length, 1);
        assert.equal(coldState.prospectEvidence[0]?.normalizedValue, "hrb 654321");
        assert.equal((await loadProspectQualificationState(pool, otherTeamId))
            .prospectEvidence.length, 0);
        const coldStore = qualificationStore(teamId, ownerId, prospectId, organizationId);
        Object.assign(coldStore, coldState);
        assert.equal(listOwnerProspectQualification(coldStore, {
            teamId,
            ownerId,
            prospectId
        }).evidence.length, 1);
        ...
        await assert.rejects(applyProspectQualificationCommandMysql(pool, coldStore, {
            ...command,
            ownerId: otherOwnerId,
            ...
            validUntil: at(100),
            createdAt: at(2)
        }), (error) => error instanceof ProspectQualificationError
        ...
        const [factRows] = await database.query("SELECT encrypted_payload FROM prospect_qualification_facts");
        assert.equal(factRows.length, 1);
        assert.equal(String(factRows[0]?.encrypted_payload || "").includes("HRB 654321"), false);
        await database.end();
        console.log("Prospect qualification MySQL persistence and isolation tests passed");
        exitCode = 0;
    }
    catch (error) {
        console.error(error);
        ...
        if (pool && testTeamId) {
            await pool.query("DELETE FROM prospect_qualification_facts WHERE team_id = ?", [testTeamId]).catch(() => undefined);
            await pool.query("DELETE FROM prospect_qualification_team_guards WHERE team_id = ?", [testTeamId]).catch(() => undefined);
        }
        if (pool)
            await pool.end();
        if (databaseCreated) {
