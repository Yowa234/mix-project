...
import { firstCountryInEnglish, freeProcurementUsage, isExcludedProcurement, positiveCursor, procurementSchemaError, procurementSearchTerms, textLimit } from "./procurement-provider-utils.js";
...
function toLead(notice, query) {
    const business = textLimit(notice.bid_description || notice.project_name || "世界银行公开采购需求", 500);
    const company = textLimit(notice.contact_organization || notice.project_name || "世界银行项目采购机构", 200);
    if (!company || isExcludedProcurement(`${company} ${business}`, query))
        return null;
    ...
    const contactInfo = textLimit(notice.contact_email || notice.contact_phone_no || "", 255);
    return {
        company,
        officialWebsite: "",
        country: textLimit(notice.contact_ctry_name || notice.project_ctry_name || "", 100),
        business,
...
export const WORLD_BANK_PROCUREMENT_PROVIDER = defineProvider({
    ...
    async search({ query, cursor }, _cred, tools) {
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "World Bank Procurement");
        let data;
        try {
