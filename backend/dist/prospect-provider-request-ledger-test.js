...
import { DeterministicFakeProspectProvider, FakeProspectProviderError } from "./prospect-fake-provider.js";
import { hasValidProspectProviderDispatchStatusFacts, isProspectProviderDispatchTransitionAllowed, prospectProviderRequestHash, prospectProviderRequestIdempotencyKey } from "./prospect-provider-request-ledger.js";
const idempotencySecret = "goodjob-provider-request-ledger-test-secret-2026";
function success(acceptedCount, overrides = {}) {
    return {
        kind: "success",
        acceptedCount,
...
assert.equal(firstRequest.requestHash, dispatchRequest({ logicalRequestNo: 1 }).requestHash);
assert.equal(firstRequest.idempotencyKey, dispatchRequest({ logicalRequestNo: 1 }).idempotencyKey);
assert.notEqual(firstRequest.requestHash, dispatchRequest({ logicalRequestNo: 2 }).requestHash);
...
assert.equal(new Set(concurrent.map((item) => item.externalRequestId)).size, 1);
assert.equal(new Set(concurrent.map((item) => item.responseHash)).size, 1);
...
assert.equal(second.step.kind === "success" ? second.step.acceptedCount : -1, 2);
await assert.rejects(provider.dispatch({
    ...secondRequest,
    idempotencyKey: firstRequest.idempotencyKey
}), (error) => error instanceof FakeProspectProviderError
...
await assert.rejects(provider.getByExternalRequestId({
...
assert.equal(calls.filter((item) => item.operation === "dispatch").length, 5);
assert.equal(calls.filter((item) => item.providerExecuted).length, 2);
assert.equal(calls.filter((item) => item.operation !== "dispatch" && item.providerExecuted).length, 0);
...
await assert.rejects(interruptedProvider.dispatch(interruptedRequest), (error) => error instanceof FakeProspectProviderError
    && error.code
...
assert.equal(recovered.step.kind === "success"
    ? recovered.step.acceptedCount
    : -1, 7);
assert.equal(interruptedProvider.listPhysicalCalls()
...
const responseA = await responseBindingProvider.dispatch(dispatchRequest({ logicalRequestNo: 20 }));
const responseB = await responseBindingProvider.dispatch(dispatchRequest({ logicalRequestNo: 21 }));
...
assert.equal(hasValidProspectProviderDispatchStatusFacts(unknownDispatch), true);
...
assert.equal(hasValidProspectProviderDispatchStatusFacts(startedDispatch), true);
assert.equal(hasValidProspectProviderDispatchStatusFacts({
    ...startedDispatch,
    confirmedAt: "2026-07-14T01:00:01.000Z"
}), false, "started 状态不得提前出现派发确认时间");
assert.equal(hasValidProspectProviderDispatchStatusFacts({
    ...startedDispatch,
    providerExecuted: true
}), false, "started 状态不得提前宣称 Provider 已执行");
assert.equal(isProspectProviderDispatchTransitionAllowed("outcome_unknown", "response_received"), true);
assert.equal(hasValidProspectProviderDispatchStatusFacts({
    ...unknownDispatch,
