...
function customer(ownerId = "sales-a", teamId = "team-a") {
    return {
        id: "customer-a",
...
}
{
    ...
    assert.deepEqual(accepted.suggestion.acceptedFields, ["country", "documentContact"]);
...
}
{
    ...
    target.country = "CH";
    assert.throws(() => acceptCustomerIntelligence(store, {
        suggestionId: suggestion.id,
        teamId: target.teamId,
        ownerId: target.ownerId,
...
}
{
    ...
    assert.throws(() => rejectCustomerIntelligence(store, {
        suggestionId: suggestion.id,
        teamId: "team-b",
        ownerId: target.ownerId
    }), /无权访问/u);
...
}
{
    ...
    assert.deepEqual(target, before);
    assert.equal(store.customerActivities.length, 1);
    assert.match(store.customerActivities[0].content, /仅采纳来源证据/u);
}
assert.throws(() => generateCustomerIntelligenceSuggestion(isolatedStore(), {
