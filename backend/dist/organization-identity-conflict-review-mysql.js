...
import { buildOrganizationIdentityConflictReview, OrganizationIdentityConflictReviewError, organizationCanonicalMappingHash, organizationIdentityConflictEtag, organizationIdentityConflictReviewHash } from "./organization-identity-conflict-review.js";
function iso(value) {
...
async function rows(source, sql, values = []) {
...
export async function ensureOrganizationIdentityConflictReviewSchema(pool) {
    await pool.query(`
...
export async function loadOrganizationIdentityConflictReviewState(source, teamId) {
    const where = teamId ? " WHERE team_id = ?" : "";
    const values = teamId ? [teamId] : [];
    const [reviewRows, mappingRows] = await Promise.all([
        rows(source, `SELECT * FROM organization_identity_conflict_reviews${where}
       ORDER BY team_id, created_at, id`, values),
        rows(source, `SELECT * FROM organization_canonical_mappings${where}
       ORDER BY team_id, created_at, id`, values)
    ]);
...
function replaceTeamState(store, teamId, state) {
    store.organizationIdentityConflictReviews.splice(0, store.organizationIdentityConflictReviews.length, ...store.organizationIdentityConflictReviews.filter((item) => item.teamId !== teamId), ...structuredClone(state.organizationIdentityConflictReviews));
    store.organizationCanonicalMappings.splice(0, store.organizationCanonicalMappings.length, ...store.organizationCanonicalMappings.filter((item) => item.teamId !== teamId), ...structuredClone(state.organizationCanonicalMappings));
}
export async function ensureOrganizationIdentityConflictReviewTeamCache(pool, store, teamId) {
    replaceTeamState(store, teamId, await loadOrganizationIdentityConflictReviewState(pool, teamId));
}
export async function reviewOrganizationIdentityConflictMysql(pool, store, input) {
    const connection = await pool.getConnection();
    let built = null;
    try {
        await connection.beginTransaction();
        const conflictRows = await rows(connection, `SELECT id, team_id
       FROM organization_identity_conflicts
       WHERE id = ? AND team_id = ?
       FOR UPDATE`, [input.conflictId, input.user.teamId]);
        if (!conflictRows.length) {
            throw new OrganizationIdentityConflictReviewError("IDENTITY_CONFLICT_NOT_FOUND", "企业身份冲突不存在或无权访问", 404);
        }
        const organizationRows = await rows(connection, `SELECT organization_id
       FROM organization_identity_conflict_organizations
       WHERE conflict_id = ? AND team_id = ?
       ORDER BY ordinal
       FOR UPDATE`, [input.conflictId, input.user.teamId]);
        const persistedState = await loadOrganizationIdentityConflictReviewState(connection, input.user.teamId);
        const conflict = store.organizationIdentityConflicts.find((item) => item.id === input.conflictId
            && item.teamId === input.user.teamId);
        if (!conflict) {
            throw new OrganizationIdentityConflictReviewError("IDENTITY_CONFLICT_NOT_FOUND", "企业身份冲突缓存尚未就绪，请刷新后重试", 409);
        }
        const transactionStore = {
            ...store,
            organizationIdentityConflicts: [{
                    ...conflict,
                    organizationIds: organizationRows.map((item) => String(item.organization_id))
                }],
            organizationIdentityConflictReviews: persistedState.organizationIdentityConflictReviews,
            organizationCanonicalMappings: persistedState.organizationCanonicalMappings
        };
        built = buildOrganizationIdentityConflictReview(transactionStore, input);
        for (const mapping of built.mappings) {
            await connection.query(`INSERT INTO organization_canonical_mappings (
        ...
        await connection.query(`INSERT INTO organization_identity_conflict_reviews (
        ...
        if (error
            ...
            throw new OrganizationIdentityConflictReviewError("IDENTITY_CONFLICT_ALREADY_REVIEWED", "该企业身份冲突已被其他管理员处理，请刷新后查看", 409);
        }
        throw error;
    }
    finally {
    ...
    const state = await loadOrganizationIdentityConflictReviewState(pool, input.user.teamId);
    replaceTeamState(store, input.user.teamId, state);
    const review = state.organizationIdentityConflictReviews.find((item) => item.id === built.review.id);
    if (!review) {
        throw new Error(`企业身份冲突复核提交后无法读取：${canonicalJsonStringify({
    ...
    return {
        review,
        mappings: state.organizationCanonicalMappings.filter((item) => item.conflictId === input.conflictId),
        revision: 2,
