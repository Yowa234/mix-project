...
import { activateProspectCampaign, createProspectCampaign, prospectCampaignEtag, transitionProspectCampaign } from "./prospect-campaigns.js";
import { createMysqlStore } from "./mysql-store.js";
import { createProspectRun, ProspectRunRequestError, transitionProspectRun } from "./prospect-runs.js";
...
async function main() {
    ...
    if (!applicationUrl || !adminConnectionUrl) {
        throw new Error("Prospect Run MySQL test requires MYSQL_TEST_ADMIN_URL, DATABASE_URL or MYSQL_URL");
    }
    const adminUrl = new URL(adminConnectionUrl);
    const appUrl = new URL(applicationUrl);
    const databaseName = `goodjob_run_test_${randomUUID().replaceAll("-", "").slice(0, 16)}`;
    ...
    try {
       ...
       CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`);
        ...
        const [baselineTables] = await admin.query(`SELECT table_name AS tableName
       ...
       ORDER BY table_name`, [databaseName]);
        assert.deepEqual(baselineTables.map((row) => String(row.tableName)), [
            "prospect_run_events",
            "prospect_run_queue_child_bindings",
            "prospect_run_queue_parent_bindings",
            "prospect_run_shards",
        ...
        const [strategyReferenceIndex] = await admin.query(`SELECT GROUP_CONCAT(
       ...
       WHERE table_schema = ?
         ...
         AND non_unique = 0`, [databaseName]);
        assert.equal(strategyReferenceIndex[0]?.columnsList, "team_id,campaign_id,campaign_version,id");
        ...
        const strategy = setupStore.prospectStrategies.find((item) => item.campaignId === campaign.id);
        assert.ok(strategy);
        ...
        const raceStrategy = setupStore.prospectStrategies.find((item) => item.campaignId === raceCampaign.id);
        ...
        const raceStoreOwner = raceStore.users.find((item) => item.id === owner.id);
        const raceStoreCampaign = raceStore.prospectCampaigns.find((item) => item.id === raceCampaign.id);
        const raceStoreStrategy = raceStore.prospectStrategies.find((item) => item.id === raceStrategy.id);
        ...
        const pauseCreateError = pauseCreateResults[1].reason;
        assert.equal(pauseCreateError instanceof ProspectRunRequestError, true);
        assert.equal(pauseCreateError.code, "RUN_NOT_READY");
        assert.equal(pauseCreateError.details.issues.some((item) => item.code === "CAMPAIGN_NOT_ACTIVE"), true);
        assert.equal(raceStore.prospectSearchRuns.some((item) => item.campaignId === raceStoreCampaign.id), false);
        ...
        const activeRaceCreated = activeRaceResults.find((item) => item.status === "fulfilled");
        const activeRaceRejected = activeRaceResults.find((item) => item.status === "rejected");
        assert.ok(activeRaceCreated);
        assert.ok(activeRaceRejected);
        assert.equal(activeRaceRejected.reason instanceof ProspectRunRequestError, true);
        ...
        await transitionProspectRun({
            ...
            ifMatch: `"${activeRaceCreated.value.run.id}:${activeRaceCreated.value.run.revision}"`,
            action: "cancel",
        ...
        const creatorOwner = creatorStore.users.find((item) => item.id === owner.id);
        const creatorStrategy = creatorStore.prospectStrategies.find((item) => item.id === strategy.id);
        ...
        const createdRun = sameStoreRequests.find((item) => !item.idempotencyReplayed);
        const sameStoreReplay = sameStoreRequests.find((item) => item.idempotencyReplayed);
        ...
        assert.equal(creatorStore.prospectRunQueueParentBindings.filter((item) => item.runId === runId).length, 1);
        assert.equal(creatorStore.prospectRunQueueChildBindings.filter((item) => item.runId === runId).length, 1);
        assert.equal(creatorStore.agentJobs.filter((item) => item.aggregateId === runId)
            .length, 2);
        stage = "concurrent exact replay";
        const replayStrategy = replayStore.prospectStrategies.find((item) => item.id === strategy.id);
        const replay = await createProspectRun({
            store: replayStore,
            user: publicUser(replayStore.users.find((item) => item.id === owner.id)),
        ...
        assert.equal(replayStore.prospectRunShards.filter((item) => item.runId === runId)
            .length, 1);
        assert.equal(replayStore.prospectRunEvents.filter((item) => item.runId === runId)
            .length, 1);
        assert.equal(replayStore.prospectRunQueueParentBindings.filter((item) => item.runId === runId).length, 1);
        assert.equal(replayStore.prospectRunQueueChildBindings.filter((item) => item.runId === runId).length, 1);
        assert.equal(replayStore.agentJobs.filter((item) => item.aggregateId === runId)
            .length, 2);
        stage = "concurrent changed request conflict";
        const changedStrategy = changedRequestStore.prospectStrategies.find((item) => item.id === strategy.id);
        await assert.rejects(createProspectRun({
            store: changedRequestStore,
            user: publicUser(changedRequestStore.users.find((item) => item.id === owner.id)),
        ...
        }), (error) => error instanceof ProspectRunRequestError
            && error.status === 409
            && error.code === "IDEMPOTENCY_KEY_CONFLICT");
        stage = "concurrent active duplicate conflict";
        const conflictStrategy = activeConflictStore.prospectStrategies.find((item) => item.id === strategy.id);
        await assert.rejects(createProspectRun({
            store: activeConflictStore,
            user: publicUser(activeConflictStore.users.find((item) => item.id === owner.id)),
        ...
        const [runRows] = await admin.query(`SELECT idempotency_key_hash, status, revision_no, queue_bridge_version
       ...
       WHERE id = ?`, [runId]);
        ...
        const [indexRows] = await admin.query(`SELECT index_name AS indexName,
         GROUP_CONCAT(column_name ORDER BY seq_in_index) AS columnsList
       FROM information_schema.statistics
       WHERE table_schema = ?
         AND table_name = 'prospect_search_runs'
         AND non_unique = 0
       GROUP BY index_name`, [databaseName]);
        const uniqueIndexes = new Map(indexRows.map((row) => [row.indexName, row.columnsList]));
        assert.equal(uniqueIndexes.get("uk_prospect_run_idempotency"), "team_id,created_by,operation_code,idempotency_key_hash");
        assert.equal(uniqueIndexes.get("uk_prospect_run_active_fingerprint"), "active_team_id,active_owner_id,active_query_fingerprint");
        const [bridgeIndexRows] = await admin.query(`SELECT table_name AS tableName, index_name AS indexName,
         GROUP_CONCAT(column_name ORDER BY seq_in_index) AS columnsList
       FROM information_schema.statistics
       WHERE table_schema = ?
         AND non_unique = 0
       ...
       GROUP BY table_name, index_name`, [databaseName]);
        const bridgeIndexes = new Map(bridgeIndexRows.map((row) => [
            `${row.tableName}.${row.indexName}`,
            row.columnsList
        ]));
        assert.equal(bridgeIndexes.get("agent_jobs.uk_agent_job_queue_bridge_ref"), "team_id,owner_id,id,job_type,parent_job_id");
        assert.equal(bridgeIndexes.get("prospect_run_shards.uk_prospect_run_shard_team_run_id"), "team_id,run_id,id");
        assert.equal(bridgeIndexes.get("prospect_run_queue_parent_bindings.uk_prospect_run_queue_parent_run"), "team_id,run_id");
        assert.equal(bridgeIndexes.get("prospect_run_queue_child_bindings.uk_prospect_run_queue_child_shard"), "team_id,run_id,shard_id");
        const [bridgeForeignKeys] = await admin.query(`SELECT constraint_name AS constraintName, delete_rule AS deleteRule
       FROM information_schema.referential_constraints
       WHERE constraint_schema = ?
         AND constraint_name LIKE 'fk_prospect_run_queue_%'`, [databaseName]);
        assert.equal(bridgeForeignKeys.length, 6);
        assert.equal(bridgeForeignKeys.every((row) => row.deleteRule === "RESTRICT"), true);
        const [bridgeCounts] = await admin.query(`SELECT
         ...
         (SELECT COUNT(*) FROM \`${databaseName}\`.
           agent_jobs WHERE aggregate_id = ?) AS jobs`, [runId, runId, runId]);
        ...
        await assert.rejects(admin.query(`DELETE FROM \`${databaseName}\`.prospect_run_queue_parent_bindings
         WHERE run_id = ?`, [runId]), (error) => error.code === "ER_ROW_IS_REFERENCED_2");
        const [columnRows] = await admin.query(`SELECT column_name AS columnName
       ...
       WHERE table_schema = ?
        ...
        await assert.rejects(admin.query(`INSERT INTO \`${databaseName}\`.prospect_run_shards (
         ...
         WHERE run_id = ?`, [`prsh_${randomUUID()}`, runId]), (error) => error.code === "ER_DUP_ENTRY");
        await assert.rejects(admin.query(`UPDATE \`${databaseName}\`.prospect_run_shards
         SET has_cursor = TRUE WHERE run_id = ?`, [runId]), (error) => error.code === "ER_CHECK_CONSTRAINT_VIOLATED");
        await assert.rejects(admin.query(`INSERT INTO \`${databaseName}\`.prospect_run_shards (
           id,team_id,run_id,provider_code,position_no,status,page_limit,
           result_limit,budget_limit,currency,adapter_version,contract_version,
           catalog_version,capabilities_json,access_mode,has_cursor,created_at,
         ...
         FROM \`${databaseName}\`.prospect_run_shards
         WHERE run_id = ?`, [`prsh_${randomUUID()}`, runId]), (error) => error.code === "ER_NO_REFERENCED_ROW_2");
        await assert.rejects(admin.query(`INSERT INTO \`${databaseName}\`.prospect_run_events (
         ...
         WHERE run_id = ? AND sequence_no = 1`, [`pre_${randomUUID()}`, runId]), (error) => error.code === "ER_NO_REFERENCED_ROW_2");
        ...
        const pauseCampaign = pauseStore.prospectCampaigns.find((item) => item.id === campaign.id);
        await transitionProspectCampaign({
            store: pauseStore,
            user: publicUser(pauseStore.users.find((item) => item.id === owner.id)),
            campaignId: campaign.id,
        ...
        const staleRun = staleStore.prospectSearchRuns.find((item) => item.id === runId);
        await assert.rejects(transitionProspectRun({
            store: staleStore,
            user: publicUser(staleStore.users.find((item) => item.id === owner.id)),
            runId,
        ...
        const rolledBackRun = staleStore.prospectSearchRuns.find((item) => item.id === runId);
        assert.equal(rolledBackRun.status, "queued");
        assert.equal(rolledBackRun.revision, 1);
        assert.equal(staleStore.prospectRunEvents.filter((item) => item.runId === runId)
            .length, 1);
        stage = "cold restart paused state";
        const pausedStore = await createMysqlStore();
        const pausedCampaign = pausedStore.prospectCampaigns.find((item) => item.id === campaign.id);
        const pausedRun = pausedStore.prospectSearchRuns.find((item) => item.id === runId);
        assert.equal(pausedCampaign.status, "paused");
        assert.equal(pausedRun.status, "paused");
        assert.equal(pausedRun.revision, 2);
        assert.equal(pausedStore.prospectRunShards.find((item) => item.runId === runId)
            ?.status, "paused");
        assert.equal(pausedStore.prospectRunEvents.filter((item) => item.runId === runId)
            .length, 2);
        assert.deepEqual(pausedStore.agentJobs
            .filter((item) => item.aggregateId === runId)
            .map((item) => item.status), ["queued", "queued"]);
        stage = "reactivate without auto resume";
        await activateProspectCampaign({
            store: pausedStore,
            user: publicUser(pausedStore.users.find((item) => item.id === owner.id)),
            campaignId: campaign.id,
        ...
        const finalRun = finalStore.prospectSearchRuns.find((item) => item.id === runId);
        assert.equal(finalRun.status, "cancelled");
        assert.equal(finalRun.revision, 4);
        assert.equal(finalStore.prospectRunShards.find((item) => item.runId === runId)
            ?.status, "cancelled");
        assert.deepEqual(finalStore.prospectRunEvents
            .filter((item) => item.runId === runId)
            .sort((left, right) => left.sequence - right.sequence)
            .map((item) => item.eventType), ["created", "paused", "resumed", "cancelled"]);
        assert.deepEqual(finalStore.agentJobs
            .filter((item) => item.aggregateId === runId)
            .map((item) => item.status), ["cancelled", "cancelled"]);
        stage = "append-only persistence guard";
        const immutableEvent = finalStore.prospectRunEvents.find((item) => item.runId === runId);
        const originalReason = immutableEvent.reason;
        immutableEvent.reason = "试图改写运行审计历史";
        await assert.rejects(finalStore.persist(), /获客搜索运行审计事件不可变历史被修改/);
        immutableEvent.reason = originalReason;
        stage = "snapshot tamper cold-start guard";
        await admin.query(`UPDATE \`${databaseName}\`.prospect_search_runs
       ...
       WHERE id = ?`, [runId]);
        await assert.rejects(createMysqlStore(), /获客搜索运行执行快照哈希不一致/);
        await admin.query(`UPDATE \`${databaseName}\`.prospect_search_runs
       SET execution_snapshot_json = ? WHERE id = ?`, [JSON.stringify(finalRun.executionSnapshot), runId]);
        stage = "event-chain tamper cold-start guard";
        const createdEvent = finalStore.prospectRunEvents
            .filter((item) => item.runId === runId)
            .sort((left, right) => left.sequence - right.sequence)[0];
        await admin.query(`UPDATE \`${databaseName}\`.prospect_run_events
       SET event_type = 'paused' WHERE id = ?`, [createdEvent.id]);
        await assert.rejects(createMysqlStore(), /revision 与首个审计事件不一致/);
        await admin.query(`UPDATE \`${databaseName}\`.prospect_run_events
       SET event_type = 'created' WHERE id = ?`, [createdEvent.id]);
        stage = "queue bridge binding tamper cold-start guard";
        const parentBinding = finalStore.prospectRunQueueParentBindings.find((item) => item.runId === runId);
        await admin.query(`UPDATE \`${databaseName}\`.prospect_run_queue_parent_bindings
       SET binding_hash = ? WHERE id = ?`, ["0".repeat(64), parentBinding.id]);
        await assert.rejects(createMysqlStore(), /父桥接绑定完整性校验失败/);
        await admin.query(`UPDATE \`${databaseName}\`.prospect_run_queue_parent_bindings
       SET binding_hash = ? WHERE id = ?`, [parentBinding.bindingHash, parentBinding.id]);
        stage = "queue bridge job digest tamper cold-start guard";
        const parentJob = finalStore.agentJobs.find((item) => item.id === parentBinding.jobId);
        await admin.query(`UPDATE \`${databaseName}\`.agent_jobs
       SET idempotency_key = ? WHERE id = ?`, ["0".repeat(64), parentJob.id]);
        await assert.rejects(createMysqlStore(), /任务字段完整性校验失败/);
        await admin.query(`UPDATE \`${databaseName}\`.agent_jobs
       SET idempotency_key = ? WHERE id = ?`, [parentJob.idempotencyKey, parentJob.id]);
        stage = "orphan reserved job cold-start guard";
        const orphanJobId = `aj_${randomUUID()}`;
        const orphanIdempotencyKey = randomUUID()
            .replaceAll("-", "")
            .repeat(2);
        await admin.query(`INSERT INTO \`${databaseName}\`.agent_jobs (
       ...
       WHERE aggregate_id = ? AND job_type = 'prospect.orchestrate'`, [orphanJobId, orphanIdempotencyKey, randomUUID(), runId]);
        await assert.rejects(createMysqlStore(), /未绑定搜索运行的桥接任务/);
        await admin.query(`DELETE FROM \`${databaseName}\`.agent_jobs WHERE id = ?`, [orphanJobId]);
        stage = "final valid cold restart";
        const verifiedStore = await createMysqlStore();
        assert.equal(verifiedStore.prospectSearchRuns.find((item) => item.id === runId)
            ?.status, "cancelled");
        console.log("Prospect Run MySQL schema, cold restart, conflict recovery, CAS, isolation and tamper tests passed");
        exitCode = 0;
    }
    catch (error) {
