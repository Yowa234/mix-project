...
function requiredUser(id) {
    ...
    if (!user)
        throw new Error(`Missing organization relation test user: ${id}`);
    return user;
}
...
store.organizations.push(organization(childId, "europe", "Relation Child GmbH"), organization(childAliasId, "europe", "Relation Child Trading GmbH"), organization(parentId, "europe", "Relation Parent Holding AG"), organization(otherParentId, "europe", "Relation Other Holding AG"), organization(hiddenId, "organization-relation-hidden-team", "Hidden Ltd"));
...
try {
    const anonymous = await request(`/api/organizations/${childId}/identity-profile`, null);
    assert.equal(anonymous.response.status, 401);
    const emptyProfile = await request(`/api/organizations/${childId}/identity-profile`, salesToken);
    ...
    const superAdminRead = await request(`/api/organizations/${childId}/identity-profile`, superAdminToken);
    assert.equal(superAdminRead.response.status, 403);
    assert.equal(superAdminRead.json.errorCode, "ORGANIZATION_PROFILE_FORBIDDEN");
    const crossTeamRead = await request(`/api/organizations/${hiddenId}/identity-profile`, managerToken);
    assert.equal(crossTeamRead.response.status, 404);
    assert.equal(crossTeamRead.json.errorCode, "ORGANIZATION_NOT_FOUND");
    const salesWrite = await post(`/api/organizations/${childId}/aliases`, salesToken, aliasBody);
    assert.equal(salesWrite.response.status, 403);
    assert.equal(salesWrite.json.errorCode, "ORGANIZATION_FACT_WRITE_FORBIDDEN");
    const crossTeamWrite = await post(`/api/organizations/${hiddenId}/aliases`, managerToken, aliasBody);
    assert.equal(crossTeamWrite.response.status, 404);
    assert.equal(crossTeamWrite.json.errorCode, "ORGANIZATION_NOT_FOUND");
    const aliasCreated = await post(`/api/organizations/${childAliasId}/aliases`, managerToken, aliasBody);
    ...
    assert.equal(aliasCreated.response.headers.get("idempotency-replayed"), "false");
    const aliasReplay = await post(`/api/organizations/${childAliasId}/aliases`, managerToken, aliasBody);
    ...
    const adminAlias = await post(`/api/organizations/${parentId}/aliases`, adminToken, {
        ...aliasBody,
        aliasType: "brand",
    ...
    const relationCreated = await post("/api/organization-relations", managerToken, parentRelationBody);
    ...
    const relationReplay = await post("/api/organization-relations", managerToken, parentRelationBody);
    assert.equal(relationReplay.response.status, 200);
    assert.equal(relationReplay.json.replayed, true);
    assert.equal(relationReplay.json.relation.id, relationCreated.json.relation.id);
    const conflictingParent = await post("/api/organization-relations", managerToken, {
        ...parentRelationBody,
    ...
    assert.equal(conflictingParent.json.errorCode, "ORGANIZATION_RELATION_CONFLICT");
    const selfRelation = await post("/api/organization-relations", managerToken, {
        ...parentRelationBody,
        targetOrganizationId: childId,
        sourceReference: "relation-test-self"
    });
    assert.equal(selfRelation.response.status, 400);
    assert.equal(selfRelation.json.errorCode, "ORGANIZATION_RELATION_INVALID");
    const cycle = await post("/api/organization-relations", adminToken, {
        ...parentRelationBody,
    ...
    const profile = await request(`/api/organizations/${childAliasId}/identity-profile`, salesToken);
    ...
    assert.equal(profile.json.profile.relations[0].relatedOrganization.id, parentId);
    const parentProfile = await request(`/api/organizations/${parentId}/identity-profile`, managerToken);
    assert.equal(parentProfile.response.status, 200);
    assert.equal(parentProfile.json.profile.relations[0].direction, "inbound");
    assert.equal(parentProfile.json.profile.relations[0].relatedOrganization.id, childId);
    assert.deepEqual({
        leads: store.leads,
        leadActivities: store.leadActivities,
        customers: store.customers,
    ...
    const aliasOperation = document.paths["/api/organizations/{id}/aliases"].post;
    const relationOperation = document.paths["/api/organization-relations"].post;
    assert.deepEqual(aliasOperation.requestBody.content["application/json"].schema.required, ["aliasType", "aliasName", "sourceLabel", "evidenceSummary"]);
    assert.deepEqual(relationOperation.requestBody.content["application/json"].schema.required, [
    ...
    console.log("Organization alias and group relation permissions, isolation, idempotency, hierarchy and Swagger tests passed");
}
finally {
