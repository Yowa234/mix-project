 ...
 * 从商机和客户信息生成报关资料
 */
export function generateCustomsDocumentFromDeal(deal, customer, tradeDocument) {
...
export function customsDocumentExportIssues(customsDoc) {
    const issues = [];
    const requiredText = [
    ...
    if (Number(customsDoc.packageCount || 0) <= 0)
        issues.push("包装件数");
    if (Number(customsDoc.grossWeight || 0) <= 0)
        issues.push("总毛重");
    if (Number(customsDoc.netWeight || 0) <= 0)
        issues.push("总净重");
    if (!Array.isArray(customsDoc.items) || !customsDoc.items.length) {
        ...
        customsDoc.items.forEach((item, index) => {
            ...
            if (!String(item.product || "").trim())
                issues.push(`${prefix}品名`);
            if (!String(item.hsCode || "").trim())
                issues.push(`${prefix}HS编码`);
            if (Number(item.quantity || 0) <= 0)
                issues.push(`${prefix}数量`);
            if (!String(item.unit || "").trim())
                issues.push(`${prefix}单位`);
            if (Number(item.weightKg || 0) <= 0)
                issues.push(`${prefix}重量`);
            if (Number(item.packageCount || 0) <= 0)
                issues.push(`${prefix}包装数`);
        });
    }
    return issues;
}
 ...
 * 导出报关资料为Excel文件
 */
export function exportCustomsDocumentToExcel(customsDoc, customer, deal) {
...
function applySheetPresentation(sheet, plan) {
    ...
    if (plan.dataRows) {
        ...
        for (let row = start; row <= end; row += 1) {
            if ((row - start) % 2 === 1)
                styleRow(sheet, row, { fill: STYLE_COLORS.stripe });
        }
    }
...
function thinBorder() {
    const side = { style: "thin", color: { rgb: STYLE_COLORS.border } };
    return { top: side, right: side, bottom: side, left: side };
}
function styleRow(sheet, rowNumber, options) {
    const range = XLSX.utils.decode_range(sheet["!ref"] || "A1:A1");
...
function applyNumberFormat(sheet, startRow, endRow, columns) {
...
function createDeclarationSheet(customsDoc, customer) {
    const data = [
        ["中华人民共和国海关出口货物报关单"],
...
function createDeclarationElementsSheet(customsDoc) {
    const data = [
        ["申报要素说明"],
...
function createInvoiceSheet(customsDoc, customer) {
    const data = [
...
function createPackingListSheet(customsDoc, customer) {
    const data = [
        ["装箱单 PACKING LIST"],
...
function createContractSheet(customsDoc, customer) {
    const data = [
        ["销售合同 SALES CONTRACT"],
...
function createAuthorizationSheet(customsDoc) {
    const data = [
        ["代理报关委托书"],
        [],
