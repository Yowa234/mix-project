...
export const PROSPECT_QUALIFICATION_CONTRACT = "prospect-qualification-gate-v1";
export const PROSPECT_ICP_SCORING_CONTRACT = "prospect-icp-scoring-v1";
...
export const PROSPECT_QUALIFICATION_RECORD_TYPES = {
    ...
    prospectContactVerificationSnapshots: "prospect_contact_verification_snapshot",
...
function qualificationSecret() {
    ...
    if (Buffer.byteLength(value, "utf8") < 32) {
        fail("QUALIFICATION_SECRET_INVALID", "PROSPECT_QUALIFICATION_MASTER_SECRET 必须至少包含 32 字节");
    }
    return value;
}
function hmac(value) {
...
function required(value, label) {
    ...
    if (!normalized)
        fail("QUALIFICATION_INPUT_INVALID", `${label}不能为空`);
    return normalized;
}
...
function commandRequestHash(command) {
    const { idempotencyKey: _idempotencyKey, createdAt: _createdAt, ...request } = command;
    return hmac({
        contract: PROSPECT_QUALIFICATION_CONTRACT,
        request
    });
}
function idempotencyHash(command) {
    return hmac({
        contract: PROSPECT_QUALIFICATION_CONTRACT,
...
function recordHash(recordType, value) {
    return hmac({
        contract: PROSPECT_QUALIFICATION_CONTRACT,
        recordType,
        value
...
function finalize(recordType, value) {
    return {
        ...value,
        recordHash: recordHash(recordType, value)
    };
}
function findReplay(rows, command) {
    ...
    if (existing && existing.requestHash !== requestHash) {
        fail("QUALIFICATION_IDEMPOTENCY_CONFLICT", "同一幂等键已用于不同资格请求");
    }
    return { existing, keyHash, requestHash };
}
function requireProspect(store, teamId, prospectId) {
    const prospect = store.tenantProspects.find((item) => item.teamId === teamId && item.id === prospectId);
    if (!prospect)
        fail("PROSPECT_NOT_FOUND", "候选不存在");
    const organization = store.organizations.find((item) => item.teamId === teamId && item.id === prospect.organizationId);
    if (!organization) {
        fail("ORGANIZATION_NOT_FOUND", "候选关联企业不存在");
    }
    return { prospect, organization };
}
function ownerEvidence(store, command, evidenceIds) {
    const ids = normalizeList(evidenceIds);
    if (!ids.length) {
        fail("EVIDENCE_REQUIRED", "至少需要一条来源证据");
    }
    const rows = ids.map((evidenceId) => {
        const evidence = store.prospectEvidence.find((item) => item.id === evidenceId
...
function latestByCreatedAt(values) {
    return values.reduce((latest, current) => !latest || current.createdAt > latest.createdAt ? current : latest, undefined);
}
function isExpired(expiresAt, at) {
    return Boolean(expiresAt && expiresAt < at);
}
function evidenceSnapshotHash(evidence) {
...
function appendEvidence(store, command) {
    const replay = findReplay(store.prospectEvidence, command);
    if (replay.existing)
        return replay.existing;
    const { organization } = requireProspect(store, command.teamId, command.prospectId);
    const normalizedValue = required(command.value, "证据值")
        .trim()
        .toLowerCase();
    const createdAt = iso(command.createdAt || new Date().toISOString(), "创建时间");
...
function computeCompanyVerification(store, command) {
    ...
    const authoritative = evidence.filter((item) => item.sourceType === "authoritative_registry");
    ...
    const hasInactive = statusValues.some((value) => INACTIVE_VALUES.has(value));
    const hasStrongIdentifier = authoritative.some((item) => item.field === "registration_number");
    const hasOfficialDomain = evidence.some((item) => item.field === "official_domain"
        && item.sourceType === "official_website");
    const independentSources = new Set(evidence.map((item) => `${item.providerCode}\u0000${item.sourceRef}`));
    const conflictFields = [
        "registration_number",
        "jurisdiction"
    ];
    const hasConflict = conflictFields.some((field) => new Set(evidence
        .filter((item) => item.field === field)
        .map((item) => item.normalizedValue)).size > 1) || (hasActive && hasInactive);
    ...
    const snapshot = finalize("company_verification_snapshot", {
        ...
        status,
        reasonCodes,
        authorityCodes: normalizeList(evidence.map((item) => item.authorityCode)),
        ...
        previousSnapshotId: latestByCreatedAt(store.companyVerificationSnapshots.filter((item) => item.teamId === command.teamId
            && item.prospectId === command.prospectId))?.id || "",
...
function reviewCompanyVerification(store, command) {
    const replay = findReplay(store.companyVerificationSnapshots, command);
    if (replay.existing)
        return replay.existing;
    requireProspect(store, command.teamId, command.prospectId);
    const source = store.companyVerificationSnapshots.find((item) => item.id === command.snapshotId
        && item.teamId === command.teamId
        && item.prospectId === command.prospectId);
    if (!source)
        fail("VERIFICATION_NOT_FOUND", "企业核验快照不存在");
    if (command.decision === "approved"
        && source.status !== "partially_verified") {
        fail("VERIFICATION_REVIEW_INVALID", "只有部分核验结论可以人工批准");
    }
    const createdAt = iso(command.createdAt || new Date().toISOString(), "创建时间");
...
function validateWeights(weights) {
    const keys = Object.keys(DEFAULT_ICP_WEIGHTS);
...
function publishIcpPolicy(store, command) {
    const replay = findReplay(store.prospectIcpPolicySnapshots, command);
    if (replay.existing)
        return replay.existing;
    requireProspect(store, command.teamId, command.prospectId);
    const campaignVersion = store.prospectCampaignVersions.find((item) => item.teamId === command.teamId
...
function validateDimensionScores(scores, weights) {
...
function latestCompanyVerification(store, teamId, prospectId) {
    return latestByCreatedAt(store.companyVerificationSnapshots.filter((item) => item.teamId === teamId && item.prospectId === prospectId));
}
function assessIcp(store, command) {
    const replay = findReplay(store.prospectIcpAssessmentSnapshots, command);
    if (replay.existing)
        return replay.existing;
    const { prospect, organization } = requireProspect(store, command.teamId, command.prospectId);
    const policy = store.prospectIcpPolicySnapshots.find((item) => item.id === command.policyId
        && item.teamId === command.teamId
        && item.ownerId === command.ownerId);
    if (!policy)
        fail("ICP_POLICY_NOT_FOUND", "ICP 规则快照不存在");
    const evidence = ownerEvidence(store, command, command.evidenceIds);
    const totalScore = validateDimensionScores(command.dimensionScores, policy.weights);
    const hardGateReasonCodes = new Set(normalizeList(command.hardGateReasonCodes || []));
    const company = latestCompanyVerification(store, command.teamId, command.prospectId);
    ...
    const assessment = finalize("prospect_icp_assessment_snapshot", {
        ...
        reviewedBy: "",
        reviewedAt: "",
        previousAssessmentId: latestByCreatedAt(store.prospectIcpAssessmentSnapshots.filter((item) => item.teamId === command.teamId
            && item.ownerId === command.ownerId
            && item.prospectId === command.prospectId
            && item.policyId === policy.id))?.id || "",
        scoringContractVersion: PROSPECT_ICP_SCORING_CONTRACT,
        idempotencyKeyHash: replay.keyHash,
        requestHash: replay.requestHash,
        createdAt
...
function reviewIcp(store, command) {
    const replay = findReplay(store.prospectIcpAssessmentSnapshots, command);
    if (replay.existing)
        return replay.existing;
    requireProspect(store, command.teamId, command.prospectId);
    const source = store.prospectIcpAssessmentSnapshots.find((item) => item.id === command.assessmentId
        && item.teamId === command.teamId
        && item.ownerId === command.ownerId
        && item.prospectId === command.prospectId);
    if (!source)
        fail("ICP_ASSESSMENT_NOT_FOUND", "ICP 评分快照不存在");
...
function requireSourceEvidence(store, command, evidenceId) {
    return ownerEvidence(store, command, [evidenceId])[0];
}
function addContact(store, command) {
...
function normalizeChannelValue(channelType, value) {
...
function ownerContact(store, command, contactId) {
    const contact = store.prospectContacts.find((item) => item.id === contactId
        && item.teamId === command.teamId
        && item.ownerId === command.ownerId
        && item.prospectId === command.prospectId);
    if (!contact)
        fail("CONTACT_NOT_FOUND", "联系人不存在或不可见");
    return contact;
}
function ownerChannel(store, command, channelId) {
    const channel = store.prospectContactChannels.find((item) => item.id === channelId
        && item.teamId === command.teamId
        && item.ownerId === command.ownerId
        && item.prospectId === command.prospectId);
    if (!channel) {
...
function addContactChannel(store, command) {
    ...
    const duplicate = store.prospectContactChannels.find((item) => item.teamId === command.teamId
        && item.ownerId === command.ownerId
...
function verifyContactChannel(store, command) {
    const replay = findReplay(store.prospectContactVerificationSnapshots, command);
    if (replay.existing)
        return replay.existing;
    const channel = ownerChannel(store, command, command.channelId);
    const current = latestByCreatedAt(store.prospectContactVerificationSnapshots.filter((item) => item.teamId === command.teamId
        && item.ownerId === command.ownerId
    ...
    if (current && terminal.has(current.status)
        ...
        fail("CONTACT_VERIFICATION_TRANSITION_INVALID", "终止状态的联系方式不能直接恢复，请新建并重新核验");
    }
...
function suppressionScopeKey(store, command, organizationId) {
    ...
    if (command.scope === "organization_channel") {
        const channelType = command.channelType
            || (command.channelId
                ...
                : undefined);
        if (!channelType) {
            fail("SUPPRESSION_INPUT_INVALID", "企业通道抑制必须指定通道类型");
        }
        return {
            contactId: "",
            channelId: "",
    ...
    const contact = ownerContact(store, command, required(command.contactId, "联系人"));
    ...
    const channel = ownerChannel(store, command, required(command.channelId, "联系方式"));
    ...
    return {
        ...
        rawKey: `channel:${channel.channelType}:${channel.normalizedValueHash}`
    };
}
function setSuppression(store, command) {
    ...
    const previous = latestByCreatedAt(store.prospectSuppressionEvents.filter((item) => item.teamId === command.teamId
...
function latestAssessment(store, input) {
    return latestByCreatedAt(store.prospectIcpAssessmentSnapshots.filter((item) => item.teamId === input.teamId
...
function activeSuppressionHashes(store, input) {
    const keys = [
        hmac({
            teamId: input.teamId,
        ...
        hmac({
            ...
            key: `organization:${input.organizationId}:${input.channel.channelType}`
        }),
        hmac({
            teamId: input.teamId,
            scope: "contact_all",
        ...
        hmac({
            ...
            key: `channel:${input.channel.channelType}:`
                + input.channel.normalizedValueHash
        })
    ];
    return keys.filter((key) => {
        const latest = latestByCreatedAt(store.prospectSuppressionEvents.filter((item) => item.teamId === input.teamId && item.scopeKeyHash === key));
...
function evaluateGateState(store, input) {
    const { prospect, organization } = requireProspect(store, input.teamId, input.prospectId);
    ...
    const company = latestCompanyVerification(store, input.teamId, input.prospectId);
    const assessment = latestAssessment(store, input);
    const verification = latestByCreatedAt(store.prospectContactVerificationSnapshots.filter((item) => item.teamId === input.teamId
        && item.ownerId === input.ownerId
        && item.channelId === channel.id));
...
function evaluateContactability(store, command) {
    ...
    const decision = finalize("prospect_contactability_decision", {
        ...
        approvedBy: "",
        approvedAt: "",
        previousDecisionId: latestByCreatedAt(store.prospectContactabilityDecisions.filter((item) => item.teamId === command.teamId
            && item.ownerId === command.ownerId
            && item.prospectId === command.prospectId
            && item.campaignId === command.campaignId
            && item.campaignVersion === command.campaignVersion
            && item.channelId === command.channelId))?.id || "",
        contractVersion: PROSPECT_QUALIFICATION_CONTRACT,
        idempotencyKeyHash: replay.keyHash,
        requestHash: replay.requestHash,
        createdAt
...
function approveContactability(store, command) {
    const replay = findReplay(store.prospectContactabilityDecisions, command);
    if (replay.existing)
        return replay.existing;
    const source = store.prospectContactabilityDecisions.find((item) => item.id === command.decisionId
        && item.teamId === command.teamId
        && item.ownerId === command.ownerId
        && item.prospectId === command.prospectId);
    if (!source)
        fail("CONTACTABILITY_NOT_FOUND", "可联系门禁结论不存在");
    ...
    if (current.status !== "eligible"
        ...
        fail("CONTACTABILITY_STALE", "资格事实已变化，请重新执行可联系门禁");
    }
    const { recordHash: _sourceRecordHash, ...sourceValue } = source;
    const decision = finalize("prospect_contactability_decision", {
        ...sourceValue,
...
export function applyProspectQualificationCommand(store, command) {
    required(command.teamId, "团队");
    required(command.ownerId, "业务员");
    required(command.actorId, "操作人");
    required(command.prospectId, "候选");
    const beforeCount = PROSPECT_QUALIFICATION_ARRAYS.reduce((count, key) => count + store[key].length, 0);
    let record;
    switch (command.kind) {
    ...
    return {
        kind: command.kind,
        record,
        replayed: PROSPECT_QUALIFICATION_ARRAYS.reduce((count, key) => count + store[key].length, 0) === beforeCount
    };
}
export function listOwnerProspectQualification(store, input) {
    requireProspect(store, input.teamId, input.prospectId);
    const ownerFilter = (rows) => rows.filter((item) => item.teamId === input.teamId
        && item.ownerId === input.ownerId
        && item.prospectId === input.prospectId);
    return {
        evidence: ownerFilter(store.prospectEvidence),
        companyVerification: latestCompanyVerification(store, input.teamId, input.prospectId) || null,
        icpPolicies: store.prospectIcpPolicySnapshots.filter((item) => item.teamId === input.teamId && item.ownerId === input.ownerId),
        ...
        contactVerifications: ownerFilter(store.prospectContactVerificationSnapshots),
        suppressions: store.prospectSuppressionEvents.filter((item) => item.teamId === input.teamId
            && item.prospectId === input.prospectId
            && (item.ownerId === input.ownerId
                || item.scope.startsWith("organization_"))).map((item) => item.ownerId === input.ownerId
            ? item
            : {
                id: item.id,
                scope: item.scope,
        ...
        contactabilityDecisions: ownerFilter(store.prospectContactabilityDecisions)
    };
}
export function currentContactabilityDecision(store, input) {
    const latest = latestByCreatedAt(store.prospectContactabilityDecisions.filter((item) => item.teamId === input.teamId
        && item.ownerId === input.ownerId
        && item.prospectId === input.prospectId
        && item.campaignId === input.campaignId
        && item.campaignVersion === input.campaignVersion
...
export function validateProspectQualificationState(store) {
    const seen = new Set();
    for (const key of PROSPECT_QUALIFICATION_ARRAYS) {
        for (const item of store[key]) {
            ...
            if (actual !== recordHash(PROSPECT_QUALIFICATION_RECORD_TYPES[key], withoutHash)) {
            ...
            if ("prospectId" in item) {
                ...
                const campaignVersion = store.prospectCampaignVersions.find((version) => version.teamId === item.teamId
                    && version.campaignId === item.campaignId
                    && version.version === item.campaignVersion);
                if (!campaignVersion) {
                    fail("QUALIFICATION_DATA_INTEGRITY", "ICP 规则关联的项目版本不存在");
                }
            }
