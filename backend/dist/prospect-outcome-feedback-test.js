...
function candidate(id, ownerId = "sales-a", teamId = "team-a") {
    return {
        id,
        company: `Buyer ${id}`,
...
function deal(id, ownerId = "sales-a", teamId = "team-a") {
    return {
        id,
...
function coverage(item, sourceHitId = "") {
    return {
        id: `coverage-${item.id}`,
...
}
{
    ...
    assert.equal(prospectPerformance(store, {
        teamId: "team-a",
        ownerId: "sales-b"
...
}
{
    ...
    for (let index = 0; index < 9; index += 1) {
        ...
        if (index < 4)
            store.prospectTouchpoints.push(outbound(item));
    }
...
}
{
    ...
    store.prospectStrategies.push(activeStrategy);
