...
import { applyProspectQualificationCommand, currentContactabilityDecision, listOwnerProspectQualification, ProspectQualificationError, validateProspectQualificationState } from "./prospect-qualification.js";
...
const registration = applyProspectQualificationCommand(store, registrationCommand);
assert.equal(registration.replayed, false);
assert.equal(applyProspectQualificationCommand(store, registrationCommand).replayed, true);
assert.throws(() => applyProspectQualificationCommand(store, {
    ...registrationCommand,
    value: "HRB 999999"
...
assert.equal("status" in companyVerification.record
...
assert.equal("result" in assessment.record ? assessment.record.result : "", "qualified");
...
assert.equal("reviewStatus" in approvedAssessment.record
...
assert.equal("value" in channel.record ? channel.record.value : "", "sales@qualification.example");
...
assert.equal("status" in blockedBeforeVerification.record
...
assert.equal("status" in eligible.record ? eligible.record.status : "", "eligible");
...
assert.equal("status" in approved.record ? approved.record.status : "", "approved_contactable");
...
assert.equal("reasonCodes" in blockedAfterSuppression.record
