...
import { FR_COMPANY_SEARCH_PROVIDER, NPPES_PROVIDER, OPENFDA_510K_PROVIDER, PUBLIC_COMPANY_PROVIDERS, ROR_PROVIDER } from "./public-company-providers.js";
import { BRAZIL_PNCP_PROVIDER, FMCSA_QCMOBILE_PROVIDER, MEXICO_DENUE_PROVIDER, PUBLIC_PROCUREMENT_PROVIDERS, SAM_OPPORTUNITIES_PROVIDER, SINGAPORE_GEBIZ_PROVIDER, UK_FIND_A_TENDER_PROVIDER, USASPENDING_AWARDS_PROVIDER } from "./public-procurement-providers.js";
import { createDefaultProviderCatalog } from "./provider-catalog.js";
import { normalizeProviderPage, normalizeProviderQuery, ProviderContractError } from "./provider-contract.js";
...
function query(country, productKeywords = "solar pump") {
...
async function searchWithMock(input) {
    ...
    const page = await input.provider.search({
        ...
        cursor: ""
    }, { apiKey: input.apiKey || "" }, tools(input.provider));
...
for (const provider of ASSISTED_SOURCE_PROVIDERS) {
    ...
    const catalogItem = catalog.find((item) => item.code === provider.id);
    assert.ok(catalogItem, `${provider.id} should be registered in provider catalog`);
    assert.throws(() => assertProviderOperationPolicy(provider, catalogItem, "search"), (error) => error instanceof ProviderContractError
...
const skippedFrance = await FR_COMPANY_SEARCH_PROVIDER.search({ query: query("Germany"), cursor: "" }, { apiKey: "" }, tools(FR_COMPANY_SEARCH_PROVIDER));
assert.equal(skippedCalls, 0);
assert.equal(skippedFrance.records.length, 0);
assert.equal(skippedFrance.usage?.requestCount, 0);
setProviderHttpTestTransport(async () => new Response(JSON.stringify({ number_of_results: "invalid", items: {} }), {
    status: 200,
    headers: { "content-type": "application/json" }
}));
await assert.rejects(() => ROR_PROVIDER.search({ query: query(""), cursor: "" }, { apiKey: "" }, tools(ROR_PROVIDER)), (error) => error instanceof ProviderContractError
    && error.code === "PROVIDER_SCHEMA_CHANGED");
setProviderHttpTestTransport(async () => new Response(JSON.stringify({ message: "rate limited" }), {
    status: 429,
    headers: { "content-type": "application/json", "retry-after": "60" }
}));
await assert.rejects(() => USASPENDING_AWARDS_PROVIDER.search({ query: query("USA"), cursor: "" }, { apiKey: "" }, tools(USASPENDING_AWARDS_PROVIDER)), (error) => error instanceof Error && /HTTP 429/.test(error.message));
