...
export function prospectScheduleEtag(schedule) {
    return `"${schedule.id}:${schedule.revision}"`;
}
function assertScheduleRole(user) {
    if (user.role === "super_admin") {
        throw new ProspectScheduleRequestError(403, "SCHEDULE_ACCESS_FORBIDDEN", "超级管理员默认不能访问团队定时获客计划");
    }
}
function scheduleApprovedStrategies(store, schedule) {
    return store.prospectStrategies.filter((item) => item.teamId === schedule.teamId
        && item.ownerId === schedule.ownerId
        && item.campaignId === schedule.campaignId
        && item.campaignVersion === schedule.campaignVersion
        && item.status === "approved");
}
function scheduleDueReviewCount(store, schedule, now = new Date()) {
    const dueAt = now.toISOString();
    const ownedProspectIds = new Set(store.prospectCoverageEvents
        .filter((item) => item.teamId === schedule.teamId
        && item.ownerId === schedule.ownerId
        && item.campaignId === schedule.campaignId
        && item.eventType === "coverage_classified")
        .map((item) => item.prospectId));
    return store.tenantProspects.filter((item) => item.teamId === schedule.teamId
        && item.status === "active"
        && Boolean(item.nextReviewAt)
        && item.nextReviewAt <= dueAt
        && ownedProspectIds.has(item.id)).length;
}
function publicSchedule(store, schedule) {
    const { teamId: _teamId, ...visible } = schedule;
    const approvedStrategies = scheduleApprovedStrategies(store, schedule);
    const rotatableStrategyCount = approvedStrategies.filter((item) => schedule.recurringCostApproved
        || !strategyNeedsRecurringCostApproval(store, item)).length;
    const lastStrategyId = store.prospectSearchRuns.find((item) => item.id === schedule.lastRunId
        && item.teamId === schedule.teamId
        && item.ownerId === schedule.ownerId)?.strategyId || "";
    return {
        ...visible,
...
function canRead(user, schedule) {
    if (user.role === "manager" || user.role === "admin") {
        return user.teamId === schedule.teamId;
    }
    return user.teamId === schedule.teamId && user.id === schedule.ownerId;
}
function findVisibleSchedule(store, user, scheduleId) {
    assertScheduleRole(user);
    const schedule = store.prospectSchedules.find((item) => item.id === scheduleId && canRead(user, item));
    if (!schedule) {
        throw new ProspectScheduleRequestError(404, "SCHEDULE_NOT_FOUND", "定时获客计划不存在或无权访问");
    }
    return schedule;
}
function assertOwnerMutation(user, schedule) {
    if (schedule.ownerId !== user.id || schedule.teamId !== user.teamId) {
        throw new ProspectScheduleRequestError(403, "SCHEDULE_MUTATION_FORBIDDEN", "只能修改本人创建的定时获客计划");
    }
}
function assertIfMatch(schedule, ifMatch) {
    if (!ifMatch) {
        throw new ProspectScheduleRequestError(428, "IF_MATCH_REQUIRED", "必须提供 If-Match 请求头");
    }
    if (ifMatch.trim() !== prospectScheduleEtag(schedule)) {
        throw new ProspectScheduleRequestError(412, "SCHEDULE_REVISION_CONFLICT", "定时获客计划已更新，请刷新后重试");
    }
}
function assertTimezone(timezone) {
    try {
        ...
        throw new ProspectScheduleRequestError(400, "INVALID_TIMEZONE", "时区无效");
    }
}
...
function zonedDateToUtc(input, timezone) {
    const wallClock = Date.UTC(input.year, input.month - 1, input.day, input.hour, input.minute, input.second);
...
export function nextProspectScheduleOccurrence(from, frequency, timezone) {
    assertTimezone(timezone);
    const source = typeof from === "string" ? new Date(from) : from;
    if (Number.isNaN(source.getTime())) {
        throw new ProspectScheduleRequestError(400, "INVALID_SCHEDULE_TIME", "定时计划时间无效");
    }
    ...
    if (frequency === "monthly") {
        ...
        local.setUTCFullYear(targetYear, targetMonth - 1, Math.min(parts.day, daysInMonth(targetYear, targetMonth)));
    }
...
export function advanceProspectScheduleBeyond(plannedAt, frequency, timezone, now) {
...
export function strategyNeedsRecurringCostApproval(store, strategy) {
    return strategy.providerPlan.some((plan) => {
        if (plan.providerId === "ai_search" || (plan.budgetLimit || 0) > 0) {
            return true;
        }
        const catalog = store.providerCatalog.find((item) => item.code === plan.providerId);
        return catalog?.licensePolicy.tier === "ai"
            || catalog?.licensePolicy.tier === "paid";
    });
}
function scheduleReferences(store, strategyId) {
    const strategy = store.prospectStrategies.find((item) => item.id === strategyId);
    const campaign = strategy
        ? store.prospectCampaigns.find((item) => item.id === strategy.campaignId && item.teamId === strategy.teamId)
        : undefined;
    return { strategy, campaign };
}
function persistMutation(store, mutation) {
...
export function listProspectSchedules(store, user) {
    assertScheduleRole(user);
...
export async function createProspectSchedule(input) {
    ...
    return persistMutation(input.store, () => {
        const { strategy, campaign } = scheduleReferences(input.store, input.strategyId);
        if (!strategy || !campaign
            || strategy.teamId !== input.user.teamId
            || strategy.ownerId !== input.user.id
            || campaign.ownerId !== input.user.id) {
            throw new ProspectScheduleRequestError(404, "STRATEGY_NOT_FOUND", "搜索策略不存在或无权创建定时计划");
        }
        if (strategy.status !== "approved"
            || campaign.status !== "active"
            || strategy.campaignVersion !== campaign.currentVersion) {
            throw new ProspectScheduleRequestError(422, "SCHEDULE_NOT_READY", "只能为当前版本已批准策略创建定时计划");
        }
        if (input.ifMatch !== prospectStrategyEtag(strategy)) {
            throw new ProspectScheduleRequestError(input.ifMatch ? 412 : 428, input.ifMatch ? "STRATEGY_REVISION_CONFLICT" : "IF_MATCH_REQUIRED", input.ifMatch ? "搜索策略已更新，请刷新后重试" : "必须提供 If-Match 请求头");
        }
        if (strategyNeedsRecurringCostApproval(input.store, strategy)
            && !input.body.recurringCostApproved) {
            throw new ProspectScheduleRequestError(422, "RECURRING_COST_APPROVAL_REQUIRED", "定期运行包含计费或 AI 数据源，需要单独确认持续费用");
        }
        const existing = input.store.prospectSchedules.find((item) => item.campaignId === campaign.id
            && item.campaignVersion === campaign.currentVersion
            && item.ownerId === input.user.id
            && item.status === "active");
        if (existing) {
            throw new ProspectScheduleRequestError(409, "ACTIVE_SCHEDULE_EXISTS", "当前获客项目版本已有启用中的定时计划", { scheduleId: existing.id });
        }
        ...
        const schedule = {
            ...
            nextRunAt: nextProspectScheduleOccurrence(now, input.body.frequency, input.body.timezone),
            lastRunAt: "",
            lastRunId: "",
            lastPlannedAt: "",
            lastFailureCode: "",
        ...
        return {
            ...
            rollback: () => {
                input.store.prospectSchedules.splice(0, input.store.prospectSchedules.length, ...before);
            }
        };
    });
}
export async function transitionProspectSchedule(input) {
    return persistMutation(input.store, () => {
        const schedule = findVisibleSchedule(input.store, input.user, input.scheduleId);
        ...
        if (target === "active") {
            const activeSchedule = input.store.prospectSchedules.find((item) => item.id !== schedule.id
                && item.teamId === schedule.teamId
                && item.ownerId === schedule.ownerId
                && item.campaignId === schedule.campaignId
                && item.campaignVersion === schedule.campaignVersion
                && item.status === "active");
            if (activeSchedule) {
                throw new ProspectScheduleRequestError(409, "ACTIVE_SCHEDULE_EXISTS", "当前获客项目版本已有启用中的定时计划", { scheduleId: activeSchedule.id });
            }
            const { strategy, campaign } = scheduleReferences(input.store, schedule.strategyId);
            if (!strategy || !campaign
                ...
                throw new ProspectScheduleRequestError(422, "SCHEDULE_NOT_READY", "项目或策略已变化，当前计划不能恢复");
            }
            schedule.nextRunAt = nextProspectScheduleOccurrence(now, schedule.frequency, schedule.timezone);
...
export function scheduleOwnerSession(store, schedule) {
