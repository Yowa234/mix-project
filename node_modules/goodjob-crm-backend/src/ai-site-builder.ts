...
import type { AiModelConfig, AiSiteBuilderProject, AiSiteBuilderSetting, SessionUser } from "./types.js";
...
async function callAiModel(config: AiModelConfig, prompt: string, maxInputChars = 12000) {
  ...
  try {
    if (protocol === "anthropic") {
      ...
      const data = await readAiJson<{ content?: Array<{ text?: string }> }>(response);
    ...
    if (error instanceof Error && error.name === "AbortError") throw new Error("This operation was aborted after 100s timeout");
    throw error;
  } finally {
...
function normalizeAiTestStatus(value: unknown): AiModelConfig["lastTestStatus"] {
...
export function registerAiSiteBuilderRoutes(app: Express) {
...
function aiSiteReferenceSitesFile(projectId: string) {
...
function aiSectionRevisionDir(projectId: string, sectionKey: AiSiteSectionKey) {
  if (!isAiSiteSectionKey(sectionKey)) throw new Error("Invalid AI site section key");
  return path.join(aiProjectDir(projectId), "revisions", sectionKey);
}

function cleanAiSiteRevisionReason(value: string) {
  return String(value || "edit").toLowerCase().replace(/[^a-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 48) || "edit";
}

async function snapshotAiSiteSection(project: AiSiteBuilderProject, sectionKey: AiSiteSectionKey, reason = "edit") {
...
async function writeAiSiteSectionHtml(project: AiSiteBuilderProject, sectionKey: AiSiteSectionKey, html: string, reason = "edit") {
...
async function undoAiSiteSectionHtml(project: AiSiteBuilderProject, sectionKey: AiSiteSectionKey) {
  const dir = aiSectionRevisionDir(project.id, sectionKey);
  const entries = (await readdir(dir).catch(() => []))
    .filter((name) => name.endsWith(".html"))
    .sort();
  const latest = entries.at(-1);
  if (!latest) throw new Error("No undo snapshot is available for this section.");
...
function uniqueCleanStrings(values: unknown[], max = 12) {
...
function stripAiReferenceHtml(html: string) {
  return html
...
function decodeBasicHtmlEntities(value: string) {
  ...
  return value.replace(/&(#x?[0-9a-f]+|[a-z]+);/gi, (_match, entity: string) => {
    ...
    if (key.startsWith("#x")) return String.fromCharCode(parseInt(key.slice(2), 16) || 32);
    if (key.startsWith("#")) return String.fromCharCode(parseInt(key.slice(1), 10) || 32);
    return named[key] || " ";
  });
}
...
function htmlAttrValues(html: string, attr: string) {
  const pattern = new RegExp(`\\s${attr}\\s*=\\s*([\"'])(.*?)\\1`, "gi");
  const values: string[] = [];
  let match: RegExpExecArray | null;
  while ((match = pattern.exec(html)) && values.length < 80) values.push(decodeBasicHtmlEntities(match[2] || ""));
  return values;
}

function htmlTagTexts(html: string, tagPattern: string, max = 12) {
  const pattern = new RegExp(`<${tagPattern}\\b[^>]*>([\\s\\S]*?)<\\/${tagPattern.split("|")[0]}>`, "gi");
  const values: string[] = [];
  let match: RegExpExecArray | null;
  while ((match = pattern.exec(html)) && values.length < max * 2) values.push(textFromHtml(match[1] || ""));
  return uniqueCleanStrings(values, max);
}

function extractAiReferenceMeta(html: string, name: string) {
  const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const pattern = new RegExp(`<meta\\b(?=[^>]*(?:name|property)=["']${escaped}["'])(?=[^>]*content=(["'])(.*?)\\1)[^>]*>`, "i");
  return textFromHtml(pattern.exec(html)?.[2] || "");
}

function extractAiReferenceColors(html: string) {
  const colors = [
...
function extractAiReferenceTypographyHints(html: string) {
  const hints = [
...
function extractAiReferenceSectionHints(html: string) {
  const values = [
...
function aiReferenceLayoutNotes(html: string, sectionHints: string[]) {
  const source = html.toLowerCase();
  const notes: string[] = [];
  if (/topbar|header/.test(sectionHints.join(" "))) notes.push("Header includes a clear navigation/top information area.");
  if (/dropdown|mega-menu|submenu/.test(source)) notes.push("Navigation likely uses dropdown or mega-menu behavior.");
  if (/hero|banner|swiper|slide|carousel/.test(source)) notes.push("Hero area appears visually prominent, possibly image or slider driven.");
  if (/product|category|catalog/.test(sectionHints.join(" "))) notes.push("Products are organized through category/catalog modules.");
  if (/blog|news/.test(sectionHints.join(" "))) notes.push("Blog/news content appears as a separate editorial listing module.");
  if (/contact|inquiry|form/.test(sectionHints.join(" "))) notes.push("Contact/inquiry conversion path is visible.");
  if (/grid|row|col-|columns|flex/.test(source)) notes.push("Layout uses grid/flex card groupings rather than plain text flow.");
  if (/animation|transition|transform|swiper|slick|owl-carousel/.test(source)) notes.push("Site likely uses motion/slider interactions; keep generated motion lightweight and export-safe.");
...
function normalizedAiReferenceUrls(value: unknown) {
...
async function assertSafeAiReferenceUrl(input: string) {
  let parsed: URL;
  try {
  ...
  if (!["http:", "https:"].includes(parsed.protocol)) throw new Error("Only http/https reference URLs are allowed");
  const host = parsed.hostname.toLowerCase();
  if (!host || host === "localhost" || host.endsWith(".local")) throw new Error("Local reference URLs are blocked");
  if (net.isIP(host) && isPrivateIpAddress(host)) throw new Error("Private IP reference URLs are blocked");
  if (!net.isIP(host)) {
    const records = await lookup(host, { all: true }).catch(() => []);
    if (records.some((item) => isPrivateIpAddress(item.address))) throw new Error("Reference URL resolves to a private network address");
  }
  return parsed.toString();
}

async function analyzeAiSiteReferenceUrl(input: string): Promise<AiSiteReferenceSiteAnalysis> {
  ...
  try {
    ...
    try {
      ...
      if (!/text\/html|application\/xhtml\+xml/i.test(contentType)) throw new Error(`Unsupported content-type: ${contentType || "unknown"}`);
...
function buildAiSiteReferenceBrief(sites: AiSiteReferenceSiteAnalysis[]) {
...
async function readAiSiteReferenceSites(project: AiSiteBuilderProject): Promise<AiSiteReferenceSitesState> {
  await ensureAiSiteSandbox(project);
  const fallback: AiSiteReferenceSitesState = { version: "ai-site-reference-sites.v1", updatedAt: "", sites: [], brief: "" };
  const stored = await readJsonFile<Partial<AiSiteReferenceSitesState>>(aiSiteReferenceSitesFile(project.id), fallback);
  return {
...
async function writeAiSiteReferenceSites(project: AiSiteBuilderProject, sites: AiSiteReferenceSiteAnalysis[]) {
  await ensureAiSiteSandbox(project);
  const state: AiSiteReferenceSitesState = {
...
function aiSiteReferenceDesignContext(project: AiSiteBuilderProject) {
...
async function readAiSiteProjectExport(project: AiSiteBuilderProject, includeHtml = false) {
  await ensureAiSiteSandbox(project);
  const manifest = await readJsonFile<Record<string, unknown> | null>(path.join(aiExportDir(project.id), "export.json"), null);
...
function aiSitePlaceholderImage(width: number, height: number, bg: string, fg: string, text: string) {
...
function aiSiteWpStyleTagFromHtml(html: string) {
  return html.match(/<style\b[^>]*>[\s\S]*?<\/style>/i)?.[0] || "";
}

function aiSiteWpCptAdapterStyle(sectionKey: "products" | "blog") {
  if (sectionKey === "products") {
...
function aiSiteCssRuleBlocks(css: string) {
...
function aiSiteGradientCount(value: string) {
  return (value.match(/(?:linear|radial|conic)-gradient\s*\(/gi) || []).length;
}

function aiSiteIsLargeSurfaceSelector(selector: string, sectionId: string) {
...
function aiSiteCssHasHardSplitGradient(css: string) {
  return /linear-gradient\([^)]*\b(\d{1,3}(?:\.\d+)?)%\s*,\s*(?:#[0-9a-f]{3,8}|rgba?\([^)]*\)|var\([^)]*\)|[a-z]+)\s+\1%/i.test(css);
}

function aiSiteUnsafeLargeGradientReason(css: string, sectionId: string) {
  ...
  for (const block of aiSiteCssRuleBlocks(css)) {
    ...
    if (!/gradient\s*\(/i.test(body) || !aiSiteIsLargeSurfaceSelector(block.selector, sectionId)) continue;
...
function aiSiteSelectedRegionContext(value: unknown, maxLength = 60000) {
  return String(value || "")
    .replace(/<script[\s\S]*?<\/script>/gi, " ")
    .replace(/\son[a-z]+\s*=\s*"[^"]*"/gi, "")
    .replace(/\son[a-z]+\s*=\s*'[^']*'/gi, "")
...
function buildAiSiteSelectedRegionRewritePrompt(project: AiSiteBuilderProject, sectionKey: AiSiteSectionKey, currentHtml: string, selectedRegion: Record<string, unknown>, userInstruction: string, repairReason = "") {
  const schema = normalizeAiSiteSchemaData(project.schemaData);
  const designSystem = aiSiteDesignSystem(project);
  const styleProfile = designSystem.styleProfile;
  const blueprint = cleanAiSiteBlueprint(project);
...
async function rewriteAiSiteSelectedRegionHtml(project: AiSiteBuilderProject, sectionKey: AiSiteSectionKey, user: SessionUser, selectedRegion: Record<string, unknown>, userInstruction: string) {
  if (aiSiteLockedSections.has(sectionKey)) throw new Error("Locked header/footer sections cannot be rewritten through selected-region editing.");
  ...
  if (process.env.NODE_ENV === "test") return currentHtml;
  const settings = getStore().aiSiteBuilderSettings.find((item) => item.ownerId === user.id);
  if (!aiSiteModelReady(settings)) throw new Error("AI site builder model settings are not ready. Please save and test the API settings first.");
  const config = aiSiteSettingsToModelConfig(settings!);
