...
try {
  ...
  if (!aiSiteList.response.ok || !(aiSiteList.json.projects || []).some((item: any) => item.id === aiSiteProject.json.project.id) || (aiSiteList.json.projects || []).some((item: any) => item.id === aiSiteDeleteCandidate.json.project.id)) {
  ...
  if (!aiSiteEditor.response.ok || !aiSiteEditor.json.sections?.some((item: any) => item.key === "header" && item.locked)) {
  ...
  if (!aiSiteGenerated.response.ok || !String(aiSiteGenerated.json.html || "").includes("About Us")) throw new Error("ai site section generate failed");
  ...
  if (!aiSiteSourceSave.response.ok) throw new Error("ai site section source save failed");
  ...
  if (!aiSiteHydratedList.response.ok || !(aiSiteHydratedList.json.projects || []).some((item: any) => item.id === aiSiteProject.json.project.id)) {
