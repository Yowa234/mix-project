...
import type { AiModelConfig, AiSiteBuilderProject, AiSiteBuilderSetting, CaseStudy, Competitor, Customer, Deal, Exam, ExamAttempt, ExamQuestion, ExamQuestionLink, ImportExportJob, KnowledgeAsset, LeadSourceConfig, Memo, OcrJob, PlanTask, PlanTemplate, ProblemItem, Reminder, Todo, TradeDocument, User, WecomMessage, WebsiteOpportunity } from "./types.js";
...
async function loadAiSiteBuilderProjects(pool: mysql.Pool): Promise<AiSiteBuilderProject[]> {
  return (await rows<Record<string, any>>(pool, "SELECT * FROM ai_site_builder_projects ORDER BY created_at DESC")).map((row) => ({
    id: row.id,
    ...
    goal: row.goal,
    tone: row.tone,
    pages: parseJsonField<string[]>(row.pages_json, []),
    schemaData: parseJsonField<Record<string, unknown>>(row.schema_json, {}),
    agentPayload: parseJsonField<Record<string, unknown>>(row.agent_payload_json, {}),
    status: row.status,
    ownerId: row.owner_id,
    teamId: row.team_id,
    createdAt: row.created_at instanceof Date ? row.created_at.toISOString() : row.created_at
...
async function loadAiSiteBuilderSettings(pool: mysql.Pool): Promise<AiSiteBuilderSetting[]> {
  return (await rows<Record<string, any>>(pool, "SELECT * FROM ai_site_builder_settings ORDER BY updated_at DESC")).map((row) => ({
    ownerId: row.owner_id,
    teamId: row.team_id,
    provider: row.provider || "openai",
    model: row.model || "",
