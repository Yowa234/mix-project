...
import { PROSPECT_LEAD_CONVERSION_CONTRACT, PROSPECT_LEAD_SOURCE_CHANNEL } from "./prospect-lead-conversion.js";
import { ProspectCoverageMemoryError, setTenantProspectDisposition } from "./prospect-coverage-memory.js";
export const PROSPECT_CUSTOMER_CONVERSION_CONTRACT = "prospect-to-customer-human-confirmation-v1";
...
function canonical(value) {
    ...
    if (typeof result !== "string") {
        fail("PROSPECT_CUSTOMER_CONVERSION_DATA_INTEGRITY", "客户转换请求无法规范化", 500);
    }
    return result;
}
...
function requiredText(value, label, max) {
    ...
    if (!text || text.length > max) {
        fail("PROSPECT_CUSTOMER_CONVERSION_INVALID", `${label}不能为空且不能超过 ${max} 个字符`, 400);
    }
    return text;
}
function normalizedIso(value) {
    ...
    if (!Number.isFinite(date.getTime())) {
        fail("PROSPECT_CUSTOMER_CONVERSION_INVALID", "转换时间无效", 400);
    }
...
function normalizeInput(raw) {
    ...
    if (!parsed.success) {
        fail("PROSPECT_CUSTOMER_CONVERSION_INVALID", parsed.error.issues[0]?.message || "客户转换参数无效", 400);
    }
    const idempotencyKey = requiredText(raw.idempotencyKey, "Idempotency-Key", 200);
    if (idempotencyKey.length < 8
        || !/^[A-Za-z0-9._:-]+$/u.test(idempotencyKey)) {
        fail("PROSPECT_CUSTOMER_CONVERSION_INVALID", "Idempotency-Key 格式无效", 400);
    }
    const coverageSecret = requiredText(raw.coverageSecret, "候选覆盖密钥", 500);
    if (Buffer.byteLength(coverageSecret, "utf8") < 32) {
        fail("PROSPECT_CUSTOMER_CONVERSION_INVALID", "候选覆盖密钥至少需要 32 字节", 500);
    }
    return {
        ...parsed.data,
...
export function prospectCustomerConversionIds(raw) {
...
function findProspect(store, input) {
    const matches = store.tenantProspects.filter((item) => item.id === input.prospectId && item.teamId === input.teamId);
    if (matches.length !== 1) {
        fail("PROSPECT_CUSTOMER_CONVERSION_NOT_FOUND", "候选客户不存在或无权访问", 404);
    }
    return matches[0];
}
function findLead(store, input, prospect) {
    ...
    if (!lead || prospect.leadId !== lead.id) {
        fail("PROSPECT_CUSTOMER_CONVERSION_NOT_FOUND", "线索不存在或无权访问", 404);
    }
    return lead;
}
function sourcePayload(event) {
    try {
        ...
        fail("PROSPECT_CUSTOMER_CONVERSION_SOURCE_INVALID", "线索来源审计载荷损坏", 500);
    }
}
function findLeadSource(store, input, prospect, lead) {
    const matches = store.leadSourceEvents.filter((item) => item.teamId === input.teamId
        && item.ownerId === input.ownerId
        && item.leadId === lead.id
        && item.channel === PROSPECT_LEAD_SOURCE_CHANNEL
        && item.externalId === prospect.id);
    if (matches.length !== 1) {
        fail("PROSPECT_CUSTOMER_CONVERSION_SOURCE_INVALID", "线索缺少唯一、可信的智能获客来源链");
    }
    ...
    if (payload.contract !== PROSPECT_LEAD_CONVERSION_CONTRACT
        ...
        fail("PROSPECT_CUSTOMER_CONVERSION_SOURCE_INVALID", "线索来源链与候选企业身份不一致");
    }
    return source;
}
function findExistingCustomer(store, input) {
    const customer = store.customers.find((item) => item.id === input.existingCustomerId
        && item.teamId === input.teamId
        && item.ownerId === input.ownerId);
    if (!customer) {
        fail("PROSPECT_CUSTOMER_CONVERSION_NOT_FOUND", "已有客户不存在或无权访问", 404);
    }
    return customer;
}
function newCustomer(input, lead, customerId) {
    return {
        id: customerId,
...
function commitLocalState(store, local) {
    store.customers.splice(0, store.customers.length, ...local.customers);
    store.customerActivities.splice(0, store.customerActivities.length, ...local.customerActivities);
    store.customerAcquisitionSourceEvents.splice(0, store.customerAcquisitionSourceEvents.length, ...local.customerAcquisitionSourceEvents);
    store.leads.splice(0, store.leads.length, ...local.leads);
    store.leadActivities.splice(0, store.leadActivities.length, ...local.leadActivities);
    store.tenantProspects.splice(0, store.tenantProspects.length, ...local.tenantProspects);
    store.prospectCoverageEvents.splice(0, store.prospectCoverageEvents.length, ...local.prospectCoverageEvents);
}
function convertOnLocalStore(local, input) {
    ...
    const replayEvents = local.customerAcquisitionSourceEvents.filter((item) => item.id === ids.sourceEventId
        || (item.teamId === input.teamId
            && item.ownerId === input.ownerId
            && item.processingKeyHash === ids.processingKeyHash));
    if (replayEvents.length > 1) {
        fail("PROSPECT_CUSTOMER_CONVERSION_DATA_INTEGRITY", "同一客户转换处理键存在多条来源审计", 500);
    }
    const prior = replayEvents[0];
    if (prior) {
        if (!sameHash(prior.requestHash, conversionRequestHash)) {
            fail("PROSPECT_CUSTOMER_CONVERSION_IDEMPOTENCY_CONFLICT", "该 Idempotency-Key 已用于不同的客户转换请求");
        }
        const customer = local.customers.find((item) => item.id === prior.customerId
            && item.teamId === input.teamId
            && item.ownerId === input.ownerId);
        const customerActivity = local.customerActivities.find((item) => item.id === ids.customerActivityId
            && item.customerId === prior.customerId);
        const leadActivity = local.leadActivities.find((item) => item.id === ids.leadActivityId && item.leadId === lead.id);
        if (!customer || !customerActivity || !leadActivity
            ...
            fail("PROSPECT_CUSTOMER_CONVERSION_DATA_INTEGRITY", "客户转换审计链与 CRM 状态不一致", 500);
        }
        ...
        if (!disposition.idempotent) {
            fail("PROSPECT_CUSTOMER_CONVERSION_DATA_INTEGRITY", "客户转换幂等覆盖事件缺失", 500);
        }
        return {
            replayed: true,
    ...
    const organizationEvents = local.customerAcquisitionSourceEvents.filter((item) => item.teamId === input.teamId
        && item.organizationId === prospect.organizationId);
    if (organizationEvents.length) {
        const ownCompleted = organizationEvents.some((item) => item.ownerId === input.ownerId
            && (item.prospectId === prospect.id || item.leadId === lead.id));
        fail(ownCompleted
            ? "PROSPECT_CUSTOMER_CONVERSION_ALREADY_COMPLETED"
            : "PROSPECT_CUSTOMER_CONVERSION_ORGANIZATION_OWNERSHIP_CONFLICT", ownCompleted
            ? "该候选客户已经完成客户转换"
            : "该企业已存在受保护的客户归属");
    }
    if (lead.convertedCustomerId || prospect.customerId) {
        if (lead.convertedCustomerId
            ...
            fail("PROSPECT_CUSTOMER_CONVERSION_CUSTOMER_BINDING_CONFLICT", "线索与候选客户绑定状态冲突");
        }
        fail("PROSPECT_CUSTOMER_CONVERSION_ALREADY_COMPLETED", "该线索或候选客户已经关联客户");
    }
    if (prospect.status !== "converted"
        || prospect.queueState !== "converted"
        || prospect.exclusionMode !== "none") {
        fail("PROSPECT_CUSTOMER_CONVERSION_NOT_ELIGIBLE", "候选客户当前不可转为客户");
    }
    const created = input.mode === "create_new";
    let customer;
    if (created) {
        if (local.customers.some((item) => item.id === ids.customerId)) {
            fail("PROSPECT_CUSTOMER_CONVERSION_DATA_INTEGRITY", "客户转换标识发生冲突", 500);
        }
        ...
        const existingBinding = local.customerAcquisitionSourceEvents.find((item) => item.customerId === customer.id
            && item.organizationId !== prospect.organizationId);
        if (existingBinding) {
            fail("PROSPECT_CUSTOMER_CONVERSION_CUSTOMER_BINDING_CONFLICT", "该客户已绑定其他企业身份");
        }
    }
    const sourceEvent = {
    ...
    try {
        ...
        if (error instanceof ProspectCoverageMemoryError
            ...
            fail("PROSPECT_CUSTOMER_CONVERSION_IDEMPOTENCY_CONFLICT", "该 Idempotency-Key 已用于不同的候选处置请求");
        }
        throw error;
    }
    return {
...
export function convertProspectToCustomer(store, raw) {
    if (store.mode === "mysql") {
        fail("PROSPECT_CUSTOMER_CONVERSION_MYSQL_TRANSACTION_REQUIRED", "MySQL 客户转换必须通过专用事务提交", 500);
    }
    const input = normalizeInput(raw);
    const local = {
        ...store,
        ...
        customerAcquisitionSourceEvents: structuredClone(store.customerAcquisitionSourceEvents),
