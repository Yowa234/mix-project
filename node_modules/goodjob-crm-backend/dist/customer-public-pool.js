...
export function isPublicCustomer(customer) {
    return customer.poolStatus === "public";
}
export function isOwnedCustomer(customer) {
    return !isPublicCustomer(customer);
}
export function canReleaseCustomer(customer, input) {
...
export function assertCustomerOwnershipMutation(store, input) {
    ...
    if (input.action === "release") {
        ...
        const hasActiveDeal = store.deals.some((deal) => deal.customerId === customer.id
            && !deal.archivedAt
            && deal.stage !== "成交"
            && deal.stage !== "丢单");
        if (hasActiveDeal) {
            fail("CUSTOMER_POOL_ACTIVE_DEAL", "该客户仍有活跃商机，请先完成、关闭或移交商机后再释放", 409);
        }
...
export function mutateCustomerOwnershipMemory(store, input) {
    ...
    if (input.action === "release") {
        ...
        for (const todo of store.todos) {
            if (todo.customerId !== customer.id || todo.done || todo.cancelledAt)
                continue;
            todo.done = true;
            todo.status = "pending";
            todo.historyAt = now;
