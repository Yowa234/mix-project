import { assertProviderBaseUrlAllowed, createProviderHttpClient } from "./provider-http-client.js";
const AI_HTTP_TIMEOUT_MS = 120_000;
const AI_MAX_RESPONSE_BYTES = 2 * 1024 * 1024;
function aiNetworkPolicy(rawBaseUrl) {
    let baseUrl;
    try {
...
export function assertAiBaseUrlAllowed(rawBaseUrl) {
    const normalized = rawBaseUrl.replace(/\/+$/, "");
    assertProviderBaseUrlAllowed(normalized, aiNetworkPolicy(normalized));
    return normalized;
}
export function createAiHttpClient(rawBaseUrl) {
