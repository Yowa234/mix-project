...
export const PROSPECT_COVERAGE_MEMORY_CONTRACT = "team-prospect-coverage-memory-v1";
...
function withTeamLock(teamId, operation) {
    if (activeTeamLocks.has(teamId)) {
        throw new ProspectCoverageMemoryError("PROSPECT_COVERAGE_TEAM_BUSY", "同一团队覆盖记忆正在处理其它写入");
    }
...
function tenantProspectWithoutHash(prospect) {
    const { prospectHash: _prospectHash, ...rest } = prospect;
    return rest;
}
function coverageEventWithoutHash(event) {
    const { eventHash: _eventHash, ...rest } = event;
    return rest;
}
export function tenantProspectFactHash(prospect, secret) {
    return hmac(secret, {
...
export function prospectCoverageEventFactHash(event, secret) {
    return hmac(secret, {
        contract: PROSPECT_COVERAGE_MEMORY_CONTRACT,
...
function normalizedEvidence(input) {
    const normalized = input.map((item) => {
        ...
        const expiresAt = normalizeOptionalIso(item.expiresAt, "覆盖证据失效时间");
...
function verifiedCoverageContext(store, input) {
    const resolutions = store.organizationIdentityResolutions.filter((item) => item.id === input.resolutionId
    ...
    if (!["new_entity", "exact_match"].includes(resolution.result)
        ...
        throw new ProspectCoverageMemoryError("PROSPECT_COVERAGE_NOT_ELIGIBLE", "只有完成强身份绑定的企业才能进入团队覆盖记忆");
    }
    const organizations = store.organizations.filter((item) => item.id === resolution.organizationId
        && item.teamId === input.teamId
        && item.status === "active");
    if (organizations.length !== 1) {
        integrityError("覆盖记忆引用的企业不存在或不唯一");
    }
    const bindings = store.organizationSourceBindings.filter((item) => item.id === resolution.bindingId
    ...
    const rawRecords = store.prospectSourceRawRecords.filter((item) => item.id === resolution.rawRecordId
        && item.teamId === input.teamId
        && item.ownerId === input.ownerId);
    if (rawRecords.length !== 1) {
        integrityError("覆盖记忆引用的 Provider Raw 不存在或不唯一");
    }
    const sourceHits = store.prospectSourceRawHits.filter((item) => item.id === input.sourceHitId
    ...
    const runs = store.prospectSearchRuns.filter((item) => item.id === sourceHit.runId
        && item.teamId === input.teamId
        && item.ownerId === input.ownerId);
    if (runs.length !== 1) {
    ...
    if (sourceHit.shardId
        && !store.prospectRunShards.some((item) => item.id === sourceHit.shardId
...
function materialEvidenceKeys(store, context, evidence, secret) {
    const claimKeys = store.organizationIdentityClaims
        .filter((item) => item.resolutionId === context.resolution.id
    ...
    const identifierKeys = context.resolution.acceptedIdentifierIds.map((id) => hmac(secret, {
...
function sourceKey(context, secret) {
    return hmac(secret, {
...
function applicableExclusion(prospect, at) {
...
function classifyExistingProspect(prospect, newEvidenceCount, coveredAt) {
    ...
    if (newEvidenceCount > 0) {
        return {
            classification: "new_intelligence",
...
function queueTransition(prospect, classification, reasonCode, at) {
    ...
    if (["net_new", "new_intelligence", "due_review"].includes(classification)) {
...
function appendCoverageEvent(store, prospect, event) {
    const index = store.tenantProspects.findIndex((item) => item.id === prospect.id && item.teamId === prospect.teamId);
    if (index < 0) {
...
function replayCoverageEvent(store, event, requestHash) {
    if (event.requestHash !== requestHash) {
        throw new ProspectCoverageMemoryError("PROSPECT_COVERAGE_REPLAY_CONFLICT", "相同覆盖处理键收到了不同请求内容");
    }
    const prospects = store.tenantProspects.filter((item) => item.id === event.prospectId && item.teamId === event.teamId);
    if (prospects.length !== 1) {
        integrityError("覆盖记忆幂等事件引用的候选不存在或不唯一");
    }
    return prospects[0];
}
export function recordProspectCoverage(store, rawInput) {
    if (store.mode === "mysql") {
        throw new ProspectCoverageMemoryError("PROSPECT_COVERAGE_MYSQL_TRANSACTION_REQUIRED", "MySQL 覆盖记忆只能通过专用事务提交");
    }
    ...
    const resolutionId = requireText(rawInput.resolutionId, "企业身份 Resolution", 200);
    const sourceHitId = requireText(rawInput.sourceHitId, "Provider 来源命中", 200);
    const coveredAt = normalizeIso(rawInput.coveredAt, "覆盖处理时间");
    const nextReviewAt = normalizeOptionalIso(rawInput.nextReviewAt, "下次复核时间");
    ...
    const incomingEvidenceKeys = materialEvidenceKeys(store, context, evidence, secret);
    ...
    return withTeamLock(teamId, () => {
        const replayEvents = store.prospectCoverageEvents.filter((item) => item.teamId === teamId
        ...
        const matches = store.tenantProspects.filter((item) => item.teamId === teamId
        ...
        const previousEvidence = new Set(previous?.materialEvidenceKeyHashes || []);
        const previousSources = new Set(previous?.sourceKeyHashes || []);
        const newEvidenceKeyHashes = incomingEvidenceKeys.filter((key) => !previousEvidence.has(key));
        ...
        if (!previous) {
            ...
            const decision = classifyExistingProspect(prospect, newEvidenceKeyHashes.length, coveredAt);
        ...
        const queueAction = queueTransition(prospect, classification, reasonCode, coveredAt);
...
function verifyCrmLink(store, teamId, type, id) {
    if (!id)
        return;
...
export function setTenantProspectDisposition(store, rawInput) {
    if (store.mode === "mysql") {
        throw new ProspectCoverageMemoryError("PROSPECT_COVERAGE_MYSQL_TRANSACTION_REQUIRED", "MySQL 候选处置只能通过专用事务提交");
    }
    ...
    const excludedUntil = normalizeOptionalIso(rawInput.excludedUntil, "排除到期时间");
    const nextReviewAt = normalizeOptionalIso(rawInput.nextReviewAt, "下次复核时间");
    ...
    return withTeamLock(teamId, () => {
        ...
        const matches = store.tenantProspects.filter((item) => item.id === prospectId && item.teamId === teamId);
        if (matches.length !== 1) {
...
export function listTenantProspects(store, input) {
    const teamId = requireText(input.teamId, "候选团队", 200);
    return structuredClone(store.tenantProspects.filter((item) => item.teamId === teamId));
}
export function listOwnerProspectCoverageEvents(store, input) {
    ...
    const prospectId = input.prospectId
        ...
        : "";
    return structuredClone(store.prospectCoverageEvents.filter((item) => item.teamId === teamId
        && item.ownerId === ownerId
        && (!prospectId || item.prospectId === prospectId)));
}
export function prospectCoverageSha256(value) {
    return sha256(value);
}
