...
store.marketOpportunityBatches.push(latestBatch, historicalReadyBatch, adminPrivateBatch, otherTeamBatch);
store.marketOpportunityCalculationEvents.push(calculation({}), calculation({
...
store.marketOpportunitySnapshots.push(snapshot({
...
async function request(token, campaignId, query = {}) {
    const url = new URL(`/api/prospect-campaigns/${campaignId}/market-opportunities`, baseUrl);
...
store.persistMutation = async (operation) => {
    mutationCalls += 1;
...
try {
    ...
    assert.equal(neverCalculated.json.absenceMeaning, "no_fact_snapshot_does_not_mean_zero_or_no_market");
    ...
    assert.equal(latest.json.selectedBatch.observationCutoffAt, "2026-07-13T10:55:00.000Z");
    ...
    assert.deepEqual(latest.json.opportunities.map((item) => item.country), ["Canada", "Germany"]);
    ...
    assert.equal(latest.json.opportunities[0].metrics.reportedImportValueSeries[2]
        .evidence.reportedImportValueUsd, 144);
    const secondPage = await request(tokenA, "campaign_market", {
        limit: 2,
        cursor: latest.json.nextCursor
    });
    assert.equal(secondPage.response.status, 200);
    assert.deepEqual(secondPage.json.opportunities.map((item) => item.country), ["United States"]);
    ...
    assert.equal(store.marketOpportunityCalculationEvents.length, calculationCountBefore);
    // Anonymous and invalid-query requests are rejected before any store read.
    assert.equal(readBarrierCalls, 15);
    const document = createOpenApiDocument(app);
    const operation = document.paths["/api/prospect-campaigns/{id}/market-opportunities"]?.get;
    assert.ok(operation);
    for (const name of [
        "batchId",
        "countryCode",
        ...
        assert.ok(operation.parameters.some((item) => item.name === name));
    }
    ...
    assert.equal(successSchema.properties.opportunities.items.required.includes("nextAction"), false);
    assert.equal(successSchema.properties.opportunities.items.additionalProperties, false);
    assert.equal(successSchema.properties.opportunities.items.properties.metrics.additionalProperties, false);
    assert.equal(successSchema.properties.opportunities.items.properties.metrics
    ...
    assert.equal(operation.responses["404"].content["application/json"].schema.$ref, "#/components/schemas/MarketOpportunityListRequestError");
    ...
    const marketAnalysisOperation = document.paths["/api/prospect-campaigns/{id}/market-analysis-runs"]?.post;
    ...
    assert.equal(analysisResult.properties.marketOpportunityCalculation.additionalProperties, false);
    ...
    assert.throws(() => validateMarketOpportunityCursorSecurity(), /生产环境必须配置至少 32 字节/);
    ...
    console.log("Market opportunity list latest/history, isolation, cursor, purity, barrier and Swagger tests passed");
}
finally {
