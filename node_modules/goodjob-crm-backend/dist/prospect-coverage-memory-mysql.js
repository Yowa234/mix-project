...
import { PROSPECT_COVERAGE_MEMORY_CONTRACT, ProspectCoverageMemoryError, prospectCoverageEventFactHash, recordProspectCoverage, setTenantProspectDisposition, tenantProspectFactHash } from "./prospect-coverage-memory.js";
import { loadOrganizationIdentityState } from "./organization-strong-identity-mysql.js";
import { convertProspectToCustomer, prospectCustomerConversionIds, ProspectCustomerConversionError } from "./prospect-customer-conversion.js";
import { convertProspectToLead, prospectLeadConversionIds, PROSPECT_LEAD_SOURCE_CHANNEL, ProspectLeadConversionError } from "./prospect-lead-conversion.js";
import { loadProspectQualificationState } from "./prospect-qualification-mysql.js";
...
const coverageCacheVersions = new WeakMap();
const EMPTY_STATE = {
    tenantProspects: [],
...
function integrityError(message) {
    throw new ProspectCoverageMemoryError("PROSPECT_COVERAGE_DATA_INTEGRITY_VIOLATION", message);
}
function configurationError(message) {
    throw new ProspectCoverageMemoryError("PROSPECT_COVERAGE_CONFIGURATION_INVALID", message);
}
...
function configuredSecrets(required) {
    ...
    if (!configured && !required)
        return null;
    const master = requireSecret("PROSPECT_COVERAGE_MASTER_SECRET 或 "
        + "ORGANIZATION_IDENTITY_MASTER_SECRET", configured);
    return {
        stateSecret: deriveKey(master, "prospect-coverage-state-hmac-v1").toString("base64url"),
        rowIntegrityKey: deriveKey(master, "prospect-coverage-row-integrity-hmac-v1"),
        metadataKey: deriveKey(master, "prospect-coverage-metadata-hmac-v1")
    };
}
...
function rowMac(table, value, secrets) {
...
function metadataMac(value, secrets) {
...
async function coverageFactCount(source) {
    const prospects = await queryRows(source, "SELECT COUNT(*) AS count FROM tenant_prospects");
    const events = await queryRows(source, "SELECT COUNT(*) AS count FROM prospect_coverage_events");
...
async function validateMetadata(source, secrets, lockForUpdate = false) {
    const rows = await queryRows(source, `SELECT * FROM prospect_coverage_contract_metadata
    ...
    if (rows.length !== 1)
        integrityError("覆盖记忆 Metadata 不唯一");
    if (!secrets) {
...
async function initializeMetadata(connection, secrets, createdAt) {
    if (await validateMetadata(connection, secrets, true))
        return;
    const expected = metadataBase(secrets, createdAt);
    await connection.query(`INSERT INTO prospect_coverage_contract_metadata (
...
async function ensureUniqueIndex(pool, table, index, columns) {
    const rows = await queryRows(pool, `SELECT column_name AS columnName, non_unique AS nonUnique
     FROM information_schema.statistics
     WHERE table_schema = DATABASE()
       AND table_name = ? AND index_name = ?
     ORDER BY seq_in_index`, [table, index]);
    const matches = rows.length === columns.length
        && rows.every((row, position) => Number(row.nonUnique) === 0
...
async function ensureOwnerForeignKey(pool, input) {
    const rows = await queryRows(pool, `SELECT column_name AS columnName,
     ...
     WHERE table_schema = DATABASE()
       ...
       AND constraint_name = ?
     ORDER BY ordinal_position`, [input.constraint]);
    const matches = rows.length === input.columns.length
        && rows.every((row, position) => row.columnName === input.columns[position]
            && row.referencedTable === input.referencedTable
            && row.referencedColumn === input.referencedColumns[position]);
    if (matches)
        return;
    if (rows.length) {
        await pool.query(`ALTER TABLE prospect_source_raw_hits
...
async function strengthenRawOwnerSchema(pool) {
    const mismatches = await queryRows(pool, `SELECT COUNT(*) AS count
    ...
    await ensureUniqueIndex(pool, "prospect_source_raw_batches", "uk_ps_raw_batch_team_owner_id", ["team_id", "owner_id", "id"]);
    await ensureUniqueIndex(pool, "prospect_source_raw_hits", "uk_ps_raw_hit_team_owner_id", ["team_id", "owner_id", "id"]);
...
export async function ensureProspectCoverageSchema(pool) {
    ...
    try {
        ...
        if (!acquired)
            configurationError("无法获取覆盖记忆 Schema 锁");
        await strengthenRawOwnerSchema(pool);
        for (const sql of SCHEMA_SQL)
            await connection.query(sql);
        const engines = await queryRows(connection, `SELECT table_name AS tableName, engine AS engineName
       ...
       WHERE table_schema = DATABASE()
        ...
        if (engines.length !== 4
            || engines.some((row) => String(row.engineName || "").toLowerCase() !== "innodb")) {
            configurationError("覆盖记忆 Schema 必须完整使用 InnoDB，当前为 "
                + canonical(engines));
        }
    }
    finally {
...
function parseHashList(value, label) {
    let parsed;
    try {
    ...
    if (!Array.isArray(parsed)
        || parsed.some((item) => typeof item !== "string" || !HASH_PATTERN.test(item))) {
...
function tenantProspectFromRow(row, secrets) {
    const prospect = {
        id: String(row.id),
        teamId: String(row.team_id),
        organizationId: String(row.organization_id),
        status: String(row.lifecycle_status),
        latestClassification: String(row.last_classification),
        ...
        sourceKeyHashes: parseHashList(row.source_key_hashes_json, "来源摘要集合"),
        materialEvidenceKeyHashes: parseHashList(row.material_evidence_key_hashes_json, "实质证据摘要集合"),
        exclusionScope: String(row.exclusion_scope),
        exclusionMode: String(row.exclusion_mode),
...
function coverageEventFromRow(row, secrets) {
    const event = {
        id: String(row.id),
        ...
        queueAction: String(row.queue_action),
        reasonCode: String(row.reason_code),
        processingKeyHash: String(row.processing_key_hash),
        requestHash: String(row.request_hash),
        newEvidenceKeyHashes: parseHashList(row.new_evidence_key_hashes_json, "事件新增证据摘要"),
        newSourceKeyHashes: parseHashList(row.new_source_key_hashes_json, "事件新增来源摘要"),
...
function snapshotHash(type, teamId, prospectId, keys, secret) {
    return hmac(secret, {
...
function validateState(state, secrets) {
    ...
    for (const prospect of state.tenantProspects) {
        ...
        for (let index = 0; index < events.length; index += 1) {
            ...
            for (const value of event.newEvidenceKeyHashes)
                evidence.add(value);
            for (const value of event.newSourceKeyHashes)
                sources.add(value);
            const evidenceKeys = [...evidence].sort();
            const sourceKeys = [...sources].sort();
            if (event.evidenceSnapshotHash !== snapshotHash("evidence", prospect.teamId, prospect.id, evidenceKeys, secrets.stateSecret) || event.sourceSnapshotHash !== snapshotHash("source", prospect.teamId, prospect.id, sourceKeys, secrets.stateSecret)) {
...
export async function loadProspectCoverageState(source, teamId) {
    const secrets = configuredSecrets(false);
    const initialized = await validateMetadata(source, secrets);
    if (!initialized)
        return structuredClone(EMPTY_STATE);
    if (!secrets)
        configurationError("覆盖记忆密钥未初始化");
    const where = teamId ? " WHERE team_id = ?" : "";
    const values = teamId ? [teamId] : [];
    const prospectRows = await queryRows(source, `SELECT * FROM tenant_prospects${where}
     ORDER BY team_id, organization_id, id`, values);
    const eventRows = await queryRows(source, `SELECT * FROM prospect_coverage_events${where}
     ORDER BY team_id, prospect_id, sequence_no`, values);
    const state = {
        tenantProspects: prospectRows.map((row) => tenantProspectFromRow(row, secrets)),
        prospectCoverageEvents: eventRows.map((row) => coverageEventFromRow(row, secrets))
    };
...
async function persistProspectAndEvent(connection, prospect, event, secrets) {
    await connection.query(`INSERT INTO tenant_prospects (
    ...
    await connection.query(`INSERT INTO prospect_coverage_events (
...
function replaceTeamState(store, teamId, state, version) {
    const versions = coverageCacheVersions.get(store)
        || new Map();
    const current = versions.get(teamId) || 0;
    if (version < current)
        return false;
    store.tenantProspects.splice(0, store.tenantProspects.length, ...store.tenantProspects.filter((item) => item.teamId !== teamId), ...structuredClone(state.tenantProspects));
    store.prospectCoverageEvents.splice(0, store.prospectCoverageEvents.length, ...store.prospectCoverageEvents.filter((item) => item.teamId !== teamId), ...structuredClone(state.prospectCoverageEvents));
    versions.set(teamId, version);
    coverageCacheVersions.set(store, versions);
    return true;
}
async function refreshTeamCache(pool, store, teamId) {
    const connection = await pool.getConnection();
    ...
    try {
        ...
        const guards = await queryRows(connection, `SELECT guard_version
       ...
       WHERE team_id = ? FOR UPDATE`, [teamId]);
        ...
        throw new ProspectCoverageMemoryError("PROSPECT_COVERAGE_CACHE_UNAVAILABLE", "覆盖记忆已保存，但当前实例暂时无法刷新团队缓存，请重试");
    }
    finally {
        if (!destroyed)
            connection.release();
...
export async function ensureProspectCoverageTeamCache(pool, store, teamId) {
    let lastError;
...
async function lockCoverageContext(connection, input) {
    const hits = await queryRows(connection, `SELECT * FROM prospect_source_raw_hits
     WHERE team_id = ? AND owner_id = ? AND id = ? FOR UPDATE`, [input.teamId, input.ownerId, input.sourceHitId]);
    if (hits.length !== 1) {
        throw new ProspectCoverageMemoryError("PROSPECT_COVERAGE_INVALID", "Provider 来源命中不存在或不属于当前业务员");
    }
    const hit = rawHitFromRow(hits[0]);
    const rawRows = await queryRows(connection, `SELECT * FROM prospect_source_raw_records
     WHERE team_id = ? AND owner_id = ? AND id = ? FOR UPDATE`, [input.teamId, input.ownerId, hit.recordId]);
    const resolutionRows = await queryRows(connection, `SELECT id FROM organization_identity_resolutions
     WHERE team_id = ? AND owner_id = ? AND id = ? FOR UPDATE`, [input.teamId, input.ownerId, input.resolutionId]);
    const runRows = await queryRows(connection, `SELECT id,team_id,owner_id,campaign_id,strategy_id
     FROM prospect_search_runs
     WHERE team_id = ? AND owner_id = ? AND id = ? FOR UPDATE`, [input.teamId, input.ownerId, hit.runId]);
    const shardRows = await queryRows(connection, `SELECT id,team_id,run_id
     FROM prospect_run_shards
     WHERE team_id = ? AND run_id = ? AND id = ? FOR UPDATE`, [input.teamId, hit.runId, hit.shardId]);
...
function transactionStore(store, coverageState, overrides = {}) {
    return {
        ...store,
        ...overrides,
        mode: "memory",
        tenantProspects: structuredClone(coverageState.tenantProspects),
        prospectCoverageEvents: structuredClone(coverageState.prospectCoverageEvents),
        async persist() {
...
async function lockTeamGuard(connection, teamId, at) {
    await connection.query(`INSERT INTO prospect_coverage_team_guards (
       team_id,guard_version,updated_at
     ) VALUES (?,1,?)
     ON DUPLICATE KEY UPDATE team_id = VALUES(team_id)`, [teamId, new Date(at)]);
    const rows = await queryRows(connection, `SELECT guard_version FROM prospect_coverage_team_guards
     WHERE team_id = ? FOR UPDATE`, [teamId]);
    if (rows.length !== 1)
        integrityError("无法锁定团队覆盖记忆 Guard");
}
function mysqlErrorNumber(error) {
    ...
    const details = error;
    if (details.errno)
        return Number(details.errno);
    if (details.code === "ER_LOCK_DEADLOCK")
        return 1213;
    if (details.code === "ER_LOCK_WAIT_TIMEOUT")
        return 1205;
    return 0;
}
...
export async function recordProspectCoverageMysql(pool, store, rawInput) {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
        ...
        try {
            ...
            const coverageState = await loadProspectCoverageState(connection, input.teamId);
            const identityState = await loadOrganizationIdentityState(connection, undefined, input.teamId);
            ...
            if (!result.idempotent) {
                await persistProspectAndEvent(connection, result.prospect, result.event, secrets);
                await connection.query(`UPDATE prospect_coverage_team_guards
           SET guard_version = guard_version + 1, updated_at = ?
           WHERE team_id = ?`, [new Date(input.coveredAt), input.teamId]);
            }
            commitStarted = true;
            await connection.commit();
            transactionStarted = false;
            ...
            if (commitStarted) {
                ...
                if (attempt < 3)
                    continue;
                throw new ProspectCoverageMemoryError("PROSPECT_COVERAGE_COMMIT_OUTCOME_UNKNOWN", "覆盖记忆事务 COMMIT 结果无法确认");
            }
            if (transactionStarted) {
                try {
                    await connection.rollback();
            ...
            if ([1205, 1213].includes(mysqlErrorNumber(error))) {
                if (attempt < 3)
                    continue;
                throw new ProspectCoverageMemoryError("PROSPECT_COVERAGE_CONCURRENCY_RETRY_EXHAUSTED", "覆盖记忆并发重试次数已耗尽");
            }
            throw error;
        }
        finally {
...
async function crmReferenceRows(connection, input) {
    const load = async (table, id) => {
        if (!id)
            return [];
        return await queryRows(connection, `SELECT id,team_id FROM \`${table}\`
       WHERE team_id = ? AND id = ? FOR UPDATE`, [input.teamId, id]);
    };
...
function conversionLeadFromRow(row) {
    return {
        id: String(row.id),
        ...
        sourceType: String(row.source_type || "outbound"),
        ...
        statusBeforeDelete: row.status_before_delete
            ? String(row.status_before_delete)
            : undefined
    };
}
function conversionSourceEventFromRow(row) {
    return {
        id: String(row.id),
        leadId: String(row.lead_id),
        sourceType: String(row.source_type || "outbound"),
...
function conversionActivityFromRow(row) {
    return {
        id: String(row.id),
        leadId: String(row.lead_id),
...
async function lockProspectQualificationGuard(connection, teamId, at) {
    await connection.query(`INSERT INTO prospect_qualification_team_guards (
       team_id,guard_version,updated_at
     ) VALUES (?,1,?)
     ON DUPLICATE KEY UPDATE team_id = VALUES(team_id)`, [teamId, new Date(at)]);
    const rows = await queryRows(connection, `SELECT guard_version FROM prospect_qualification_team_guards
     WHERE team_id = ? FOR UPDATE`, [teamId]);
    if (rows.length !== 1) {
        throw new ProspectLeadConversionError("PROSPECT_LEAD_CONVERSION_DATA_INTEGRITY", "无法锁定候选资格团队 Guard", 500);
    }
}
async function lockProspectLeadConversionRows(connection, input, ids) {
    const sourceRows = await queryRows(connection, `SELECT * FROM lead_source_events
     WHERE id = ?
        OR (
    ...
    const leadRows = leadIds.length
        ? await queryRows(connection, `SELECT * FROM leads
         WHERE id IN (${leadIds.map(() => "?").join(",")})
         FOR UPDATE`, leadIds)
        : [];
    const activityRows = await queryRows(connection, "SELECT * FROM lead_activities WHERE id = ? FOR UPDATE", [ids.activityId]);
    return {
...
async function insertConvertedLead(connection, lead) {
...
async function insertLeadConversionAudit(connection, sourceEvent, activity) {
    await connection.query(`INSERT INTO lead_source_events (
    ...
    await connection.query(`INSERT INTO lead_activities (
...
function conversionCustomerFromRow(row) {
    return {
        id: String(row.id),
        company: String(row.company || ""),
        country: String(row.country || ""),
...
function conversionCustomerActivityFromRow(row) {
    return {
        id: String(row.id),
        customerId: String(row.customer_id),
...
function conversionCustomerSourceFromRow(row) {
    return {
        id: String(row.id),
        teamId: String(row.team_id),
        ownerId: String(row.owner_id),
        ...
        sourceCampaign: String(row.source_campaign || ""),
        sourceUrl: String(row.source_url || ""),
        mode: String(row.conversion_mode),
        processingKeyHash: String(row.processing_key_hash),
        requestHash: String(row.request_hash),
        createdAt: iso(row.created_at)
    };
}
async function lockProspectCustomerConversionRows(connection, input, ids, organizationId) {
    const sourceRows = await queryRows(connection, `SELECT * FROM lead_source_events
    ...
    const leadRows = await queryRows(connection, "SELECT * FROM leads WHERE id = ? FOR UPDATE", [input.leadId]);
    const acquisitionRows = await queryRows(connection, `SELECT * FROM customer_acquisition_source_events
     WHERE id = ?
        OR (
    ...
    const acquisitionEvents = acquisitionRows.map(conversionCustomerSourceFromRow);
    ...
    const customerRows = customerIds.length
        ? await queryRows(connection, `SELECT * FROM customers
         WHERE id IN (${customerIds.map(() => "?").join(",")})
         FOR UPDATE`, customerIds)
        : [];
    const customerActivityRows = await queryRows(connection, "SELECT * FROM customer_activities WHERE id = ? FOR UPDATE", [ids.customerActivityId]);
    const leadActivityRows = await queryRows(connection, "SELECT * FROM lead_activities WHERE id = ? FOR UPDATE", [ids.leadActivityId]);
    return {
        leads: leadRows.map(conversionLeadFromRow),
        leadSourceEvents: sourceRows.map(conversionSourceEventFromRow),
        customers: customerRows.map(conversionCustomerFromRow),
        customerActivities: customerActivityRows.map(conversionCustomerActivityFromRow),
        customerAcquisitionSourceEvents: acquisitionEvents,
        leadActivities: leadActivityRows.map(conversionActivityFromRow)
    };
}
async function insertConvertedCustomer(connection, customer) {
...
async function insertCustomerConversionAudit(connection, sourceEvent, customerActivity, leadActivity) {
    await connection.query(`INSERT INTO customer_acquisition_source_events (
    ...
    await connection.query(`INSERT INTO customer_activities (
...
async function updateConvertedCustomerLead(connection, lead, expectedOwnerId, expectedTeamId) {
    const [result] = await connection.query(`UPDATE leads
    ...
    if (result.affectedRows !== 1) {
        throw new ProspectCustomerConversionError("PROSPECT_CUSTOMER_CONVERSION_CUSTOMER_BINDING_CONFLICT", "线索客户绑定已发生变化");
    }
}
function upsertConversionCache(target, value) {
...
export async function convertProspectToLeadMysql(pool, store, rawInput) {
    const conversionDate = new Date(rawInput.convertedAt);
    if (!Number.isFinite(conversionDate.getTime())) {
        throw new ProspectLeadConversionError("PROSPECT_LEAD_CONVERSION_INVALID", "转换时间无效", 400);
    }
    const input = {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
        ...
        try {
            ...
            await lockProspectQualificationGuard(connection, input.teamId, input.convertedAt);
            const coverageState = await loadProspectCoverageState(connection, input.teamId);
            const qualificationState = await loadProspectQualificationState(connection, input.teamId);
            const identityState = await loadOrganizationIdentityState(connection, undefined, input.teamId);
            const crmState = await lockProspectLeadConversionRows(connection, input, ids);
            const local = transactionStore(store, coverageState, {
                ...identityState,
                ...qualificationState,
                ...crmState
            ...
            if (!result.replayed) {
                ...
                await insertLeadConversionAudit(connection, result.sourceEvent, result.activity);
                await persistProspectAndEvent(connection, result.prospect, result.coverageEvent, secrets);
                await connection.query(`UPDATE prospect_coverage_team_guards
           SET guard_version = guard_version + 1,updated_at = ?
           WHERE team_id = ?`, [new Date(input.convertedAt), input.teamId]);
            }
            commitStarted = true;
            await connection.commit();
            transactionStarted = false;
            ...
            if (commitStarted) {
                ...
                if (attempt < 3)
                    continue;
                throw new ProspectLeadConversionError("PROSPECT_LEAD_CONVERSION_COMMIT_OUTCOME_UNKNOWN", "候选转线索事务 COMMIT 结果无法确认", 503);
            }
            if (transactionStarted) {
                try {
                    await connection.rollback();
                    ...
                    destroyed = true;
                    connection.destroy();
                    throw new ProspectLeadConversionError("PROSPECT_LEAD_CONVERSION_DATA_INTEGRITY", "候选转线索事务回滚结果无法确认", 500);
                }
            }
            if ([1205, 1213].includes(mysqlErrorNumber(error))) {
                if (attempt < 3)
                    continue;
                throw new ProspectLeadConversionError("PROSPECT_LEAD_CONVERSION_CONCURRENCY_RETRY_EXHAUSTED", "候选转线索并发重试次数已耗尽", 503);
            }
            throw error;
        }
        finally {
    ...
    throw new ProspectLeadConversionError("PROSPECT_LEAD_CONVERSION_DATA_INTEGRITY", "候选转线索事务未返回结果", 500);
}
export async function convertProspectToCustomerMysql(pool, store, rawInput) {
    const conversionDate = new Date(rawInput.convertedAt);
    if (!Number.isFinite(conversionDate.getTime())) {
        throw new ProspectCustomerConversionError("PROSPECT_CUSTOMER_CONVERSION_INVALID", "转换时间无效", 400);
    }
    const input = {
        ...structuredClone(rawInput),
        convertedAt: conversionDate.toISOString()
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
        ...
        try {
            ...
            await lockTeamGuard(connection, input.teamId, input.convertedAt);
            const coverageState = await loadProspectCoverageState(connection, input.teamId);
            const prospect = coverageState.tenantProspects.find((item) => item.id === input.prospectId);
            if (!prospect) {
                throw new ProspectCustomerConversionError("PROSPECT_CUSTOMER_CONVERSION_NOT_FOUND", "候选客户不存在或无权访问", 404);
            }
            const crmState = await lockProspectCustomerConversionRows(connection, input, ids, prospect.organizationId);
            ...
            if (!result.replayed) {
                ...
                await insertCustomerConversionAudit(connection, result.sourceEvent, result.customerActivity, result.leadActivity);
                await updateConvertedCustomerLead(connection, result.lead, input.ownerId, input.teamId);
                await persistProspectAndEvent(connection, result.prospect, result.coverageEvent, secrets);
                await connection.query(`UPDATE prospect_coverage_team_guards
           SET guard_version = guard_version + 1,updated_at = ?
           WHERE team_id = ?`, [new Date(input.convertedAt), input.teamId]);
            ...
            upsertConversionCache(store.customerAcquisitionSourceEvents, result.sourceEvent);
            upsertConversionCache(store.customerActivities, result.customerActivity);
            upsertConversionCache(store.leads, result.lead);
            ...
            if (commitStarted) {
                ...
                if (attempt < 3)
                    continue;
                throw new ProspectCustomerConversionError("PROSPECT_CUSTOMER_CONVERSION_COMMIT_OUTCOME_UNKNOWN", "候选转客户事务 COMMIT 结果无法确认", 503);
            }
            if (transactionStarted) {
                try {
                    await connection.rollback();
                    ...
                    destroyed = true;
                    connection.destroy();
                    throw new ProspectCustomerConversionError("PROSPECT_CUSTOMER_CONVERSION_DATA_INTEGRITY", "候选转客户事务回滚结果无法确认", 500);
                }
            }
            if ([1205, 1213].includes(mysqlErrorNumber(error))) {
                if (attempt < 3)
                    continue;
                throw new ProspectCustomerConversionError("PROSPECT_CUSTOMER_CONVERSION_CONCURRENCY_RETRY_EXHAUSTED", "候选转客户并发重试次数已耗尽", 503);
            }
            throw error;
        }
        finally {
    ...
    throw new ProspectCustomerConversionError("PROSPECT_CUSTOMER_CONVERSION_DATA_INTEGRITY", "候选转客户事务未返回结果", 500);
}
export async function setTenantProspectDispositionMysql(pool, store, rawInput) {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
        ...
        try {
            ...
            if (!result.idempotent) {
                ...
                await connection.query(`UPDATE prospect_coverage_team_guards
           SET guard_version = guard_version + 1, updated_at = ?
           WHERE team_id = ?`, [new Date(input.effectiveAt), input.teamId]);
            }
            commitStarted = true;
            await connection.commit();
            transactionStarted = false;
            ...
            if (commitStarted) {
                ...
                if (attempt < 3)
                    continue;
                throw new ProspectCoverageMemoryError("PROSPECT_COVERAGE_COMMIT_OUTCOME_UNKNOWN", "候选处置事务 COMMIT 结果无法确认");
            }
            if (transactionStarted) {
                try {
                    await connection.rollback();
            ...
            if ([1205, 1213].includes(mysqlErrorNumber(error))) {
                if (attempt < 3)
                    continue;
                throw new ProspectCoverageMemoryError("PROSPECT_COVERAGE_CONCURRENCY_RETRY_EXHAUSTED", "候选处置并发重试次数已耗尽");
            }
            throw error;
        }
        finally {
