...
function pathParameters(path, method) {
    const parameters = [...path.matchAll(/:([A-Za-z0-9_]+)/g)].map((match) => ({
        name: match[1],
        in: "path",
        required: true,
    ...
    if (method !== "get" && (path === "/api/prospect-campaigns/:id"
    ...
    if (method !== "get" && (path === "/api/prospect-schedules/:id/pause"
    ...
    if (method !== "get" && (path === "/api/prospect-strategies/:id"
    ...
    if (method !== "get" && (path === "/api/prospect-runs/:id/pause"
    ...
    if (method === "post"
        ...
        parameters.push({
            ...
            schema: {
                type: "string",
                example: "\"organization-identity-conflict:oic_00000000-0000-4000-8000-000000000000:1\""
            }
        });
    }
            ...
            ? {
                ...
                properties: {
                    ...
                    canonicalOrganizationId: {
                        type: "string",
                        maxLength: 90,
                        description: "action=merge 时必填，且必须是当前冲突内的候选企业；保持独立时必须为空。"
                    },
                    note: {
                        type: "string",
                        minLength: 2,
                    ...
                    ? {
                        ...
                        properties: {
                            ...
                            relationType: {
                                ...
                                description: "方向固定为来源企业指向母公司、最终母公司、所属分支、所属品牌主体或关联企业。"
                            },
                            sourceLabel: { type: "string", minLength: 2, maxLength: 120 },
                            sourceReference: {
                                type: "string",
                                        ...
                                        : method === "post" && path === "/api/prospect-campaigns/:id/strategies"
                                            ...
                                            : method === "patch" && path === "/api/prospect-strategies/:id"
                                                ...
                                                : method === "post" && path === "/api/prospect-strategies/:id/preview"
                                                    ...
                                                    : method === "post" && (path === "/api/prospect-strategies/:id/approve"
                                                        || path === "/api/prospect-strategies/:id/disable")
                                                        ? {
                                                            type: "object",
                                                            properties: {
                                                        ...
                                                        : method === "post" && (path === "/api/prospect-strategies/:id/runs"
                                                            ...
                                                            : method === "post" && (path === "/api/prospect-campaigns/:id/activate"
                                                                || path === "/api/prospect-campaigns/:id/pause"
                                                                || path === "/api/prospect-campaigns/:id/complete"
                                                                || path === "/api/prospect-campaigns/:id/archive")
                                                                ? {
...
function registeredApiRoutes(app) {
    const stack = app._router?.stack || [];
    return stack.flatMap((layer) => {
        const path = layer.route?.path;
        if (typeof path !== "string" || !path.startsWith("/api/") || path.startsWith("/api/docs"))
            return [];
        return Object.entries(layer.route?.methods || {})
            .filter(([, enabled]) => enabled)
            .map(([method]) => method.toLowerCase())
            .filter((method) => method in methodLabels)
...
function responsesFor(method, path) {
    const responses = {
        "200": {
            description: "请求成功",
            content: {
    ...
    const campaignMutation = method !== "get" && (path === "/api/prospect-campaigns/:id"
        || path === "/api/prospect-campaigns/:id/versions"
        || path === "/api/prospect-campaigns/:id/strategies"
        || path === "/api/prospect-campaigns/:id/activate"
        || path === "/api/prospect-campaigns/:id/pause"
        || path === "/api/prospect-campaigns/:id/complete"
        || path === "/api/prospect-campaigns/:id/archive");
    const strategyMutation = method !== "get" && (path === "/api/prospect-strategies/:id"
        || path === "/api/prospect-strategies/:id/approve"
        || path === "/api/prospect-strategies/:id/disable");
    const runMutation = method !== "get" && (path === "/api/prospect-strategies/:id/runs"
        || path === "/api/prospect-runs/:id/pause"
        || path === "/api/prospect-runs/:id/resume"
        || path === "/api/prospect-runs/:id/cancel");
    const identityConflictReviewMutation = method === "post"
        && path === "/api/organization-identity-conflicts/:id/review";
    const organizationFactMutation = method === "post"
        && (path === "/api/organizations/:id/aliases"
            || path === "/api/organization-relations");
    ...
    if (identityConflictReviewMutation) {
        responses["409"] = {
            description: "冲突已经复核、企业已有其他规范映射，或本次合并会形成映射循环"
        };
        responses["412"] = {
...
function requireApiDocsAdmin(req, res, next) {
    requireAuth(req, res, () => {
