...
function eurostatMonth(period) {
    return `${period.slice(0, 4)}-${period.slice(4, 6)}`;
}
function validateQuery(query, baseUrl) {
    if (baseUrl.trim()) {
    ...
    if (sortedPeriods.some((period, index) => !/^\d{6}$/.test(period)
...
function parseResponse(value, query) {
    ...
    for (const [dimension, code] of Object.entries(fixedCodes)) {
        const coordinate = indexes[dimension].get(code);
...
async function fetchTrade(query, baseUrl, fetcher) {
    ...
    if (!response.ok)
        throw providerHttpStatusError(response, "Eurostat Comext");
