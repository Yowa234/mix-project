...
async function fetchWithTimeout(url, init = {}, timeout = DEFAULT_TIMEOUT) {
...
const brave = {
    ...
    async test(cred) {
        ...
        if (response.status === 401 || response.status === 422)
            return { ok: false, message: "Subscription Token 无效" };
        if (!response.ok)
            return { ok: false, message: `连接失败：HTTP ${response.status}` };
        const remaining = response.headers.get("X-RateLimit-Remaining") || "";
...
const gleif = {
    ...
    async search(query, cred) {
        ...
        if (!response.ok)
            throw new Error(`GLEIF HTTP ${response.status}`);
        const data = (await response.json());
...
const opencorporates = {
    ...
    async search(query, cred) {
        ...
        if (!response.ok)
            throw new Error(`OpenCorporates HTTP ${response.status}`);
        const data = (await response.json());
    ...
    async test(cred) {
        ...
        if (response.status === 401 || response.status === 403)
            return { ok: false, message: "API Token 无效或未授权" };
        if (!response.ok)
            return { ok: false, message: `连接失败：HTTP ${response.status}` };
...
const companiesHouse = {
    ...
    async search(query, cred) {
        ...
        if (!response.ok)
            throw new Error(`Companies House HTTP ${response.status}`);
        const data = (await response.json());
...
const peopledatalabs = {
    ...
    async search(query, cred) {
        ...
        if (!response.ok)
            throw new Error(`PDL HTTP ${response.status}`);
        const data = (await response.json());
        const leads = (data.data || []).map((item) => ({
    ...
    async test(cred) {
        ...
        if (response.status === 402)
            return { ok: true, message: "Key 有效，但账户额度受限（HTTP 402）" };
        if (!response.ok)
            return { ok: false, message: `连接失败：HTTP ${response.status}` };
