...
function companyQueryText(query) {
...
function probableOfficialWebsite(rawUrl) {
    try {
...
function webResultToLead(title, link, snippet, query) {
    if (!link)
        return null;
    if (NON_COMPANY_HOSTS.test(link))
        return null;
...
function usage(display = "", requestCount = 1) {
    return {
        requestCount,
        quotaUsed: null,
        quotaRemaining: null,
...
function providerPage(records, options = {}) {
    return {
        records,
...
const serper = defineProvider({
    ...
    async search({ query }, cred, tools) {
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "Serper");
...
const brave = defineProvider({
    ...
    async search({ query }, cred, tools) {
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "Brave");
        const data = (await response.json());
...
const serpapi = defineProvider({
    ...
    async search({ query }, cred, tools) {
        ...
        const response = await tools.http.fetch(url);
        if (!response.ok)
            throw providerHttpStatusError(response, "SerpApi");
        const data = (await response.json());
...
const gleif = defineProvider({
    ...
    async search({ query }, cred, tools) {
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "GLEIF");
        const data = (await response.json());
        const leads = (data.data || []).map((item) => {
...
const wikidata = defineProvider({
    ...
    async search({ query }, cred, tools) {
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "Wikidata");
        const data = (await response.json());
...
const opencorporates = defineProvider({
    ...
    async search({ query }, cred, tools) {
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "OpenCorporates");
        const data = (await response.json());
        const leads = (data.results?.companies || []).map((wrap) => {
...
const companiesHouse = defineProvider({
    ...
    async search({ query }, cred, tools) {
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "Companies House");
        const data = (await response.json());
...
const apollo = defineProvider({
    ...
    async search({ query }, cred, tools) {
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "Apollo");
        const data = (await response.json());
        const orgs = [...(data.organizations || []), ...(data.accounts || [])];
        const leads = orgs.slice(0, query.limit).map((org) => ({
...
const peopledatalabs = defineProvider({
    ...
    async search({ query }, cred, tools) {
        ...
        if (industry)
            must.push({ match: { industry: industry } });
        if (country)
            must.push({ match: { location_country: country.toLowerCase() } });
        const body = {
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "PDL");
        const data = (await response.json());
        const leads = (data.data || []).map((item) => ({
    ...
    async health(cred, tools) {
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "People Data Labs");
...
const hunter = defineProvider({
    ...
    async search({ query }, cred, tools) {
        ...
        if (!company)
            return providerPage([], { requestCount: 0 });
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "Hunter");
        const data = (await response.json());
        const d = data.data;
        if (!d || !d.domain)
            return providerPage([]);
...
const DEFAULT_LEAD_SEARCH_PROVIDER_ID_SET = new Set(DEFAULT_LEAD_SEARCH_PROVIDER_IDS);
