...
function createStore(observations) {
    return {
        ...memoryStore,
...
assert.deepEqual(usa.metrics.reportedImportValueSeries.map((item) => [item.period, item.tradeValueUsd]), [["2022", 100], ["2023", 121], ["2024", 144]]);
...
assert.equal(first.snapshots.some((item) => item.reporterCode === "276" || item.reporterCode === "251"), false);
...
assert.equal(invalidChina.snapshots[0]?.metrics.chinaMainlandSupplyShareReason, "china_mainland_value_exceeds_world");
...
assert.deepEqual(suppressedLatest.snapshots[0]?.metrics.reportedImportValueSeries.map((item) => item.period), ["2021", "2022", "2023"]);
...
assert.deepEqual(renamedReporter.snapshots[0]?.metrics.reportedImportValueSeries.map((item) => item.period), ["2022", "2023", "2024"]);
...
assert.ok(nonContiguous.snapshots[0]?.insufficiencyReasons.includes("non_contiguous_three_year_series"));
assert.ok(nonContiguous.snapshots[0]?.insufficiencyReasons.includes("missing_world_observation:2023"));
