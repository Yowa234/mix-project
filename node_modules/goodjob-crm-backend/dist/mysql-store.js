...
export async function createMysqlStore() {
    ...
    const store = {
        ...
        aiSiteBuilderProjects: await loadAiSiteBuilderProjects(pool),
        aiSiteBuilderSettings: await loadAiSiteBuilderSettings(pool),
...
async function ensureSchema(pool) {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS ai_site_builder_projects (
    id VARCHAR(64) PRIMARY KEY,
    ...
    agent_payload_json LONGTEXT,
    status VARCHAR(40) NOT NULL,
    owner_id VARCHAR(64) NOT NULL,
    team_id VARCHAR(64) NOT NULL,
    created_at DATETIME NOT NULL,
    INDEX idx_ai_site_owner(owner_id),
    INDEX idx_ai_site_team(team_id)
  )`);
    await pool.query(`CREATE TABLE IF NOT EXISTS ai_site_builder_settings (
    ...
    INDEX idx_ai_site_setting_team(team_id)
  )`);
...
async function loadCustomers(pool) {
    return (await rows(pool, "SELECT * FROM customers ORDER BY created_at DESC")).map((row) => ({
...
async function loadAiModelConfigs(pool) {
    return (await rows(pool, "SELECT * FROM ai_model_configs ORDER BY updated_at DESC")).map((row) => ({
        id: row.id,
...
async function loadAiSiteBuilderProjects(pool) {
    return (await rows(pool, "SELECT * FROM ai_site_builder_projects ORDER BY created_at DESC")).map((row) => ({
        id: row.id,
        ...
        schemaData: parseJsonField(row.schema_json, {}),
        agentPayload: parseJsonField(row.agent_payload_json, {}),
        status: row.status,
        ownerId: row.owner_id,
        teamId: row.team_id,
        createdAt: row.created_at instanceof Date ? row.created_at.toISOString() : row.created_at
...
async function loadAiSiteBuilderSettings(pool) {
    return (await rows(pool, "SELECT * FROM ai_site_builder_settings ORDER BY updated_at DESC")).map((row) => ({
        ownerId: row.owner_id,
        teamId: row.team_id,
        provider: row.provider || "openai",
        model: row.model || "",
        ...
        enabled: Boolean(row.enabled),
        lastTestStatus: row.last_test_status || "untested",
        lastTestMessage: row.last_test_message || "未检查",
        updatedAt: row.updated_at instanceof Date ? row.updated_at.toISOString() : row.updated_at
    }));
}
...
async function persistAll(pool, store) {
    ...
    try {
        ...
        await replaceRows(connection, "ai_site_builder_projects", store.aiSiteBuilderProjects, (item) => [item.id, item.taskName, item.siteName, item.industry, item.goal, item.tone, JSON.stringify(item.pages || []), JSON.stringify(item.schemaData || {}), JSON.stringify(item.agentPayload || {}), item.status, item.ownerId, item.teamId, mysqlDate(item.createdAt)], "(id,task_name,site_name,industry,goal,tone,pages_json,schema_json,agent_payload_json,status,owner_id,team_id,created_at)");
        await replaceRows(connection, "ai_site_builder_settings", store.aiSiteBuilderSettings, (item) => [item.ownerId, item.teamId, item.provider, item.model, item.baseUrl, item.apiKey, item.enabled, item.lastTestStatus || "untested", item.lastTestMessage || "", mysqlDate(item.updatedAt)], "(owner_id,team_id,provider,model,base_url,api_key,enabled,last_test_status,last_test_message,updated_at)");
