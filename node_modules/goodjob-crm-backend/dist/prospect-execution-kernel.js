...
import { PROSPECT_RUN_PROVIDER_FETCH_JOB_TYPE } from "./agent-jobs.js";
import { validateProspectRunQueueBridge } from "./prospect-run-queue-bridge.js";
import { canonicalJsonStringify } from "./canonical-json.js";
import { PROSPECT_PROVIDER_REQUEST_LEDGER_CONTRACT, prospectProviderAccountingEvidenceHash, prospectProviderAccountingEvidenceRef, prospectProviderDispatchConfirmationRef, prospectProviderRequestHash, prospectProviderRequestIdempotencyKey, prospectProviderResponseComponentHashes, prospectProviderResponseEvidenceRef, prospectProviderResponseHash, prospectProviderSettlementHash, sha256CanonicalJson } from "./prospect-provider-request-ledger.js";
import { PROSPECT_SOURCE_RAW_SCHEMA_VERSION, ProspectSourceRawError, appendProspectSourceRawBatch, normalizeProspectProviderSourceRecords, prospectProviderRawArtifactHash } from "./prospect-source-raw.js";
import { prospectStrategySourcePositionIdentity, prospectStrategySourcePositionIdentityHash } from "./prospect-strategy-source-position.js";
...
function latestIso(...values) {
    ...
    if (!candidates.length) {
        throw new ProspectExecutionKernelError("EXECUTION_TIME_INVALID", "执行内核缺少可比较的时间");
    }
...
function validIso(value) {
    ...
    if (!Number.isFinite(time)) {
        throw new ProspectExecutionKernelError("EXECUTION_TIME_INVALID", "执行内核时间格式无效");
    }
    return time;
}
function safeProviderOutcomeCode(step) {
    ...
    const code = step.errorCode.trim()
        ...
        .slice(0, 100);
    if (!code) {
        throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_INVALID", "Provider 失败响应缺少安全结果码");
    }
    return code;
}
function validateProviderResponseStep(step) {
    if (!step || typeof step !== "object"
        || (step.kind !== "success" && step.kind !== "failure")) {
        throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_INVALID", "Provider 响应结果结构无效");
    }
    ...
    if (!usage
        ...
        throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_INVALID", "Provider 响应用量或费用结构无效");
    }
    if (step.kind === "success") {
        const counts = [
            step.acceptedCount,
        ...
        if (counts.some((item) => !Number.isInteger(item) || item < 0)
            ...
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_INVALID", "Provider 成功响应计数或游标结构无效");
        }
        ...
        if (hasRawFields) {
            try {
                ...
                const sourceRecords = normalizeProspectProviderSourceRecords(step.sourceRecords, step.rawCount);
                ...
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_INVALID", "Provider 成功响应的原始记录合同或工件摘要无效");
            }
        }
        return;
    }
    if (!step.errorCode.trim()
        ...
        throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_INVALID", "Provider 失败响应错误事实无效");
    }
}
...
function persistedSourcePositionIdentity(position) {
    return {
...
function restore(store, before) {
    store.prospectSearchRuns.splice(0, store.prospectSearchRuns.length, ...before.runs);
    store.prospectRunShards.splice(0, store.prospectRunShards.length, ...before.shards);
    store.prospectRunEvents.splice(0, store.prospectRunEvents.length, ...before.runEvents);
    store.agentJobs.splice(0, store.agentJobs.length, ...before.jobs);
    store.prospectExecutionKernelStates.splice(0, store.prospectExecutionKernelStates.length, ...before.kernelStates);
    store.prospectExecutionCheckpoints.splice(0, store.prospectExecutionCheckpoints.length, ...before.checkpoints);
    store.prospectStrategySourcePositions.splice(0, store.prospectStrategySourcePositions.length, ...before.sourcePositions);
    store.prospectExecutionLeases.splice(0, store.prospectExecutionLeases.length, ...before.leases);
    store.prospectExecutionAttempts.splice(0, store.prospectExecutionAttempts.length, ...before.attempts);
    store.prospectProviderRequestLedgers.splice(0, store.prospectProviderRequestLedgers.length, ...before.providerRequestLedgers);
    store.prospectProviderRequestDispatches.splice(0, store.prospectProviderRequestDispatches.length, ...before.providerRequestDispatches);
    store.prospectProviderRequestEvents.splice(0, store.prospectProviderRequestEvents.length, ...before.providerRequestEvents);
    store.prospectProviderRequestAttemptBindings.splice(0, store.prospectProviderRequestAttemptBindings.length, ...before.providerRequestAttemptBindings);
    store.prospectProviderRequestAccountingEvidence.splice(0, store.prospectProviderRequestAccountingEvidence.length, ...before.providerRequestAccountingEvidence);
    store.prospectSourceRawBatches.splice(0, store.prospectSourceRawBatches.length, ...before.sourceRawBatches);
    store.prospectSourceRawRecords.splice(0, store.prospectSourceRawRecords.length, ...before.sourceRawRecords);
    store.prospectSourceRawHits.splice(0, store.prospectSourceRawHits.length, ...before.sourceRawHits);
    store.prospectExecutionPages.splice(0, store.prospectExecutionPages.length, ...before.pages);
    store.prospectExecutionEvents.splice(0, store.prospectExecutionEvents.length, ...before.events);
    store.prospectExecutionThrottleBuckets.splice(0, store.prospectExecutionThrottleBuckets.length, ...before.throttles);
}
async function persistMutation(store, mutation) {
...
function nextRunEventSequence(store, run) {
...
function appendRunStartedEvent(store, run, previousStatus, previousRevision, now) {
    const event = {
        id: `pre_${randomUUID()}`,
        teamId: run.teamId,
        runId: run.id,
...
function appendRunTransitionEvent(store, input) {
    const event = {
        id: `pre_${randomUUID()}`,
...
function appendExecutionEvent(store, input) {
    const event = {
        id: `pexe_${randomUUID()}`,
        teamId: input.teamId,
...
export class ProspectExecutionKernel {
    ...
    providerRequestEnvelopeKey;
    providerResponseEnvelopeKey;
    ...
    kernelEpoch = 0;
    constructor(options) {
        const allowedRunIds = new Set([...(options.allowedRunIds || [])]
            .map((item) => item.trim())
            .filter(Boolean));
        const allowPersistedRuns = options.allowPersistedRuns === true;
        if (process.env.NODE_ENV !== "test" && !allowPersistedRuns) {
            throw new ProspectExecutionKernelError("EXECUTION_PERSISTED_RUN_SCOPE_REQUIRED", "生产执行内核必须显式启用持久化运行作用域");
        }
        if (!allowPersistedRuns && !allowedRunIds.size) {
            throw new ProspectExecutionKernelError("EXECUTION_ALLOWED_RUNS_REQUIRED", "测试执行内核必须显式提供非空 allowedRunIds");
        }
        if (Buffer.byteLength(options.claimSecret, "utf8") < 32) {
            throw new ProspectExecutionKernelError("EXECUTION_CLAIM_SECRET_WEAK", "测试执行内核 claimSecret 至少需要 32 字节");
        }
        const cursorSecret = options.cursorSecret || options.claimSecret;
        if (Buffer.byteLength(cursorSecret, "utf8") < 32) {
            throw new ProspectExecutionKernelError("EXECUTION_CURSOR_SECRET_WEAK", "测试执行内核 cursorSecret 至少需要 32 字节");
        }
        const providerRequestIdempotencySecret = options.providerRequestIdempotencySecret
        ...
        if (Buffer.byteLength(providerRequestIdempotencySecret, "utf8") < 32) {
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_IDEMPOTENCY_SECRET_WEAK", "测试执行内核 Provider 请求幂等密钥至少需要 32 字节");
        }
        const providerRequestEnvelopeSecret = options.providerRequestEnvelopeSecret
            || createHmac("sha256", options.claimSecret)
                .update("goodjob-provider-request-envelope-v1")
                .digest("hex");
        if (Buffer.byteLength(providerRequestEnvelopeSecret, "utf8") < 32) {
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_ENVELOPE_SECRET_WEAK", "测试执行内核 Provider 请求信封密钥至少需要 32 字节");
        }
        const providerResponseEnvelopeSecret = options.providerResponseEnvelopeSecret
            || createHmac("sha256", options.claimSecret)
                .update("goodjob-provider-response-envelope-v1")
                .digest("hex");
        if (Buffer.byteLength(providerResponseEnvelopeSecret, "utf8") < 32) {
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_ENVELOPE_SECRET_WEAK", "测试执行内核 Provider 响应信封密钥至少需要 32 字节");
        }
        ...
        if (Buffer.byteLength(providerRawEnvelopeSecret, "utf8") < 32) {
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RAW_ENVELOPE_SECRET_WEAK", "测试执行内核 Provider 原始记录加密密钥至少需要 32 字节");
        }
        const providerRawIdentitySecret = options.providerRawIdentitySecret
            || createHmac("sha256", options.claimSecret)
                .update("goodjob-provider-raw-identity-v1")
                .digest("hex");
        if (Buffer.byteLength(providerRawIdentitySecret, "utf8") < 32) {
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RAW_IDENTITY_SECRET_WEAK", "测试执行内核 Provider 原始记录身份密钥至少需要 32 字节");
        }
        const providerRawPolicies = new Map();
        for (const [rawProviderCode, rawPolicy] of Object.entries(options.providerRawPolicies || {})) {
            ...
            if (!providerCode
                ...
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RAW_POLICY_INVALID", "测试执行内核 Provider 原始记录许可或保留策略无效");
            }
        ...
        if (!options.workerId.trim()) {
            throw new ProspectExecutionKernelError("EXECUTION_WORKER_ID_REQUIRED", "测试执行内核必须提供 workerId");
        }
        ...
        this.leaseMs = Math.max(1_000, Math.trunc(options.leaseMs ?? DEFAULT_LEASE_MS));
        this.deadlineMs = Math.max(this.leaseMs, Math.trunc(options.deadlineMs ?? DEFAULT_DEADLINE_MS));
        this.throttleIntervalMs = Math.max(0, Math.trunc(options.throttleIntervalMs ?? 0));
    }
    isRunAllowed(runId) {
        if (this.allowedRunIds.has(runId))
            return true;
        if (!this.allowPersistedRuns)
            return false;
        const run = this.store.prospectSearchRuns.find((item) => item.id === runId);
        if (!run || run.queueBridgeVersion !== "v1")
            return false;
        const binding = this.store.prospectRunQueueParentBindings.find((item) => item.runId === run.id
            && item.teamId === run.teamId
            && item.ownerId === run.ownerId
            && item.bridgeVersion === "v1");
        return Boolean(binding && this.store.agentJobs.some((item) => item.id === binding.jobId
            && item.teamId === run.teamId
            && item.ownerId === run.ownerId
            && item.jobType === "prospect.orchestrate"));
    }
    providerConnectionId(input) {
        if (!this.allowPersistedRuns) {
            return `fake:${input.providerCode}`;
        }
        return this.store.providerConnections.find((item) => item.providerId === input.providerCode
            && item.teamId === input.teamId
            && item.ownerId === input.ownerId
            && item.scope === "personal"
            && item.status === "active")?.id || `builtin:${input.providerCode}`;
    }
    isRawCapableProviderSuccess(step) {
        return step.kind === "success"
            && step.responseSchemaVersion === PROSPECT_SOURCE_RAW_SCHEMA_VERSION
            && typeof step.rawArtifactHash === "string"
            && Array.isArray(step.sourceRecords);
    }
    appendSettledProviderRawFacts(input) {
        ...
        if (!policy) {
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RAW_POLICY_REQUIRED", "Provider 原始记录未配置许可与保留策略，不能完成成功结算");
        }
        const existingBatches = this.store.prospectSourceRawBatches.filter((item) => item.ledgerId === ledger.id
            && item.teamId === ledger.teamId
            && item.ownerId === ledger.ownerId);
        if (input.requireExisting && existingBatches.length !== 1) {
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RAW_STATE_INVALID", "已成功结算的 Provider 响应缺少唯一原始批次");
        }
        if (ledger.status !== "settled"
            ...
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RAW_STATE_INVALID", "Provider 原始记录与成功结算、执行页或响应数量不一致");
        }
        try {
            ...
            if (error instanceof ProspectSourceRawError) {
                throw new ProspectExecutionKernelError(error.code === "PROSPECT_SOURCE_RAW_CONFLICT"
                    ? "EXECUTION_PROVIDER_RAW_CONFLICT"
                    : "EXECUTION_PROVIDER_RAW_INVALID", error.message);
            }
            throw error;
        }
    }
    providerRequestEnvelopeAad(ledger) {
    ...
    encryptProviderRequestEnvelope(ledger, value) {
        const iv = randomBytes(12);
        const cipher = createCipheriv("aes-256-gcm", this.providerRequestEnvelopeKey, iv);
    ...
    decryptProviderRequestEnvelope(ledger) {
        const [version, ivText, tagText, payloadText] = ledger.encryptedRequestEnvelope.split(".");
        if (version !== "provider-request-v1"
            || !ivText
            || !tagText
            || !payloadText) {
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_ENVELOPE_INVALID", "Provider 请求信封格式无效");
        }
        try {
            ...
            const decipher = createDecipheriv("aes-256-gcm", this.providerRequestEnvelopeKey, iv, { authTagLength: 16 });
            ...
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_ENVELOPE_INVALID", "Provider 请求信封完整性校验失败");
        }
    }
    providerResponseEnvelopeAad(ledger, dispatch) {
        return Buffer.from(canonicalJsonStringify({
            contract: PROSPECT_PROVIDER_REQUEST_LEDGER_CONTRACT,
    ...
    encryptProviderResponseEnvelope(ledger, dispatch, value) {
        const iv = randomBytes(12);
        const cipher = createCipheriv("aes-256-gcm", this.providerResponseEnvelopeKey, iv);
    ...
    decryptProviderResponseEnvelope(ledger, dispatch) {
        const [version, ivText, tagText, payloadText] = ledger.encryptedResponseEnvelope.split(".");
        if (version !== "provider-response-v1"
            || !ivText
            || !tagText
            || !payloadText) {
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_ENVELOPE_INVALID", "Provider 响应信封格式无效");
        }
        try {
            const iv = Buffer.from(ivText, "base64url");
            const tag = Buffer.from(tagText, "base64url");
            ...
            const decipher = createDecipheriv("aes-256-gcm", this.providerResponseEnvelopeKey, iv, { authTagLength: 16 });
            ...
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_ENVELOPE_INVALID", "Provider 响应信封完整性校验失败");
        }
    }
    settlePreparedProviderRequestsBeforeDispatch(run, now) {
        for (const ledger of this.store.prospectProviderRequestLedgers) {
            ...
            const bindings = this.store.prospectProviderRequestAttemptBindings
                .filter((item) => item.teamId === ledger.teamId
                && item.ownerId === ledger.ownerId
    ...
    transitionProviderDispatchOutcomeUnknown(input) {
        const dispatches = this.store.prospectProviderRequestDispatches.filter((item) => item.teamId === input.lease.teamId
        ...
        if (dispatches.length !== 1) {
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_DISPATCH_CONFLICT", "同一执行尝试存在多个未结算的 Provider 派发");
        }
        const dispatch = dispatches[0];
        const ledger = this.store.prospectProviderRequestLedgers.find((item) => item.id === dispatch.ledgerId
        ...
        const binding = this.store.prospectProviderRequestAttemptBindings.find((item) => item.teamId === dispatch.teamId
            && item.ownerId === dispatch.ownerId
        ...
        if (!ledger || !binding
            ...
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_DISPATCH_SCOPE_INVALID", "Provider 派发记录与请求账本或执行尝试不一致");
        }
        const previousStatus = ledger.status;
    ...
    terminateProviderRequestAsOutcomeUnknown(input) {
        const lease = this.store.prospectExecutionLeases.find((item) => item.id === input.leaseId
            && this.isRunAllowed(item.runId));
        if (!lease) {
            throw new ProspectExecutionKernelError("EXECUTION_LEASE_NOT_ACTIVE", "执行租约不存在、已失效或不在白名单内");
        }
        this.assertClaimToken(lease, input.claimToken);
        if (lease.status !== "active") {
            const attempt = this.store.prospectExecutionAttempts.find((item) => item.leaseId === lease.id
                && item.teamId === lease.teamId
                && item.jobId === lease.jobId);
            const dispatch = this.store.prospectProviderRequestDispatches.find((item) => item.teamId === lease.teamId
            ...
            const ledger = dispatch
                ? this.store.prospectProviderRequestLedgers.find((item) => item.id === dispatch.ledgerId
                    && item.teamId === dispatch.teamId
                    && item.ownerId === dispatch.ownerId
                    && item.status === "outcome_unknown")
                : null;
            ...
            throw new ProspectExecutionKernelError("EXECUTION_LEASE_NOT_ACTIVE", "执行租约已经失效且不存在可复用的结果未知事实");
        }
        const run = this.store.prospectSearchRuns.find((item) => item.id === lease.runId
            && item.teamId === lease.teamId
            && item.ownerId === lease.ownerId);
        const shard = this.store.prospectRunShards.find((item) => item.id === lease.shardId
            && item.runId === lease.runId
            && item.teamId === lease.teamId);
        const job = this.store.agentJobs.find((item) => item.id === lease.jobId
            && item.teamId === lease.teamId
            && item.ownerId === lease.ownerId
        ...
        const attempt = this.store.prospectExecutionAttempts.find((item) => item.leaseId === lease.id
            && item.teamId === lease.teamId
            && item.jobId === lease.jobId);
        const checkpoint = this.store.prospectExecutionCheckpoints.find((item) => item.teamId === lease.teamId
            && item.runId === lease.runId
            && item.shardId === lease.shardId
            && item.jobId === lease.jobId);
        if (!run || !shard || !job || !attempt || !checkpoint
            ...
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_DISPATCH_SCOPE_INVALID", "Provider 派发中断时的运行、分片或尝试状态无效");
        }
        if (!this.transitionProviderDispatchOutcomeUnknown({
            ...
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_DISPATCH_MISSING", "Provider 派发中断时缺少持久化派发事实");
        }
    ...
    claimTokenHmac(lease, claimToken) {
    ...
    assertClaimToken(lease, claimToken) {
        ...
        const actual = Buffer.from(this.claimTokenHmac(lease, claimToken), "hex");
        if (expected.length !== actual.length
            || !timingSafeEqual(expected, actual)) {
            throw new ProspectExecutionKernelError("EXECUTION_CLAIM_TOKEN_INVALID", "执行租约令牌无效");
        }
    }
    cursorAad(input) {
        return Buffer.from([
    ...
    decryptCursor(encrypted, expectedHash, input) {
        if (!encrypted) {
            if (expectedHash) {
                throw new ProspectExecutionKernelError("EXECUTION_CURSOR_INVALID", "执行游标密文与摘要状态不一致");
            }
            return "";
        }
        const parts = encrypted.split(".");
        if (parts.length !== 4) {
            throw new ProspectExecutionKernelError("EXECUTION_CURSOR_INVALID", "执行游标密文格式无效");
        }
        ...
        if (version !== "v1"
            ...
            throw new ProspectExecutionKernelError("EXECUTION_CURSOR_INVALID", "执行游标密文或摘要格式无效");
        }
        try {
            ...
            const decipher = createDecipheriv("aes-256-gcm", this.cursorKey, iv, { authTagLength: 16 });
            ...
            throw new ProspectExecutionKernelError("EXECUTION_CURSOR_INVALID", "执行游标完整性校验失败");
        }
    }
    sourcePositionAad(position) {
        return Buffer.from([
    ...
    encryptSourcePositionCursor(cursor, position) {
        if (!cursor)
            return { encrypted: "", hash: "" };
        const iv = randomBytes(12);
        const cipher = createCipheriv("aes-256-gcm", this.cursorKey, iv);
    ...
    decryptSourcePositionCursor(position) {
        if (position.status !== "continuable"
            || !position.encryptedCursor
            || !/^[a-f0-9]{64}$/.test(position.cursorHash)) {
            throw new ProspectExecutionKernelError("EXECUTION_SOURCE_POSITION_INVALID", "获客数据源续搜位置状态无效，已停止自动从头搜索");
        }
        const parts = position.encryptedCursor.split(".");
        if (parts.length !== 4 || parts[0] !== "v1") {
            throw new ProspectExecutionKernelError("EXECUTION_SOURCE_POSITION_INVALID", "获客数据源续搜位置密文格式无效，已停止自动从头搜索");
        }
        try {
            const decodeSegment = (text) => {
                if (!/^[A-Za-z0-9_-]+$/.test(text)) {
            ...
            throw new ProspectExecutionKernelError("EXECUTION_SOURCE_POSITION_INVALID", "获客数据源续搜位置完整性校验失败，已停止自动从头搜索");
        }
    }
    sourcePositionScope(input) {
        const identity = prospectStrategySourcePositionIdentity({
            ...input,
            endpointCode: PROVIDER_ENDPOINT_CODE
        });
        const identityHash = prospectStrategySourcePositionIdentityHash(identity);
        const matches = this.store.prospectStrategySourcePositions.filter((item) => item.teamId === identity.teamId
            && item.ownerId === identity.ownerId
            && item.identityHash === identityHash);
        if (matches.length > 1) {
            throw new ProspectExecutionKernelError("EXECUTION_SOURCE_POSITION_CONFLICT", "获客数据源续搜位置存在重复作用域");
        }
        const position = matches[0];
        if (position
            && !isDeepStrictEqual(persistedSourcePositionIdentity(position), identity)) {
            throw new ProspectExecutionKernelError("EXECUTION_SOURCE_POSITION_CONFLICT", "获客数据源续搜位置身份摘要发生冲突");
        }
    ...
    upsertSourcePosition(input) {
        const scope = this.sourcePositionScope(input);
        if (input.onlyIfMissing && scope.position)
        ...
        if (input.hasMore && !input.cursor) {
            throw new ProspectExecutionKernelError("EXECUTION_SOURCE_POSITION_INVALID", "Provider 声明仍有后续数据但未返回续搜位置");
        }
        ...
        const encryptedCursor = this.encryptSourcePositionCursor(input.hasMore ? input.cursor : "", position);
    ...
    activeLease(input) {
        ...
        const checkpoint = this.store.prospectExecutionCheckpoints.find((item) => item.teamId === lease.teamId
            ...
            && item.jobId === lease.jobId);
        if (!run || !shard || !job || !attempt || !checkpoint) {
            throw new ProspectExecutionKernelError("EXECUTION_LEASE_SCOPE_INVALID", "执行租约引用了不存在或跨团队的执行事实");
        }
        ...
        if (lease.kernelEpoch !== this.kernelEpoch
            ...
            throw new ProspectExecutionKernelError("EXECUTION_LEASE_FENCE_INVALID", "执行租约 epoch、fence 或有效期校验失败");
        }
        return { lease, run, shard, job, attempt, checkpoint, cancelledEpoch };
    }
    assertStarted() {
        if (this.kernelEpoch < 1) {
            throw new ProspectExecutionKernelError("EXECUTION_KERNEL_NOT_STARTED", "执行内核尚未启动");
        }
    }
    verifyAllowedRunsExist() {
        for (const runId of this.allowedRunIds) {
            if (!this.store.prospectSearchRuns.some((item) => item.id === runId)) {
                throw new ProspectExecutionKernelError("EXECUTION_ALLOWED_RUN_NOT_FOUND", `allowedRunIds 包含不存在的搜索运行：${runId}`);
            }
        }
    }
    parentJob(run) {
        const binding = this.store.prospectRunQueueParentBindings.find((item) => item.teamId === run.teamId && item.runId === run.id);
        const job = binding
            ? this.store.agentJobs.find((item) => item.id === binding.jobId
                && item.teamId === binding.teamId
                && item.ownerId === binding.ownerId)
            : null;
        if (!binding || !job) {
            throw new ProspectExecutionKernelError("EXECUTION_PARENT_JOB_MISSING", "搜索运行缺少父聚合任务");
        }
        return job;
    }
    childJob(run, shard) {
        const binding = this.store.prospectRunQueueChildBindings.find((item) => item.teamId === run.teamId
            && item.runId === run.id
        ...
        const job = binding
            ...
            : null;
        if (!binding || !job) {
            throw new ProspectExecutionKernelError("EXECUTION_CHILD_JOB_MISSING", "搜索运行分片缺少子执行任务");
        }
        return job;
    }
    releaseLease(lease, now, reason) {
        lease.status = "released";
        lease.releasedAt = now;
        lease.releaseReason = reason;
        lease.version += 1;
    }
    settlePauseIfReady(run, now) {
        if (run.status !== "pause_requested"
            || this.store.prospectExecutionLeases.some((item) => item.teamId === run.teamId
                && item.runId === run.id
                && item.status === "active")) {
            return false;
        }
        const shards = this.store.prospectRunShards.filter((item) => item.teamId === run.teamId && item.runId === run.id);
    ...
    finishRunIfTerminal(run, now) {
        ...
        if (run.status === "cancel_requested"
            ...
            const failedCount = shards.filter((item) => item.status === "failed").length;
            const partialCount = shards.filter((item) => item.status === "partial_success").length;
            const completedCount = shards.filter((item) => item.status === "succeeded"
                || item.status === "succeeded_empty"
                || item.status === "partial_success").length;
    ...
    finishShardAfterPersistedResponse(input) {
        input.shard.status = input.status;
        input.shard.updatedAt = input.now;
        input.job.status = input.status === "failed"
            ? (input.job.attemptCount >= input.job.maxAttempts
    ...
    fallbackRetryAt(input) {
        const exponential = Math.min(60_000, 1_000 * (2 ** Math.max(0, input.checkpointCallNo - 1)));
    ...
    async start(now = new Date().toISOString()) {
        ...
        return persistMutation(this.store, () => {
            ...
            const previousEpoch = this.store.prospectExecutionKernelStates.reduce((highest, state) => Math.max(highest, state.kernelEpoch), 0);
            ...
            this.store.prospectExecutionKernelStates.splice(0, this.store.prospectExecutionKernelStates.length, state);
    ...
    recoverLeasesInMutation(now, previousKernelOnly) {
        ...
        for (const lease of this.store.prospectExecutionLeases) {
            ...
            const job = this.store.agentJobs.find((item) => item.id === lease.jobId
                ...
                && item.jobType === PROSPECT_RUN_PROVIDER_FETCH_JOB_TYPE);
            if (!run || !shard || !job) {
                throw new ProspectExecutionKernelError("EXECUTION_LEASE_SCOPE_INVALID", "执行租约引用了不存在或跨团队的运行、分片或任务");
            }
            const attempt = this.store.prospectExecutionAttempts.find((item) => item.leaseId === lease.id
                && item.teamId === lease.teamId
                && item.jobId === lease.jobId);
            if (!attempt) {
                throw new ProspectExecutionKernelError("EXECUTION_ATTEMPT_MISSING", "执行租约缺少对应尝试事实");
            }
            if (lease.requestStartedAt) {
                const responseDispatch = this.store.prospectProviderRequestDispatches.find((item) => item.teamId === lease.teamId
                    && item.ownerId === lease.ownerId
                    && item.runId === lease.runId
                    && item.shardId === lease.shardId
                    && item.attemptId === attempt.id
                ...
                const responseLedger = responseDispatch
                    ? this.store.prospectProviderRequestLedgers.find((item) => item.id === responseDispatch.ledgerId
    ...
    async claimNext(now = new Date().toISOString()) {
        ...
        return persistMutation(this.store, () => {
            ...
            const candidates = this.store.prospectRunQueueChildBindings
                .map((binding) => {
                const run = this.store.prospectSearchRuns.find((item) => item.id === binding.runId
                    && item.teamId === binding.teamId
                    && item.ownerId === binding.ownerId);
                const shard = this.store.prospectRunShards.find((item) => item.id === binding.shardId
                    && item.runId === binding.runId
                    && item.teamId === binding.teamId);
                const job = this.store.agentJobs.find((item) => item.id === binding.jobId
                    && item.teamId === binding.teamId
                    && item.ownerId === binding.ownerId
                    && item.jobType === PROSPECT_RUN_PROVIDER_FETCH_JOB_TYPE);
                ...
                .filter(({ run, shard, job }) => this.isRunAllowed(run.id)
                ...
                && !this.store.prospectExecutionLeases.some((lease) => lease.runId === run.id && lease.status === "active"))
            ...
            for (const selected of candidates) {
                ...
                let checkpoint = this.store.prospectExecutionCheckpoints.find((item) => item.teamId === selected.run.teamId
                ...
                if (selected.run.status === "queued") {
                    ...
                    appendRunStartedEvent(this.store, selected.run, previousStatus, previousRevision, now);
                }
                ...
                const fenceToken = this.store.prospectExecutionLeases.reduce((highest, lease) => lease.teamId === selected.run.teamId
    ...
    async prepareProviderRequest(input) {
        ...
        return persistMutation(this.store, () => {
            ...
            if (scope.run.status === "pause_requested"
                ...
                throw new ProspectExecutionKernelError("EXECUTION_REQUEST_BLOCKED_BY_PAUSE", "暂停请求已生效，不能准备新的 Provider 请求");
            }
            if (scope.attempt.status !== "claimed"
                || scope.lease.requestStartedAt) {
                throw new ProspectExecutionKernelError("EXECUTION_REQUEST_ALREADY_STARTED", "当前租约已经开始或完成 Provider 请求");
            }
            if (scope.checkpoint.checkpointCallCount >= 3) {
                throw new ProspectExecutionKernelError("EXECUTION_CHECKPOINT_ATTEMPTS_EXHAUSTED", "当前 checkpoint 的三次总调用已耗尽");
            }
            const providerSnapshot = scope.run.executionSnapshot.providerPlan.find((item) => item.providerCode === scope.shard.providerCode);
            if (!providerSnapshot
                ...
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_SNAPSHOT_INVALID", "Provider 执行快照与分片配置不一致");
            }
            const cursor = this.decryptCursor(scope.checkpoint.encryptedCursor, scope.checkpoint.cursorHash, {
            ...
            const checkpointCallNo = scope.checkpoint.checkpointCallCount + 1;
            const connectionId = this.providerConnectionId({
                teamId: scope.run.teamId,
            ...
            const providerPayload = {
                ...
                limits: {
                    ...
                    remainingResultLimit: Math.max(0, scope.shard.resultLimit - scope.checkpoint.acceptedCount)
                }
            };
            ...
            const existing = this.store.prospectProviderRequestLedgers.find((item) => item.teamId === scope.run.teamId
            ...
            if (existing) {
                ...
                if (existing.status !== "prepared"
                    ...
                    || !isDeepStrictEqual(envelope.dispatchRequest, dispatchRequest)
                    || !isDeepStrictEqual(envelope.providerRequest, providerRequest)) {
                    throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_PREPARED_REQUEST_CONFLICT", "已准备的 Provider 请求与当前逻辑请求不一致");
                }
                const existingBinding = this.store.prospectProviderRequestAttemptBindings.find((item) => item.teamId === existing.teamId
                    && item.ownerId === existing.ownerId
                    && item.ledgerId === existing.id
                    && item.attemptId === scope.attempt.id);
                if (!existingBinding) {
                    const bindingNo = this.store.prospectProviderRequestAttemptBindings.reduce((highest, item) => item.teamId === existing.teamId
                        && item.ledgerId === existing.id
    ...
    async startPreparedProviderDispatch(input) {
        ...
        return persistMutation(this.store, () => {
            ...
            if (scope.run.status === "pause_requested"
                || scope.shard.status === "pause_requested") {
                throw new ProspectExecutionKernelError("EXECUTION_REQUEST_BLOCKED_BY_PAUSE", "暂停请求已生效，不能派发 Provider 请求");
            }
            if (scope.attempt.status !== "claimed"
                || scope.lease.requestStartedAt) {
                throw new ProspectExecutionKernelError("EXECUTION_REQUEST_ALREADY_STARTED", "当前租约已经开始或完成 Provider 请求");
            ...
            const ledger = this.store.prospectProviderRequestLedgers.find((item) => item.id === input.ledgerId
                && item.teamId === scope.run.teamId
                && item.ownerId === scope.run.ownerId
                && item.runId === scope.run.id
                && item.shardId === scope.shard.id
                && item.jobId === scope.job.id);
            const binding = this.store.prospectProviderRequestAttemptBindings.find((item) => item.teamId === scope.run.teamId
                && item.ownerId === scope.run.ownerId
                && item.ledgerId === input.ledgerId
                && item.attemptId === scope.attempt.id);
            if (!ledger || !binding || ledger.status !== "prepared") {
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_PREPARED_REQUEST_INVALID", "Provider 请求账本不存在、未绑定当前尝试或已离开准备状态");
            }
            if (validIso(ledger.preparedAt) > validIso(now)
                ...
                || ledger.requestSchemaVersion
                    !== PROVIDER_REQUEST_SCHEMA_VERSION) {
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_PREPARED_REQUEST_CONFLICT", "Provider 请求账本与当前 checkpoint 或执行快照不一致");
            }
            if (this.store.prospectProviderRequestDispatches.some((item) => item.teamId === ledger.teamId && item.ledgerId === ledger.id)) {
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_DISPATCH_ALREADY_EXISTS", "当前 Provider 请求已经存在派发事实");
            }
            const providerSnapshot = scope.run.executionSnapshot.providerPlan.find((item) => item.providerCode === scope.shard.providerCode);
            if (!providerSnapshot
                ...
                || providerSnapshot.pageLimit !== scope.shard.pageLimit
                || providerSnapshot.resultLimit !== scope.shard.resultLimit) {
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_SNAPSHOT_INVALID", "Provider 执行快照与已准备请求不一致");
            }
            ...
            const expectedIdempotencyKey = prospectProviderRequestIdempotencyKey({
                teamId: ledger.teamId,
                ownerId: ledger.ownerId,
                connectionId: ledger.connectionId,
                endpointCode: ledger.endpointCode,
            ...
            if (ledger.connectionConfigHash !== expectedConnectionConfigHash
                ...
                || !isDeepStrictEqual(envelope.dispatchRequest, expectedDispatchRequest)
                || !isDeepStrictEqual(envelope.providerRequest, expectedProviderRequest)) {
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_PREPARED_REQUEST_CONFLICT", "Provider 请求信封与持久化身份、游标或哈希不一致");
            }
            ...
            let throttle = this.store.prospectExecutionThrottleBuckets.find((item) => item.id === throttleId
                && item.teamId === scope.run.teamId
            ...
            throttle.availableAt = plusMilliseconds(now, this.throttleIntervalMs);
            throttle.version += 1;
            ...
            const dispatchNo = this.store.prospectProviderRequestDispatches.reduce((highest, item) => item.teamId === ledger.teamId
                && item.ledgerId === ledger.id
    ...
    providerResponseFacts(dispatchRequest, response) {
        ...
        if (!response.externalRequestId.trim()
            ...
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_INVALID", "Provider 响应缺少有效外部请求编号或 HTTP 状态");
        }
        const componentHashes = prospectProviderResponseComponentHashes(response.step);
        ...
        if (response.rawResponseHash !== componentHashes.rawResponseHash
            ...
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_INVALID", "Provider 响应摘要与服务端复算结果不一致");
        }
        return {
            ...componentHashes,
            responseHash,
    ...
    verifiedProviderResponse(ledger, dispatch) {
        ...
        const expectedConfirmationRef = prospectProviderDispatchConfirmationRef({
            ledgerId: ledger.id,
            dispatchId: dispatch.id,
            externalRequestId: ledger.externalRequestId,
            responseHash: ledger.responseHash
        ...
        if (envelope.providerOutcomeCode !== facts.providerOutcomeCode
            ...
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_CONFLICT", "Provider 响应证据与持久化摘要不一致");
        }
        return response;
    }
    persistedProviderResponse(input) {
        this.assertStarted();
        const lease = this.store.prospectExecutionLeases.find((item) => item.id === input.leaseId
            && this.isRunAllowed(item.runId));
        if (!lease)
        ...
        const dispatch = attempt
            ? this.store.prospectProviderRequestDispatches.find((item) => item.ledgerId === ledger.id
                && item.teamId === ledger.teamId
                && item.ownerId === ledger.ownerId
                && item.attemptId === attempt.id
                && item.status === "response_received")
            : null;
        const leaseReleaseMatchesAttempt = attempt?.status === "request_started"
        ...
        if (!attempt
            ...
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_SCOPE_INVALID", "Provider 响应与租约、账本或派发作用域不一致");
        }
    ...
    async persistPreparedProviderResponse(input) {
        ...
        return persistMutation(this.store, () => {
            ...
            if (!attempt || !ledger || !dispatch
                || dispatch.requestHash !== ledger.requestHash
                || dispatch.idempotencyKey !== ledger.idempotencyKey) {
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_SCOPE_INVALID", "Provider 响应与租约、账本、尝试或派发作用域不一致");
            }
            const dispatchRequest = {
                teamId: ledger.teamId,
                ownerId: ledger.ownerId,
            ...
            const facts = this.providerResponseFacts(dispatchRequest, input.response);
            if (ledger.status === "response_received") {
                ...
                if (!persisted
                    ...
                    throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_CONFLICT", "同一 Provider 请求不能绑定不同响应");
                }
                return {
                    value: persisted,
                    rollback: () => restore(this.store, before)
            ...
            const recoveredUnknownResponseScope = ["released", "expired"].includes(lease.status)
                && attempt.status === "request_outcome_unknown"
                && ledger.status === "outcome_unknown"
                && dispatch.status === "outcome_unknown"
                && [
            ...
            if ((!activeResponseScope && !recoveredUnknownResponseScope)
                ...
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_SCOPE_INVALID", "Provider 响应到达时执行状态已经失效或时间发生回退");
            }
            ...
            const responseObservationFloor = latestIso(now, dispatch.startedAt, dispatch.finishedAt, ledger.updatedAt, ledger.unknownAt, lease.releasedAt);
            ...
            const responseReceivedAt = plusMilliseconds(latestIso(responseObservationFloor, confirmationAt), 1);
            if (needsDispatchConfirmation) {
                ...
                || dispatch.externalRequestId
                    !== input.response.externalRequestId) {
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RESPONSE_CONFLICT", "Provider 响应与已确认派发事实不一致");
            }
            ...
            if (lease.status === "active") {
                this.releaseLease(lease, responseReceivedAt, "RESPONSE_RECEIVED_PENDING_SETTLEMENT");
            }
            return {
                value: {
                    kind: "response_received",
    ...
    async dispatchPreparedProviderRequest(provider, input) {
        const now = input.now || new Date().toISOString();
        const persisted = this.persistedProviderResponse(input);
        if (persisted)
            return persisted;
        ...
        let response;
        try {
            response = await provider.dispatch(started.dispatchRequest, started.providerRequest);
        }
        catch (error) {
            const unknownAt = plusMilliseconds(now, Math.max(1, Math.ceil(performance.now() - startedAtMonotonic)));
            ...
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_OUTCOME_UNKNOWN", "Provider 请求已派发但结果无法安全确认，系统不会自动重试");
        }
        const confirmedAt = plusMilliseconds(now, Math.max(1, Math.ceil(performance.now() - startedAtMonotonic)));
    ...
    async settlePersistedProviderResponse(input) {
        ...
        return persistMutation(this.store, () => {
            const before = snapshot(this.store);
            const run = this.store.prospectSearchRuns.find((item) => item.id === input.runId
                && item.teamId === input.teamId
                && item.ownerId === input.ownerId
                && this.isRunAllowed(item.id));
            const ledger = run
                ? this.store.prospectProviderRequestLedgers.find((item) => item.id === input.ledgerId
                    && item.teamId === input.teamId
                    && item.ownerId === input.ownerId
                    && item.runId === input.runId)
                : null;
            if (!run || !ledger) {
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_SETTLEMENT_SCOPE_INVALID", "Provider 响应结算不在当前团队、业务员或运行作用域内");
            }
            if (!/^[a-f0-9]{64}$/.test(input.expectedResponseHash)
                || ledger.responseHash !== input.expectedResponseHash) {
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_SETTLEMENT_CONFLICT", "Provider 响应结算摘要与持久化响应不一致");
            }
            const shard = this.store.prospectRunShards.find((item) => item.id === ledger.shardId
                && item.teamId === ledger.teamId
                && item.runId === ledger.runId
                && item.providerCode === ledger.providerCode);
            const job = this.store.agentJobs.find((item) => item.id === ledger.jobId
                && item.teamId === ledger.teamId
                && item.ownerId === ledger.ownerId
                && item.jobType === PROSPECT_RUN_PROVIDER_FETCH_JOB_TYPE);
            const checkpoint = this.store.prospectExecutionCheckpoints.find((item) => item.teamId === ledger.teamId
                && item.ownerId === ledger.ownerId
                && item.runId === ledger.runId
                && item.shardId === ledger.shardId
                && item.jobId === ledger.jobId);
            const responseDispatches = this.store.prospectProviderRequestDispatches.filter((item) => item.ledgerId === ledger.id
                && item.teamId === ledger.teamId
                && item.ownerId === ledger.ownerId
                && item.runId === ledger.runId
                && item.shardId === ledger.shardId
            ...
            const attempt = dispatch
                ? this.store.prospectExecutionAttempts.find((item) => item.id === dispatch.attemptId
                    && item.teamId === ledger.teamId
                    && item.ownerId === ledger.ownerId
                    && item.runId === ledger.runId
                    && item.shardId === ledger.shardId
                ...
                : null;
            const lease = attempt
                ? this.store.prospectExecutionLeases.find((item) => item.id === attempt.leaseId
            ...
            const binding = attempt
                ? this.store.prospectProviderRequestAttemptBindings.find((item) => item.ledgerId === ledger.id
                    && item.attemptId === attempt.id
                    && item.teamId === ledger.teamId
                    && item.ownerId === ledger.ownerId)
                : null;
            ...
            if (!shard
                ...
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_SETTLEMENT_SCOPE_INVALID", "Provider 响应结算引用的运行、派发、尝试或终态租约不一致");
            }
            const response = this.verifiedProviderResponse(ledger, dispatch);
            const accountingEvidenceRows = this.store.prospectProviderRequestAccountingEvidence.filter((item) => item.ledgerId === ledger.id
                && item.teamId === ledger.teamId
                && item.ownerId === ledger.ownerId);
            const pages = this.store.prospectExecutionPages.filter((item) => item.attemptId === attempt.id
                && item.teamId === ledger.teamId
                && item.ownerId === ledger.ownerId
                && item.runId === ledger.runId
                && item.shardId === ledger.shardId);
            if (ledger.status === "settled"
                ...
                if (!validKind
                    ...
                    throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_SETTLEMENT_CONFLICT", "Provider 响应已有结算事实不完整或摘要不一致");
                }
                const rawBatch = page
                ...
                if (settlementKind === "success" && page) {
                    if (response.step.kind !== "success") {
                        throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_SETTLEMENT_CONFLICT", "Provider 成功结算与响应结果不一致");
                    }
            ...
            if (ledger.status !== "response_received"
                ...
                || pages.length
                || !["request_started", "request_outcome_unknown"].includes(attempt.status)) {
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_SETTLEMENT_STATE_INVALID", "Provider 响应尚未进入可结算状态或已存在冲突事实");
            }
            ...
            if (!normalSettlement && !lateSettlement) {
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_SETTLEMENT_STATE_INVALID", "Provider 响应与当前运行状态不允许结算");
            }
            const evidenceHash = prospectProviderAccountingEvidenceHash(response.step);
            if (evidenceHash !== ledger.responseAccountingEvidenceHash) {
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_SETTLEMENT_CONFLICT", "Provider 成本证据摘要与响应组件不一致");
            }
            const accountingEvidence = {
                ...
                evidenceHash,
                estimationMethodVersion: response.step.cost.kind === "estimated"
                    ? "fake-provider-estimate-v1"
                    : "",
                createdAt: now
            };
            this.store.prospectProviderRequestAccountingEvidence.push(accountingEvidence);
            ...
            if (lateSettlement) {
                ...
                attempt.retryable = false;
                const remaining = Math.max(0, shard.resultLimit - checkpoint.acceptedCount);
                const acceptedCount = Math.min(remaining, Math.trunc(response.step.acceptedCount));
                ...
                if (!naturalEnd) {
                    ...
                    const encryptedCursor = this.encryptCursor(response.step.cursor, {
                        teamId: run.teamId,
                        runId: run.id,
                        shardId: shard.id,
                        providerCode: shard.providerCode,
    ...
    async persistSettledProviderRawBatch(input) {
        ...
        const ledger = run
            ...
            : null;
        if (!run || !ledger) {
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RAW_SCOPE_INVALID", "Provider 原始批次不在当前团队、业务员或运行作用域内");
        }
        if (!/^[a-f0-9]{64}$/.test(input.expectedResponseHash)
            || !/^[a-f0-9]{64}$/.test(input.expectedSettlementHash)
            || ledger.responseHash !== input.expectedResponseHash
            || ledger.settlementHash !== input.expectedSettlementHash) {
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RAW_CONFLICT", "Provider 原始批次摘要与持久化响应或结算不一致");
        }
        const dispatches = this.store.prospectProviderRequestDispatches.filter((item) => item.ledgerId === ledger.id
            && item.teamId === ledger.teamId
            && item.ownerId === ledger.ownerId
            && item.runId === ledger.runId
            && item.shardId === ledger.shardId
        ...
        const attempt = dispatch
            ...
            : null;
        const pages = attempt
            ? this.store.prospectExecutionPages.filter((item) => item.attemptId === attempt.id
                && item.teamId === ledger.teamId
                && item.ownerId === ledger.ownerId
                && item.runId === ledger.runId
                && item.shardId === ledger.shardId)
        ...
        if (!dispatch
            ...
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RAW_STATE_INVALID", "Provider 原始批次引用的成功结算事实不完整或已被篡改");
        }
        const response = this.verifiedProviderResponse(ledger, dispatch);
        ...
        if (!result) {
            throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_RAW_UNAVAILABLE", "该 Provider 成功响应未声明逐条原始记录合同");
        }
        return result;
    }
    async beginRequest(input) {
        ...
        return persistMutation(this.store, () => {
            ...
            if (scope.run.status === "pause_requested"
                || scope.shard.status === "pause_requested") {
                throw new ProspectExecutionKernelError("EXECUTION_REQUEST_BLOCKED_BY_PAUSE", "暂停请求已生效，不能再发起新的 Provider 请求");
            }
            if (scope.attempt.status !== "claimed"
                || scope.lease.requestStartedAt) {
                throw new ProspectExecutionKernelError("EXECUTION_REQUEST_ALREADY_STARTED", "当前租约已经开始或完成 Provider 请求");
    ...
    async completePage(input) {
        ...
        return persistMutation(this.store, () => {
            ...
            if (scope.attempt.status !== "request_started") {
                throw new ProspectExecutionKernelError("EXECUTION_REQUEST_NOT_STARTED", "Provider 请求尚未开始或已经结算");
            }
            ...
            const remaining = Math.max(0, scope.shard.resultLimit - scope.checkpoint.acceptedCount);
            const acceptedCount = Math.min(remaining, Math.trunc(input.result.acceptedCount));
            ...
            if (!naturalEnd) {
                ...
                const encryptedCursor = this.encryptCursor(input.result.cursor, {
                    teamId: scope.run.teamId,
                    runId: scope.run.id,
                    shardId: scope.shard.id,
                    providerCode: scope.shard.providerCode,
    ...
    async requestPause(runId, now = new Date().toISOString()) {
        this.assertStarted();
        validIso(now);
        return persistMutation(this.store, () => {
            const before = snapshot(this.store);
            const run = this.store.prospectSearchRuns.find((item) => item.id === runId && this.isRunAllowed(item.id));
            if (!run || (run.status !== "queued" && run.status !== "running")) {
                throw new ProspectExecutionKernelError("EXECUTION_PAUSE_STATE_INVALID", "当前搜索运行状态不能暂停");
            }
            validateProspectRunQueueBridge(this.store, run);
            const activeLeases = this.store.prospectExecutionLeases.filter((item) => item.teamId === run.teamId
                && item.runId === run.id
                && item.status === "active");
            if (activeLeases.length > 1) {
                throw new ProspectExecutionKernelError("EXECUTION_ACTIVE_LEASE_CONFLICT", "同一搜索运行存在多个活动租约");
            }
            const activeLease = activeLeases[0];
            const pendingResponseLedgers = this.store.prospectProviderRequestLedgers.filter((item) => item.teamId === run.teamId
                && item.ownerId === run.ownerId
                && item.runId === run.id
                && item.status === "response_received");
            if (pendingResponseLedgers.length > 1
                || (activeLease && pendingResponseLedgers.length)) {
                throw new ProspectExecutionKernelError("EXECUTION_PROVIDER_SETTLEMENT_CONFLICT", "同一搜索运行存在冲突的活动租约或待结算响应");
            }
            ...
            const requestInFlight = activeRequestInFlight || settlementOutstanding;
            if (activeLease && !activeRequestInFlight) {
                const attempt = this.store.prospectExecutionAttempts.find((item) => item.teamId === activeLease.teamId
                    && item.leaseId === activeLease.id
                    && item.jobId === activeLease.jobId);
                if (!attempt || attempt.status !== "claimed") {
                    throw new ProspectExecutionKernelError("EXECUTION_ATTEMPT_SCOPE_INVALID", "暂停前的执行尝试状态或作用域无效");
                }
                ...
                attempt.finishedAt = now;
                attempt.version += 1;
                const activeJob = this.store.agentJobs.find((item) => item.id === activeLease.jobId
                    && item.teamId === activeLease.teamId
                    && item.ownerId === activeLease.ownerId);
                if (!activeJob) {
                    throw new ProspectExecutionKernelError("EXECUTION_CHILD_JOB_MISSING", "暂停前的执行租约缺少子任务");
                }
            ...
            const activeShardId = requestInFlight
                ...
                : "";
            for (const shard of this.store.prospectRunShards.filter((item) => item.teamId === run.teamId && item.runId === run.id)) {
                if ([
                    "cancelled",
                    "succeeded",
                    "succeeded_empty",
    ...
    async resume(runId, now = new Date().toISOString()) {
        this.assertStarted();
        validIso(now);
        return persistMutation(this.store, () => {
            const before = snapshot(this.store);
            ...
            if (!run || run.status !== "paused") {
                throw new ProspectExecutionKernelError("EXECUTION_RESUME_STATE_INVALID", "只有完全暂停的搜索运行可以恢复");
            }
            if (this.store.prospectExecutionLeases.some((item) => item.teamId === run.teamId
                && item.runId === run.id
                && item.status === "active")) {
                throw new ProspectExecutionKernelError("EXECUTION_RESUME_LEASE_ACTIVE", "存在活动租约时不能恢复搜索运行");
            }
            validateProspectRunQueueBridge(this.store, run);
            const previousStatus = run.status;
            const previousRevision = run.revision;
            ...
            run.updatedAt = now;
            run.pausedAt = "";
            for (const shard of this.store.prospectRunShards.filter((item) => item.teamId === run.teamId
                && item.runId === run.id
                && item.status === "paused")) {
                const checkpoint = this.store.prospectExecutionCheckpoints.find((item) => item.teamId === run.teamId
                    && item.runId === run.id
                    && item.shardId === shard.id);
                const job = this.childJob(run, shard);
                if (checkpoint?.encryptedCursor) {
                    const cursor = this.decryptCursor(checkpoint.encryptedCursor, checkpoint.cursorHash, {
                        teamId: run.teamId,
                        runId: run.id,
                        shardId: shard.id,
                        providerCode: shard.providerCode,
    ...
    async requestCancel(runId, now = new Date().toISOString()) {
        this.assertStarted();
        validIso(now);
        return persistMutation(this.store, () => {
            const before = snapshot(this.store);
            ...
            if (!run || ![
                ...
                throw new ProspectExecutionKernelError("EXECUTION_CANCEL_STATE_INVALID", "当前搜索运行状态不能取消");
            }
            validateProspectRunQueueBridge(this.store, run);
            const activeLeases = this.store.prospectExecutionLeases.filter((item) => item.teamId === run.teamId
                && item.runId === run.id
            ...
            if (activeLease && !activeRequestInFlight) {
                const attempt = this.store.prospectExecutionAttempts.find((item) => item.teamId === activeLease.teamId
                    ...
                    && item.jobId === activeLease.jobId);
                if (!attempt || attempt.status !== "claimed") {
                    throw new ProspectExecutionKernelError("EXECUTION_ATTEMPT_SCOPE_INVALID", "取消前的执行尝试状态或作用域无效");
                }
    ...
    async executeNext(provider, now = new Date().toISOString()) {
        const startedAtMonotonic = performance.now();
        ...
        const settlementNow = plusMilliseconds(now, Math.max(1, Math.ceil(performance.now() - startedAtMonotonic)));
        if (response.step.kind === "success") {
            return {
                kind: "success",
    ...
    async heartbeat(input) {
        ...
        return persistMutation(this.store, () => {
            ...
            lease.expiresAt = new Date(Math.min(nowTime + this.leaseMs, validIso(lease.deadlineAt))).toISOString();
            lease.version += 1;
            appendExecutionEvent(this.store, {
                teamId: lease.teamId,
                ownerId: lease.ownerId,
