...
import { createProspectCampaign, createProspectCampaignVersion, prospectCampaignEtag, prospectCampaignSnapshotHash } from "./prospect-campaigns.js";
...
async function main() {
    ...
    if (!configuredUrl) {
        throw new Error("Prospect Campaign MySQL test requires MYSQL_TEST_ADMIN_URL, DATABASE_URL or MYSQL_URL");
    }
    ...
    try {
        ...
        assert.match(campaignId, /^pc_[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
        ...
        const reloadedDraft = secondStore.prospectCampaigns.find((item) => item.id === campaignId);
        assert.ok(reloadedDraft);
        assert.equal(reloadedDraft.name, "德国工业照明获客项目");
        assert.equal(reloadedDraft.currentVersion, 1);
        assert.equal(secondStore.prospectCampaignVersions.filter((item) => item.campaignId === campaignId).length, 1);
        assert.equal(secondStore.prospectCampaignEvents.filter((item) => item.campaignId === campaignId).length, 1);
        ...
        const reloadedVersioned = thirdStore.prospectCampaigns.find((item) => item.id === campaignId);
        ...
        assert.equal(persistedVersions[1]?.contentHash, prospectCampaignSnapshotHash(fullSnapshot));
        assert.equal(thirdStore.prospectCampaignEvents.filter((item) => item.campaignId === campaignId).length, 2);
        const persistedVersion = persistedVersions[0];
        const originalVersion = structuredClone(persistedVersion);
        const linkedStrategy = thirdStore.prospectStrategies.find((item) => item.campaignId === campaignId
        ...
        persistedVersion.contentHash = prospectCampaignSnapshotHash(persistedVersion.snapshot);
        ...
        await assert.rejects(thirdStore.persist(), /获客项目版本不可变历史被删除或修改/);
        Object.assign(persistedVersion, originalVersion);
        linkedStrategy.queryFingerprint = originalStrategyFingerprint;
        const campaign = thirdStore.prospectCampaigns.find((item) => item.id === campaignId);
        const originalCurrentVersion = campaign.currentVersion;
        campaign.currentVersion = 99;
        await assert.rejects(thirdStore.persist(), /获客项目当前版本指针无效/);
        campaign.currentVersion = originalCurrentVersion;
        const malformedVersion = structuredClone(persistedVersions[1]);
        ...
        await assert.rejects(thirdStore.persist(), /获客项目版本快照或内容哈希不一致/);
        thirdStore.prospectCampaignVersions.pop();
        const formalNamespaceJob = {
            id: `job_${randomUUID()}`,
            teamId: owner.teamId,
            ownerId: owner.id,
        ...
        await assert.rejects(thirdStore.persist(), /pc_ 正式命名空间中的旧兼容市场数据/);
        thirdStore.agentJobs.pop();
        const invalidExistingVersion = structuredClone(persistedVersions[1]);
        ...
        await assert.rejects(createProspectCampaign({
            store: thirdStore,
            user: publicUser(thirdStore.users.find((item) => item.id === owner.id)),
            body: { name: "必须回滚的项目" },
        ...
        assert.equal(thirdStore.prospectCampaigns.some((item) => item.name === "必须回滚的项目"), false);
        thirdStore.prospectCampaignVersions.pop();
        await thirdStore.persist();
        const [campaignRows] = await admin.query(`SELECT id, current_version, revision_no
       FROM \`${databaseName}\`.prospect_campaigns WHERE id = ?`, [campaignId]);
        ...
        assert.equal(finalStore.prospectCampaignVersions.filter((item) => item.campaignId === campaignId).length, 2);
        assert.equal(finalStore.prospectCampaignEvents.filter((item) => item.campaignId === campaignId).length, 2);
        console.log("Prospect Campaign MySQL cold restart, append-only, rollback and validation tests passed");
        exitCode = 0;
    }
    catch (error) {
        console.error(error);
