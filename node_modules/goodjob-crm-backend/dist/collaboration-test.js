...
if (!address || typeof address === "string")
    throw new Error("Cannot start collaboration test server");
...
try {
    ...
    assert(store.dailyReports.filter((item) => item.ownerId === "u_sales_shirley" && item.reportDate === reportDate).length === 1, "same user and date must have exactly one report");
    const ownReports = await request("/api/daily-reports", { headers: auth(shirleyToken) });
    assert(ownReports.response.ok && ownReports.json.reports?.some((item) => item.id === reportId), "salesperson must see own report");
    const peerReports = await request("/api/daily-reports", { headers: auth(miaToken) });
    assert(!peerReports.json.reports?.some((item) => item.id === reportId), "salesperson must not see a peer report");
    const managerReports = await request("/api/daily-reports", { headers: auth(alexToken) });
    assert(managerReports.json.reports?.some((item) => item.id === reportId), "team manager must see team report");
    const otherTeamReports = await request("/api/daily-reports", { headers: auth(otherManagerToken) });
    assert(!otherTeamReports.json.reports?.some((item) => item.id === reportId), "other team manager must not see report");
    ...
    const commentId = managerComment.json.comment?.id;
    const salespersonInboxAfterComment = await request("/api/internal-messages?box=inbox", {
        headers: auth(shirleyToken)
    });
    const commentNotification = salespersonInboxAfterComment.json.messages?.find((item) => item.relatedId === reportId && item.senderId === "u_manager_alex");
    ...
    assert(reply.response.status === 201 && reply.json.comment?.parentId === commentId, "comment reply must retain parent relation");
    const managerInboxBeforeRead = await request("/api/internal-messages?box=inbox", { headers: auth(alexToken) });
    const replyNotification = managerInboxBeforeRead.json.messages?.find((item) => item.relatedId === reportId && item.subject.includes("回复"));
    assert(replyNotification, "comment reply must notify original commenter");
    const managerUnreadBefore = managerInboxBeforeRead.json.unreadCount;
    ...
    assert(!otherTeamRecipients.json.recipients?.some((item) => item.teamId === "europe"), "recipient selector must not expose another team");
    ...
    const directMessageId = directMessage.json.message?.id;
    const miaInboxBeforeRead = await request("/api/internal-messages?box=inbox", { headers: auth(miaToken) });
    assert(miaInboxBeforeRead.json.messages?.some((item) => item.id === directMessageId), "recipient inbox must contain direct message");
    const alexInbox = await request("/api/internal-messages?box=inbox", { headers: auth(alexToken) });
    assert(!alexInbox.json.messages?.some((item) => item.id === directMessageId), "unrelated team member must not see direct message");
    const shirleySent = await request("/api/internal-messages?box=sent", { headers: auth(shirleyToken) });
    assert(shirleySent.json.messages?.some((item) => item.id === directMessageId), "sender sent box must contain direct message");
    ...
    const miaUnreadBefore = miaInboxBeforeRead.json.unreadCount;
