...
import { createCredentialRef, encryptProviderConfiguration } from "./credential-security.js";
import { createProspectCampaign, prospectCampaignEtag, transitionProspectCampaign, updateProspectCampaign } from "./prospect-campaigns.js";
import { normalizeProspectStrategyProviderPlan, normalizeProspectStrategyQuery, prospectStrategyFingerprint } from "./prospect-strategies.js";
...
try {
    ...
    const defaultStrategy = store.prospectStrategies.find((item) => item.campaignId === campaignId);
    ...
    assert.equal(visibleDuplicatePreview.json.duplicate.strategyId, store.prospectStrategies.find((item) => item.campaignId === visibleDuplicateCampaign.campaign.id)?.id);
    ...
    const isolatedOwnerStrategy = store.prospectStrategies.find((item) => item.campaignId === isolatedOwnerCampaign.campaign.id);
    ...
    assert.equal(managerTeamDuplicatePreview.json.duplicate.strategyId, store.prospectStrategies.find((item) => item.campaignId === teamDuplicateCampaign.campaign.id)?.id);
    ...
    assert.deepEqual(updated.json.strategy.query.exclusionDomains, ["example.com"]);
    ...
    assert.equal(limitOnlyUpdate.json.strategy.queryFingerprint, fingerprintBeforeLimitChange);
    ...
    assert.equal(copied.response.headers.get("x-campaign-etag"), prospectCampaignEtag(store.prospectCampaigns[0]));
    ...
    assert.equal(duplicateApprove.json.errorCode, "STRATEGY_DUPLICATE_APPROVED");
    ...
    const secondStrategyBeforeTransfer = store.prospectStrategies.find((item) => item.id === secondCreated.json.strategy.id);
    ...
    assert.ok(store.prospectStrategies
        .filter((item) => item.campaignId === campaignId)
        .every((item) => item.ownerId === salesA2.id));
    assert.equal(store.prospectStrategies.find((item) => item.id === secondStrategyBeforeTransfer.id)?.revision, secondRevisionBeforeTransfer + 1);
    ...
    const inconsistentCampaignRecord = store.prospectCampaigns.find((item) => item.id === inconsistentCampaign.campaign.id);
    const inconsistentStrategy = store.prospectStrategies.find((item) => item.campaignId === inconsistentCampaignRecord.id);
    ...
    const directVersion = store.prospectCampaignVersions.find((item) => item.campaignId === campaignId && item.version === 1);
    ...
    assert.equal(prospectStrategyFingerprint({
    ...
    const keyStrategy = store.prospectStrategies.find((item) => item.campaignId === keyCampaign.campaign.id);
    ...
    const keyActivation = await request({
        ...
        method: "POST",
        user: salesA,
        ifMatch: prospectCampaignEtag(store.prospectCampaigns.find((item) => item.id === keyCampaign.campaign.id)),
        body: {}
    });
    assert.equal(keyActivation.response.status, 422);
    assert.equal(keyActivation.json.errorCode, "CAMPAIGN_PROVIDER_NOT_READY");
    assert.ok(keyActivation.json.issues.some((item) => item.code === "PROVIDER_CONNECTION_REQUIRED"));
    ...
    assert.ok(legacyScopeActivation.json.issues.some((item) => item.code === "PROVIDER_CONNECTION_REQUIRED"));
    store.providerConnections.splice(store.providerConnections.findIndex((item) => item.id === legacyConnectionId), 1);
    const gleifCatalog = store.providerCatalog.find((item) => item.code === "gleif");
    ...
    const manualStrategy = store.prospectStrategies.find((item) => item.campaignId === manualCampaign.campaign.id);
    ...
    assert.ok(manualApproval.json.issues.some((item) => item.code === "PROVIDER_RUNTIME_POLICY_INVALID"));
    ...
    const ghostStrategy = store.prospectStrategies.find((item) => item.campaignId === ghostCampaign.campaign.id);
    ...
    assert.ok(ghostApproval.json.issues.some((item) => item.code === "PROVIDER_ADAPTER_MISSING"));
    store.providerCatalog.splice(store.providerCatalog.findIndex((item) => item.code === "strategy_ghost"), 1);
    ...
    const emailOnlyStrategy = store.prospectStrategies.find((item) => item.campaignId === emailOnlyCampaign.campaign.id);
    ...
    assert.ok(emailOnlyApproval.json.issues.some((item) => item.code === "PROVIDER_CAPABILITY_INVALID"));
    ...
    assert.equal(store.prospectCampaigns.some((item) => item.name === "策略原子回滚"), false);
    store.persist = original.persist;
    const openApi = createOpenApiDocument(app);
    const createStrategy = openApi.paths["/api/prospect-campaigns/{id}/strategies"]?.post;
    const patchStrategy = openApi.paths["/api/prospect-strategies/{id}"]?.patch;
    const previewStrategy = openApi.paths["/api/prospect-strategies/{id}/preview"]?.post;
    const approveStrategy = openApi.paths["/api/prospect-strategies/{id}/approve"]?.post;
    assert.ok(createStrategy);
    assert.ok(createStrategy.parameters.some((item) => item.name === "If-Match" && item.required === true));
    assert.ok(patchStrategy.parameters.some((item) => item.name === "If-Match" && item.required === true));
    ...
    store.providerCatalog.splice(0, store.providerCatalog.length, ...original.providerCatalog);
    store.providerConnections.splice(0, store.providerConnections.length, ...original.providerConnections);
    store.aiModelConfigs.splice(0, store.aiModelConfigs.length, ...original.aiModelConfigs);
    store.persist = original.persist;
