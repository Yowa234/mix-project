...
import { applyProspectQualificationCommand, PROSPECT_QUALIFICATION_ARRAYS, PROSPECT_QUALIFICATION_CONTRACT, PROSPECT_QUALIFICATION_RECORD_TYPES, ProspectQualificationError, validateProspectQualificationState } from "./prospect-qualification.js";
...
const ARRAY_BY_RECORD_TYPE = Object.fromEntries(Object.entries(PROSPECT_QUALIFICATION_RECORD_TYPES)
...
const qualificationQueues = new WeakMap();
function canonical(value) {
    const result = canonicalJsonStringify(value);
    if (typeof result !== "string") {
        throw new ProspectQualificationError("QUALIFICATION_CANONICAL_INVALID", "资格事实无法规范化");
    }
    return result;
}
...
function configurationError(message) {
    throw new ProspectQualificationError("QUALIFICATION_CONFIGURATION_INVALID", message);
}
function integrityError(message) {
    throw new ProspectQualificationError("QUALIFICATION_DATA_INTEGRITY", message);
}
function configuredSecrets(required) {
    ...
    if (!master || Buffer.byteLength(master, "utf8") < 32) {
        configurationError("PROSPECT_QUALIFICATION_MASTER_SECRET 必须至少包含 32 字节");
    }
    const derive = (info) => Buffer.from(hkdfSync("sha256", Buffer.from(master, "utf8"), HKDF_SALT, Buffer.from(info, "utf8"), 32));
...
async function factCount(source) {
    const rows = await queryRows(source, "SELECT COUNT(*) AS count FROM prospect_qualification_facts");
    return Number(rows[0]?.count || 0);
}
async function validateMetadata(source, secrets, lockForUpdate = false) {
    const rows = await queryRows(source, `SELECT * FROM prospect_qualification_contract_metadata
...
async function initializeMetadata(connection, secrets, createdAt) {
    ...
    await connection.query(`INSERT INTO prospect_qualification_contract_metadata (
...
function factIndex(record) {
    const recordTypeEntry = Object.entries(PROSPECT_QUALIFICATION_RECORD_TYPES).find(([arrayName]) => (recordTypeRows(record, arrayName)));
    if (!recordTypeEntry)
        integrityError("无法识别资格事实类型");
    ...
    const organizationId = "organizationId" in record ? record.organizationId : "";
...
function recordTypeRows(record, arrayName) {
    switch (arrayName) {
...
async function insertFact(connection, record, secrets) {
    ...
    await connection.query(`INSERT INTO prospect_qualification_facts (
...
export async function ensureProspectQualificationSchema(pool) {
    const connection = await pool.getConnection();
    let acquired = false;
    try {
        const rows = await queryRows(connection, "SELECT GET_LOCK(?, 30) AS acquired", [SCHEMA_LOCK]);
        acquired = Number(rows[0]?.acquired || 0) === 1;
        if (!acquired)
            configurationError("无法获取资格 Schema 锁");
...
export async function loadProspectQualificationState(source, teamId) {
    const secrets = configuredSecrets(false);
    if (!await validateMetadata(source, secrets)) {
        return structuredClone(EMPTY_STATE);
    }
    const rows = await queryRows(source, `SELECT * FROM prospect_qualification_facts
     ${teamId ? "WHERE team_id = ?" : ""}
     ORDER BY created_at,id`, teamId ? [teamId] : []);
    ...
    for (const raw of rows) {
        ...
        const arrayName = ARRAY_BY_RECORD_TYPE[row.recordType];
        if (!arrayName)
            integrityError("资格事实类型不受支持");
        state[arrayName].push(record);
    }
    return state;
}
function replaceTeamState(store, teamId, state) {
    for (const key of PROSPECT_QUALIFICATION_ARRAYS) {
        const target = store[key];
        const retained = target.filter((item) => item.teamId !== teamId);
        target.splice(0, target.length, ...retained, ...state[key]);
    }
}
function enqueueTeam(store, teamId, operation) {
    ...
    return current.finally(() => {
        if (queues?.get(teamId) === current)
            queues.delete(teamId);
    });
}
export async function applyProspectQualificationCommandMysql(pool, store, command) {
    return enqueueTeam(store, command.teamId, async () => {
        ...
        try {
         ...
         ON DUPLICATE KEY UPDATE team_id = VALUES(team_id)`, [command.teamId, new Date(createdAt)]);
            await connection.query(`SELECT guard_version FROM prospect_qualification_team_guards
         WHERE team_id = ? FOR UPDATE`, [command.teamId]);
            const state = await loadProspectQualificationState(connection, command.teamId);
            const local = {
                ...store,
                ...state,
                mode: "memory"
            ...
            if (!result.replayed) {
                ...
                await connection.query(`UPDATE prospect_qualification_team_guards
           SET guard_version = guard_version + 1,updated_at = ?
           WHERE team_id = ?`, [new Date(createdAt), command.teamId]);
            }
...
function stateFromStore(store) {
    return {
        ...
        prospectContactVerificationSnapshots: store.prospectContactVerificationSnapshots,
        prospectSuppressionEvents: store.prospectSuppressionEvents,
        prospectContactabilityDecisions: store.prospectContactabilityDecisions
    };
}
export async function ensureProspectQualificationTeamCache(pool, store, teamId) {
