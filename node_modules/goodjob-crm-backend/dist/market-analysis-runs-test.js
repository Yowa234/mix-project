...
import { decryptAgentJobPayload, encryptAgentJobPayload } from "./agent-job-security.js";
...
if (!address || typeof address === "string")
    throw new Error("Cannot start market analysis test server");
...
async function request(token, idempotencyKey, body = baseBody) {
...
try {
    ...
    assert.equal(teamAFirst.response.headers.get("location"), `/api/prospect-agent-jobs/${teamAFirst.json.job.id}`);
    ...
    assert.deepEqual(teamADuplicate.json.result.querySummary, teamAFirst.json.result.querySummary);
    assert.deepEqual(teamADuplicate.json.result.observations, teamAFirst.json.result.observations);
    assert.equal(externalRequests, 1);
    assert.equal(store.agentJobs.length, 1);
    ...
    const validOutput = decryptAgentJobPayload(teamAJob, "output", teamAJob.outputJsonEncrypted);
    ...
    assert.equal(store.marketTradeObservations.filter((item) => item.teamId === teamAUser.teamId && item.ownerId === teamAUser.id).length, 1);
    assert.equal(store.marketTradeObservations.filter((item) => item.teamId === teamAUser2.teamId && item.ownerId === teamAUser2.id).length, 1);
    ...
    assert.equal(store.marketTradeObservations.filter((item) => item.teamId === teamAUser.teamId).length, 2);
    assert.equal(store.marketTradeObservations.filter((item) => item.teamId === teamBUser.teamId).length, 1);
    assert.equal(store.providerRequestLogs.filter((item) => item.teamId === teamAUser.teamId).length, 2);
    assert.equal(store.providerRequestLogs.filter((item) => item.teamId === teamBUser.teamId).length, 1);
    ...
    assert.equal(store.marketTradeObservations.filter((item) => item.period === "2022").length, 0);
    assert.equal(store.agentJobs.find((item) => item.id === rateLimited.json.job.id)?.teamId, teamAUser.teamId);
    assert.equal(rateLimited.response.headers.get("location"), `/api/prospect-agent-jobs/${rateLimited.json.job.id}`);
    const rateLimitedWithNewKey = await request(tokenA, "market-run-rate-limited-new-key", {
        ...baseBody,
        periods: ["2022"]
    });
    ...
    assert.equal(store.agentJobIdempotencyAliases.length, aliasesBeforeRateLimit + 1);
    ...
    assert.equal(store.marketTradeObservations.filter((item) => item.period === "2022").length, 1);
    ...
    const genericRetryJob = store.agentJobs.find((item) => item.id === genericRetryFailure.json.job.id);
    ...
    const crossOwnerRetry = await fetch(`${baseUrl}/api/prospect-agent-jobs/${genericRetryJob.id}/retry`, {
        method: "POST",
        headers: {
    ...
    const genericRetryResponse = await fetch(`${baseUrl}/api/prospect-agent-jobs/${genericRetryJob.id}/retry`, {
        method: "POST",
        headers: {
    ...
    assert.equal(genericRetryResponse.headers.get("location"), `/api/prospect-agent-jobs/${genericRetryJob.id}`);
    let releaseInitialPersistFailure;
    let markInitialPersistStarted;
    const initialPersistFailureGate = new Promise((resolve) => {
        releaseInitialPersistFailure = resolve;
    });
    const initialPersistStarted = new Promise((resolve) => {
    ...
    const initialPersistFailureFirstPromise = request(tokenA, "market-run-initial-persist-a", initialPersistFailureBody);
    await initialPersistStarted;
    const initialPersistFailureSecondPromise = request(tokenA, "market-run-initial-persist-b", initialPersistFailureBody);
    await new Promise((resolve) => setTimeout(resolve, 20));
    assert.equal(store.agentJobIdempotencyAliases.length, aliasesBeforeInitialPersistFailure);
    ...
    assert.equal(store.agentJobIdempotencyAliases.length, aliasesBeforeInitialPersistFailure);
    let releaseConcurrentRequest;
    let markConcurrentRequestStarted;
    const concurrentGate = new Promise((resolve) => {
        releaseConcurrentRequest = resolve;
    });
    const concurrentRequestStarted = new Promise((resolve) => {
    ...
    const runningJob = store.agentJobs.find((item) => item.ownerId === teamAUser.id
        && item.aggregateId === "campaign_shared"
        && item.jobType === "prospect.market_analysis"
        && item.status === "running");
    assert.ok(runningJob);
    const cancelResponse = await fetch(`${baseUrl}/api/prospect-agent-jobs/${runningJob.id}/cancel`, {
        method: "POST",
        headers: {
            authorization: `Bearer ${tokenA}`,
            "content-type": "application/json"
    ...
    const concurrentSecondPromise = request(tokenA, "market-run-concurrent-b", concurrentBody);
    const activeConflictingReuse = await request(tokenA, "market-run-shared-key", concurrentBody);
    ...
    assert.equal(concurrentProviderCalls, 1);
    assert.equal(store.agentJobIdempotencyAliases.length, aliasesBeforeConcurrent + 1);
    const concurrentAlias = store.agentJobIdempotencyAliases.find((item) => item.jobId === concurrentFirst.json.job.id);
    assert.ok(concurrentAlias);
    assert.match(concurrentAlias.idempotencyKey, /^[a-f0-9]{64}$/);
    assert.notEqual(concurrentAlias.idempotencyKey, "market-run-concurrent-b");
    assert.equal(store.agentJobs.filter((item) => item.ownerId === teamAUser.id
        && item.aggregateId === "campaign_shared"
        && item.jobType === "prospect.market_analysis"
        && item.status === "succeeded"
        && item.createdAt === concurrentFirst.json.job.createdAt).length, 1);
    const concurrentSecondReplay = await request(tokenA, "market-run-concurrent-b", concurrentBody);
    ...
    assert.equal(concurrentProviderCalls, 1);
    assert.equal(store.agentJobs.length, jobsBeforeConcurrent + 1);
    const concurrentAliasConflict = await request(tokenA, "market-run-concurrent-b", { ...baseBody, periods: ["2017"] });
    ...
    assert.equal(concurrentProviderCalls, 1);
    assert.equal(store.agentJobs.length, jobsBeforeConcurrent + 1);
    let releaseRuntimeBindingProvider;
    let markRuntimeBindingProviderStarted;
    const runtimeBindingProviderGate = new Promise((resolve) => {
        releaseRuntimeBindingProvider = resolve;
    });
    const runtimeBindingProviderStarted = new Promise((resolve) => {
    ...
    const runtimeBindingPrimaryPromise = request(tokenA, "market-run-runtime-binding-primary", runtimeBindingBody);
    ...
    let releaseRuntimeBindingMutation;
    let markRuntimeBindingMutationStarted;
    const runtimeBindingMutationGate = new Promise((resolve) => {
        releaseRuntimeBindingMutation = resolve;
    });
    const runtimeBindingMutationStarted = new Promise((resolve) => {
        markRuntimeBindingMutationStarted = resolve;
    });
    let runtimeBindingMutationCount = 0;
    let runtimeBindingMutationTail = Promise.resolve();
    store.persistMutation = (mutation) => {
    ...
    const runtimeBindingAliasPromise = request(tokenA, "market-run-runtime-binding-shared", runtimeBindingBody);
    await runtimeBindingMutationStarted;
    const runtimeBindingConflictPromise = request(tokenA, "market-run-runtime-binding-shared", { ...baseBody, periods: ["2012"] });
    await new Promise((resolve) => setTimeout(resolve, 20));
    ...
    assert.equal(store.agentJobIdempotencyAliases.length, aliasesBeforeRuntimeBinding + 1);
    ...
    assert.equal(runtimeBindingProviderCalls, 1);
    let releaseConcurrentFailure;
    let markConcurrentFailureStarted;
    const concurrentFailureGate = new Promise((resolve) => {
        releaseConcurrentFailure = resolve;
    });
    const concurrentFailureStarted = new Promise((resolve) => {
    ...
    const concurrentFailureFirstPromise = request(tokenA, "market-run-concurrent-failure-a", concurrentFailureBody);
    await concurrentFailureStarted;
    const concurrentFailureSecondPromise = request(tokenA, "market-run-concurrent-failure-b", concurrentFailureBody);
    await new Promise((resolve) => setTimeout(resolve, 20));
    assert.equal(concurrentFailureProviderCalls, 1);
    assert.equal(store.agentJobs.length, jobsBeforeConcurrentFailure + 1);
    assert.equal(store.agentJobIdempotencyAliases.length, aliasesBeforeConcurrentFailure + 1);
    ...
    assert.equal(concurrentFailureFirst.json.job.id, concurrentFailureSecond.json.job.id);
    assert.equal(concurrentFailureFirst.json.job.status, "failed");
    assert.ok(concurrentFailureFirst.json.job.nextAttemptAt);
    const concurrentFailureReplay = await request(tokenA, "market-run-concurrent-failure-b", concurrentFailureBody);
    assert.equal(concurrentFailureReplay.response.status, 429);
    assert.equal(concurrentFailureReplay.json.job.id, concurrentFailureFirst.json.job.id);
    assert.equal(concurrentFailureProviderCalls, 1);
    assert.equal(store.agentJobs.length, jobsBeforeConcurrentFailure + 1);
    const concurrentFailureJob = store.agentJobs.find((item) => item.id === concurrentFailureFirst.json.job.id);
    ...
    const concurrentFailureRetried = await request(tokenA, "market-run-concurrent-failure-b", concurrentFailureBody);
    assert.equal(concurrentFailureRetried.response.status, 200);
    assert.equal(concurrentFailureRetried.json.job.id, concurrentFailureFirst.json.job.id);
    assert.equal(concurrentFailureRetried.json.job.attemptCount, 2);
    assert.equal(concurrentFailureProviderCalls, 2);
    assert.equal(store.agentJobs.length, jobsBeforeConcurrentFailure + 1);
    let releaseAliasRollbackProvider;
    let markAliasRollbackProviderStarted;
    const aliasRollbackProviderGate = new Promise((resolve) => {
        releaseAliasRollbackProvider = resolve;
    });
    const aliasRollbackProviderStarted = new Promise((resolve) => {
    ...
    const aliasRollbackFirstPromise = request(tokenA, "market-run-alias-rollback-a", aliasRollbackBody);
    ...
    let releaseAliasRollbackPersist;
    let markAliasRollbackPersistStarted;
    const aliasRollbackPersistGate = new Promise((resolve) => {
        releaseAliasRollbackPersist = resolve;
    });
    const aliasRollbackPersistStarted = new Promise((resolve) => {
    ...
    const aliasRollbackSecondPromise = request(tokenA, "market-run-alias-rollback-b", aliasRollbackBody);
    ...
    await new Promise((resolve) => setTimeout(resolve, 20));
    assert.equal(aliasRollbackFirstSettled, false, "主任务完成持久化必须等待并发别名写入完成或回滚");
    ...
    const rollbackJob = store.agentJobs.find((item) => item.ownerId === teamAUser.id
        && item.aggregateId === "campaign_shared"
        && item.status === "running");
    ...
    const operation = document.paths["/api/prospect-campaigns/{id}/market-analysis-runs"]?.post;
    assert.ok(operation);
    assert.equal(operation.tags[0], "获客项目");
    assert.ok(operation.parameters.some((item) => item.in === "header" && item.name === "Idempotency-Key" && item.required === true));
    ...
    assert.equal(operation.responses["200"].content["application/json"].schema
        .properties.retryMode.enum[0], "manual");
    assert.equal(operation.responses["201"].content["application/json"].schema
        .properties.result.properties.updatedCount.type, "integer");
    assert.equal(operation.responses["201"].content["application/json"].schema
        .properties.result.properties.resultScope.enum[0], "job_execution");
    assert.equal(operation.responses["201"].content["application/json"].schema
        .properties.result.properties.querySummary.additionalProperties, false);
    assert.equal(operation.responses["201"].content["application/json"].schema
        .properties.result.properties.observations.items.additionalProperties, false);
    ...
    assert.equal(operation.responses["429"].content["application/json"].schema.$ref, "#/components/schemas/MarketAnalysisProviderError");
    assert.match(operation.responses["201"].headers.Location.description, /完整查询结果需重放本 POST/);
