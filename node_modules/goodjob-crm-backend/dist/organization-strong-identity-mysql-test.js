...
import { ORGANIZATION_STRONG_IDENTITY_CONTRACT, OrganizationStrongIdentityError, resolveOrganizationStrongIdentity } from "./organization-strong-identity.js";
import { ensureOrganizationIdentitySchema, loadOrganizationIdentityState, resolveOrganizationStrongIdentityMysql } from "./organization-strong-identity-mysql.js";
import { ensureProspectCoverageSchema } from "./prospect-coverage-memory-mysql.js";
import { PROSPECT_SOURCE_RAW_SCHEMA_VERSION, appendProspectSourceRawBatch, prospectProviderRawArtifactHash } from "./prospect-source-raw.js";
import { createMysqlStore } from "./mysql-store.js";
import { getStore } from "./store.js";
const masterSecret = "organization-identity-mysql-master-secret-v1-".repeat(2);
const rawEnvelopeSecret = "organization-identity-mysql-raw-envelope-v1-".repeat(2);
const rawIdentitySecret = "organization-identity-mysql-raw-id-v1-".repeat(2);
...
const identityHkdfSalt = Buffer.from("goodjob-organization-identity-v1", "utf8");
...
function identityTestKey(info) {
    return Buffer.from(hkdfSync("sha256", Buffer.from(masterSecret, "utf8"), identityHkdfSalt, Buffer.from(info, "utf8"), 32));
}
...
function encryptTestField(value, key, aad) {
...
function mysqlDuplicateError(key, overrides = {}) {
...
function isInsertInto(sql, table) {
    return sql.startsWith(`INSERT INTO \`${table}\``);
}
function namedDuplicateRacePool(base, options) {
    const state = {
...
async function rewriteIdentifierNormalizedValue(connection, identifierId, normalizedValue) {
    const [rows] = await connection.query(`SELECT * FROM organization_accepted_identifiers
    ...
    const base = {
        ...
        normalized_value_encrypted: encryptTestField(normalizedValue, encryptionKey, testFieldAad({
    ...
    await connection.query(`UPDATE organization_accepted_identifiers
...
function lei(value, subjectRef = "company") {
    return baseClaim({
        kind: "lei",
        value,
...
function registration(value, scheme = "registry-a", subjectRef = "company") {
    return baseClaim({
        kind: "registration_number",
        value,
...
function createRawFixture(teamId, ownerId, payload = {}) {
    rawSequence += 1;
    const suffix = String(rawSequence).padStart(4, "0");
    const fetchedAt = new Date(Date.UTC(2026, 6, 14, 2, 0, rawSequence)).toISOString();
    const sourceRecords = [{
...
async function insertRawRecord(connection, fixture) {
    const record = fixture.record;
    await connection.query(`INSERT INTO prospect_source_raw_records (
...
function persistedInput(raw, claims, overrides = {}) {
    ...
    return {
        ...
        resolvedAt: new Date(Date.UTC(2026, 6, 14, 3, 0, resolutionSequence)).toISOString(),
...
async function resolvePersisted(store, raw, claims, overrides = {}) {
    assert.ok(store.resolveOrganizationStrongIdentity);
    return store.resolveOrganizationStrongIdentity(persistedInput(raw, claims, overrides));
}
function isIdentityError(code) {
    return (error) => error instanceof OrganizationStrongIdentityError
        && error.code === code;
}
async function identityFactCount(connection) {
    let count = 0;
...
function snapshotBusinessArrays(store) {
    return Object.fromEntries(protectedBusinessArrays.map((name) => [
        name,
        structuredClone(store[name])
    ]));
}
function snapshotTeamIdentity(store, teamId) {
    return {
        organizations: structuredClone(store.organizations.filter((item) => item.teamId === teamId)),
        claims: structuredClone(store.organizationIdentityClaims.filter((item) => item.teamId === teamId)),
        identifiers: structuredClone(store.organizationAcceptedIdentifiers.filter((item) => item.teamId === teamId)),
        resolutions: structuredClone(store.organizationIdentityResolutions.filter((item) => item.teamId === teamId)),
        bindings: structuredClone(store.organizationSourceBindings.filter((item) => item.teamId === teamId)),
        conflicts: structuredClone(store.organizationIdentityConflicts.filter((item) => item.teamId === teamId)),
        events: structuredClone(store.organizationIdentityEvents.filter((item) => item.teamId === teamId))
    };
}
async function databaseIdentitySnapshot(connection) {
    return databaseTableSnapshot(connection, identityTables);
}
async function databaseIdentityFactSnapshot(connection) {
    return databaseTableSnapshot(connection, identityFactTables);
}
async function databaseTableSnapshot(connection, tables) {
    const snapshot = {};
    for (const table of tables) {
        const [rows] = await connection.query(`SELECT * FROM \`${table}\``);
        snapshot[table] = JSON.stringify(rows.map((row) => JSON.stringify(row)).sort());
    }
    return snapshot;
}
async function assertColdStartRejected(code) {
    await assert.rejects(createMysqlStore(), isIdentityError(code));
}
async function assertSchemaMutationRejected(mutate, restore) {
    await mutate();
    try {
...
async function dropIdentitySchema(connection) {
...
function instrumentPool(base, hooks) {
                        ...
                        get(connectionTarget, connectionProperty) {
                            if (connectionProperty === "query") {
                                return (...args) => {
                                    ...
                                    const operation = Reflect.get(connectionTarget, connectionProperty);
                                    const proceed = () => Promise.resolve(Reflect.apply(operation, connectionTarget, args));
...
async function main() {
    ...
    if (!applicationUrl || !adminConnectionUrl) {
        throw new Error("Organization Identity MySQL test requires MYSQL_TEST_ADMIN_URL, "
            + "DATABASE_URL or MYSQL_URL");
    }
    const adminUrl = new URL(adminConnectionUrl);
    const appUrl = new URL(applicationUrl);
    const databaseName = `goodjob_identity_test_${randomUUID().replaceAll("-", "").slice(0, 16)}`;
    ...
    try {
       ...
       CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`);
        ...
        const [accountRows] = await appProbe.query("SELECT CURRENT_USER() AS account");
        await appProbe.end();
        ...
        await admin.query(`GRANT ALL PRIVILEGES ON \`${databaseName}\`.* TO ${sqlString(accountUser)}@${sqlString(accountHost)}`);
        ...
        try {
            const schemaSql = await readFile(new URL("../schema.mysql.sql", import.meta.url), "utf8");
        ...
        const [tableRows] = await database.query(`SELECT table_name AS tableName
       FROM information_schema.tables
       WHERE table_schema = ?
         AND table_name IN (${identityTables.map(() => "?").join(",")})
       ORDER BY table_name`, [databaseName, ...identityTables]);
        assert.deepEqual(tableRows.map((row) => String(row.tableName)), [...identityTables].sort());
        ...
        const [indexRows] = await database.query(`SELECT table_name AS tableName,index_name AS indexName,
         GROUP_CONCAT(column_name ORDER BY seq_in_index) AS columnsList,
         MAX(non_unique) AS nonUnique
       FROM information_schema.statistics
       WHERE table_schema = ?
         AND table_name IN (${identityTables.map(() => "?").join(",")})
       GROUP BY table_name,index_name`, [databaseName, ...identityTables]);
        for (const [key, columns] of expectedUniqueIndexes) {
            const [table, index] = key.split(":");
            const row = indexRows.find((item) => item.tableName === table && item.indexName === index);
        ...
        const [foreignKeyRows] = await database.query(`SELECT constraint_name AS constraintName,
         update_rule AS updateRule,delete_rule AS deleteRule
       FROM information_schema.referential_constraints
       WHERE constraint_schema = ?
         AND constraint_name IN (${expectedForeignKeys.map(() => "?").join(",")})`, [databaseName, ...expectedForeignKeys]);
        assert.equal(foreignKeyRows.length, expectedForeignKeys.length);
        assert.ok(foreignKeyRows.every((row) => row.updateRule === "RESTRICT" && row.deleteRule === "RESTRICT"));
        ...
        const [checkRows] = await database.query(`SELECT constraint_name AS constraintName
       FROM information_schema.table_constraints
       WHERE constraint_schema = ?
         AND constraint_type = 'CHECK'
         AND constraint_name IN (${expectedChecks.map(() => "?").join(",")})`, [databaseName, ...expectedChecks]);
        assert.deepEqual(checkRows.map((row) => String(row.constraintName)).sort(), expectedChecks.sort());
        const [guardDefinitionRows] = await database.query("SHOW CREATE TABLE organization_identity_team_guards");
        const guardDefinition = String(guardDefinitionRows[0]?.["Create Table"] || "");
        ...
        const [partialEmptyBeforeRows] = await database.query("SHOW CREATE TABLE organization_identity_team_guards");
        const partialEmptyBefore = String(partialEmptyBeforeRows[0]?.["Create Table"] || "");
        await ensureOrganizationIdentitySchema(pool);
        const [partialEmptyAfterRows] = await database.query("SHOW CREATE TABLE organization_identity_team_guards");
        assert.equal(String(partialEmptyAfterRows[0]?.["Create Table"] || ""), partialEmptyBefore);
        const [completedPartialRows] = await database.query(`SELECT table_name AS tableName
         FROM information_schema.tables
         WHERE table_schema = ?
           AND table_name IN (${identityTables.map(() => "?").join(",")})
         ORDER BY table_name`, [databaseName, ...identityTables]);
        assert.deepEqual(completedPartialRows.map((row) => String(row.tableName)), [...identityTables].sort());
        const [completedGuardRows] = await database.query(`SELECT COUNT(*) AS count
        ...
        await dropIdentitySchema(database);
        await database.query(guardDefinition);
        await database.query(`INSERT INTO organization_identity_team_guards (
         team_id,guard_version,updated_at
       ) VALUES ('identity-partial-state',1,UTC_TIMESTAMP(3))`);
        await assert.rejects(ensureOrganizationIdentitySchema(pool), isIdentityError("IDENTITY_DATA_INTEGRITY_VIOLATION"));
        const [rejectedPartialRows] = await database.query(`SELECT table_name AS tableName
         FROM information_schema.tables
         WHERE table_schema = ?
           AND table_name IN (${identityTables.map(() => "?").join(",")})`, [databaseName, ...identityTables]);
        assert.deepEqual(rejectedPartialRows.map((row) => String(row.tableName)), ["organization_identity_team_guards"]);
        const [preservedGuardRows] = await database.query(`SELECT team_id AS teamId,guard_version AS guardVersion
         FROM organization_identity_team_guards`);
        assert.deepEqual(preservedGuardRows.map((row) => ({
            teamId: String(row.teamId),
            guardVersion: Number(row.guardVersion)
        })), [{ teamId: "identity-partial-state", guardVersion: 1 }]);
        await database.query(`DELETE FROM organization_identity_team_guards
       WHERE team_id = 'identity-partial-state'`);
        await ensureOrganizationIdentitySchema(pool);
        stage = "partial schema rejects before creating missing tables";
        await dropIdentitySchema(database);
        await database.query(`ALTER TABLE prospect_source_raw_hits
       DROP FOREIGN KEY fk_ps_raw_hit_record`);
        await database.query(`ALTER TABLE prospect_source_raw_records
       DROP INDEX uk_ps_raw_record_team_owner_id`);
        await database.query(`CREATE TABLE organizations (
         id VARCHAR(91) PRIMARY KEY
       )`);
        await assertColdStartRejected("IDENTITY_DATA_INTEGRITY_VIOLATION");
        const [partialTableRows] = await database.query(`SELECT table_name AS tableName
       FROM information_schema.tables
       WHERE table_schema = ?
         AND table_name IN (${identityTables.map(() => "?").join(",")})`, [databaseName, ...identityTables]);
        assert.deepEqual(partialTableRows.map((row) => String(row.tableName)), ["organizations"]);
        const [rawOwnerIndexRows] = await database.query(`SELECT column_name AS columnName,non_unique AS nonUnique
       FROM information_schema.statistics
       WHERE table_schema = ?
         AND table_name = 'prospect_source_raw_records'
         AND index_name = 'uk_ps_raw_record_team_owner_id'
       ORDER BY seq_in_index`, [databaseName]);
        assert.deepEqual(rawOwnerIndexRows.map((row) => String(row.columnName)), ["team_id", "owner_id", "id"]);
        assert.ok(rawOwnerIndexRows.every((row) => Number(row.nonUnique) === 0));
        ...
        await assertSchemaMutationRejected(async () => {
            await database.query(`ALTER TABLE prospect_source_raw_records
        ...
        await assertSchemaMutationRejected(async () => {
            await database.query(`ALTER TABLE organization_identity_team_guards ENGINE=MyISAM`);
        }, async () => {
            await database.query(`ALTER TABLE organization_identity_team_guards ENGINE=InnoDB`);
        });
        stage = "identity column drift rejection";
        await assertSchemaMutationRejected(async () => {
            await database.query(`ALTER TABLE organization_identity_team_guards
        ...
        await assertSchemaMutationRejected(async () => {
            await database.query(`ALTER TABLE organization_identity_claims
        ...
        await assertSchemaMutationRejected(async () => {
            await database.query(`ALTER TABLE organization_identity_events
        ...
        await database.query(`INSERT INTO organization_identity_authority_profiles (
         profile_code,profile_version,canonical_json,profile_hash,
         profile_mac,created_at,row_mac
       ) VALUES (?,?,?,REPEAT('a',64),REPEAT('b',64),
         UTC_TIMESTAMP(3),REPEAT('c',64))`, ["identity-orphan-profile", "v1", "{}"]);
        try {
            await assertColdStartRejected("IDENTITY_DATA_INTEGRITY_VIOLATION");
        }
        finally {
            await database.query(`DELETE FROM organization_identity_authority_profiles
        ...
        await assert.rejects(ensureOrganizationIdentitySchema(acquisitionFailurePool), /forced schema lock acquisition response loss/);
        ...
        await assert.rejects(ensureOrganizationIdentitySchema(releaseFailurePool), isIdentityError("IDENTITY_DATA_INTEGRITY_VIOLATION"));
        assert.equal(schemaConnectionDestroyed, true);
        assert.equal(schemaConnectionReleased, false);
        await ensureOrganizationIdentitySchema(pool);
        for (const released of [0, null]) {
            ...
            await assert.rejects(ensureOrganizationIdentitySchema(uncertainReleasePool), isIdentityError("IDENTITY_DATA_INTEGRITY_VIOLATION"));
        ...
        for (const table of identityTables) {
            const [emptyRows] = await database.query(`SELECT COUNT(*) AS count FROM \`${table}\``);
            assert.equal(Number(emptyRows[0]?.count || 0), 0, `Expected empty identity table before first DML: ${table}`);
        }
        stage = "synchronous resolver rejection and zero writes";
        const syncRaw = createRawFixture("identity-sync-team", "owner-a");
        assert.throws(() => resolveOrganizationStrongIdentity(primaryStore, {
        ...
        const first = await resolvePersisted(primaryStore, sharedRawA, sharedClaims);
        ...
        const firstTeamSnapshot = snapshotTeamIdentity(primaryStore, sharedRawA.teamId);
        const coldStore = await createMysqlStore();
        assert.deepEqual(snapshotTeamIdentity(coldStore, sharedRawA.teamId), firstTeamSnapshot);
        stage = "runtime metadata deletion fails closed";
        const [metadataRows] = await database.query(`SELECT * FROM organization_identity_contract_metadata
       WHERE id = 1`);
        assert.equal(metadataRows.length, 1);
        const metadataRow = metadataRows[0];
        await database.query("DELETE FROM organization_identity_contract_metadata WHERE id = 1");
        const missingMetadataRaw = createRawFixture("identity-metadata-missing-team", "owner-a");
        await insertRawRecord(database, missingMetadataRaw);
        const missingMetadataSnapshot = await databaseIdentitySnapshot(database);
        try {
            await assert.rejects(resolvePersisted(coldStore, missingMetadataRaw, [
                registration("METADATA-MISSING-100")
            ]), isIdentityError("IDENTITY_DATA_INTEGRITY_VIOLATION"));
            assert.deepEqual(await databaseIdentitySnapshot(database), missingMetadataSnapshot);
        }
        finally {
            await database.query(`INSERT INTO organization_identity_contract_metadata (
        ...
        await assert.rejects(resolvePersisted(coldStore, sharedRawA, [
            ...sharedClaims,
            legalName("Changed Parser Output GmbH")
        ]), isIdentityError("IDENTITY_CLAIM_REPLAY_CONFLICT"));
        assert.deepEqual(await databaseIdentitySnapshot(database), factsBeforeReplayConflict);
        ...
        assert.equal(secondOwner.resolution.organizationId, first.resolution.organizationId);
        assert.ok(secondOwner.claims.every((claim) => claim.ownerId === "owner-b"));
        assert.equal(coldStore.organizations.filter((item) => item.teamId === "identity-shared-team").length, 1);
        await assert.rejects(resolvePersisted(coldStore, { ...sharedRawA, ownerId: "owner-b" }, [lei("5493001KJTIIGC8Y1R12")]), isIdentityError("IDENTITY_INVALID"));
        ...
        assert.notEqual(crossTeam.resolution.organizationId, first.resolution.organizationId);
        ...
        assert.equal(coldStore.organizationSourceBindings.filter((item) => item.rawRecordId === parserRaw.record.id).length, 1);
        stage = "binding conflict without partial structural writes";
        const bindingSourceRaw = createRawFixture("identity-binding-team", "owner-source");
        const bindingTargetRaw = createRawFixture("identity-binding-team", "owner-target");
        await insertRawRecord(database, bindingSourceRaw);
        await insertRawRecord(database, bindingTargetRaw);
        const bindingSource = await resolvePersisted(coldStore, bindingSourceRaw, [registration("BINDING-SOURCE-100")]);
        const bindingTarget = await resolvePersisted(coldStore, bindingTargetRaw, [registration("BINDING-TARGET-200")]);
        const bindingStructuralTables = [
            "organizations",
            "organization_accepted_identifiers",
            "organization_source_bindings"
        ];
        const bindingStructuralDatabaseBefore = await databaseTableSnapshot(database, bindingStructuralTables);
        const bindingConflict = await resolvePersisted(coldStore, bindingSourceRaw, [registration("BINDING-TARGET-200")], { parserVersion: "identity-parser-v2" });
        ...
        assert.notEqual(bindingSource.resolution.organizationId, bindingTarget.resolution.organizationId);
        assert.deepEqual(await databaseTableSnapshot(database, bindingStructuralTables), bindingStructuralDatabaseBefore);
        const bindingConflictColdStore = await createMysqlStore();
        assert.ok(bindingConflictColdStore.organizationIdentityConflicts.some((item) => item.id === bindingConflict.conflict?.id));
        const bindingConflictReplay = await resolvePersisted(bindingConflictColdStore, bindingSourceRaw, [registration("BINDING-TARGET-200")], { parserVersion: "identity-parser-v2" });
        assert.equal(bindingConflictReplay.idempotent, true);
        assert.equal(bindingConflictReplay.binding, null);
        assert.deepEqual(bindingConflictReplay.resolution, bindingConflict.resolution);
        ...
        const splitStructuralBefore = await databaseTableSnapshot(database, bindingStructuralTables);
        ...
        assert.deepEqual(await databaseTableSnapshot(database, bindingStructuralTables), splitStructuralBefore);
        const splitConflictColdStore = await createMysqlStore();
        assert.ok(splitConflictColdStore.organizationIdentityConflicts.some((item) => item.id === splitConflict.conflict?.id));
        stage = "authority profile drift rejection";
        const [profileRows] = await database.query(`SELECT canonical_json AS canonicalJson
       FROM organization_identity_authority_profiles
       WHERE profile_code = ? AND profile_version = ?`, ["identity-authority-test", "v1"]);
        const canonicalProfile = String(profileRows[0]?.canonicalJson || "");
        await database.query(`UPDATE organization_identity_authority_profiles
       SET canonical_json = CONCAT(canonical_json, ' ')
       WHERE profile_code = ? AND profile_version = ?`, ["identity-authority-test", "v1"]);
        await assertColdStartRejected("IDENTITY_CONFIGURATION_INVALID");
        await database.query(`UPDATE organization_identity_authority_profiles
       SET canonical_json = ?
       WHERE profile_code = ? AND profile_version = ?`, [canonicalProfile, "identity-authority-test", "v1"]);
        ...
        const [organizationRows] = await database.query(`SELECT id,legal_name_encrypted AS encrypted,row_mac AS rowMac
       FROM organizations ORDER BY created_at,id LIMIT 1`);
        ...
        await database.query("UPDATE organizations SET legal_name_encrypted = ? WHERE id = ?", [`${encryptedName}A`, tamperOrganizationId]);
        await assertColdStartRejected("IDENTITY_DATA_INTEGRITY_VIOLATION");
        await database.query(`UPDATE organizations
       SET legal_name_encrypted = ?,row_mac = ? WHERE id = ?`, [encryptedName, organizationRowMac, tamperOrganizationId]);
        stage = "row mac tamper rejection";
        await database.query("UPDATE organizations SET row_mac = ? WHERE id = ?", ["f".repeat(64), tamperOrganizationId]);
        await assertColdStartRejected("IDENTITY_DATA_INTEGRITY_VIOLATION");
        await database.query("UPDATE organizations SET row_mac = ? WHERE id = ?", [organizationRowMac, tamperOrganizationId]);
        stage = "relation tamper rejection";
        const [relationRows] = await database.query(`SELECT team_id AS teamId,owner_id AS ownerId,
       ...
       ORDER BY resolution_id,ordinal LIMIT 1`);
        ...
        const changedRole = relation.role === "accepted_new"
            ? "accepted_existing"
            : "accepted_new";
        await database.query(`UPDATE organization_identity_resolution_identifiers
       SET relation_role = ?
       ...
       WHERE team_id = ? AND owner_id = ?
        ...
        const [eventRows] = await database.query(`SELECT id,event_hash AS eventHash,row_mac AS rowMac
       FROM organization_identity_events
       ORDER BY created_at,id LIMIT 1`);
        const event = eventRows[0];
        await database.query("UPDATE organization_identity_events SET event_hash = ? WHERE id = ?", ["e".repeat(64), event.id]);
        await assertColdStartRejected("IDENTITY_DATA_INTEGRITY_VIOLATION");
        await database.query(`UPDATE organization_identity_events
       SET event_hash = ?,row_mac = ? WHERE id = ?`, [event.eventHash, event.rowMac, event.id]);
        stage = "two stale stores serialize same team identity";
        const concurrentRawA = createRawFixture("identity-concurrent-team", "owner-a");
        const concurrentRawB = createRawFixture("identity-concurrent-team", "owner-b");
        ...
        assert.deepEqual(concurrentResults.map((item) => item.resolution.result).sort(), ["exact_match", "new_entity"]);
        const concurrentColdStore = await createMysqlStore();
        assert.equal(concurrentColdStore.organizations.filter((item) => item.teamId === "identity-concurrent-team").length, 1);
        ...
        const retryInput = persistedInput(retryRaw, [registration("RETRY-100")]);
        ...
        const retryResult = await resolveOrganizationStrongIdentityMysql(retryPool, concurrentColdStore, retryInput);
        assert.equal(retryFailures, 2);
        assert.equal(retryInputMutated, true);
        assert.equal(retryResult.resolution.result, "new_entity");
        const retryState = await loadOrganizationIdentityState(pool, undefined, retryRaw.teamId);
        assert.equal(retryState.organizationAcceptedIdentifiers[0]?.normalizedValue, "RETRY-100");
        const retryCeilingRaw = createRawFixture("identity-retry-ceiling-team", "owner-a");
        ...
        await assert.rejects(resolveOrganizationStrongIdentityMysql(ceilingPool, concurrentColdStore, persistedInput(retryCeilingRaw, [registration("RETRY-CEILING-100")])), isIdentityError("IDENTITY_CONCURRENCY_RETRY_EXHAUSTED"));
        assert.equal(ceilingFailures, 3);
        stage = "decision closure stays team scoped and selection bound";
        const closureRaw = createRawFixture("identity-decision-closure-team", "owner-a");
        await insertRawRecord(database, closureRaw);
        const decisionFactSelects = new Map();
            ...
            connection: {
                query(sql, proceed) {
                    ...
                    if (!decisionWriteCommitted) {
                        const table = identityFactTables.find((candidate) => normalizedSql.startsWith(`SELECT * FROM ${candidate} `));
                        if (table) {
        ...
        const closureResult = await resolveOrganizationStrongIdentityMysql(closurePool, concurrentColdStore, persistedInput(closureRaw, [registration("CLOSURE-100")]));
        assert.equal(closureResult.resolution.result, "new_entity");
        assert.deepEqual([...decisionFactSelects.keys()].sort(), [...identityFactTables].sort());
        for (const queries of decisionFactSelects.values()) {
            assert.equal(queries.every((sql) => sql.includes(" WHERE team_id = ? AND ")), true);
        }
        const namedDuplicateStore = await createMysqlStore();
        stage = "named duplicate processing key replay";
        const processingReplayRaw = createRawFixture("identity-duplicate-processing-replay-team", "owner-a");
        await insertRawRecord(database, processingReplayRaw);
        const processingReplayInput = persistedInput(processingReplayRaw, [registration("DUPLICATE-PROCESSING-REPLAY-100")]);
        let processingReplayWinnerSnapshot = null;
        const processingReplayRace = namedDuplicateRacePool(pool, {
            ...
            async afterRollback() {
                const winner = await resolveOrganizationStrongIdentityMysql(pool, namedDuplicateStore, processingReplayInput);
        ...
        const processingReplayResult = await resolveOrganizationStrongIdentityMysql(processingReplayRace.pool, namedDuplicateStore, processingReplayInput);
        ...
        assert.deepEqual(await databaseIdentityFactSnapshot(database), processingReplayWinnerSnapshot);
        stage = "named duplicate processing key changed claim conflict";
        const processingConflictRaw = createRawFixture("identity-duplicate-processing-conflict-team", "owner-a");
        await insertRawRecord(database, processingConflictRaw);
        const processingConflictWinnerInput = persistedInput(processingConflictRaw, [registration("DUPLICATE-PROCESSING-WINNER-100")]);
        const processingConflictLoserInput = {
            ...processingConflictWinnerInput,
            claims: [registration("DUPLICATE-PROCESSING-LOSER-200")]
        };
        let processingConflictWinnerSnapshot = null;
        const processingConflictRace = namedDuplicateRacePool(pool, {
            table: "organization_identity_resolutions",
            key: "uk_oi_resolution_processing",
            async afterRollback() {
                await resolveOrganizationStrongIdentityMysql(pool, namedDuplicateStore, processingConflictWinnerInput);
                processingConflictWinnerSnapshot =
                    await databaseIdentityFactSnapshot(database);
            }
        });
        await assert.rejects(resolveOrganizationStrongIdentityMysql(processingConflictRace.pool, namedDuplicateStore, processingConflictLoserInput), isIdentityError("IDENTITY_CLAIM_REPLAY_CONFLICT"));
        ...
        assert.deepEqual(await databaseIdentityFactSnapshot(database), processingConflictWinnerSnapshot);
        stage = "named duplicate identifier exact match and owner isolation";
        const identifierExactLoserRaw = createRawFixture("identity-duplicate-identifier-exact-team", "owner-loser");
        const identifierExactWinnerRaw = createRawFixture("identity-duplicate-identifier-exact-team", "owner-winner");
        ...
        const identifierExactLoserInput = persistedInput(identifierExactLoserRaw, identifierExactClaims);
        const identifierExactWinnerInput = persistedInput(identifierExactWinnerRaw, identifierExactClaims);
        let identifierExactWinnerOrganizationId = "";
        const identifierExactRace = namedDuplicateRacePool(pool, {
            table: "organization_accepted_identifiers",
            key: "uk_oi_identifier_lookup",
            async afterRollback() {
                const winner = await resolveOrganizationStrongIdentityMysql(pool, namedDuplicateStore, identifierExactWinnerInput);
                identifierExactWinnerOrganizationId =
                    winner.resolution.organizationId;
            }
        });
        const identifierExactResult = await resolveOrganizationStrongIdentityMysql(identifierExactRace.pool, namedDuplicateStore, identifierExactLoserInput);
        ...
        assert.equal(identifierExactResult.resolution.organizationId, identifierExactWinnerOrganizationId);
        ...
        assert.ok(identifierExactResult.claims.every((item) => item.ownerId === identifierExactLoserRaw.ownerId));
        assert.ok(identifierExactResult.events.every((item) => item.ownerId === identifierExactLoserRaw.ownerId));
        assert.equal(identifierExactResult.binding?.ownerId, identifierExactLoserRaw.ownerId);
        assert.equal(namedDuplicateStore.organizations.filter((item) => item.teamId === identifierExactLoserRaw.teamId).length, 1);
        assert.equal(namedDuplicateStore.organizationIdentityResolutions.filter((item) => item.teamId === identifierExactLoserRaw.teamId).length, 2);
        stage = "named duplicate identifier binding conflict";
        const identifierBindingRaw = createRawFixture("identity-duplicate-identifier-binding-team", "owner-binding");
        const identifierBindingTargetRaw = createRawFixture("identity-duplicate-identifier-binding-team", "owner-target");
        const identifierBindingWinnerRaw = createRawFixture("identity-duplicate-identifier-binding-team", "owner-winner");
        ...
        const identifierBindingSeed = await resolvePersisted(namedDuplicateStore, identifierBindingRaw, [lei("529900T8BM49AURSDO55")]);
        const identifierBindingTarget = await resolvePersisted(namedDuplicateStore, identifierBindingTargetRaw, [lei("213800D1EI4B9WTWWD28")]);
        assert.notEqual(identifierBindingSeed.resolution.organizationId, identifierBindingTarget.resolution.organizationId);
        const identifierBindingLoserInput = persistedInput(identifierBindingRaw, [registration("DUPLICATE-IDENTIFIER-BINDING-B-200")], { parserVersion: "identity-parser-v2" });
        const identifierBindingWinnerInput = persistedInput(identifierBindingWinnerRaw, [
            lei("213800D1EI4B9WTWWD28"),
            registration("DUPLICATE-IDENTIFIER-BINDING-B-200")
        ]);
        let identifierBindingStructuralAfterWinner = null;
        const identifierBindingRace = namedDuplicateRacePool(pool, {
            table: "organization_accepted_identifiers",
            key: "organization_accepted_identifiers.uk_oi_identifier_lookup",
            async afterRollback() {
                const winner = await resolveOrganizationStrongIdentityMysql(pool, namedDuplicateStore, identifierBindingWinnerInput);
        ...
        const identifierBindingConflict = await resolveOrganizationStrongIdentityMysql(identifierBindingRace.pool, namedDuplicateStore, identifierBindingLoserInput);
        ...
        assert.equal(identifierBindingConflict.resolution.result, "conflict");
        assert.equal(identifierBindingConflict.conflict?.conflictType, "binding_conflict");
        ...
        assert.deepEqual(await databaseTableSnapshot(database, bindingStructuralTables), identifierBindingStructuralAfterWinner);
        stage = "named duplicate identifier hash collision";
        const identifierCollisionLoserRaw = createRawFixture("identity-duplicate-identifier-collision-team", "owner-loser");
        const identifierCollisionWinnerRaw = createRawFixture("identity-duplicate-identifier-collision-team", "owner-winner");
        ...
        const identifierCollisionLoserInput = persistedInput(identifierCollisionLoserRaw, identifierCollisionClaims);
        const identifierCollisionWinnerInput = persistedInput(identifierCollisionWinnerRaw, identifierCollisionClaims);
        let identifierCollisionWinnerSnapshot = null;
        const identifierCollisionRace = namedDuplicateRacePool(pool, {
            table: "organization_accepted_identifiers",
            key: "uk_oi_identifier_lookup",
            async afterRollback() {
                const winner = await resolveOrganizationStrongIdentityMysql(pool, namedDuplicateStore, identifierCollisionWinnerInput);
                assert.equal(winner.createdIdentifiers.length, 1);
                await rewriteIdentifierNormalizedValue(database, winner.createdIdentifiers[0].id, "DIFFERENT-FULL-VALUE");
                identifierCollisionWinnerSnapshot =
                    await databaseIdentityFactSnapshot(database);
            }
        });
        await assert.rejects(resolveOrganizationStrongIdentityMysql(identifierCollisionRace.pool, namedDuplicateStore, identifierCollisionLoserInput), isIdentityError("IDENTITY_IDENTIFIER_HASH_COLLISION"));
        ...
        assert.deepEqual(await databaseIdentityFactSnapshot(database), identifierCollisionWinnerSnapshot);
        stage = "named duplicate active binding reuse";
        const bindingReuseSeedRaw = createRawFixture("identity-duplicate-binding-reuse-team", "owner-seed");
        const bindingReuseRaw = createRawFixture("identity-duplicate-binding-reuse-team", "owner-binding");
        await insertRawRecord(database, bindingReuseSeedRaw);
        await insertRawRecord(database, bindingReuseRaw);
        await resolvePersisted(namedDuplicateStore, bindingReuseSeedRaw, [registration("DUPLICATE-BINDING-REUSE-100")]);
        const bindingReuseLoserInput = persistedInput(bindingReuseRaw, [registration("DUPLICATE-BINDING-REUSE-100")]);
        const bindingReuseWinnerInput = persistedInput(bindingReuseRaw, [registration("DUPLICATE-BINDING-REUSE-100")], { parserVersion: "identity-parser-v2" });
        let bindingReuseWinnerBindingId = "";
        let bindingReuseStructuralAfterWinner = null;
        const bindingReuseRace = namedDuplicateRacePool(pool, {
            ...
            async afterRollback() {
                const winner = await resolveOrganizationStrongIdentityMysql(pool, namedDuplicateStore, bindingReuseWinnerInput);
                bindingReuseWinnerBindingId = winner.binding?.id || "";
                bindingReuseStructuralAfterWinner =
                    await databaseTableSnapshot(database, bindingStructuralTables);
            }
        });
        const bindingReuseResult = await resolveOrganizationStrongIdentityMysql(bindingReuseRace.pool, namedDuplicateStore, bindingReuseLoserInput);
        ...
        assert.equal(bindingReuseResult.resolution.bindingRelationRole, "reused_existing");
        assert.equal(bindingReuseResult.binding?.id, bindingReuseWinnerBindingId);
        assert.ok(bindingReuseStructuralAfterWinner);
        assert.deepEqual(await databaseTableSnapshot(database, bindingStructuralTables), bindingReuseStructuralAfterWinner);
        stage = "named duplicate active binding conflict";
        const bindingConflictSeedARaw = createRawFixture("identity-duplicate-binding-conflict-team", "owner-seed-a");
        const bindingConflictSeedBRaw = createRawFixture("identity-duplicate-binding-conflict-team", "owner-seed-b");
        const bindingConflictRaceRaw = createRawFixture("identity-duplicate-binding-conflict-team", "owner-binding");
        ...
        await resolvePersisted(namedDuplicateStore, bindingConflictSeedARaw, [registration("DUPLICATE-BINDING-CONFLICT-A-100")]);
        await resolvePersisted(namedDuplicateStore, bindingConflictSeedBRaw, [registration("DUPLICATE-BINDING-CONFLICT-B-200")]);
        const bindingConflictLoserInput = persistedInput(bindingConflictRaceRaw, [registration("DUPLICATE-BINDING-CONFLICT-A-100")]);
        const bindingConflictWinnerInput = persistedInput(bindingConflictRaceRaw, [registration("DUPLICATE-BINDING-CONFLICT-B-200")], { parserVersion: "identity-parser-v2" });
        let bindingConflictStructuralAfterWinner = null;
        const bindingConflictRace = namedDuplicateRacePool(pool, {
            table: "organization_source_bindings",
            key: "organization_source_bindings.uk_oi_binding_active_raw",
            async afterRollback() {
                await resolveOrganizationStrongIdentityMysql(pool, namedDuplicateStore, bindingConflictWinnerInput);
                bindingConflictStructuralAfterWinner =
                    await databaseTableSnapshot(database, bindingStructuralTables);
            }
        });
        const namedBindingConflict = await resolveOrganizationStrongIdentityMysql(bindingConflictRace.pool, namedDuplicateStore, bindingConflictLoserInput);
        ...
        assert.equal(namedBindingConflict.conflict?.conflictType, "binding_conflict");
        assert.equal(namedBindingConflict.binding, null);
        assert.ok(bindingConflictStructuralAfterWinner);
        assert.deepEqual(await databaseTableSnapshot(database, bindingStructuralTables), bindingConflictStructuralAfterWinner);
        stage = "named duplicate fail closed matrix";
        const failClosedCases = [
            {
                label: "claim ordinal",
                table: "organization_identity_claims",
                duplicate: mysqlDuplicateError("organization_identity_claims.uk_oi_claim_resolution_ordinal")
            },
            {
                label: "event ordinal",
                table: "organization_identity_events",
                duplicate: mysqlDuplicateError("organization_identity_events.uk_oi_event_resolution_sequence")
            },
            {
            ...
            },
            {
                ...
                duplicate: mysqlDuplicateError("uk_oi_resolution_processing", {
                    sqlMessage: "Duplicate entry 'sensitive-value' "
            ...
            },
            {
                ...
                table: "organization_identity_resolutions",
                duplicate: mysqlDuplicateError("uk_oi_resolution_processing", {
                    sqlMessage: "Synthetic duplicate 'sensitive-value' "
                        + "for key 'uk_oi_resolution_processing'"
                })
            },
            {
            ...
            },
            {
                ...
                table: "organization_identity_resolutions",
                duplicate: mysqlDuplicateError("organization_accepted_identifiers.uk_oi_identifier_lookup")
            },
            {
        ...
        for (const [index, item] of failClosedCases.entries()) {
            ...
            const raw = createRawFixture(`identity-duplicate-fail-closed-${index}`, "owner-a");
            ...
            await assert.rejects(resolveOrganizationStrongIdentityMysql(failClosedPool, namedDuplicateStore, persistedInput(raw, [
            ...
            assert.deepEqual(await databaseIdentityFactSnapshot(database), before);
        }
        stage = "named duplicate rollback failure destroys connection";
        const rollbackFailureRaw = createRawFixture("identity-duplicate-rollback-failure-team", "owner-a");
        await insertRawRecord(database, rollbackFailureRaw);
        const rollbackFailureBefore = await databaseIdentityFactSnapshot(database);
            ...
            connection: {
                query(sql, proceed) {
                    if (!rollbackDuplicateInjected
                        ...
                        throw mysqlDuplicateError("organization_identity_resolutions"
        ...
        await assert.rejects(resolveOrganizationStrongIdentityMysql(rollbackFailurePool, namedDuplicateStore, persistedInput(rollbackFailureRaw, [
        ...
        assert.deepEqual(await databaseIdentityFactSnapshot(database), rollbackFailureBefore);
        stage = "named duplicate recovery second duplicate fails closed";
        const secondDuplicateLoserRaw = createRawFixture("identity-duplicate-second-team", "owner-loser");
        const secondDuplicateWinnerRaw = createRawFixture("identity-duplicate-second-team", "owner-winner");
        ...
        const secondDuplicateLoserInput = persistedInput(secondDuplicateLoserRaw, secondDuplicateClaims);
        const secondDuplicateWinnerInput = persistedInput(secondDuplicateWinnerRaw, secondDuplicateClaims);
        let firstDuplicateInjected = false;
        let secondDuplicateInjected = false;
        let secondDuplicateWinnerCommitted = false;
        let secondDuplicateWinnerSnapshot = null;
        const secondDuplicatePool = instrumentPool(pool, {
            connection: {
                query(sql, proceed) {
                    if (!firstDuplicateInjected
                        && isInsertInto(sql, "organization_accepted_identifiers")) {
                        firstDuplicateInjected = true;
                        throw mysqlDuplicateError("organization_accepted_identifiers"
                ...
                async rollback(proceed) {
                    ...
                    if (firstDuplicateInjected && !secondDuplicateWinnerCommitted) {
                        await resolveOrganizationStrongIdentityMysql(pool, namedDuplicateStore, secondDuplicateWinnerInput);
        ...
        await assert.rejects(resolveOrganizationStrongIdentityMysql(secondDuplicatePool, namedDuplicateStore, secondDuplicateLoserInput), isIdentityError("IDENTITY_DATA_INTEGRITY_VIOLATION"));
        ...
        assert.deepEqual(await databaseIdentityFactSnapshot(database), secondDuplicateWinnerSnapshot);
        stage = "named duplicate recovery rollback failure destroys connection";
        const recoveryRollbackLoserRaw = createRawFixture("identity-duplicate-recovery-rollback-team", "owner-loser");
        const recoveryRollbackWinnerRaw = createRawFixture("identity-duplicate-recovery-rollback-team", "owner-winner");
        ...
        const recoveryRollbackLoserInput = persistedInput(recoveryRollbackLoserRaw, recoveryRollbackClaims);
        const recoveryRollbackWinnerInput = persistedInput(recoveryRollbackWinnerRaw, recoveryRollbackClaims);
        ...
        let recoveryRollbackWinnerSnapshot = null;
            ...
            connection: {
                ...
                async rollback(proceed) {
                    ...
                    if (recoveryRollbackCalls === 1) {
                        await proceed();
                        await resolveOrganizationStrongIdentityMysql(pool, namedDuplicateStore, recoveryRollbackWinnerInput);
        ...
        await assert.rejects(resolveOrganizationStrongIdentityMysql(recoveryRollbackPool, namedDuplicateStore, recoveryRollbackLoserInput), isIdentityError("IDENTITY_DATA_INTEGRITY_VIOLATION"));
        ...
        assert.deepEqual(await databaseIdentityFactSnapshot(database), recoveryRollbackWinnerSnapshot);
        stage = "named duplicate recovery database error is redacted";
        const recoveryRedactionLoserRaw = createRawFixture("identity-duplicate-recovery-redaction-team", "owner-loser");
        const recoveryRedactionWinnerRaw = createRawFixture("identity-duplicate-recovery-redaction-team", "owner-winner");
        ...
        const recoveryRedactionLoserInput = persistedInput(recoveryRedactionLoserRaw, recoveryRedactionClaims);
        const recoveryRedactionWinnerInput = persistedInput(recoveryRedactionWinnerRaw, recoveryRedactionClaims);
        ...
        let recoveryRedactionWinnerSnapshot = null;
            ...
            connection: {
                query(sql, proceed) {
                    ...
                    if (recoveryRedactionWinnerCommitted
                        ...
                        throw Object.assign(new Error("sensitive-value organization_identity_resolutions"), {
                            code: "ER_BAD_FIELD_ERROR",
                            errno: 1054,
                            sql: "INSERT INTO secret_table VALUES ('sensitive-value')",
                            sqlMessage: "Unknown column 'private_column' in 'secret_table'"
                        });
                    }
                    return proceed();
                },
                async rollback(proceed) {
                    ...
                    if (recoveryRedactionDuplicateInjected
                        ...
                        await resolveOrganizationStrongIdentityMysql(pool, namedDuplicateStore, recoveryRedactionWinnerInput);
        ...
        await assert.rejects(resolveOrganizationStrongIdentityMysql(recoveryRedactionPool, namedDuplicateStore, recoveryRedactionLoserInput), (error) => {
            if (!(error instanceof OrganizationStrongIdentityError)
                || error.code !== "IDENTITY_DATA_INTEGRITY_VIOLATION") {
                return false;
            }
            assert.equal(error.message.includes("sensitive-value"), false);
            assert.equal(error.message.includes("organization_identity_resolutions"), false);
        ...
        assert.deepEqual(await databaseIdentityFactSnapshot(database), recoveryRedactionWinnerSnapshot);
        stage = "named duplicate recovery commit response loss converges";
        const recoveryCommitLoserRaw = createRawFixture("identity-duplicate-recovery-commit-team", "owner-loser");
        const recoveryCommitWinnerRaw = createRawFixture("identity-duplicate-recovery-commit-team", "owner-winner");
        ...
        const recoveryCommitLoserInput = persistedInput(recoveryCommitLoserRaw, recoveryCommitClaims);
        const recoveryCommitWinnerInput = persistedInput(recoveryCommitWinnerRaw, recoveryCommitClaims);
            ...
            connection: {
                ...
                async rollback(proceed) {
                    ...
                    if (recoveryCommitDuplicateInjected
                        ...
                        await resolveOrganizationStrongIdentityMysql(pool, namedDuplicateStore, recoveryCommitWinnerInput);
        ...
        const recoveryCommitResult = await resolveOrganizationStrongIdentityMysql(recoveryCommitPool, namedDuplicateStore, recoveryCommitLoserInput);
        assert.equal(recoveryCommitResult.resolution.result, "exact_match");
        ...
        const recoveryCommitState = await loadOrganizationIdentityState(pool, undefined, recoveryCommitLoserRaw.teamId);
        ...
        const recoveryAbsentLoserRaw = createRawFixture("identity-duplicate-recovery-absent-team", "owner-loser");
        const recoveryAbsentWinnerRaw = createRawFixture("identity-duplicate-recovery-absent-team", "owner-winner");
        ...
        const recoveryAbsentLoserInput = persistedInput(recoveryAbsentLoserRaw, recoveryAbsentClaims);
        const recoveryAbsentWinnerInput = persistedInput(recoveryAbsentWinnerRaw, recoveryAbsentClaims);
            ...
            connection: {
                ...
                async rollback(proceed) {
                    ...
                    if (recoveryAbsentDuplicateInjected
                        ...
                        await resolveOrganizationStrongIdentityMysql(pool, namedDuplicateStore, recoveryAbsentWinnerInput);
                ...
                async commit(proceed) {
                    ...
                    if (!recoveryAbsentCommitFailed) {
                        ...
                        recoveryAbsentLoserInput.claims[0].normalizedValue =
                            "DUPLICATE-RECOVERY-ABSENT-MUTATED-100";
                        throw new Error("forced named duplicate recovery commit before persistence");
                    }
                    await proceed();
                },
                destroy() {
        ...
        const recoveryAbsentResult = await resolveOrganizationStrongIdentityMysql(recoveryAbsentPool, namedDuplicateStore, recoveryAbsentLoserInput);
        ...
        const recoveryAbsentState = await loadOrganizationIdentityState(pool, undefined, recoveryAbsentLoserRaw.teamId);
        assert.equal(recoveryAbsentState.organizations.length, 1);
        assert.equal(recoveryAbsentState.organizationAcceptedIdentifiers[0]?.normalizedValue, "DUPLICATE-RECOVERY-ABSENT-100");
        assert.equal(recoveryAbsentState.organizationIdentityResolutions.length, 2);
        assert.equal(recoveryAbsentState.organizationIdentityResolutions.filter((item) => item.rawRecordId === recoveryAbsentLoserRaw.record.id).length, 1);
        stage = "named duplicate cross team non leakage";
        const crossTeamDuplicateLoserRaw = createRawFixture("identity-duplicate-isolated-loser-team", "owner-a");
        const crossTeamDuplicateWinnerRaw = createRawFixture("identity-duplicate-isolated-winner-team", "owner-a");
        ...
        const crossTeamDuplicateLoserInput = persistedInput(crossTeamDuplicateLoserRaw, crossTeamDuplicateClaims);
        const crossTeamDuplicateWinnerInput = persistedInput(crossTeamDuplicateWinnerRaw, crossTeamDuplicateClaims);
        let crossTeamDuplicateWinnerSnapshot = null;
        const crossTeamDuplicateRace = namedDuplicateRacePool(pool, {
            table: "organization_accepted_identifiers",
            key: "organization_accepted_identifiers.uk_oi_identifier_lookup",
            async afterRollback() {
                await resolveOrganizationStrongIdentityMysql(pool, namedDuplicateStore, crossTeamDuplicateWinnerInput);
                crossTeamDuplicateWinnerSnapshot =
                    await databaseIdentityFactSnapshot(database);
            }
        });
        await assert.rejects(resolveOrganizationStrongIdentityMysql(crossTeamDuplicateRace.pool, namedDuplicateStore, crossTeamDuplicateLoserInput), isIdentityError("IDENTITY_DATA_INTEGRITY_VIOLATION"));
        ...
        assert.deepEqual(await databaseIdentityFactSnapshot(database), crossTeamDuplicateWinnerSnapshot);
        assert.equal(namedDuplicateStore.organizations.some((item) => item.teamId === crossTeamDuplicateLoserRaw.teamId), false);
        assert.equal(namedDuplicateStore.organizations.some((item) => item.teamId === crossTeamDuplicateWinnerRaw.teamId), true);
        stage = "unknown commit before persistence retries frozen command";
        const absentCommitRaw = createRawFixture("identity-absent-commit-team", "owner-a");
        await insertRawRecord(database, absentCommitRaw);
        const absentCommitInput = persistedInput(absentCommitRaw, [registration("ABSENT-COMMIT-100")]);
        ...
        const absentCommitResult = await resolveOrganizationStrongIdentityMysql(absentCommitPool, concurrentColdStore, absentCommitInput);
        ...
        const absentCommitState = await loadOrganizationIdentityState(pool, undefined, absentCommitRaw.teamId);
        assert.equal(absentCommitState.organizationAcceptedIdentifiers[0]?.normalizedValue, "ABSENT-COMMIT-100");
        assert.equal(absentCommitState.organizationIdentityResolutions.length, 1);
        stage = "unknown commit after persistence recovers frozen command";
        const unknownCommitRaw = createRawFixture("identity-unknown-commit-team", "owner-a");
        await insertRawRecord(database, unknownCommitRaw);
        const unknownCommitInput = persistedInput(unknownCommitRaw, [registration("UNKNOWN-COMMIT-100")]);
        ...
        const recovered = await resolveOrganizationStrongIdentityMysql(unknownCommitPool, concurrentColdStore, unknownCommitInput);
        ...
        const recoveredState = await loadOrganizationIdentityState(pool, undefined, unknownCommitRaw.teamId);
        assert.equal(recoveredState.organizationAcceptedIdentifiers[0]?.normalizedValue, "UNKNOWN-COMMIT-100");
        stage = "guard version rejects stale cache refresh";
        const cacheVersionRawA = createRawFixture("identity-cache-version-team", "owner-a");
        const cacheVersionRawB = createRawFixture("identity-cache-version-team", "owner-b");
            ...
            connection: {
                async commit(proceed) {
                    ...
                    if (cacheCommitOrdinal === 2) {
                        const newer = await resolveOrganizationStrongIdentityMysql(pool, cacheVersionStore, persistedInput(cacheVersionRawB, [registration("CACHE-VERSION-B-100")]));
        ...
        const older = await resolveOrganizationStrongIdentityMysql(cacheVersionPool, cacheVersionStore, persistedInput(cacheVersionRawA, [registration("CACHE-VERSION-A-100")]));
        assert.equal(older.resolution.result, "new_entity");
        assert.equal(newerRefreshCompleted, true);
        assert.equal(cacheVersionStore.organizations.filter((item) => item.teamId === cacheVersionRawA.teamId).length, 2);
        assert.equal(cacheVersionStore.organizationIdentityResolutions.filter((item) => item.teamId === cacheVersionRawA.teamId).length, 2);
        stage = "cache refresh transient failure retries before success";
        const refreshRaw = createRawFixture("identity-refresh-failure-team", "owner-a");
        ...
        const refreshResult = await resolveOrganizationStrongIdentityMysql(refreshFailurePool, concurrentColdStore, persistedInput(refreshRaw, [registration("REFRESH-100")]));
        assert.equal(refreshResult.resolution.result, "new_entity");
        assert.equal(refreshFaultInjected, true);
        assert.equal(concurrentColdStore.organizations.some((item) => item.teamId === refreshRaw.teamId), true);
        assert.equal(concurrentColdStore.organizationIdentityResolutions.filter((item) => item.teamId === refreshRaw.teamId).length, 1);
        stage = "cache unavailable preserves snapshot and idempotent retry";
        const persistentRefreshStore = await createMysqlStore();
        const persistentRefreshRawA = createRawFixture("identity-refresh-persistent-team", "owner-a");
        const persistentRefreshRawB = createRawFixture("identity-refresh-persistent-team", "owner-b");
        await insertRawRecord(database, persistentRefreshRawA);
        await insertRawRecord(database, persistentRefreshRawB);
        await resolveOrganizationStrongIdentityMysql(pool, persistentRefreshStore, persistedInput(persistentRefreshRawA, [registration("REFRESH-PERSISTENT-100")]));
        assert.equal(persistentRefreshStore.organizations.filter((item) => item.teamId === persistentRefreshRawA.teamId).length, 1);
        ...
        const persistentRefreshInput = persistedInput(persistentRefreshRawB, [registration("REFRESH-PERSISTENT-200")]);
        await assert.rejects(resolveOrganizationStrongIdentityMysql(persistentRefreshPool, persistentRefreshStore, persistentRefreshInput), isIdentityError("IDENTITY_CACHE_UNAVAILABLE"));
        assert.equal(persistentRefreshFailures, 2);
        assert.equal(persistentRefreshStore.organizations.filter((item) => item.teamId === persistentRefreshRawA.teamId).length, 1);
        const persistentDatabaseState = await loadOrganizationIdentityState(pool, undefined, persistentRefreshRawA.teamId);
        assert.equal(persistentDatabaseState.organizations.length, 2);
        assert.deepEqual(new Set(persistentDatabaseState.organizationIdentityClaims.map((item) => item.ownerId)), new Set(["owner-a", "owner-b"]));
        const persistentRefreshRetry = await resolveOrganizationStrongIdentityMysql(pool, persistentRefreshStore, persistentRefreshInput);
        assert.equal(persistentRefreshRetry.idempotent, true);
        assert.equal(persistentRefreshStore.organizations.filter((item) => item.teamId === persistentRefreshRawA.teamId).length, 2);
        assert.equal((await loadOrganizationIdentityState(pool, undefined, persistentRefreshRawA.teamId)).organizations.length, 2);
        ...
        assert.deepEqual(await databaseIdentitySnapshot(database), identityBeforePersist);
        stage = "business arrays remain unchanged";
        assert.deepEqual(snapshotBusinessArrays(primaryStore), businessBefore);
        await database.end();
        await pool.end();
        console.log("Organization Identity MySQL schema, persistence, isolation, "
        ...
        if (databaseCreated) {
            if (grantedAccount) {
                ...
                const accountUser = grantedAccount.slice(0, separator);
                const accountHost = grantedAccount.slice(separator + 1);
                await admin.query(`REVOKE ALL PRIVILEGES ON \`${databaseName}\`.* FROM ${sqlString(accountUser)}@${sqlString(accountHost)}`).catch(() => undefined);
            }
