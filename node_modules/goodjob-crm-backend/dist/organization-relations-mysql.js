import { buildOrganizationAliasFact, buildOrganizationRelationFact, organizationAliasFactHash, OrganizationRelationError, organizationRelationFactHash } from "./organization-relations.js";
function iso(value) {
...
export async function ensureOrganizationRelationSchema(pool) {
    await pool.query(`
...
export async function loadOrganizationRelationState(source, teamId) {
    const where = teamId ? " WHERE team_id = ?" : "";
    const values = teamId ? [teamId] : [];
    const [aliasRows, relationRows] = await Promise.all([
        rows(source, `SELECT * FROM organization_alias_facts${where}
       ORDER BY team_id, observed_at, id`, values),
        rows(source, `SELECT * FROM organization_relation_facts${where}
       ORDER BY team_id, observed_at, id`, values)
    ]);
    return {
...
function replaceTeamState(store, teamId, state) {
    store.organizationAliasFacts.splice(0, store.organizationAliasFacts.length, ...store.organizationAliasFacts.filter((item) => item.teamId !== teamId), ...structuredClone(state.organizationAliasFacts));
    store.organizationRelationFacts.splice(0, store.organizationRelationFacts.length, ...store.organizationRelationFacts.filter((item) => item.teamId !== teamId), ...structuredClone(state.organizationRelationFacts));
}
export async function ensureOrganizationRelationTeamCache(pool, store, teamId) {
    replaceTeamState(store, teamId, await loadOrganizationRelationState(pool, teamId));
}
async function requirePersistedOrganization(connection, teamId, organizationId) {
    const found = await rows(connection, `SELECT id FROM organizations
     WHERE id = ? AND team_id = ?
     FOR UPDATE`, [organizationId, teamId]);
    if (!found.length) {
        throw new OrganizationRelationError("ORGANIZATION_NOT_FOUND", "企业不存在或无权访问", 404);
    }
}
export async function recordOrganizationAliasMysql(pool, store, input) {
    const connection = await pool.getConnection();
    let built = null;
    try {
        await connection.beginTransaction();
        const persisted = await loadOrganizationRelationState(connection, input.user.teamId);
        ...
        await requirePersistedOrganization(connection, input.user.teamId, built.alias.organizationId);
        if (!built.replayed) {
            const alias = built.alias;
            await connection.query(`INSERT INTO organization_alias_facts (
    ...
    if (!built)
        throw new Error("企业别名事务未生成结果");
    const state = await loadOrganizationRelationState(pool, input.user.teamId);
    replaceTeamState(store, input.user.teamId, state);
    const alias = state.organizationAliasFacts.find((item) => item.factKeyHash === built.alias.factKeyHash);
    if (!alias)
        throw new Error("企业别名提交后无法读取");
    return { alias, replayed: built.replayed };
}
export async function recordOrganizationRelationMysql(pool, store, input) {
    const connection = await pool.getConnection();
    let built = null;
    try {
        await connection.beginTransaction();
        ...
        await requirePersistedOrganization(connection, input.user.teamId, built.relation.sourceOrganizationId);
        await requirePersistedOrganization(connection, input.user.teamId, built.relation.targetOrganizationId);
        if (!built.replayed) {
            const relation = built.relation;
            await connection.query(`INSERT INTO organization_relation_facts (
    ...
    if (!built)
        throw new Error("企业关系事务未生成结果");
    const state = await loadOrganizationRelationState(pool, input.user.teamId);
    replaceTeamState(store, input.user.teamId, state);
    const relation = state.organizationRelationFacts.find((item) => item.factKeyHash === built.relation.factKeyHash);
    if (!relation)
        throw new Error("企业关系提交后无法读取");
