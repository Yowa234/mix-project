...
export function signToken(user) {
    return jwt.sign({ ver: user.authVersion }, jwtSecret(), {
        subject: user.id,
        issuer: TOKEN_ISSUER,
...
export async function verifyPassword(stored, supplied) {
    ...
    const actual = await scrypt(supplied, Buffer.from(saltValue, "base64url"), expected.length);
    return {
...
export function canSeeTeam(user, teamId) {
...
export function canManageAccount(operator, target) {
    if (operator.role === "super_admin")
        return true;
    if (operator.role !== "admin")
        return false;
