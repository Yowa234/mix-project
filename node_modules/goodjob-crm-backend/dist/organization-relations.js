...
const hierarchicalRelationTypes = new Set([
    "direct_parent",
    "ultimate_parent",
    "branch_of",
    "brand_of"
...
export function organizationAliasFactHash(fact) {
    return sha256({
        contract: "organization-alias-fact-v1",
        ...fact
    });
}
export function organizationRelationFactHash(fact) {
    return sha256({
        contract: "organization-relation-fact-v1",
        ...fact
    });
}
function requireProfileReader(user) {
    if (!["sales", "manager", "admin"].includes(user.role)) {
        throw new OrganizationRelationError("ORGANIZATION_PROFILE_FORBIDDEN", "企业画像只允许当前团队成员读取", 403);
    }
}
function requireFactWriter(user) {
    if (user.role !== "manager" && user.role !== "admin") {
        throw new OrganizationRelationError("ORGANIZATION_FACT_WRITE_FORBIDDEN", "只有本团队主管或管理员可以维护企业别名和集团关系", 403);
    }
}
function organizationForTeam(store, user, organizationId) {
    const canonicalId = canonicalOrganizationId(store, user.teamId, organizationId);
    const organization = store.organizations.find((item) => item.id === canonicalId && item.teamId === user.teamId);
    if (!organization) {
        throw new OrganizationRelationError("ORGANIZATION_NOT_FOUND", "企业不存在或无权访问", 404);
    }
    return organization;
}
function aliasFactKey(teamId, organizationId, body, normalizedAlias) {
    return sha256({
...
function relationFactKey(teamId, sourceOrganizationId, targetOrganizationId, body) {
    return sha256({
...
export function buildOrganizationAliasFact(store, input) {
    requireFactWriter(input.user);
    const organization = organizationForTeam(store, input.user, input.organizationId);
    const normalizedAlias = normalizeAlias(input.body.aliasName);
    const factKeyHash = aliasFactKey(input.user.teamId, organization.id, input.body, normalizedAlias);
    const existing = store.organizationAliasFacts.find((item) => item.teamId === input.user.teamId
        && item.factKeyHash === factKeyHash);
    if (existing)
        return { alias: existing, replayed: true };
...
function relationEdges(store, teamId) {
    return store.organizationRelationFacts
        .filter((item) => item.teamId === teamId
        && hierarchicalRelationTypes.has(item.relationType))
        .map((item) => ({
        source: canonicalOrganizationId(store, teamId, item.sourceOrganizationId),
        target: canonicalOrganizationId(store, teamId, item.targetOrganizationId)
    }))
        .filter((item) => item.source !== item.target);
}
function hasPath(edges, start, destination) {
...
export function buildOrganizationRelationFact(store, input) {
    requireFactWriter(input.user);
    const source = organizationForTeam(store, input.user, input.body.sourceOrganizationId);
    const target = organizationForTeam(store, input.user, input.body.targetOrganizationId);
    if (source.id === target.id) {
        throw new OrganizationRelationError("ORGANIZATION_RELATION_INVALID", "规范企业不能与自身建立集团关系", 400);
    }
    const factKeyHash = relationFactKey(input.user.teamId, source.id, target.id, input.body);
    const existing = store.organizationRelationFacts.find((item) => item.teamId === input.user.teamId
        && item.factKeyHash === factKeyHash);
    if (existing)
        return { relation: existing, replayed: true };
    if (hierarchicalRelationTypes.has(input.body.relationType)) {
        const conflicting = store.organizationRelationFacts.find((item) => item.teamId === input.user.teamId
            && item.sourceOrganizationId === source.id
            && item.relationType === input.body.relationType
            && item.targetOrganizationId !== target.id);
        if (conflicting) {
            throw new OrganizationRelationError("ORGANIZATION_RELATION_CONFLICT", "该企业已存在同类型但目标不同的集团关系，请先人工复核", 409);
        }
        if (hasPath(relationEdges(store, input.user.teamId), target.id, source.id)) {
            throw new OrganizationRelationError("ORGANIZATION_RELATION_CYCLE", "本次集团关系会形成层级循环，已拒绝保存", 409);
        }
    }
    const createdAt = new Date().toISOString();
    const base = {
...
export async function recordOrganizationAlias(store, input) {
    ...
    try {
        ...
        store.organizationAliasFacts.splice(store.organizationAliasFacts.indexOf(result.alias), 1);
        throw error;
    }
    return result;
}
export async function recordOrganizationRelation(store, input) {
    ...
    try {
        ...
        store.organizationRelationFacts.splice(store.organizationRelationFacts.indexOf(result.relation), 1);
        throw error;
    }
    return result;
}
export function organizationIdentityProfile(store, user, organizationId) {
    ...
    const aliases = store.organizationAliasFacts
        ...
        && canonicalOrganizationId(store, user.teamId, item.organizationId) === organization.id)
        .sort((left, right) => right.observedAt.localeCompare(left.observedAt));
    const relations = store.organizationRelationFacts
        .filter((item) => item.teamId === user.teamId)
        .map((item) => ({
        fact: item,
        sourceOrganizationId: canonicalOrganizationId(store, user.teamId, item.sourceOrganizationId),
        targetOrganizationId: canonicalOrganizationId(store, user.teamId, item.targetOrganizationId)
    }))
        .filter((item) => item.sourceOrganizationId !== item.targetOrganizationId
        ...
        const related = store.organizations.find((candidate) => candidate.id === relatedId
