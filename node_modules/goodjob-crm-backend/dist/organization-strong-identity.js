...
import { readProspectSourceRawRecord } from "./prospect-source-raw.js";
export const ORGANIZATION_STRONG_IDENTITY_CONTRACT = "organization-strong-identity-v1";
const MAX_CLAIMS = 200;
const MAX_TEXT_LENGTH = 2_000;
const activeTeamLocks = new Set();
...
function factConflict(message) {
    throw new OrganizationStrongIdentityError("IDENTITY_FACT_CONFLICT", message);
}
...
function factHash(secret, value) {
...
function normalizeDomain(value) {
    ...
    try {
        const url = new URL(/^[a-z][a-z0-9+.-]*:\/\//iu.test(raw)
            ? raw
            : `https://${raw}`);
...
export function normalizeOrganizationIdentityAuthorityProfile(profile) {
    const normalized = {
        ...
        allowMultiIdentifierSubjectBinding: profile.allowMultiIdentifierSubjectBinding === true,
        rules: profile.rules
    };
    ...
    const rules = profile.rules.map((rule) => {
        ...
        return {
            ...
            jurisdictions: [...new Set(rule.jurisdictions.map((item) => item === "*" ? "*" : normalizeJurisdiction(item)))].sort(),
            entityTypes: [...new Set(rule.entityTypes)].sort(),
            normalizerVersions: [...new Set(rule.normalizerVersions.map((item) => requireText(item, "权威规范化器版本", 200)))].sort(),
            validatorVersions: [...new Set(rule.validatorVersions.map((item) => requireText(item, "权威校验器版本", 200)))].sort()
        };
...
export function organizationIdentityAuthorityProfileHash(profile) {
    return sha256({
...
function matchingAuthorityRule(profile, claim, sourceMatches) {
    if (!sourceMatches
    ...
    return profile.rules.some((rule) => rule.kind === claim.kind
...
function normalizeClaims(claims, profile, source) {
    if (!Array.isArray(claims) || claims.length > MAX_CLAIMS) {
        invalid("企业身份声明数量无效或超过上限");
    }
    const sourceMatches = profile.providerCode === source.providerCode
    ...
    const prepared = claims.map((claim) => {
        ...
        const normalizerVersion = requireText(claim.normalizerVersion, "声明规范化器版本", 200);
        const validatorVersion = requireText(claim.validatorVersion, "声明校验器版本", 200);
        ...
        let scheme = "";
        let jurisdiction = "";
        let entityType = claim.entityType || "unknown";
        if (claim.kind === "legal_name") {
            ...
            || claim.kind === "vat") {
            normalizedValue = normalizeCanonicalIdentifier(claim.normalizedValue || "");
            scheme = normalizeNamespace(claim.scheme || "", "企业标识体系");
    ...
    const initiallyEligible = prepared.filter((claim) => claim.authorityEligible);
    if (initiallyEligible.length > 1) {
        const subjects = new Set(initiallyEligible.map((claim) => claim.subjectRef));
        if (!profile.allowMultiIdentifierSubjectBinding
            || subjects.size !== 1) {
            for (const claim of prepared)
                claim.authorityEligible = false;
        }
    }
    const byCanonicalClaim = new Map();
    for (const claim of prepared) {
        const key = canonical({
            kind: claim.kind,
...
function identifierKey(claim, teamId, lookupSecret) {
...
function identifierSlotKey(value) {
    return canonical({
        kind: value.kind,
        scheme: value.scheme,
...
function identifierExactKey(value) {
    return canonical(value);
}
function organizationWithoutHash(organization) {
    const { organizationHash: _hash, ...withoutHash } = organization;
    return withoutHash;
}
function claimWithoutHash(claim) {
    const { claimFactHash: _hash, ...withoutHash } = claim;
    return withoutHash;
}
function identifierWithoutHash(identifier) {
    const { identifierHash: _hash, ...withoutHash } = identifier;
    return withoutHash;
}
function resolutionWithoutHash(resolution) {
...
function conflictWithoutHash(conflict) {
    const { conflictHash: _hash, ...withoutHash } = conflict;
    return withoutHash;
}
export function organizationIdentityResolutionRelations(resolution) {
    ...
    const bindings = resolution.bindingId && resolution.bindingRelationRole
        ? [{
...
export function organizationIdentityResolutionRelationHash(resolution, integritySecret) {
...
function conflictKeyType(identifierKey) {
    if (identifierKey.startsWith("raw-binding:"))
        return "raw_binding";
    try {
        const parsed = JSON.parse(identifierKey);
...
export function organizationIdentityConflictRelations(conflict, matchedOrganizationIds, existingBindingOrganizationId) {
    const organizations = [
        ...[...new Set(matchedOrganizationIds)].map((organizationId) => ({
            organizationId,
            role: "identifier_match"
        })),
        ...(existingBindingOrganizationId
            ? [{
                    organizationId: existingBindingOrganizationId,
                    role: "existing_binding"
                }]
            : [])
    ]
        .filter((item) => conflict.organizationIds.includes(item.organizationId))
        .sort((left, right) => `${left.role}:${left.organizationId}`
...
export function organizationIdentityConflictRelationHash(relations, integritySecret) {
    return factHash(integritySecret, {
        contract: ORGANIZATION_STRONG_IDENTITY_CONTRACT,
...
function replayResult(store, resolution, integritySecret) {
    ...
    const createdIdentifiers = store.organizationAcceptedIdentifiers
        .filter((item) => claims.some((claim) => claim.id === item.sourceClaimId));
    const binding = resolution.bindingId
        ? store.organizationSourceBindings.find((item) => item.id === resolution.bindingId
            && item.teamId === resolution.teamId
            && item.ownerId === resolution.ownerId) || null
        : null;
    const conflict = resolution.conflictId
        ? store.organizationIdentityConflicts.find((item) => item.id === resolution.conflictId
            && item.teamId === resolution.teamId
            && item.ownerId === resolution.ownerId) || null
        : null;
    ...
    const createdOrganization = resolution.result === "new_entity"
        ? store.organizations.find((item) => item.id === resolution.organizationId
            && item.teamId === resolution.teamId) || null
        : null;
...
function integrityViolation(message) {
...
export function validateOrganizationIdentityFacts(store, integritySecret) {
    const groups = [
    ...
    const resolutionByScope = new Map(store.organizationIdentityResolutions.map((item) => [
        `${item.teamId}\u0000${item.ownerId}\u0000${item.id}`,
        item
    ]));
    ...
    for (const claim of store.organizationIdentityClaims) {
        if (!resolutionByScope.has(`${claim.teamId}\u0000${claim.ownerId}\u0000${claim.resolutionId}`)
            || !rawByScope.has(`${claim.teamId}\u0000${claim.ownerId}\u0000${claim.rawRecordId}`)
    ...
    for (const identifier of store.organizationAcceptedIdentifiers) {
        const claim = claimByScope.get(`${identifier.teamId}\u0000${identifier.sourceOwnerId}`
            + `\u0000${identifier.sourceClaimId}`);
        if (!organizationByScope.has(`${identifier.teamId}\u0000${identifier.organizationId}`)
            || !claim
            || claim.rawRecordId !== identifier.sourceRawRecordId
            || !rawByScope.has(`${identifier.teamId}\u0000${identifier.sourceOwnerId}`
    ...
    for (const binding of store.organizationSourceBindings) {
        if (!resolutionByScope.has(`${binding.teamId}\u0000${binding.ownerId}`
            + `\u0000${binding.resolutionId}`)
            || !organizationByScope.has(`${binding.teamId}\u0000${binding.organizationId}`)
            || !rawByScope.has(`${binding.teamId}\u0000${binding.ownerId}`
                + `\u0000${binding.rawRecordId}`)
    ...
    for (const conflict of store.organizationIdentityConflicts) {
        if (!resolutionByScope.has(`${conflict.teamId}\u0000${conflict.ownerId}`
            + `\u0000${conflict.resolutionId}`)
            || !rawByScope.has(`${conflict.teamId}\u0000${conflict.ownerId}`
                + `\u0000${conflict.rawRecordId}`)
            || conflict.organizationIds.some((organizationId) => !organizationByScope.has(`${conflict.teamId}\u0000${organizationId}`))
    ...
    for (const resolution of store.organizationIdentityResolutions) {
        ...
        const claims = store.organizationIdentityClaims
            .filter((item) => item.teamId === resolution.teamId
            && item.ownerId === resolution.ownerId
            && item.resolutionId === resolution.id)
            .sort((left, right) => left.ordinal - right.ordinal);
        if (!claims.length
            || claims.some((claim, index) => claim.ordinal !== index + 1
        ...
        const recomputedClaimHash = claimSetHash(claims.map((claim) => ({
            ...
            validatorVersion: claim.validatorVersion,
            observedAt: claim.observedAt,
            authorityEligible: claim.classification === "strong_identifier_eligible"
        })));
        if (recomputedClaimHash !== resolution.claimHash
            || resolution.relationHash
                !== organizationIdentityResolutionRelationHash(resolution, integritySecret)) {
            integrityViolation("Identity Resolution 声明或关系摘要不一致");
        }
        if (resolution.organizationId
            && !organizationByScope.has(`${resolution.teamId}\u0000${resolution.organizationId}`)) {
            integrityViolation("Identity Resolution Organization 引用无效");
        }
        if (resolution.bindingId
            && !store.organizationSourceBindings.some((item) => item.id === resolution.bindingId
                && item.teamId === resolution.teamId
                && item.ownerId === resolution.ownerId)) {
            integrityViolation("Identity Resolution Binding 引用无效");
        }
        if (resolution.conflictId
            && !store.organizationIdentityConflicts.some((item) => item.id === resolution.conflictId
                && item.teamId === resolution.teamId
                && item.ownerId === resolution.ownerId)) {
            integrityViolation("Identity Resolution Conflict 引用无效");
        }
        try {
            replayResult(store, resolution, integritySecret);
        }
        catch (error) {
            if (error instanceof OrganizationStrongIdentityError) {
    ...
    const resolutionIds = new Set(store.organizationIdentityResolutions.map((item) => `${item.teamId}\u0000${item.ownerId}\u0000${item.id}`));
    if (store.organizationIdentityEvents.some((event) => !resolutionIds.has(`${event.teamId}\u0000${event.ownerId}\u0000${event.resolutionId}`))) {
        integrityViolation("Identity Event 存在孤立 Resolution 引用");
    }
}
function withTeamLock(teamId, operation) {
    if (activeTeamLocks.has(teamId)) {
        throw new OrganizationStrongIdentityError("IDENTITY_TEAM_BUSY", "当前团队正在提交企业身份事实");
    }
...
function commitFacts(store, facts) {
    const lengths = {
...
export function resolveOrganizationStrongIdentity(store, rawInput) {
    if (store.mode === "mysql") {
        throw new OrganizationStrongIdentityError("IDENTITY_MYSQL_TRANSACTION_REQUIRED", "MySQL 企业身份事实只能通过专用事务提交");
    }
    if (rawInput.resolverVersion !== ORGANIZATION_STRONG_IDENTITY_CONTRACT) {
        invalid("企业强身份 Resolver 合同版本无效");
    }
    const identitySecret = requireText(rawInput.identitySecret, "企业身份幂等密钥", 10_000);
    ...
    const deterministicIdSecret = rawInput.deterministicIdSecret || identitySecret;
    ...
    const rawRecordId = requireText(rawInput.rawRecordId, "Provider 原始记录标识", 500);
    const parserVersion = requireText(rawInput.parserVersion, "企业身份解析器版本", 200);
    const normalizerVersion = requireText(rawInput.normalizerVersion, "企业身份规范化版本", 200);
    const resolvedAt = normalizeIso(rawInput.resolvedAt, "企业身份处理时间");
    const authorityProfile = normalizeOrganizationIdentityAuthorityProfile(rawInput.authorityProfile);
    const authorityProfileHash = organizationIdentityAuthorityProfileHash(authorityProfile);
    ...
    const normalizedClaims = normalizeClaims(rawInput.claims, authorityProfile, immutableRaw);
    ...
    return withTeamLock(teamId, () => {
        const existingResolutions = store.organizationIdentityResolutions.filter((item) => item.processingKeyHash === processingKeyHash
            && item.teamId === teamId
        ...
        if (existingResolutions.length === 1) {
            ...
            if (existing.claimHash !== claimHash) {
                throw new OrganizationStrongIdentityError("IDENTITY_CLAIM_REPLAY_CONFLICT", "同一 Provider 原始工件在相同处理版本下产生了不同身份声明");
            }
            return replayResult(store, existing, factIntegritySecret);
        }
        const resolutionId = `oir_${processingKeyHash.slice(0, 40)}`;
        if (store.organizationIdentityResolutions.some((item) => item.id === resolutionId)) {
            factConflict("企业身份 Resolution 标识已被其它事实占用");
        }
        const claimRows = normalizedClaims.map((claim, index) => {
            const row = {
                id: `oic_${sha256({
        ...
        const eligibleClaims = normalizedClaims.filter((claim) => claim.authorityEligible);
        const eligibleKeys = eligibleClaims.map((claim) => identifierKey(claim, teamId, identifierLookupSecret));
        const exactKeyToClaim = new Map();
        for (const claim of claimRows) {
            if (claim.classification !== "strong_identifier_eligible")
                continue;
            const normalized = normalizedClaims[claim.ordinal - 1];
            exactKeyToClaim.set(identifierExactKey(identifierKey(normalized, teamId, identifierLookupSecret)), claim);
        }
        const existingBindings = store.organizationSourceBindings.filter((item) => item.teamId === teamId
            && item.ownerId === ownerId
        ...
        for (const key of eligibleKeys) {
            const hashMatches = store.organizationAcceptedIdentifiers.filter((item) => item.teamId === teamId
                && item.status === "active"
                && item.kind === key.kind
            ...
            for (const identifier of hashMatches) {
                if (identifier.normalizedValue !== key.normalizedValue) {
                    throw new OrganizationStrongIdentityError("IDENTITY_IDENTIFIER_HASH_COLLISION", "企业强标识摘要冲突，已拒绝自动合并");
                }
                matchedIdentifiers.push(identifier);
                matchedOrganizationIds.add(identifier.organizationId);
            }
        }
        const duplicateStoredIdentifier = matchedIdentifiers.some((identifier, index) => matchedIdentifiers.some((other, otherIndex) => otherIndex !== index
        ...
        for (const key of eligibleKeys) {
            ...
            const values = incomingBySlot.get(slot) || new Set();
        ...
        if (!conflictType && targetOrganizationId) {
            const existingTarget = store.organizations.filter((item) => item.id === targetOrganizationId
                && item.teamId === teamId
                && item.status === "active");
            if (existingTarget.length !== 1) {
                factConflict("企业强标识引用的 Organization 不存在或不唯一");
            }
            const targetIdentifiers = store.organizationAcceptedIdentifiers.filter((item) => item.organizationId === targetOrganizationId
                && item.teamId === teamId
                && item.status === "active");
            for (const key of eligibleKeys) {
                const slot = identifierSlotKey(key);
                const incompatible = targetIdentifiers.some((item) => identifierSlotKey(item) === slot
        ...
        if (!eligibleClaims.length) {
            ...
            conflict.relationHash = organizationIdentityConflictRelationHash(organizationIdentityConflictRelations(conflict, [...matchedOrganizationIds], existingBinding?.organizationId || ""), factIntegritySecret);
            ...
            if (!targetOrganizationId) {
                const firstKey = [...eligibleKeys]
                    .sort((left, right) => identifierExactKey(left).localeCompare(identifierExactKey(right)))[0];
                targetOrganizationId = `org_${hmac(deterministicIdSecret, {
                    version: "organization-id-v1",
                    teamId,
                    firstIdentifier: firstKey
                }).slice(0, 40)}`;
                if (store.organizations.some((item) => item.id === targetOrganizationId)) {
                    factConflict("Organization 标识已被其它事实占用");
                }
                const subjectRef = eligibleClaims[0]?.subjectRef || "";
                const nameClaim = normalizedClaims.find((claim) => claim.kind === "legal_name"
                    && (!subjectRef || claim.subjectRef === subjectRef)) || normalizedClaims.find((claim) => claim.kind === "legal_name");
                const organization = {
            ...
            for (const key of eligibleKeys) {
                const existing = matchedIdentifiers.find((item) => item.organizationId === targetOrganizationId
                    && item.kind === key.kind
                    && item.scheme === key.scheme
                    && item.jurisdiction === key.jurisdiction
                    && item.normalizedValue === key.normalizedValue);
                ...
                if (store.organizationAcceptedIdentifiers.some((item) => item.id === identifier.id)) {
            ...
            if (!binding) {
                ...
                if (store.organizationSourceBindings.some((item) => item.id === bindingId)) {
        ...
        const resolution = {
            ...
            bindingId: result === "new_entity" || result === "exact_match"
                ...
                : "",
            conflictId,
            matchedIdentifierIds: [...new Set(matchedIdentifiers.map((item) => item.id))].sort(),
            acceptedIdentifierIds,
            bindingRelationRole: (result === "new_entity" || result === "exact_match") && binding
        ...
        resolution.relationHash = organizationIdentityResolutionRelationHash(resolution, factIntegritySecret);
        const stagedEvents = [];
