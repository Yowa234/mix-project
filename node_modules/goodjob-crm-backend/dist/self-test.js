...
if (!address || typeof address === "string")
    throw new Error("Cannot start test server");
...
try {
    ...
    if (!customerFollow.response.ok || customerFollow.json.customer.nextReminder !== "明天 14:00")
        throw new Error("customer activity create failed");
    const customersAfterFollow = await request("/api/customers", { headers: { authorization: `Bearer ${salesToken}` } });
    const followedCustomer = customersAfterFollow.json.customers.find((customer) => customer.id === "c1");
    if (followedCustomer?.ownerName !== "Shirley" || followedCustomer?.activities?.[0]?.content !== followContent)
        throw new Error("customer activity must remain readable with real owner");
    ...
    if (crossCustomerFollow.response.status !== 404)
        throw new Error("sales customer activity must be owner isolated");
    ...
    if (!boundaryCustomer.response.ok)
        throw new Error("boundary customer create failed");
    ...
    if (!managerBoundaryTodo.response.ok || !salesBoundaryTodo.response.ok)
        throw new Error("boundary todo create failed");
    const boundaryDelete = await request("/api/customers/bulk-delete", {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` },
        body: JSON.stringify({ ids: [boundaryCustomer.json.customer.id] })
    });
    if (!boundaryDelete.response.ok)
        throw new Error("boundary customer delete failed");
    ...
    if (!managerTodosAfterBoundaryDelete.json.todos.some((todo) => todo.id === managerBoundaryTodo.json.todo.id))
        throw new Error("customer delete must not remove manager personal todo");
    if (salesTodosAfterBoundaryDelete.json.todos.some((todo) => todo.id === salesBoundaryTodo.json.todo.id))
        throw new Error("customer delete should remove current user's related todo");
    ...
    if (!recognizedOcr.response.ok)
        throw new Error("ocr recognize failed");
    ...
    if (!ocrRepeat.response.ok || !ocrRepeat.json.duplicate || ocrRepeat.json.lead.id !== ocr.json.lead.id)
        throw new Error("ocr sync must be idempotent");
    if (!leadsAfterOcr.json.leads.some((lead) => lead.id === ocr.json.lead.id))
        throw new Error("ocr lead must be persisted");
    const managerOcr = await request("/api/tools/ocr/jobs/ocr1", {
        headers: { authorization: `Bearer ${managerToken}` }
    });
    if (!managerOcr.response.ok || managerOcr.json.job.ownerId !== "u_manager_alex")
        throw new Error("manager personal ocr job failed");
    const crossOcr = await request(`/api/tools/ocr/jobs/${managerOcr.json.job.id}`, {
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (crossOcr.response.status !== 404)
        throw new Error("ocr job must be personal isolated");
    ...
    if (invalidOcr.response.status !== 400)
        throw new Error("ocr recognition input must be bounded");
    ...
    if (!aiConfig.response.ok || aiConfig.json.config?.apiKey !== "****1234" || !aiConfig.json.config?.hasApiKey)
    ...
    if (!aiConfigSecond.response.ok || (aiConfigSecond.json.configs || []).length < 2 || aiConfigSecond.json.config?.apiKey !== "****5678")
        throw new Error("ai multi config save failed");
    const aiConfigRead = await request("/api/tools/ai-config", {
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!aiConfigRead.response.ok || (aiConfigRead.json.configs || []).length < 2 || aiConfigRead.json.config?.apiKey !== "****5678")
        throw new Error("ai config read failed");
    const aiConfigDelete = await request(`/api/tools/ai-config/${aiConfig.json.config.id}`, {
        method: "DELETE",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!aiConfigDelete.response.ok || (aiConfigDelete.json.configs || []).some((item) => item.id === aiConfig.json.config.id) || aiConfigDelete.json.config?.apiKey !== "****5678")
        throw new Error("ai config delete failed");
    ...
    if (!leadCreate.response.ok || leadCreate.json.lead?.company !== "自动化线索工作区 GmbH")
        throw new Error("lead workspace create failed");
    ...
    const activityTypes = (leadDetail.json.activities || []).map((activity) => activity.type);
    const sourceEvent = (leadDetail.json.sourceEvents || []).find((event) => event.leadId === testLeadId);
    const testLeadOriginalStatus = leadDetail.json.lead?.status;
    if (!leadDetail.response.ok
    ...
    if (!matchingCustomer.response.ok)
        throw new Error("matching customer create failed");
    ...
    if (!existingConversion.response.ok || existingConversion.json.deal?.customerId !== matchingCustomer.json.customer.id)
        throw new Error("existing customer conversion failed");
    const customersAfterExistingConversion = await request("/api/customers", { headers: { authorization: `Bearer ${salesToken}` } });
    if (customersAfterExistingConversion.json.customers.length !== customersBeforeExistingConversion.json.customers.length)
        throw new Error("existing conversion must not duplicate customer");
    const aggregatedCustomer = customersAfterExistingConversion.json.customers.find((customer) => customer.id === matchingCustomer.json.customer.id);
    ...
    if (convertedLeadDelete.response.status !== 400)
        throw new Error("converted lead must not be deleted");
    ...
    if (!leadTrashList.json.leads.some((lead) => lead.id === testLeadId))
        throw new Error("lead trash list failed");
    const leadActiveList = await request("/api/leads", { headers: { authorization: `Bearer ${salesToken}` } });
    if (leadActiveList.json.leads.some((lead) => lead.id === testLeadId))
        throw new Error("trashed lead must leave active list");
    const leadRestore = await request(`/api/leads/${testLeadId}/restore`, {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!leadRestore.response.ok
    ...
    if (!leadTrashAgain.response.ok || !leadTrashAgain.json.lead?.deletedAt)
        throw new Error("lead second trash failed");
    ...
    if (websiteSyncBeforeVerify.response.status !== 400)
    ...
    if (excludeSyncedWebsite.response.status !== 400)
        throw new Error("synced opportunity must not be excluded");
    ...
    if (forgedSourceSync.response.status !== 404)
        throw new Error("unstored website opportunity must not bypass verification");
    ...
    if (shirleyOpportunities.json.opportunities.some((item) => item.id === miaOpportunity.id)) {
        throw new Error("sales must not read another salesperson website opportunities");
    }
    if (!managerOpportunities.json.opportunities.some((item) => item.id === miaOpportunity.id)) {
    ...
    if (!salesAssignees.response.ok || salesAssignees.json.assignees.length !== 0)
        throw new Error("sales must not receive prospect assignees");
    if (!managerAssignees.response.ok
        || !managerAssignees.json.assignees.some((item) => item.id === "u_sales_shirley")
        || !managerAssignees.json.assignees.some((item) => item.id === "u_sales_mia")) {
    ...
    if (salesAssign.response.status !== 403)
        throw new Error("sales must not assign prospect");
    ...
    if (managerUnknownAssign.response.status !== 400)
        throw new Error("manager must not assign outside available team");
    ...
    if (anonymousCatalog.response.status !== 401)
        throw new Error("provider catalog must require authentication");
    ...
    if (!salesCatalog.response.ok || catalogProviders.length < 8)
        throw new Error("provider catalog read failed");
    ...
    const serperCatalog = catalogProviders.find((item) => item.code === "serper");
    if (!serperCatalog
    ...
    if (!leadProviders.response.ok || providerList.length < 8)
        throw new Error("lead providers list failed");
    const freeReady = providerList.filter((item) => !item.requiresKey && item.ready).length;
    const hunterMeta = providerList.find((item) => item.id === "hunter");
    if (freeReady < 2 || !hunterMeta || hunterMeta.tier !== "paid")
        throw new Error("lead providers metadata failed");
    const aiSearchMeta = providerList.find((item) => item.id === "ai_search");
    if (!aiSearchMeta || aiSearchMeta.tier !== "ai" || aiSearchMeta.requiresKey !== false)
        throw new Error("ai search source metadata failed");
    const disabledCatalogProvider = getStore().providerCatalog.find((item) => item.code === "gleif");
    if (!disabledCatalogProvider)
        throw new Error("disabled provider status test setup failed");
    ...
    if (anonymousLeadProviders.response.status !== 401)
        throw new Error("lead providers must require authentication");
    ...
    if (anonymousLeadSourceSave.response.status !== 401)
        throw new Error("lead source config write must require authentication");
    ...
    const serperStatus = (leadSourceSave.json.providers || []).find((item) => item.id === "serper");
    ...
    const shirleyConnection = getStore().providerConnections.find((item) => item.providerId === "serper" && item.ownerId === "u_sales_shirley" && item.teamId === "europe");
    ...
    const decryptedShirleyConnection = decryptProviderConfiguration(shirleyConnection, shirleyConnection.configurationEncrypted);
    ...
    for (const tamperedContext of [
        ...
        if (!tamperBlocked)
            throw new Error("provider credential AAD tampering must be rejected");
    }
    ...
    const miaSerperBeforeSave = (miaBeforeSave.json.providers || []).find((item) => item.id === "serper");
    ...
    const miaConnection = getStore().providerConnections.find((item) => item.providerId === "serper" && item.ownerId === "u_sales_mia" && item.teamId === "europe");
    if (!miaConnection
    ...
    try {
        ...
        const firstFreeByProvider = new Map((firstFreeSearch.json.opportunities || []).map((item) => [
        ...
        const firstFreeSourceIds = new Set((firstFreeSearch.json.sourceStats || []).map((item) => item.id || ""));
        ...
        if (!freeManualEdit.response.ok)
            throw new Error("free provider opportunity manual edit failed");
        const secondFreeSearch = await freeSearchRequest();
        const secondFreeGleif = secondFreeSearch.json.opportunities
            ?.find((item) => item.sourceEvidence?.some((evidence) => evidence.providerId === "gleif"));
        const secondFreeWikidata = secondFreeSearch.json.opportunities
            ?.find((item) => item.sourceEvidence?.some((evidence) => evidence.providerId === "wikidata"));
        ...
        if (!serpapiCatalog || !gleifCatalog)
            throw new Error("provider catalog policy test setup failed");
        ...
        try {
            ...
            const temporaryConnectionIndex = getStore().providerConnections.findIndex((item) => item.providerId === "serpapi" && item.ownerId === "u_sales_shirley");
            if (temporaryConnectionIndex >= 0)
                getStore().providerConnections.splice(temporaryConnectionIndex, 1);
            await getStore().persist();
        }
        ...
        const unavailableHunterStat = unavailableHunter.json.sourceStats
            ?.find((item) => item.id === "hunter");
        const unavailableHunterLog = getStore().providerRequestLogs.find((item) => item.runId === unavailableHunter.json.runId
        ...
        const unavailableAiStat = unavailableAiSearch.json.sourceStats
            ?.find((item) => item.id === "ai_search");
        const unavailableAiLog = getStore().providerRequestLogs.find((item) => item.runId === unavailableAiSearch.json.runId
        ...
        const sameDomainDifferentCountry = {
            ...firstProviderOpportunity,
        ...
        if (!manualProviderEdit.response.ok)
            throw new Error("provider opportunity manual verification edit failed");
    ...
    if (!providerLeadSync.response.ok
        ...
        || !syncedProviderOpportunity.sourceEvidence.some((item) => item.providerId === "serper")
        || !syncedProviderOpportunity.sourceEvidence.some((item) => item.providerId === "hunter" && item.recordType === "contact_evidence")
        ...
        || !syncedProviderRawPayload.sourceEvidence.some((item) => item.providerId === "hunter" && item.recordType === "contact_evidence")) {
    ...
    if (anonymousProviderLogs.response.status !== 401)
        throw new Error("provider request logs must require authentication");
    ...
    if (!shirleyProviderLogs.response.ok
        ...
        || !loggedRequests.every((item) => item.runId === providerSearch.json.runId
            ...
            && item.durationMs >= 0
            && item.responseSize > 0)
        || !loggedRequests.some((item) => item.providerId === "serper" && item.endpointCode === "search")
        || !loggedRequests.some((item) => item.providerId === "hunter" && item.endpointCode === "enrich")) {
    ...
    if (!(connectionTestLogs.json.logs || []).some((item) => item.endpointCode === "connection_test" && item.connectionId === shirleyConnection.id && item.httpStatus === 200)) {
    ...
    if ((miaProviderLogs.json.logs || []).length)
        throw new Error("provider request logs leaked to another salesperson");
    ...
    const expectAgentJobError = (label, action) => {
        let failed = false;
        try {
            action();
        }
    ...
    expectAgentJobError("agent job must require team", () => enqueueAgentJob(agentStore, { ...baseAgentJob, teamId: "" }));
    expectAgentJobError("agent job must require owner", () => enqueueAgentJob(agentStore, { ...baseAgentJob, ownerId: "" }));
    expectAgentJobError("agent job must require idempotency key", () => enqueueAgentJob(agentStore, { ...baseAgentJob, idempotencyKey: "" }));
    expectAgentJobError("agent job owner must belong to team", () => enqueueAgentJob(agentStore, { ...baseAgentJob, teamId: "all" }));
    ...
    for (const tamperedContext of [
        ...
        expectAgentJobError("agent job AAD tampering must be rejected", () => decryptAgentJobPayload(tamperedContext, "input", encryptedInput));
    }
    ...
    expectAgentJobError("agent job parent must remain in the same team", () => enqueueAgentJob(agentStore, {
        ...baseAgentJob,
        teamId: "all",
        ownerId: "u_super_admin",
    ...
    expectAgentJobError("agent job must reject invalid retry time", () => failAgentJob(scheduledAgentJob, "PROVIDER_TIMEOUT", "not-a-date"));
    ...
    expectAgentJobError("scheduled agent job must not start before due time", () => startAgentJob(scheduledAgentJob));
    scheduledAgentJob.nextAttemptAt = new Date(Date.now() - 1_000).toISOString();
    startAgentJob(scheduledAgentJob);
    failAgentJob(scheduledAgentJob, "PROVIDER_TIMEOUT");
    if (String(scheduledAgentJob.status) !== "failed")
        throw new Error("agent job failure state transition failed");
    retryAgentJob(scheduledAgentJob);
    if (String(scheduledAgentJob.status) !== "queued")
        throw new Error("agent job manual retry failed");
    ...
    if (deadLetterAgentJob.status !== "dead_letter")
        throw new Error("agent job dead-letter transition failed");
    ...
    if (repeatCancelAgentJob.response.status !== 409)
        throw new Error("agent job repeat cancel must be rejected");
    ...
    const serperReadBack = (leadSourceReadBack.json.providers || []).find((item) => item.id === "serper");
    if (!serperReadBack?.hasApiKey || !serperReadBack?.ready)
        throw new Error("lead source config persist failed");
    ...
    const corruptedSerper = (corruptedConnectionRead.json.providers || []).find((item) => item.id === "serper");
    ...
    const serperAfterDelete = (leadSourceDelete.json.providers || []).find((item) => item.id === "serper");
    if (!leadSourceDelete.response.ok || serperAfterDelete?.hasApiKey)
        throw new Error("lead source config delete failed");
    const miaAfterShirleyDelete = await request("/api/lead-finder/providers", {
        headers: { authorization: `Bearer ${miaToken}` }
    });
    const miaSerperAfterShirleyDelete = (miaAfterShirleyDelete.json.providers || []).find((item) => item.id === "serper");
    ...
    if (!miaLeadSourceDelete.response.ok)
        throw new Error("second salesperson provider connection cleanup failed");
    const hunterLeadSourceDelete = await request("/api/lead-finder/source-config/hunter", {
        method: "DELETE",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!hunterLeadSourceDelete.response.ok)
        throw new Error("hunter provider connection cleanup failed");
    ...
    if (deals.json.deals.length < 5)
        throw new Error("manager deals scope failed");
    ...
    if (!newDeal.response.ok || newDeal.json.deal.title !== "自动化新增商机" || newDeal.json.deal.amount !== 12000)
        throw new Error("deal create failed");
    ...
    if (!editedDeal.response.ok || editedDeal.json.deal.product !== "自动化智能家居产品" || editedDeal.json.deal.amount !== 10800)
        throw new Error("deal edit failed");
    ...
    if (crossDealEdit.response.status !== 404)
        throw new Error("sales deal must be owner isolated");
    ...
    if (invalidJump.response.status !== 400)
        throw new Error("manager stage jump should require reason");
    let wonDeal = newDeal;
    for (const [index, targetStage] of ["已联系", "已报价", "样品", "谈判", "成交"].entries()) {
        ...
        if (!wonDeal.response.ok)
            throw new Error(`deal stage failed: ${targetStage}`);
    }
    if (!wonDeal.response.ok || wonDeal.json.deal.stage !== "成交")
        throw new Error("deal won stage failed");
    ...
    if (lostAfterWon.response.status !== 400)
        throw new Error("won deal should not move to lost");
    const archivedDeal = await request(`/api/deals/${newDeal.json.deal.id}/archive`, {
        method: "POST",
        headers: { authorization: `Bearer ${managerToken}` }
    });
    if (!archivedDeal.response.ok || !archivedDeal.json.deal.archivedAt)
        throw new Error("deal archive failed");
    ...
    if (!salesReport.response.ok || !managerReport.response.ok || !adminReport.response.ok)
        throw new Error("executive report request failed");
    if (salesReport.json.scope?.label !== "本人业务")
        throw new Error("sales report scope label failed");
    if (managerReport.json.scope?.label !== "本团队业务")
        throw new Error("manager report scope label failed");
    if (adminReport.json.scope?.label !== "全公司业务")
        throw new Error("admin report scope label failed");
    ...
    const salesPerformanceOwners = salesReport.json.performance.map((row) => row.owner);
    const managerPerformanceOwners = managerReport.json.performance.map((row) => row.owner);
    if (salesPerformanceOwners.some((owner) => owner !== "Shirley"))
        throw new Error("sales report leaks another salesperson");
    ...
    if (reportMoneyCollections.some((row) => !row.currency || typeof row.amount !== "number")) {
    ...
    const reportStages = { 询盘: 0.05, 已联系: 0.1, 已报价: 0.3, 样品: 0.5, 谈判: 0.7 };
    const salesDeals = await request("/api/deals", { headers: { authorization: `Bearer ${salesToken}` } });
    const expectedWeightedByCurrency = new Map();
    ...
    for (const row of salesReport.json.metrics.weightedForecast) {
    ...
    if (!Array.isArray(salesReport.json.riskRows) || salesReport.json.riskRows.some((row) => row.owner !== "Shirley")) {
    ...
    if (!commissionProduct.response.ok || commissionProduct.json.product.name !== "自动化智能家居产品")
        throw new Error("commission product create failed");
    ...
    if (!commissionRule.response.ok || commissionRule.json.rule.rate !== 0.04)
        throw new Error("commission rule create failed");
    ...
    if (invalidCommissionRule.response.status !== 400)
        throw new Error("commission rate above 100% must be rejected");
    ...
    if (!editedCommissionProduct.response.ok || editedCommissionProduct.json.product.model !== "AUTO-T-EDIT")
        throw new Error("commission product edit failed");
    ...
    if (!editedCommissionRule.response.ok || editedCommissionRule.json.rule.remark !== "自动化测试规则已编辑")
        throw new Error("commission rule edit failed");
    ...
    const commissionRecord = commissionSync.json.records?.find((item) => item.dealId === newDeal.json.deal.id);
    if (!commissionSync.response.ok || !commissionRecord)
        throw new Error("commission sync from archived deal failed");
    const crossCommissionRead = await request(`/api/commission/sales-records?month=${commissionMonth}&ownerId=u_sales_shirley`, {
        headers: { authorization: `Bearer ${miaToken}` }
    });
    if (crossCommissionRead.response.status !== 403)
        throw new Error("sales must not read another salesperson commission records");
    const adminCommissionRead = await request(`/api/commission/sales-records?month=${commissionMonth}&ownerId=u_sales_shirley`, {
        headers: { authorization: `Bearer ${adminToken}` }
    });
    if (!adminCommissionRead.response.ok || !adminCommissionRead.json.records.some((item) => item.id === commissionRecord.id) || !adminCommissionRead.json.canSelectOwner) {
    ...
    if (!confirmedCommissionRecord.response.ok || confirmedCommissionRecord.json.record.status !== "confirmed")
        throw new Error("commission record confirm failed");
    const commissionAudits = await request(`/api/commission/sales-records/${commissionRecord.id}/audits`, {
        headers: { authorization: `Bearer ${adminToken}` }
    });
    if (!commissionAudits.response.ok || !commissionAudits.json.audits.length)
        throw new Error("commission audits read failed");
    const commissionRecalc = await request("/api/commission/calculations/recalculate", {
        method: "POST",
        headers: { authorization: `Bearer ${adminToken}` },
        body: JSON.stringify({ month: commissionMonth, ownerId: "u_sales_shirley" })
    });
    const commissionCalculation = commissionRecalc.json.calculations?.find((item) => item.ownerId === archivedDeal.json.deal.ownerId);
    ...
    const calculatedItem = commissionRecalc.json.items?.find((item) => item.recordId === commissionRecord.id);
    if (!calculatedItem?.ruleSnapshotJson?.includes("82080 × 4% = 3283.2"))
        throw new Error("commission calculation snapshot must explain formula");
    ...
    if (salesManualCommission.response.status !== 403)
        throw new Error("sales must not adjust commission manually");
    ...
    if (!commissionManual.response.ok || commissionManual.json.calculation.finalCommission <= commissionCalculation.finalCommission)
        throw new Error("commission manual item failed");
    const reviewedCommission = await request(`/api/commission/calculations/${commissionCalculation.id}/review`, {
        method: "POST",
        headers: { authorization: `Bearer ${adminToken}` }
    });
    if (!reviewedCommission.response.ok || reviewedCommission.json.calculation.status !== "reviewed")
        throw new Error("commission review failed");
    const lockedCommission = await request(`/api/commission/calculations/${commissionCalculation.id}/lock`, {
        method: "POST",
        headers: { authorization: `Bearer ${adminToken}` }
    });
    if (!lockedCommission.response.ok || lockedCommission.json.calculation.status !== "locked")
        throw new Error("commission lock failed");
    ...
    if (editLockedCommissionRecord.response.status !== 400)
        throw new Error("locked commission record must reject edit");
    const recalcLockedCommission = await request("/api/commission/calculations/recalculate", {
        method: "POST",
        headers: { authorization: `Bearer ${adminToken}` },
        body: JSON.stringify({ month: commissionMonth, ownerId: "u_sales_shirley" })
    });
    if (recalcLockedCommission.response.status !== 409)
        throw new Error("locked commission must reject recalculate");
    ...
    if (adjustLockedCommission.response.status !== 400)
        throw new Error("locked commission must reject manual adjustment");
    ...
    if (!commissionExport.response.ok || !commissionExport.json.rows?.length || commissionExport.json.rows.some((row) => "finalCommission" in row)) {
    ...
    if (salesUnlockCommission.response.status !== 403)
        throw new Error("sales must not unlock commission calculation");
    ...
    if (!markedLost.response.ok || markedLost.json.deal.stage !== "丢单" || !markedLost.json.deal.closedAt || markedLost.json.deal.lostReasonCategory !== "价格原因")
        throw new Error("deal lost failed");
    ...
    if (!stage.response.ok || stage.json.deal.stage !== "样品")
        throw new Error("deal stage update failed");
    const dealsAfterWorkflow = await request("/api/deals", { headers: { authorization: `Bearer ${managerToken}` } });
    if (!dealsAfterWorkflow.json.events.some((event) => event.dealId === newDeal.json.deal.id && event.type === "won"))
        throw new Error("deal events should persist workflow history");
    ...
    if (!closedDeals.response.ok || closedDeals.json.total < 1 || closedDeals.json.deals[0]?.lostReasonCategory !== "价格原因")
        throw new Error("closed deal query failed");
    ...
    if (!dashboard.response.ok || !dashboard.json.briefing?.title || !Array.isArray(dashboard.json.schedule))
        throw new Error("dashboard summary aggregation failed");
    if (!Array.isArray(dashboard.json.leadFunnel?.stages) || dashboard.json.leadFunnel.stages.length !== 5)
        throw new Error("lead funnel aggregation failed");
    if (dashboard.json.leadFunnel.stages[0]?.count !== salesLeadScope.json.leads.length)
        throw new Error("sales lead funnel must show the current salesperson's active visible lead scope");
    if (dashboard.json.leadFunnel.filteredOut !== salesLeadTrashScope.json.leads.length)
        throw new Error("sales lead funnel must report filtered leads separately");
    if (dashboard.json.leadFunnel.stages.some((stage) => stage.count < 0 || stage.conversionRate < 0 || stage.conversionRate > 100))
        throw new Error("lead funnel values must stay within valid ranges");
    if (!dashboard.json.briefing?.basis || !dashboard.json.briefing?.action || !dashboard.json.briefing?.impact)
        throw new Error("dashboard briefing guidance failed");
    if (dashboard.json.scopeLabels?.business !== "本人业务" || dashboard.json.scopeLabels?.todos !== "本人待办")
        throw new Error("sales dashboard scope labels failed");
    if (dashboard.json.briefing?.riskLabel !== "本人名下风险")
        throw new Error("sales dashboard risk label failed");
    if (!dashboard.json.periods?.today || !dashboard.json.periods?.week || !dashboard.json.periods?.month)
        throw new Error("dashboard period summaries missing");
    for (const period of Object.values(dashboard.json.periods)) {
        ...
        const briefing = period.briefing;
    ...
    const salesVisibleDealIds = new Set(salesDeals.json.deals.map((deal) => deal.id));
    const expectedSalesMonthDeals = salesDeals.json.deals.filter((deal) => !deal.archivedAt &&
        deal.stage !== "成交" &&
        deal.stage !== "丢单" &&
        Boolean(deal.expectedCloseAt) &&
        deal.expectedCloseAt.slice(0, 7) === dashboard.json.periods.month.start.slice(0, 7));
    if (dashboard.json.periods.month.expectedDeals !== expectedSalesMonthDeals.length)
        throw new Error("sales dashboard month forecast count failed");
    const expectedSalesMonthAmounts = new Map();
    expectedSalesMonthDeals.forEach((deal) => {
    ...
    if (dashboard.json.periods.month.expectedAmounts.some((row) => !row.currency || typeof row.amount !== "number")) {
        throw new Error("dashboard period forecast must retain currency");
    }
    if (dashboard.json.periods.month.expectedAmounts.length !== expectedSalesMonthAmounts.size)
        throw new Error("dashboard period forecast currency grouping failed");
    for (const row of dashboard.json.periods.month.expectedAmounts) {
        if (row.amount !== expectedSalesMonthAmounts.get(row.currency))
            throw new Error("dashboard period forecast amount must match scoped deals");
    }
    if (expectedSalesMonthDeals.some((deal) => !salesVisibleDealIds.has(deal.id)))
        throw new Error("dashboard period forecast leaks another salesperson");
    if (typeof dashboard.json.metrics?.riskCustomers !== "number")
        throw new Error("dashboard risk customer metric failed");
    if (typeof dashboard.json.metrics?.wecomBoundRate !== "number" || typeof dashboard.json.todoInsights?.completionRate !== "number")
        throw new Error("dashboard metrics aggregation failed");
    if (!Array.isArray(dashboard.json.pipelineHealth) || typeof dashboard.json.pipelineHealth[0]?.count !== "number")
        throw new Error("pipeline health aggregation failed");
    if (dashboard.json.pipelineHealth.some((item) => item.stage === "丢单"))
        throw new Error("pipeline health should exclude lost deals");
    if (dashboard.json.pipelineHealth.some((item) => item.stage === "成交"))
        throw new Error("pipeline health should exclude won deals");
    if (dashboard.json.pipelineHealth.some((item) => item.count === 0 && item.width !== 0))
        throw new Error("empty pipeline stages should have zero width");
    if (!dashboard.json.priorityTasks?.[0]?.reason || typeof dashboard.json.priorityTasks?.[0]?.score !== "number")
        throw new Error("priority task scoring failed");
    const managerDashboard = await request("/api/dashboard/summary", { headers: { authorization: `Bearer ${managerToken}` } });
    if (!managerDashboard.response.ok || managerDashboard.json.scope !== "本团队业务数据，本人待办")
        throw new Error("manager dashboard scope failed");
    if (managerDashboard.json.scopeLabels?.business !== "本团队业务" || managerDashboard.json.scopeLabels?.todos !== "本人待办" || managerDashboard.json.briefing?.riskLabel !== "团队风险金额")
        throw new Error("manager dashboard scope labels failed");
    if (managerDashboard.json.periods.month.expectedDeals < dashboard.json.periods.month.expectedDeals)
        throw new Error("manager dashboard period scope should include the salesperson");
    const adminDashboard = await request("/api/dashboard/summary", { headers: { authorization: `Bearer ${adminToken}` } });
    if (!adminDashboard.response.ok || adminDashboard.json.scope !== "本团队业务数据，本人待办")
        throw new Error("admin dashboard personal todo scope failed");
    if (adminDashboard.json.scopeLabels?.business !== "本团队业务" || adminDashboard.json.scopeLabels?.todos !== "本人待办" || adminDashboard.json.briefing?.riskLabel !== "团队风险金额")
        throw new Error("admin dashboard scope labels failed");
    const priorityBatch = await request("/api/dashboard/priority-tasks/batch-process", {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!priorityBatch.response.ok || !Array.isArray(priorityBatch.json.created))
        throw new Error("priority batch process failed");
    if (priorityBatch.json.created.some((item) => item.ownerId !== "u_sales_shirley"))
        throw new Error("priority batch should create personal todos only");
    const managerTodos = await request("/api/todos", { headers: { authorization: `Bearer ${managerToken}` } });
    if (!managerTodos.response.ok || managerTodos.json.todos.some((item) => item.ownerId !== "u_manager_alex"))
        throw new Error("manager should only see own todos");
    const adminTodos = await request("/api/todos", { headers: { authorization: `Bearer ${adminToken}` } });
    if (!adminTodos.response.ok || adminTodos.json.todos.some((item) => item.ownerId !== "u_admin"))
        throw new Error("admin should only see own todos");
    ...
    if (!planTask.response.ok || planTask.json.task.ownerId !== "u_sales_shirley" || planTask.json.task.customerId !== "c1" || planTask.json.task.dealId !== "d1")
        throw new Error("plan task create failed");
    const editedPlanTask = await request(`/api/plan-tasks/${planTask.json.task.id}`, {
        method: "PATCH",
        headers: { authorization: `Bearer ${salesToken}` },
        body: JSON.stringify({ status: "active", target: "已更新验收目标" })
    });
    if (!editedPlanTask.response.ok || editedPlanTask.json.task.status !== "active" || editedPlanTask.json.task.target !== "已更新验收目标")
        throw new Error("plan task edit failed");
    const bypassPlanTaskCompletion = await request(`/api/plan-tasks/${planTask.json.task.id}`, {
        method: "PATCH",
        headers: { authorization: `Bearer ${salesToken}` },
        body: JSON.stringify({ status: "done" })
    });
    if (bypassPlanTaskCompletion.response.ok)
        throw new Error("plan task completion should require dedicated action");
    ...
    if (mismatchedPlanTask.response.ok)
        throw new Error("plan task should reject mismatched customer and deal");
    ...
    if (mixedLeadPlanTask.response.ok)
        throw new Error("plan task should reject lead mixed with customer");
    ...
    if (!completedPlanTask.response.ok || completedPlanTask.json.task.status !== "done" || !completedPlanTask.json.task.completedAt || completedPlanTask.json.task.completionResult !== "报价已重发，客户确认收到")
        throw new Error("plan task complete failed");
    const miaPlanTasks = await request("/api/plan-tasks", { headers: { authorization: `Bearer ${miaToken}` } });
    if (!miaPlanTasks.response.ok || miaPlanTasks.json.tasks.some((item) => item.id === planTask.json.task.id))
        throw new Error("other sales should not see personal plan tasks");
    const managerPlanTasks = await request("/api/plan-tasks", { headers: { authorization: `Bearer ${managerToken}` } });
    if (!managerPlanTasks.response.ok || managerPlanTasks.json.tasks.some((item) => item.id === planTask.json.task.id))
        throw new Error("manager should not see personal plan tasks");
    const deletedPlanTask = await request(`/api/plan-tasks/${planTask.json.task.id}`, {
        method: "DELETE",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!deletedPlanTask.response.ok || deletedPlanTask.json.ok !== true)
        throw new Error("plan task delete failed");
    const cancelledPlanTask = await request("/api/plan-tasks", {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` },
        body: JSON.stringify({ title: "取消与改期自测", dueAt: "2026-07-13T08:00", leadId: "l1" })
    });
    if (!cancelledPlanTask.response.ok || cancelledPlanTask.json.task.leadId !== "l1")
        throw new Error("lead-linked plan task create failed");
    ...
    if (!rescheduledPlanTask.response.ok || rescheduledPlanTask.json.task.dueAt !== "2026-07-14T09:30" || rescheduledPlanTask.json.task.rescheduledFrom !== "2026-07-13T08:00")
        throw new Error("plan task reschedule failed");
    ...
    if (!cancelledResult.response.ok || cancelledResult.json.task.status !== "cancelled" || !cancelledResult.json.task.cancelledAt || cancelledResult.json.task.cancellationReason !== "线索确认无效，不再跟进")
        throw new Error("plan task cancel failed");
    ...
    if (!planTemplates.response.ok || planTemplates.json.templates.length < 15)
        throw new Error("plan templates seed failed");
    if (!planTemplates.json.templates.some((item) => item.section === "execution"))
        throw new Error("execution templates seed failed");
    ...
    if (!editedTemplate.response.ok || editedTemplate.json.template.title !== "自动化模板编辑")
        throw new Error("plan template edit failed");
    const deletedTemplate = await request(`/api/plan-templates/${templateId}`, {
        method: "DELETE",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!deletedTemplate.response.ok || deletedTemplate.json.ok !== true)
        throw new Error("plan template delete failed");
    const reminders = await request("/api/reminders", { headers: { authorization: `Bearer ${salesToken}` } });
    if (reminders.json.reminders.length < 2)
        throw new Error("reminders scope failed");
    ...
    if (!reminder.response.ok || reminder.json.reminder.title !== "手工提醒规则" || reminder.json.reminder.status !== "enabled" || reminder.json.reminder.targetOwnerId !== "u_sales_shirley")
    ...
    if (!reminderPreview.response.ok || typeof reminderPreview.json.creatableCount !== "number")
        throw new Error("reminder preview failed");
    const reminderRun = await request(`/api/reminders/${reminder.json.reminder.id}/run`, {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!reminderRun.response.ok || reminderRun.json.reminder.lastRunBy !== "u_sales_shirley" || reminderRun.json.reminder.lastMatchedCount !== reminderRun.json.matchedCount)
        throw new Error("reminder rule run failed");
    const managerReminderRun = await request(`/api/reminders/${reminder.json.reminder.id}/run`, {
        method: "POST",
        headers: { authorization: `Bearer ${managerToken}` }
    });
    if (managerReminderRun.response.status !== 403)
        throw new Error("manager must not execute salesperson personal reminder rule");
    const reminderRunAgain = await request(`/api/reminders/${reminder.json.reminder.id}/run`, {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!reminderRunAgain.response.ok || reminderRunAgain.json.createdCount !== 0 || reminderRunAgain.json.skippedCount !== reminderRunAgain.json.matchedCount)
        throw new Error("reminder stable dedupe failed");
    const salesReminderTodos = await request("/api/todos", { headers: { authorization: `Bearer ${salesToken}` } });
    const generatedReminderTodo = salesReminderTodos.json.todos.find((item) => item.reminderRuleId === reminder.json.reminder.id);
    if (!generatedReminderTodo || generatedReminderTodo.ownerId !== "u_sales_shirley" || !generatedReminderTodo.customerId || !generatedReminderTodo.triggerKey || generatedReminderTodo.historyAt)
        throw new Error(`reminder todo ownership or linkage failed: ${JSON.stringify({ run: reminderRun.json, todo: generatedReminderTodo })}`);
    const managerReminderTodos = await request("/api/todos", { headers: { authorization: `Bearer ${managerToken}` } });
    if (managerReminderTodos.json.todos.some((item) => item.reminderRuleId === reminder.json.reminder.id))
        throw new Error("manager should not receive sales reminder todo");
    ...
    if (!reminderSnoozed.response.ok || reminderSnoozed.json.todo.snoozeCount !== 1 || !reminderSnoozed.json.todo.snoozedFrom)
        throw new Error("reminder snooze audit failed");
    ...
    if (!reminderCompleted.response.ok || !reminderCompleted.json.todo.completedAt || reminderCompleted.json.todo.completionResult !== "客户已回复，等待确认数量")
        throw new Error("reminder completion audit failed");
    const reminderDelete = await request(`/api/todos/${generatedReminderTodo.id}`, {
        method: "DELETE",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (reminderDelete.response.ok)
        throw new Error("reminder todo must not be physically deleted");
    const reminderToggle = await request(`/api/reminders/${reminder.json.reminder.id}/toggle`, {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!reminderToggle.response.ok || reminderToggle.json.reminder.enabled !== false || reminderToggle.json.reminder.status !== "disabled")
        throw new Error("reminder toggle failed");
    const todo = await request("/api/todos", {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` },
        body: JSON.stringify({ title: "可撤回待办", type: "other", priority: "normal", dueAt: "现在", related: "自测" })
    });
    if (!todo.response.ok || todo.json.todo.done !== false)
    ...
    if (!todoDone.response.ok || todoDone.json.todo.done !== true)
        throw new Error("todo complete toggle failed");
    const todoUndone = await request(`/api/todos/${todo.json.todo.id}`, {
        method: "PATCH",
        headers: { authorization: `Bearer ${salesToken}` },
        body: JSON.stringify({ done: false })
    });
    if (!todoUndone.response.ok || todoUndone.json.todo.done !== false)
        throw new Error("todo undo failed");
    const todoStarted = await request(`/api/todos/${todo.json.todo.id}`, {
        method: "PATCH",
        headers: { authorization: `Bearer ${salesToken}` },
        body: JSON.stringify({ status: "in_progress" })
    });
    if (!todoStarted.response.ok || todoStarted.json.todo.status !== "in_progress")
        throw new Error("todo in-progress status failed");
    ...
    if (!todoOrdered.response.ok || todoOrdered.json.todos[0].pinState !== "top" || todoOrdered.json.todos[0].sortOrder !== 1)
        throw new Error("todo reorder failed");
    ...
    if (!archivedTodo.response.ok || !archivedTodo.json.todo.historyAt)
        throw new Error("todo should archive when due date is past");
    const restoredTodo = await request(`/api/todos/${archivedTodo.json.todo.id}/restore`, {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!restoredTodo.response.ok || restoredTodo.json.todo.historyAt)
        throw new Error("todo restore from history failed");
    ...
    if (!todoDeleted.response.ok || todoDeleted.json.ok !== true)
    ...
    if (!problem.response.ok || problem.json.problem.title !== "自动化问题清单")
        throw new Error("problem create failed");
    const resolvedProblem = await request(`/api/problems/${problem.json.problem.id}/status`, {
        method: "PATCH",
        headers: { authorization: `Bearer ${managerToken}` },
        body: JSON.stringify({ status: "resolved" })
    });
    if (!resolvedProblem.response.ok || resolvedProblem.json.problem.status !== "resolved")
        throw new Error("problem status failed");
    const memo = await request("/api/memos", {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` },
        body: JSON.stringify({ title: "自动化备忘录", content: "记录客户临时要求", category: "客户备忘", tags: "自动化,客户", dealId: "d1" })
    });
    if (!memo.response.ok || memo.json.memo.title !== "自动化备忘录" || memo.json.memo.customerId !== "c1")
        throw new Error("memo create or relation failed");
    const pinnedMemo = await request(`/api/memos/${memo.json.memo.id}`, {
        method: "PATCH",
        headers: { authorization: `Bearer ${salesToken}` },
        body: JSON.stringify({ pinned: true })
    });
    if (!pinnedMemo.response.ok || pinnedMemo.json.memo.pinned !== true)
        throw new Error("memo pin failed");
    const deletedMemo = await request(`/api/memos/${memo.json.memo.id}`, {
        method: "DELETE",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!deletedMemo.response.ok || !deletedMemo.json.memo.deletedAt)
        throw new Error("memo soft delete failed");
    const visibleMemos = await request("/api/memos", { headers: { authorization: `Bearer ${salesToken}` } });
    if (visibleMemos.json.memos.some((item) => item.id === memo.json.memo.id))
        throw new Error("deleted memo should not remain active");
    const deletedMemos = await request("/api/memos?trash=true", { headers: { authorization: `Bearer ${salesToken}` } });
    if (!deletedMemos.json.memos.some((item) => item.id === memo.json.memo.id))
        throw new Error("deleted memo should be visible in personal trash");
    ...
    if (!managerDeletedMemos.response.ok
        || managerDeletedMemos.json.memos.some((item) => item.ownerId !== "u_manager_alex")
        || managerDeletedMemos.json.memos.some((item) => item.id === memo.json.memo.id)) {
    ...
    if (!adminDeletedMemos.response.ok
        || adminDeletedMemos.json.memos.some((item) => item.ownerId !== "u_admin")
        || adminDeletedMemos.json.memos.some((item) => item.id === memo.json.memo.id)) {
    ...
    if (managerDeleteSalesMemo.response.status !== 404)
        throw new Error("manager must not delete salesperson memo");
    const managerRestoreSalesMemo = await request(`/api/memos/${memo.json.memo.id}/restore`, {
        method: "POST",
        headers: { authorization: `Bearer ${managerToken}` }
    });
    if (managerRestoreSalesMemo.response.status !== 404)
        throw new Error("manager must not restore salesperson memo");
    const restoredMemo = await request(`/api/memos/${memo.json.memo.id}/restore`, {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!restoredMemo.response.ok || restoredMemo.json.memo.deletedAt)
        throw new Error("memo restore failed");
    ...
    if (!permanentlyDeletedMemo.response.ok || permanentlyDeletedMemo.json.ok !== true)
        throw new Error("memo permanent delete failed");
    const invalidMemoRelation = await request("/api/memos", {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` },
        body: JSON.stringify({ title: "错误关联备忘", customerId: "c2", dealId: "d1" })
    });
    if (invalidMemoRelation.response.status !== 400)
        throw new Error("memo relation mismatch should be rejected");
    const managerMemos = await request("/api/memos", { headers: { authorization: `Bearer ${managerToken}` } });
    if (!managerMemos.response.ok || managerMemos.json.memos.some((item) => item.ownerId !== "u_manager_alex"))
        throw new Error("manager should only see own memos");
    const adminMemos = await request("/api/memos", { headers: { authorization: `Bearer ${adminToken}` } });
    if (!adminMemos.response.ok || adminMemos.json.memos.some((item) => item.ownerId !== "u_admin"))
        throw new Error("admin should only see own memos");
    ...
    if (!competitor.response.ok || competitor.json.competitor.company !== "Auto Competitor")
        throw new Error("competitor create failed");
    ...
    if (!competitorThreat.response.ok || competitorThreat.json.competitor.threatLevel !== "low")
        throw new Error("competitor threat failed");
    ...
    if (!caseStudy.response.ok || caseStudy.json.caseStudy.title !== "自动化成功案例")
        throw new Error("case study create failed");
    const publishedCase = await request(`/api/case-studies/${caseStudy.json.caseStudy.id}/publish`, {
        method: "PATCH",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!publishedCase.response.ok || publishedCase.json.caseStudy.status !== "published")
        throw new Error("case study publish failed");
    ...
    if (!customerImport.response.ok || customerImport.json.result.created !== 1 || customerImport.json.result.updated !== 1)
        throw new Error("customer import failed");
    const customerExport = await request("/api/import-export/customers/export", {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!customerExport.response.ok || !Array.isArray(customerExport.json.customers) || customerExport.json.customers.length < 1)
    ...
    if (!tradeDocument.response.ok || tradeDocument.json.document.type !== "CI")
        throw new Error("trade document create failed");
    const tradeDocuments = await request("/api/trade-documents", {
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!tradeDocuments.response.ok || !tradeDocuments.json.documents.some((item) => item.id === tradeDocument.json.document.id))
        throw new Error("trade document list failed");
    ...
    if (!tradeDocumentSubmit.response.ok || tradeDocumentSubmit.json.document.status !== "pending_approval")
        throw new Error("trade document submit approval failed");
    ...
    if (!tradeDocumentApprove.response.ok || tradeDocumentApprove.json.document.status !== "approved")
        throw new Error("trade document approval failed");
    const tradeDocumentExport = await request(`/api/trade-documents/${tradeDocument.json.document.id}/export`, {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!tradeDocumentExport.response.ok || tradeDocumentExport.json.document.status !== "exported")
        throw new Error("trade document export failed");
    ...
    if (examDetail.json.questions.some((question) => question.answerIndex !== -1 || question.answerIndexes?.length || question.explanation)) {
    ...
    const examAnswers = Object.fromEntries(managerExamDetail.json.questions.map((question) => [
        question.id,
    ...
    if (!exam.response.ok || exam.json.attempt.passed !== true || exam.json.attempt.score !== 100)
    ...
    if (salesQuestionForbidden.response.status !== 403)
        throw new Error("sales must not maintain question bank");
    const salesExamForbidden = await request("/api/exams", {
        method: "POST",
        headers: { authorization: `Bearer ${salesToken}` },
        body: JSON.stringify({ title: "销售不能发布考试", category: "权限测试", questionIds: ["q1"] })
    });
    if (salesExamForbidden.response.status !== 403)
        throw new Error("sales must not create exams");
    ...
    if (!bankQuestion.response.ok || bankQuestion.json.question.tags[0] !== "产品")
        throw new Error("bank question create failed");
    ...
    if (!importedBankQuestions.response.ok || importedBankQuestions.json.importedCount !== 2)
        throw new Error("bank question import failed");
    const exportedBank = await request("/api/exam-questions/export", {
        headers: { authorization: `Bearer ${managerToken}` }
    });
    if (!exportedBank.response.ok || exportedBank.json.questions.length < 3)
        throw new Error("bank question export failed");
    ...
    if (!deletedBankQuestion.response.ok || deletedBankQuestion.json.question.id !== deleteCandidate.id)
        throw new Error("bank question delete failed");
    ...
    if (!createdExam.response.ok || createdExam.json.exam.questionCount !== questionIds.length)
        throw new Error("exam create from bank failed");
    const publishedExam = await request(`/api/exams/${createdExam.json.exam.id}/publish`, {
        method: "PATCH",
        headers: { authorization: `Bearer ${managerToken}` }
    });
    if (!publishedExam.response.ok || publishedExam.json.exam.status !== "published")
        throw new Error("exam publish failed");
    const createdExamDetail = await request(`/api/exams/${createdExam.json.exam.id}/detail`, {
        headers: { authorization: `Bearer ${salesToken}` }
    });
    if (!createdExamDetail.response.ok || createdExamDetail.json.questions.length !== questionIds.length)
        throw new Error("created exam detail failed");
    if (createdExamDetail.json.questions.some((question) => question.answerIndex !== -1 || question.answerIndexes?.length || question.explanation)) {
    ...
    const salesExamAnswers = Object.fromEntries(managerCreatedExamDetail.json.questions.map((question) => [
        question.id,
        question.answerIndexes?.length ? question.answerIndexes : question.answerIndex
    ]));
    ...
    if (!salesExamAttempt.response.ok || salesExamAttempt.json.attempt.score !== 100)
        throw new Error("multi-select exam scoring failed");
    const managerAccounts = await request("/api/accounts", {
        headers: { authorization: `Bearer ${managerToken}` }
    });
    if (managerAccounts.response.status !== 403)
        throw new Error("manager must not access account management");
    const accountList = await request("/api/accounts", {
        headers: { authorization: `Bearer ${adminToken}` }
    });
    if (!accountList.response.ok
        || accountList.json.accounts.some((item) => item.role === "super_admin")
        || accountList.json.accounts.some((item) => item.teamId !== "europe")) {
    ...
    if (forbiddenSuper.response.status !== 403)
        throw new Error("admin must not create super admin");
    ...
    if (!account.response.ok || account.json.account.name !== "Auto Sales")
    ...
    if (loginDeleted.response.status !== 401)
        throw new Error("deleted account must not login");
