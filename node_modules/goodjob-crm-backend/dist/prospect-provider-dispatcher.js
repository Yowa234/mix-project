...
import { providerErrorFromUnknown } from "./provider-contract.js";
import { prospectProviderResponseComponentHashes, prospectProviderResponseHash } from "./prospect-provider-request-ledger.js";
import { PROSPECT_SOURCE_RAW_SCHEMA_VERSION, prospectProviderRawArtifactHash } from "./prospect-source-raw.js";
import { executeProviderSearch } from "./provider-runtime.js";
function joined(values) {
    return values.map((item) => item.trim()).filter(Boolean).join(", ");
}
function validHttpStatus(value, fallback) {
    return Number.isInteger(value) && value >= 100 && value <= 599
        ? value
        : fallback;
}
function responseFromStep(request, step, httpStatus) {
...
function sourceUrl(provider, catalogDocsUrl, catalogBaseUrl, record) {
...
function successStep(provider, catalogDocsUrl, catalogBaseUrl, page) {
    const sourceRecords = page.records.map((record) => ({
        providerRecordId: record.providerRecordId || record.payloadHash,
        sourceUrl: sourceUrl(provider, catalogDocsUrl, catalogBaseUrl, record),
...
export class ProspectProviderDispatcher {
    ...
    async dispatch(request, providerRequest) {
        try {
            const run = this.store.prospectSearchRuns.find((item) => item.id === request.runId
                && item.teamId === request.teamId
                && item.ownerId === request.ownerId);
            const shard = this.store.prospectRunShards.find((item) => item.id === request.shardId
                && item.runId === request.runId
                && item.teamId === request.teamId
                && item.providerCode === request.providerCode);
            const catalog = this.store.providerCatalog.find((item) => item.code === request.providerCode);
            ...
            const connection = request.connectionId.startsWith("builtin:")
                ? undefined
                : this.store.providerConnections.find((item) => item.id === request.connectionId
            ...
            const remaining = Math.max(1, shard.resultLimit - (checkpoint?.acceptedCount || 0));
            const query = {
                goal: [
            ...
            return responseFromStep(request, successStep(resolved.provider, catalog.officialDocsUrl, catalog.baseUrl, page), 200);
        }
        catch (error) {
            ...
            }, validHttpStatus(failure.httpStatus, failure.retryable ? 503 : 400));
        }
    }
}
