...
import { ProviderContractError, normalizeProviderEnrichment, normalizeProviderHealthResult, normalizeProviderPage, normalizeProviderQuery, normalizeProviderTradePage, normalizeTradeQuery, providerErrorFromUnknown } from "./provider-contract.js";
...
import { providerRequestFingerprint, recordProviderCacheHit, recordProviderHttpResult, recordProviderHttpResultIfEmpty, withProviderRequestLogging } from "./provider-request-logging.js";
const tradeConnectionBudgets = new Map();
const activeTradeFlights = new Map();
function contractError(code, message, operation) {
...
function rateLimitError(retryAfterAt, phase) {
...
function acquireTradeConnectionBudget(input) {
    if (!input.connection)
        return () => undefined;
    ...
    if (!maxConcurrent && !minIntervalMs && !requestsPerMinute)
        return () => undefined;
    const now = Date.now();
    ...
    if (retryAt)
        throw rateLimitError(new Date(retryAt).toISOString(), input.operation);
    state.active += 1;
...
function cloneProviderFailure(error) {
...
export function providerRequiresKey(provider, catalog) {
    return typeof catalog?.licensePolicy.requiresKey === "boolean"
        ? catalog.licensePolicy.requiresKey
        : provider.requiresKey;
}
export function assertProviderOperationPolicy(provider, catalog, operation) {
    ...
    const sharedCapabilities = provider.capabilities.filter((item) => catalog.capabilities.includes(item));
    if (operation === "search" && (!("search" in provider)
        || !provider.search
...
function validateRuntime(input) {
    ...
    for (const [label, value] of [
        ...
        if (!value.trim())
            throw contractError("PROVIDER_CONNECTION_INVALID", `Provider ${label} context is required`, context.operation);
    }
...
async function runLogged(input, endpointCode, fingerprintValue, operation) {
...
export async function executeProviderPreflight(input) {
    return await runLogged(input, `${input.context.operation}_preflight`, { preflight: true }, async () => true);
}
export async function executeProviderSearch(input) {
    const runtimeInput = {
        ...input,
    ...
    if (cursor.length > 2000)
        throw contractError("PROVIDER_POLICY_BLOCKED", "数据源游标超过允许大小", "search");
    const { limit: _limit, ...fingerprintQuery } = normalizedQuery;
    return await runLogged(runtimeInput, "search", {
        query: fingerprintQuery,
        cursor
    }, async (credential) => {
        const page = await input.provider.search({ query: normalizedQuery, cursor }, credential, { http: createProviderHttpClient(input.provider.networkPolicy) });
...
export async function executeProviderHealth(input) {
    const runtimeInput = {
        ...input,
    ...
    return await runLogged(runtimeInput, "connection_test", { health: true }, async (credential) => {
        ...
        const releaseBudget = tradeProvider
            ...
            : () => undefined;
        try {
            return normalizeProviderHealthResult(await input.provider.health(credential, { http: createProviderHttpClient(input.provider.networkPolicy) }));
        }
        finally {
            releaseBudget();
        }
...
export async function executeProviderEnrich(input) {
    const runtimeInput = {
        ...input,
    ...
    return await runLogged(runtimeInput, "enrich", { domain }, async (credential) => normalizeProviderEnrichment({
        provider: input.provider,
        catalogPolicyVersion: input.catalog.version,
        sourceLevel: input.catalog.sourceLevel,
        allowedFields: input.catalog.allowedFields,
        retentionPolicy: input.catalog.retentionPolicy,
        domain,
        result: await input.provider.enrich({ domain }, credential, { http: createProviderHttpClient(input.provider.networkPolicy) })
    }));
}
function tradeCachePolicy(catalog) {
...
export async function executeProviderTradeQuery(input) {
    const runtimeInput = {
        ...input,
    ...
    const cursor = (input.cursor || "").trim();
    if (cursor.length > 2000)
        throw contractError("PROVIDER_POLICY_BLOCKED", "数据源游标超过允许大小", "trade");
    ...
    return await runLogged(runtimeInput, "trade", fingerprintValue, async (credential) => {
        if (cacheKey) {
            const cached = await readProviderResponseCache(cacheKey);
            if (cached) {
            ...
            if (!flight) {
                flight = Promise.resolve().then(async () => {
                    ...
                    try {
                        ...
                        const adapterPage = await input.provider.trade({ query, cursor }, credential, { http: createProviderHttpClient(input.provider.networkPolicy) });
