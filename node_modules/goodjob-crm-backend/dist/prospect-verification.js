...
function evidenceTime(evidence, fallback) {
    return evidence
...
function isIdentityEvidence(item) {
    return item.sourceLevel === "identity"
        || /(registry|registration|legal[_ -]?entity|company[_ -]?registry|lei)/iu.test(`${item.recordType} ${item.evidenceSummary}`);
}
function check(code, label, status, summary, source, checkedAt) {
    return { code, label, status, summary, source, checkedAt };
}
export function buildProspectVerificationReport(opportunity, generatedAt = new Date().toISOString()) {
    ...
    const evidenceDomains = [...new Set(evidenceWithDomains.map((item) => item.domain))];
    const matchingDomainProviders = [...new Set(evidenceWithDomains
            .filter((item) => item.domain === candidateDomain)
            .map((item) => item.providerId))];
    const domainsConsistent = Boolean(candidateDomain
    ...
    const contactEvidence = evidence.some((item) => item.matchedFields.some((field) => /(contact|email|phone|whatsapp|wechat)/iu.test(field)));
    const manuallyVerified = Boolean(opportunity.verifiedAt
    ...
    const checks = [
        check("crawler_free_policy", "网页访问策略", "passed", "系统未访问、下载、解析或探测企业网页，仅保存链接和授权数据源返回字段。", "系统安全策略", generatedAt),
        check("website_reference", "官网链接", candidateDomain ? "partial" : "manual_required", candidateDomain
            ? `已登记域名 ${candidateDomain}；链接格式有效不代表官网真实性。`
            : "未登记可用官网链接，需要人工补充。", "链接登记", opportunity.createdAt || generatedAt),
        check("provider_evidence", "来源证据", evidence.length ? "partial" : "manual_required", evidence.length
            ? `已保留 ${evidence.length} 条来源记录，来自 ${providers.length} 个独立数据源。`
            : "当前没有结构化来源证据，仅可作为人工核验入口。", providers.join("、") || opportunity.sourceLabel || opportunity.source || "人工登记", checkedAt),
        check("enterprise_identity", "企业身份", identityEvidence.length ? "passed" : "unverified", identityEvidence.length
            ? `存在 ${identityEvidence.length} 条企业登记或强身份来源证据。`
            : "尚无企业登记或强身份来源证据。", identityEvidence.map((item) => item.providerId).join("、") || "待人工核实", checkedAt),
        check("multi_source_consistency", "多源一致性", multiSourceDomainVerified
            ? "passed"
            : providers.length >= 2 && evidenceDomains.length
                ? "partial"
                : "unverified", multiSourceDomainVerified
                ...
                : !candidateDomain
                    ...
                    : evidenceDomains.length === 0
                        ...
                        : domainsConsistent
                            ...
                            : `${providers.length} 个独立来源存在域名差异，需要人工判断是否为同一主体。`, matchingDomainProviders.join("、") || providers.join("、") || "待补充来源", checkedAt),
        check("contact_information", "联系方式", manuallyVerified && hasValue(opportunity.contactInfo || opportunity.contact)
            ? "passed"
            : contactEvidence && hasValue(opportunity.contactInfo || opportunity.contact)
                ? "partial"
                : "manual_required", manuallyVerified && hasValue(opportunity.contactInfo || opportunity.contact)
            ? "业务员已人工确认联系方式可用于触达。"
            : contactEvidence && hasValue(opportunity.contactInfo || opportunity.contact)
                ? "来源包含联系方式字段，但尚未经过业务员人工确认。"
                : "尚无已确认的联系方式，需要人工核实。", manuallyVerified ? "业务员人工核验" : contactEvidence ? "授权数据源" : "待人工核实", opportunity.verifiedAt || checkedAt)
    ];
    if (replied) {
        checks.push(check("real_interaction", "真实互动", "passed", "已记录真实回复或有效互动，触达结果已进入业务闭环。", "CRM 触达记录", generatedAt));
    }
    return {
        level,
...
export function withProspectVerificationReport(opportunity, generatedAt = new Date().toISOString()) {
    opportunity.verificationReport = buildProspectVerificationReport(opportunity, generatedAt);
    return opportunity;
}
export function prospectVerificationReferenceTime(opportunity) {
    const timestamps = [
...
export function ensureProspectVerificationReport(opportunity) {
    if (!opportunity.verificationReport) {
        withProspectVerificationReport(opportunity, prospectVerificationReferenceTime(opportunity));
    }
    return opportunity;
}
...
export function normalizeWebsiteReference(rawUrl) {
    const raw = rawUrl.trim();
    if (!raw)
        throw new Error("官网链接不能为空");
...
export function companyNameFromWebsiteReference(website) {
