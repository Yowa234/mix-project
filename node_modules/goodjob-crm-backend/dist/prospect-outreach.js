...
function followUpSchedule(classification, occurredAt) {
...
function activeFollowUpTodo(store, candidate, channel) {
    return store.todos.find((todo) => todo.ownerId === candidate.ownerId
...
function nextTodoSortOrder(store, ownerId) {
    return Math.min(0, ...store.todos
        .filter((todo) => todo.ownerId === ownerId)
        .map((todo) => typeof todo.sortOrder === "number" ? todo.sortOrder : 0)) - 1;
}
export function ensureProspectFollowUpTodo(store, input) {
...
function cancelChannelTodos(store, candidate, channel, reason, cancelledAt) {
    const cancelled = [];
...
function matchingFormalChannel(store, candidate, channel, contactValue) {
    if (!candidate.tenantProspectId)
        return undefined;
    const formalType = channel === "call" ? "phone" : channel;
    const matches = store.prospectContactChannels.filter((item) => item.teamId === candidate.teamId
    ...
    return matches.find((item) => normalizeContactValue(channel, item.value) === normalized);
}
async function writeFormalReplyFact(store, input, occurredAt) {
    const candidate = input.candidate;
    const channel = matchingFormalChannel(store, candidate, input.channel, input.contactValue || "");
    ...
    if (input.replyClassification === "rejected"
        ...
        await store.applyProspectQualification({
            ...
            idempotencyKey: `outreach:${candidate.id}:${input.requestId}:suppression`
        });
        return true;
    }
    return false;
}
export async function recordProspectTouchpoint(store, input) {
    const candidate = input.candidate;
    ...
    if (!requestId)
        throw new Error("触达请求编号不能为空");
    const replay = store.prospectTouchpoints.find((item) => item.ownerId === candidate.ownerId
    ...
    if (classification === "rejected" || classification === "unsubscribed") {
        ...
        const cancelled = cancelChannelTodos(store, candidate, input.channel, reason, occurredAt);
    ...
    if (classification === "bounced") {
        ...
        candidate.nextFollowAt = "";
        const cancelled = cancelChannelTodos(store, candidate, input.channel, "联系方式退信，待补充有效联系方式", occurredAt);
        return { touchpoint, cancelled, replayed: false };
    }
...
export function migrateProspectFollowUpTodos(store, candidate, leadId) {
...
export function prospectReplyLabel(value) {
