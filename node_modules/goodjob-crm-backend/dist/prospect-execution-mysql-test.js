...
import { activateProspectCampaign, createProspectCampaign, prospectCampaignEtag } from "./prospect-campaigns.js";
...
import { PROSPECT_SOURCE_RAW_SCHEMA_VERSION } from "./prospect-source-raw.js";
import { approveProspectStrategy, prospectStrategyEtag, updateProspectStrategy } from "./prospect-strategies.js";
...
function executionStateSnapshot(store) {
    return {
...
function restoreExecutionState(store, snapshot) {
    store.prospectSearchRuns.splice(0, store.prospectSearchRuns.length, ...snapshot.runs);
    store.prospectRunShards.splice(0, store.prospectRunShards.length, ...snapshot.shards);
    store.prospectRunEvents.splice(0, store.prospectRunEvents.length, ...snapshot.runEvents);
    store.agentJobs.splice(0, store.agentJobs.length, ...snapshot.jobs);
    store.prospectRunQueueParentBindings.splice(0, store.prospectRunQueueParentBindings.length, ...snapshot.parentBindings);
    store.prospectRunQueueChildBindings.splice(0, store.prospectRunQueueChildBindings.length, ...snapshot.childBindings);
    store.prospectExecutionKernelStates.splice(0, store.prospectExecutionKernelStates.length, ...snapshot.kernelStates);
    store.prospectExecutionCheckpoints.splice(0, store.prospectExecutionCheckpoints.length, ...snapshot.checkpoints);
    store.prospectStrategySourcePositions.splice(0, store.prospectStrategySourcePositions.length, ...snapshot.sourcePositions);
    store.prospectExecutionLeases.splice(0, store.prospectExecutionLeases.length, ...snapshot.leases);
    store.prospectExecutionAttempts.splice(0, store.prospectExecutionAttempts.length, ...snapshot.attempts);
    store.prospectProviderRequestLedgers.splice(0, store.prospectProviderRequestLedgers.length, ...snapshot.providerRequestLedgers);
    store.prospectProviderRequestDispatches.splice(0, store.prospectProviderRequestDispatches.length, ...snapshot.providerRequestDispatches);
    store.prospectProviderRequestAttemptBindings.splice(0, store.prospectProviderRequestAttemptBindings.length, ...snapshot.providerRequestAttemptBindings);
    store.prospectProviderRequestEvents.splice(0, store.prospectProviderRequestEvents.length, ...snapshot.providerRequestEvents);
    store.prospectProviderRequestAccountingEvidence.splice(0, store.prospectProviderRequestAccountingEvidence.length, ...snapshot.providerRequestAccountingEvidence);
    store.prospectExecutionPages.splice(0, store.prospectExecutionPages.length, ...snapshot.pages);
    store.prospectExecutionEvents.splice(0, store.prospectExecutionEvents.length, ...snapshot.events);
    store.prospectExecutionThrottleBuckets.splice(0, store.prospectExecutionThrottleBuckets.length, ...snapshot.throttles);
}
async function expectRejectedMutation(store, mutate, pattern) {
...
class MysqlInspectingFakeProspectProvider extends DeterministicFakeProspectProvider {
    inspect;
...
async function main() {
    ...
    if (!applicationUrl || !adminConnectionUrl) {
        throw new Error("Prospect execution MySQL test requires MYSQL_TEST_ADMIN_URL, "
            + "DATABASE_URL or MYSQL_URL");
    }
    const adminUrl = new URL(adminConnectionUrl);
    const appUrl = new URL(applicationUrl);
    const databaseName = `goodjob_execution_test_${randomUUID().replaceAll("-", "").slice(0, 16)}`;
    ...
    let timelineTick = 0;
    const tick = () => new Date(timelineStart + timelineTick++ * 1_000).toISOString();
    try {
       ...
       CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`);
        ...
        const [executionTables] = await admin.query(`SELECT table_name AS tableName
       ...
       ORDER BY table_name`, [databaseName]);
        assert.deepEqual(executionTables.map((row) => String(row.tableName)), [
            "prospect_execution_attempts",
            "prospect_execution_checkpoints",
            "prospect_execution_events",
            "prospect_execution_leases",
        ...
        const owner = setupStore.users.find((item) => item.id === "u_sales_shirley");
        assert.ok(owner);
        ...
        const strategy = setupStore.prospectStrategies.find((item) => item.campaignId === createdCampaign.campaign.id);
        assert.ok(strategy);
        ...
        const cursorSecret = "mysql-execution-cursor-secret-at-least-32-characters";
        ...
        if (!firstStarted.ready)
            throw new Error("first request was deferred");
        ...
        const pausedRun = pausedStore.prospectSearchRuns.find((item) => item.id === firstRunId);
        assert.ok(pausedRun);
        assert.equal(pausedRun.status, "paused");
        const pausedCheckpoint = pausedStore.prospectExecutionCheckpoints.find((item) => item.runId === firstRunId);
        ...
        assert.equal(pausedCheckpoint.encryptedCursor.includes("mysql-private-cursor"), false);
        const pausedSourcePosition = pausedStore.prospectStrategySourcePositions.find((item) => item.sourceRunId === firstRunId);
        ...
        assert.equal(pausedSourcePosition.encryptedCursor.includes("mysql-private-cursor"), false);
        const [cursorRows] = await admin.query(`SELECT encrypted_cursor, cursor_hash, 'checkpoint' AS cursor_type
       ...
       WHERE source_run_id = ?`, [firstRunId, firstRunId]);
        assert.equal(cursorRows.length, 2);
        assert.equal(cursorRows.every((row) => /^v1\./.test(String(row.encrypted_cursor))
            && /^[a-f0-9]{64}$/.test(String(row.cursor_hash))), true);
        assert.equal(JSON.stringify(cursorRows).includes("mysql-private-cursor"), false);
        ...
        const resumedResult = await resumedKernel.executeNext(resumedProvider, tick());
        ...
        const finishedCheckpoint = finishedStore.prospectExecutionCheckpoints.find((item) => item.runId === firstRunId);
        ...
        const finishedSourcePosition = finishedStore.prospectStrategySourcePositions.find((item) => item.id === pausedSourcePosition.id);
        ...
        const finishedAttempts = finishedStore.prospectExecutionAttempts
            ...
            .sort((left, right) => left.providerAttemptNo - right.providerAttemptNo);
        assert.equal(finishedAttempts.length, 2);
        assert.deepEqual(finishedAttempts.map((item) => item.status), ["succeeded", "succeeded"]);
        assert.deepEqual(finishedAttempts.map((item) => item.costAmount), [0.01, 0.02]);
        assert.deepEqual(finishedAttempts.map((item) => JSON.parse(item.usageJson)), [
            { requestUnits: 1, resultUnits: 3 },
            { requestUnits: 1, resultUnits: 1 }
        ]);
        assert.equal(finishedAttempts.every((item) => /^[a-f0-9]{64}$/.test(item.requestHash)
            && /^[a-f0-9]{64}$/.test(item.responseHash)), true);
        assert.equal(finishedStore.prospectExecutionPages.filter((item) => item.runId === firstRunId).length, 2);
        assert.equal(finishedStore.prospectExecutionLeases.filter((item) => item.runId === firstRunId).every((item) => item.status === "released"
            && Boolean(item.releasedAt)
            && Boolean(item.releaseReason)), true);
        const firstRunEventTypes = finishedStore.prospectExecutionEvents
            .filter((item) => item.runId === firstRunId)
            .map((item) => item.eventType);
        assert.equal(firstRunEventTypes.filter((item) => item === "page_accepted").length, 2);
        assert.equal(firstRunEventTypes.includes("pause_settled"), true);
        assert.equal(firstRunEventTypes.includes("run_completed"), true);
        const [secretScanRows] = await admin.query(`SELECT
         ...
         (SELECT GROUP_CONCAT(detail_hash SEPARATOR '')
          ...
          WHERE run_id = ?) AS eventFacts`, [firstRunId, firstRunId, firstRunId, firstRunId]);
        assert.equal(JSON.stringify(secretScanRows).includes("mysql-private-cursor"), false);
        stage = "cold-start lease recovery and fence advancement";
        const finishedOwner = finishedStore.users.find((item) => item.id === owner.id);
        const finishedStrategy = finishedStore.prospectStrategies.find((item) => item.id === strategy.id);
        assert.ok(finishedOwner);
        assert.ok(finishedStrategy);
        const resetSourcePositionMemoryForScenario = async (store) => {
            await admin.query(`DELETE FROM \`${databaseName}\`.prospect_strategy_source_positions`);
            store.prospectStrategySourcePositions.splice(0, store.prospectStrategySourcePositions.length);
        };
        ...
        const pauseBeforeRequestClaim = await pauseBeforeRequestKernel.claimNext(tick());
        assert.ok(pauseBeforeRequestClaim);
        const pausedBeforeRequest = await pauseBeforeRequestKernel.requestPause(pauseBeforeRequestRunId, tick());
        assert.equal(pausedBeforeRequest.status, "paused");
        const pauseBeforeRequestStore = await createMysqlStore();
        const persistedPauseRun = pauseBeforeRequestStore.prospectSearchRuns.find((item) => item.id === pauseBeforeRequestRunId);
        const persistedPauseCheckpoint = pauseBeforeRequestStore.prospectExecutionCheckpoints.find((item) => item.runId === pauseBeforeRequestRunId);
        const persistedPauseLease = pauseBeforeRequestStore.prospectExecutionLeases.find((item) => item.id === pauseBeforeRequestClaim.lease.id);
        const persistedPauseAttempt = pauseBeforeRequestStore.prospectExecutionAttempts.find((item) => item.leaseId === pauseBeforeRequestClaim.lease.id);
        const persistedPauseShard = pauseBeforeRequestStore.prospectRunShards.find((item) => item.runId === pauseBeforeRequestRunId);
        ...
        assert.equal(persistedPauseLease?.releaseReason, "PAUSED_BEFORE_REQUEST");
        assert.equal(persistedPauseAttempt?.status, "failed");
        assert.equal(persistedPauseAttempt?.errorCode, "PAUSED_BEFORE_REQUEST");
        ...
        assert.equal((await pauseCleanupKernel.requestCancel(pauseBeforeRequestRunId, tick())).status, "cancelled");
        ...
        const cancelBeforeRequestRunResult = await createProspectRun({
            store: pauseBeforeRequestStore,
            user: publicUser(pauseBeforeRequestStore.users.find((item) => item.id === owner.id)),
            strategyId: finishedStrategy.id,
            ifMatch: prospectStrategyEtag(pauseBeforeRequestStore.prospectStrategies.find((item) => item.id === finishedStrategy.id)),
        ...
        const cancelBeforeRequestRunId = cancelBeforeRequestRunResult.run.id;
        ...
        assert.equal((await cancelBeforeRequestKernel.executeNext(checkpointProvider, tick())).kind, "success");
        const cancelClaim = await cancelBeforeRequestKernel.claimNext(tick());
        assert.ok(cancelClaim);
        const cancelledBeforeRequest = await cancelBeforeRequestKernel.requestCancel(cancelBeforeRequestRunId, tick());
        assert.equal(cancelledBeforeRequest.status, "cancelled");
        const cancelledBeforeRequestStore = await createMysqlStore();
        const persistedCancelledRun = cancelledBeforeRequestStore.prospectSearchRuns.find((item) => item.id === cancelBeforeRequestRunId);
        const persistedCancelledCheckpoint = cancelledBeforeRequestStore.prospectExecutionCheckpoints.find((item) => item.runId === cancelBeforeRequestRunId);
        const persistedCancelledLease = cancelledBeforeRequestStore.prospectExecutionLeases.find((item) => item.id === cancelClaim.lease.id);
        const persistedCancelledAttempt = cancelledBeforeRequestStore.prospectExecutionAttempts.find((item) => item.leaseId === cancelClaim.lease.id);
        ...
        assert.equal(persistedCancelledCheckpoint?.completionReason, "CANCELLED_BY_USER");
        assert.equal(persistedCancelledCheckpoint?.lastErrorCode, "CANCELLED_BY_USER");
        assert.equal(persistedCancelledLease?.status, "released");
        assert.equal(persistedCancelledLease?.releaseReason, "CANCELLED_BEFORE_REQUEST");
        assert.equal(persistedCancelledAttempt?.status, "failed");
        assert.equal(persistedCancelledAttempt?.errorCode, "CANCELLED_BEFORE_REQUEST");
        assert.equal(JSON.stringify({
            checkpoint: persistedCancelledCheckpoint,
            attempts: cancelledBeforeRequestStore.prospectExecutionAttempts.filter((item) => item.runId === cancelBeforeRequestRunId)
        }).includes("mysql-cancel-private-cursor"), false);
        stage = "rollback wrong cursor key before MySQL persistence";
        const cursorRollbackOwner = cancelledBeforeRequestStore.users.find((item) => item.id === owner.id);
        const cursorRollbackStrategy = cancelledBeforeRequestStore.prospectStrategies.find((item) => item.id === finishedStrategy.id);
        ...
        const cursorRollbackProvider = new DeterministicFakeProspectProvider({
            gleif: [{
                    kind: "success",
                    acceptedCount: 1,
                    rawCount: 1,
        ...
        assert.equal((await cursorRollbackWriter.executeNext(cursorRollbackProvider, tick())).kind, "success");
        ...
        const cursorRollbackClaim = await cursorRollbackReader.claimNext(tick());
        assert.ok(cursorRollbackClaim);
        const beforeCursorRollback = executionStateSnapshot(cancelledBeforeRequestStore);
        await assert.rejects(cursorRollbackReader.beginRequest({
            leaseId: cursorRollbackClaim.lease.id,
            claimToken: cursorRollbackClaim.claimToken,
            now: tick()
        }), /执行游标完整性校验失败/);
        assert.deepEqual(executionStateSnapshot(cancelledBeforeRequestStore), beforeCursorRollback);
        const cursorRollbackColdStore = await createMysqlStore();
        const cursorRollbackCheckpoint = cursorRollbackColdStore.prospectExecutionCheckpoints.find((item) => item.runId === cursorRollbackRunId);
        const cursorRollbackAttempt = cursorRollbackColdStore.prospectExecutionAttempts.find((item) => item.leaseId === cursorRollbackClaim.lease.id);
        const cursorRollbackLease = cursorRollbackColdStore.prospectExecutionLeases.find((item) => item.id === cursorRollbackClaim.lease.id);
        ...
        await cursorRollbackReader.requestCancel(cursorRollbackRunId, tick());
        stage = "serialize concurrent claims in one MySQL kernel";
        const concurrentClaimOwner = cancelledBeforeRequestStore.users.find((item) => item.id === owner.id);
        const concurrentClaimStrategy = cancelledBeforeRequestStore.prospectStrategies.find((item) => item.id === finishedStrategy.id);
        ...
        const concurrentClaims = await Promise.all(concurrentClaimTimes.map((claimAt) => concurrentClaimKernel.claimNext(claimAt)));
        const acceptedConcurrentClaims = concurrentClaims.filter((item) => item !== null);
        assert.equal(acceptedConcurrentClaims.length, 1);
        assert.equal(concurrentClaims.filter((item) => item === null).length, 1);
        const acceptedConcurrentClaim = acceptedConcurrentClaims[0];
        const concurrentClaimColdStore = await createMysqlStore();
        assert.equal(concurrentClaimColdStore.prospectExecutionLeases.filter((item) => item.runId === concurrentClaimRunId
            && item.status === "active").length, 1);
        assert.equal(concurrentClaimColdStore.prospectExecutionAttempts.filter((item) => item.runId === concurrentClaimRunId).length, 1);
        assert.equal(concurrentClaimColdStore.prospectExecutionLeases.find((item) => item.runId === concurrentClaimRunId)?.id, acceptedConcurrentClaim.lease.id);
        await concurrentClaimKernel.requestCancel(concurrentClaimRunId, tick());
        await resetSourcePositionMemoryForScenario(concurrentClaimColdStore);
        const recoveryRunResult = await createProspectRun({
            store: cancelledBeforeRequestStore,
            user: publicUser(cancelledBeforeRequestStore.users.find((item) => item.id === owner.id)),
            strategyId: finishedStrategy.id,
            ifMatch: prospectStrategyEtag(cancelledBeforeRequestStore.prospectStrategies.find((item) => item.id === finishedStrategy.id)),
        ...
        assert.equal(abandonedStore.prospectExecutionLeases.find((item) => item.id === abandonedClaim.lease.id)?.status, "active");
        ...
        const recoveredLease = abandonedStore.prospectExecutionLeases.find((item) => item.id === abandonedClaim.lease.id);
        const recoveredAttempt = abandonedStore.prospectExecutionAttempts.find((item) => item.leaseId === abandonedClaim.lease.id);
        assert.equal(recoveredLease?.status, "expired");
        assert.equal(recoveredLease?.releaseReason, "LEASE_RECOVERED_BEFORE_REQUEST");
        ...
        const recoveryResponse = await recoveryProvider.search(recoveredStarted.request);
        ...
        assert.equal(abandonedStore.prospectSearchRuns.find((item) => item.id === recoveryRunId)?.status, "succeeded");
        ...
        await assert.rejects(staleKernelStore.persist(), /搜索执行内核 epoch CAS 冲突/);
        const staleCheckpointStore = await createMysqlStore();
        const staleCheckpoint = staleCheckpointStore.prospectExecutionCheckpoints.find((item) => item.runId === firstRunId);
        assert.ok(staleCheckpoint);
        await admin.query(`UPDATE \`${databaseName}\`.prospect_execution_checkpoints
       SET version_no = version_no + 1 WHERE id = ?`, [staleCheckpoint.id]);
        await assert.rejects(staleCheckpointStore.persist(), /搜索执行 checkpoint CAS 冲突或作用域被修改/);
        await admin.query(`UPDATE \`${databaseName}\`.prospect_execution_checkpoints
       SET version_no = ? WHERE id = ?`, [staleCheckpoint.version, staleCheckpoint.id]);
        stage = "create active guard run";
        const guardStore = await createMysqlStore();
        const guardOwner = guardStore.users.find((item) => item.id === owner.id);
        const guardStrategy = guardStore.prospectStrategies.find((item) => item.id === strategy.id);
        ...
        assert.equal((await guardKernel.executeNext(guardFirstProvider, tick())).kind, "success");
        ...
        const activeGuardLease = guardStore.prospectExecutionLeases.find((item) => item.id === guardClaim.lease.id);
        ...
        await expectRejectedMutation(guardStore, () => {
            const checkpoint = guardStore.prospectExecutionCheckpoints.find((item) => item.runId === guardRunId);
            const page = guardStore.prospectExecutionPages.find((item) => item.runId === guardRunId);
        ...
        await expectRejectedMutation(guardStore, () => {
            const checkpoint = guardStore.prospectExecutionCheckpoints.find((item) => item.runId === firstRunId);
        ...
        await expectRejectedMutation(guardStore, () => {
            const lease = guardStore.prospectExecutionLeases.find((item) => item.runId === firstRunId);
            lease.releaseReason = "TAMPERED_RELEASE_REASON";
            lease.version += 1;
        }, /搜索执行租约终态不允许修改/);
        await expectRejectedMutation(guardStore, () => {
            const attempt = guardStore.prospectExecutionAttempts.find((item) => item.runId === firstRunId && item.status === "succeeded");
            const page = guardStore.prospectExecutionPages.find((item) => item.attemptId === attempt.id);
        ...
        await expectRejectedMutation(guardStore, () => {
            const throttle = guardStore.prospectExecutionThrottleBuckets[0];
            throttle.availableAt = new Date(Date.parse(throttle.availableAt) - 1_000).toISOString();
            throttle.updatedAt = tick();
            throttle.version += 1;
        }, /搜索执行限流桶 CAS 冲突或作用域被修改/);
        await expectRejectedMutation(guardStore, () => {
            const lease = guardStore.prospectExecutionLeases.find((item) => item.id === guardClaim.lease.id);
            const attempt = guardStore.prospectExecutionAttempts.find((item) => item.leaseId === lease.id);
        ...
        await expectRejectedMutation(guardStore, () => {
            const checkpoint = guardStore.prospectExecutionCheckpoints.find((item) => item.runId === guardRunId);
            const otherOwner = guardStore.users.find((item) => item.id !== checkpoint.ownerId);
            assert.ok(otherOwner);
            checkpoint.ownerId = otherOwner.id;
        }, /搜索执行 checkpoint 引用、计数或作用域无效/);
        await expectRejectedMutation(guardStore, () => {
            const parent = guardStore.prospectRunQueueParentBindings.find((item) => item.runId === guardRunId);
            parent.bindingHash = "0".repeat(64);
        }, /父桥接绑定完整性校验失败/);
        await expectRejectedMutation(guardStore, () => {
            const index = guardStore.prospectRunQueueChildBindings.findIndex((item) => item.runId === guardRunId);
            guardStore.prospectRunQueueChildBindings.splice(index, 1);
        }, /搜索执行 checkpoint 引用、计数或作用域无效/);
        await expectRejectedMutation(guardStore, () => {
            const event = guardStore.prospectExecutionEvents.find((item) => item.runId === firstRunId);
            event.detailHash = "0".repeat(64);
        }, /搜索执行事件不可变历史被删除或修改/);
        await expectRejectedMutation(guardStore, () => {
            const index = guardStore.prospectExecutionEvents.findIndex((item) => item.runId === firstRunId);
        ...
        try {
            const [kernelRows] = await transactionConnection.query(`SELECT instance_id AS instanceId
           ...
           WHERE id = 'search_execution_kernel_v1'`);
            ...
            const guardCheckpoint = guardStore.prospectExecutionCheckpoints.find((item) => item.runId === guardRunId);
            const otherOwner = guardStore.users.find((item) => item.id !== guardCheckpoint.ownerId);
            assert.ok(otherOwner);
            for (const [column, value] of [
                ...
                try {
                    await transactionConnection.query(`UPDATE search_execution_kernel_state
             SET instance_id = 'must-rollback'
             WHERE id = 'search_execution_kernel_v1'`);
                    await assert.rejects(transactionConnection.query(`UPDATE prospect_execution_checkpoints
               SET ${column} = ? WHERE id = ?`, [value, guardCheckpoint.id]));
                }
                finally {
                    await transactionConnection.rollback();
                }
                const [rolledBackRows] = await transactionConnection.query(`SELECT instance_id AS instanceId
             FROM search_execution_kernel_state
             WHERE id = 'search_execution_kernel_v1'`);
                assert.equal(String(rolledBackRows[0]?.instanceId || ""), originalInstanceId);
            }
        }
        finally {
        ...
        if (!guardStarted.ready)
            throw new Error("guard request was deferred");
        assert.equal(guardStarted.request.cursor, "guard-private-cursor");
        const guardResponse = await guardFinalProvider.search(guardStarted.request);
        ...
        assert.equal(verifiedStore.prospectSearchRuns.find((item) => item.id === guardRunId)?.status, "succeeded");
        assert.equal(verifiedStore.prospectExecutionPages.filter((item) => item.runId === guardRunId).length, 2);
        assert.equal(JSON.stringify({
        ...
        }).includes("guard-private-cursor"), false);
        stage = "persist prepared request and physical dispatch facts";
        const dispatchOwner = verifiedStore.users.find((item) => item.id === owner.id);
        const dispatchStrategy = verifiedStore.prospectStrategies.find((item) => item.id === strategy.id);
        ...
        const providerRequestIdempotencySecret = "mysql-provider-idempotency-secret-at-least-32-characters";
        const providerRequestEnvelopeSecret = "mysql-provider-envelope-secret-at-least-32-characters";
        const providerRawEnvelopeSecret = "mysql-provider-raw-envelope-secret-at-least-32-characters";
        const providerRawIdentitySecret = "mysql-provider-raw-identity-secret-at-least-32-characters";
        ...
        const mysqlRawPlaintextMarker = "MYSQL_RAW_PLAINTEXT_MUST_NOT_APPEAR";
        const mysqlRawCompanyName = `${mysqlRawPlaintextMarker} Industrial Lighting GmbH`;
        ...
        const persistedPrepared = preparedStore.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id);
        assert.equal(persistedPrepared?.status, "prepared");
        assert.match(persistedPrepared?.encryptedRequestEnvelope || "", /^provider-request-v1\./);
        assert.equal((persistedPrepared?.encryptedRequestEnvelope || "")
            .includes("工业照明"), false);
        await expectRejectedMutation(verifiedStore, () => {
            const ledger = verifiedStore.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id);
        ...
        dispatchProvider = new MysqlInspectingFakeProspectProvider({
            ...
            const startedLedger = startedStore.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id);
            const startedDispatch = startedStore.prospectProviderRequestDispatches.find((item) => item.ledgerId === prepared.ledger.id);
            const startedAttempt = startedStore.prospectExecutionAttempts.find((item) => item.id === dispatchClaim.attempt.id);
        ...
        const confirmedLedger = confirmedStore.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id);
        const confirmedDispatch = confirmedStore.prospectProviderRequestDispatches.find((item) => item.ledgerId === prepared.ledger.id);
        ...
        assert.equal((confirmedLedger?.encryptedResponseEnvelope || "")
            .includes("requestUnits"), false);
        assert.equal(confirmedStore.prospectExecutionLeases.find((item) => item.id === dispatchClaim.lease.id)?.releaseReason, "RESPONSE_RECEIVED_PENDING_SETTLEMENT");
        assert.equal(confirmedStore.prospectExecutionAttempts.find((item) => item.id === dispatchClaim.attempt.id)?.status, "request_started");
        assert.equal(confirmedStore.prospectExecutionPages.some((item) => item.runId === dispatchRunId), false);
        assert.equal(confirmedStore.prospectProviderRequestAccountingEvidence.some((item) => item.ledgerId === prepared.ledger.id), false);
        assert.deepEqual(confirmedStore.prospectProviderRequestEvents
        ...
        assert.equal(dispatchProvider.listPhysicalCalls().length, 1);
        const replayedResponse = await dispatchRecoveryKernel.dispatchPreparedProviderRequest(dispatchProvider, {
            leaseId: dispatchClaim.lease.id,
            claimToken: dispatchClaim.claimToken,
            ledgerId: prepared.ledger.id,
            now: tick()
        ...
        assert.equal(dispatchProvider.listPhysicalCalls().length, 1);
        const mutableConfirmedLedger = confirmedStore.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id);
        const originalResponseEnvelope = mutableConfirmedLedger.encryptedResponseEnvelope;
        ...
        await assert.rejects(dispatchRecoveryKernel.dispatchPreparedProviderRequest(dispatchProvider, {
            leaseId: dispatchClaim.lease.id,
            claimToken: dispatchClaim.claimToken,
            ledgerId: prepared.ledger.id,
            now: tick()
        ...
        await expectRejectedMutation(confirmedStore, () => {
            const ledger = confirmedStore.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id);
        ...
        await expectRejectedMutation(confirmedStore, () => {
            const responseEvent = confirmedStore.prospectProviderRequestEvents.find((item) => item.ledgerId === prepared.ledger.id
                && item.eventType === "response_received");
            responseEvent.detailHash = "0".repeat(64);
        }, /Provider 响应接收事件摘要无效/);
        await expectRejectedMutation(confirmedStore, () => {
            const ledger = confirmedStore.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id);
            ledger.ownerId = confirmedStore.users.find((item) => item.id !== ledger.ownerId
                && item.teamId === ledger.teamId)?.id || "cross-owner";
        }, /Provider 请求账本身份、哈希、时间或状态无效/);
        const recoveredDispatchStore = await createMysqlStore();
        assert.equal(recoveredDispatchStore.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id)?.status, "response_received");
        assert.equal(recoveredDispatchStore.prospectProviderRequestDispatches.find((item) => item.ledgerId === prepared.ledger.id)?.status, "response_received");
        assert.equal(recoveredDispatchStore.prospectExecutionAttempts.find((item) => item.id === dispatchClaim.attempt.id)?.status, "request_started");
        stage = "persist late response after outcome unknown recovery";
        const lateResponseOwner = recoveredDispatchStore.users.find((item) => item.id === owner.id);
        ...
        const lateResponseStrategy = recoveredDispatchStore.prospectStrategies.find((item) => item.campaignId === lateResponseCampaign.campaign.id);
        ...
        const lateResponsePrepared = await lateResponseKernel.prepareProviderRequest({
            leaseId: lateResponseClaim.lease.id,
            claimToken: lateResponseClaim.claimToken,
            now: tick()
        });
        let releaseLateResponse;
        let observeDurableDispatch;
        const lateResponseGate = new Promise((resolve) => {
            releaseLateResponse = resolve;
        });
        const durableDispatchGate = new Promise((resolve) => {
            observeDurableDispatch = resolve;
        });
        const lateResponseProvider = new MysqlInspectingFakeProspectProvider({
            gleif: [{
                    kind: "success",
                    acceptedCount: 1,
                    rawCount: 1,
        ...
        const lateResponseInFlight = lateResponseKernel.dispatchPreparedProviderRequest(lateResponseProvider, {
            leaseId: lateResponseClaim.lease.id,
            claimToken: lateResponseClaim.claimToken,
        ...
        const unknownResponseLedger = unknownResponseStore.prospectProviderRequestLedgers.find((item) => item.id === lateResponsePrepared.ledger.id);
        ...
        const unknownResponseLease = unknownResponseStore.prospectExecutionLeases.find((item) => item.id === lateResponseClaim.lease.id);
        assert.ok(unknownResponseLease);
        const unknownResponseLeaseSnapshot = structuredClone(unknownResponseLease);
        assert.equal(unknownResponseStore.prospectProviderRequestDispatches.find((item) => item.ledgerId === lateResponsePrepared.ledger.id)?.status, "outcome_unknown");
        releaseLateResponse();
        ...
        const persistedLateResponseLedger = persistedLateResponseStore.prospectProviderRequestLedgers.find((item) => item.id === lateResponsePrepared.ledger.id);
        const persistedLateResponseDispatch = persistedLateResponseStore.prospectProviderRequestDispatches.find((item) => item.ledgerId === lateResponsePrepared.ledger.id);
        assert.equal(persistedLateResponseLedger?.status, "response_received");
        assert.equal(persistedLateResponseDispatch?.status, "response_received");
        ...
        assert.deepEqual(persistedLateResponseStore.prospectExecutionLeases.find((item) => item.id === lateResponseClaim.lease.id), unknownResponseLeaseSnapshot);
        assert.equal(persistedLateResponseStore.prospectExecutionAttempts.find((item) => item.id === lateResponseClaim.attempt.id)?.status, "request_outcome_unknown");
        assert.equal(persistedLateResponseStore.prospectExecutionPages.some((item) => item.runId === lateResponseRunId), false);
        assert.equal(persistedLateResponseStore
            .prospectProviderRequestAccountingEvidence.some((item) => item.ledgerId === lateResponsePrepared.ledger.id), false);
        assert.deepEqual(persistedLateResponseStore.prospectProviderRequestEvents
        ...
        const replayedLateResponse = await lateResponseRecoveryKernel.dispatchPreparedProviderRequest(lateResponseProvider, {
            leaseId: lateResponseClaim.lease.id,
            claimToken: lateResponseClaim.claimToken,
            ledgerId: lateResponsePrepared.ledger.id,
            now: tick()
        ...
        const settlementLedgerA = settlementStoreA.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id);
        assert.ok(settlementLedgerA);
        assert.equal(settlementLedgerA.status, "response_received");
        const settlementLeaseBefore = structuredClone(settlementStoreA.prospectExecutionLeases.find((item) => item.id === dispatchClaim.lease.id));
        const crmFactsBeforeSettlement = {
            ...
            websiteOpportunities: settlementStoreA.websiteOpportunities.length
        };
        ...
        const settlementLedgerB = settlementStoreB.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id);
        const otherSettlementOwner = settlementStoreB.users.find((item) => item.id !== settlementLedgerB.ownerId);
        assert.ok(otherSettlementOwner);
        await assert.rejects(settlementKernelB.settlePersistedProviderResponse({
            teamId: "cross-team",
        ...
        const originalSettlementEnvelope = settlementLedgerB.encryptedResponseEnvelope;
        settlementLedgerB.encryptedResponseEnvelope =
            `${originalSettlementEnvelope.slice(0, -1)}${originalSettlementEnvelope.endsWith("A") ? "B" : "A"}`;
        await assert.rejects(settlementKernelB.settlePersistedProviderResponse({
            teamId: settlementLedgerB.teamId,
            ownerId: settlementLedgerB.ownerId,
            runId: settlementLedgerB.runId,
            ...
            && error.code === "EXECUTION_PROVIDER_RESPONSE_ENVELOPE_INVALID");
        settlementStoreB.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id).encryptedResponseEnvelope = originalSettlementEnvelope;
        ...
        const fulfilledSettlements = concurrentSettlements.filter((item) => item.status === "fulfilled");
        const rejectedSettlements = concurrentSettlements.filter((item) => item.status === "rejected");
        assert.equal(fulfilledSettlements.length, 1);
        assert.equal(rejectedSettlements.length, 1);
        assert.match(String(rejectedSettlements[0].reason), /CAS|不可变证据|历史不允许删除/);
        const winnerIndex = concurrentSettlements.findIndex((item) => item.status === "fulfilled");
        const winningKernel = winnerIndex === 0
            ? settlementKernelA
            : settlementKernelB;
        const winningSettlement = concurrentSettlements[winnerIndex].value;
        ...
        const winningRawState = {
            batches: structuredClone(winningSettlementStore.prospectSourceRawBatches),
            records: structuredClone(winningSettlementStore.prospectSourceRawRecords),
            hits: structuredClone(winningSettlementStore.prospectSourceRawHits)
        };
        const settledDispatchStore = await createMysqlStore();
        assert.deepEqual(settledDispatchStore.prospectSourceRawBatches, winningRawState.batches);
        assert.deepEqual(settledDispatchStore.prospectSourceRawRecords, winningRawState.records);
        assert.deepEqual(settledDispatchStore.prospectSourceRawHits, winningRawState.hits);
        const settledDispatchLedger = settledDispatchStore.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id);
        assert.equal(settledDispatchLedger?.status, "settled");
        assert.equal(settledDispatchLedger?.settlementKind, "success");
        assert.equal(settledDispatchStore.prospectExecutionAttempts.find((item) => item.id === dispatchClaim.attempt.id)?.status, "succeeded");
        assert.equal(settledDispatchStore.prospectExecutionPages.filter((item) => item.attemptId === dispatchClaim.attempt.id).length, 1);
        assert.equal(settledDispatchStore.prospectProviderRequestAccountingEvidence
            .filter((item) => item.ledgerId === prepared.ledger.id).length, 1);
        assert.deepEqual(settledDispatchStore.prospectExecutionLeases.find((item) => item.id === dispatchClaim.lease.id), settlementLeaseBefore);
        assert.deepEqual({
            ...
            websiteOpportunities: settledDispatchStore.websiteOpportunities.length
        }, crmFactsBeforeSettlement);
        const settledRawBatch = settledDispatchStore.prospectSourceRawBatches.find((item) => item.ledgerId === prepared.ledger.id);
        assert.ok(settledRawBatch);
        assert.equal(settledRawBatch.recordCount, 2);
        assert.equal(settledDispatchStore.prospectSourceRawRecords.filter((item) => item.teamId === settledRawBatch.teamId
            ...
            && item.endpointCode === settledRawBatch.endpointCode).length, 1);
        const settledRawHits = settledDispatchStore.prospectSourceRawHits.filter((item) => item.batchId === settledRawBatch.id);
        assert.deepEqual(settledRawHits.map((item) => item.ordinal), [1, 2]);
        assert.equal(settledRawHits[0]?.recordId, settledRawHits[1]?.recordId);
        const restoredRawState = {
            batches: structuredClone(settledDispatchStore.prospectSourceRawBatches),
            records: structuredClone(settledDispatchStore.prospectSourceRawRecords),
        ...
        assert.deepEqual(settledDispatchStore.prospectSourceRawBatches, restoredRawState.batches);
        assert.deepEqual(settledDispatchStore.prospectSourceRawRecords, restoredRawState.records);
        assert.deepEqual(settledDispatchStore.prospectSourceRawHits, restoredRawState.hits);
        const [rawRecordRows] = await admin.query(`SELECT *
       FROM \`${databaseName}\`.prospect_source_raw_records
       WHERE team_id = ? AND id = ?`, [settledRawBatch.teamId, settledRawHits[0]?.recordId]);
        assert.equal(rawRecordRows.length, 1);
        assert.match(String(rawRecordRows[0]?.encrypted_envelope || ""), /^provider-raw-v1\./);
        const [rawBatchRows] = await admin.query(`SELECT *
       FROM \`${databaseName}\`.prospect_source_raw_batches
       WHERE team_id = ? AND id = ?`, [settledRawBatch.teamId, settledRawBatch.id]);
        const [rawHitRows] = await admin.query(`SELECT *
       FROM \`${databaseName}\`.prospect_source_raw_hits
       WHERE team_id = ? AND batch_id = ?
       ORDER BY ordinal`, [settledRawBatch.teamId, settledRawBatch.id]);
        ...
        const replayedSettlement = await winningKernel.settlePersistedProviderResponse({
        ...
        const auditLedger = settledDispatchStore.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id);
        const originalAuditEnvelope = auditLedger.encryptedResponseEnvelope;
        auditLedger.encryptedResponseEnvelope =
            `${originalAuditEnvelope.slice(0, -1)}${originalAuditEnvelope.endsWith("A") ? "B" : "A"}`;
        await assert.rejects(settlementAuditKernel.settlePersistedProviderResponse({
            ...
            && error.code === "EXECUTION_PROVIDER_RESPONSE_ENVELOPE_INVALID");
        settledDispatchStore.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id).encryptedResponseEnvelope = originalAuditEnvelope;
        await expectRejectedMutation(settledDispatchStore, () => {
            const evidence = settledDispatchStore
                .prospectProviderRequestAccountingEvidence.find((item) => item.ledgerId === prepared.ledger.id);
        ...
        await expectRejectedMutation(settledDispatchStore, () => {
            const ledger = settledDispatchStore.prospectProviderRequestLedgers.find((item) => item.id === prepared.ledger.id);
            ledger.settlementHash = "0".repeat(64);
        }, /Provider 响应结算事件或不可变摘要无效/);
        await expectRejectedMutation(settledDispatchStore, () => {
            const page = settledDispatchStore.prospectExecutionPages.find((item) => item.attemptId === dispatchClaim.attempt.id);
        ...
        const lateSettlementLedger = lateSettlementStore.prospectProviderRequestLedgers.find((item) => item.id === lateResponsePrepared.ledger.id);
        assert.ok(lateSettlementLedger);
        const lateSettlementLeaseBefore = structuredClone(lateSettlementStore.prospectExecutionLeases.find((item) => item.id === lateResponseClaim.lease.id));
        ...
        const lateSettlement = await lateSettlementKernel.settlePersistedProviderResponse({
        ...
        assert.equal(lateSettlementStore.prospectExecutionPages.some((item) => item.attemptId === lateResponseClaim.attempt.id), false);
        assert.deepEqual(lateSettlementStore.prospectExecutionLeases.find((item) => item.id === lateResponseClaim.lease.id), lateSettlementLeaseBefore);
        stage = "cold-start failed Provider response settlement";
        const failureOwner = lateSettlementStore.users.find((item) => item.id === owner.id);
        const failureStrategy = lateSettlementStore.prospectStrategies.find((item) => item.id === strategy.id);
        ...
        const failurePrepared = await failureDispatchKernel.prepareProviderRequest({
            leaseId: failureClaim.lease.id,
            claimToken: failureClaim.claimToken,
            now: tick()
        });
        const failureRetryAt = new Date(timelineStart + 60 * 60_000).toISOString();
        ...
        const failureResponse = await failureDispatchKernel.dispatchPreparedProviderRequest(failureProvider, {
            leaseId: failureClaim.lease.id,
            claimToken: failureClaim.claimToken,
        ...
        const failurePendingLedger = failurePendingStore.prospectProviderRequestLedgers.find((item) => item.id === failurePrepared.ledger.id);
        ...
        const failureSettlement = await failureSettlementKernel.settlePersistedProviderResponse({
        ...
        assert.equal(failureSettlement.accountingEvidence.provenance, "estimated");
        assert.equal(failureSettlement.accountingEvidence.estimationMethodVersion, "fake-provider-estimate-v1");
        assert.equal(failurePendingStore.prospectExecutionPages.some((item) => item.attemptId === failureClaim.attempt.id), false);
        const coldFailureStore = await createMysqlStore();
        assert.equal(coldFailureStore.prospectProviderRequestLedgers.find((item) => item.id === failurePrepared.ledger.id)?.settlementKind, "failure");
        assert.equal(coldFailureStore.prospectProviderRequestAccountingEvidence.filter((item) => item.ledgerId === failurePrepared.ledger.id).length, 1);
        assert.equal(coldFailureStore.prospectExecutionPages.some((item) => item.attemptId === failureClaim.attempt.id), false);
        console.log("Prospect execution MySQL persistence, CAS, recovery, "
