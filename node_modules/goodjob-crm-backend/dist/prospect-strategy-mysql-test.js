...
import { activateProspectCampaign, createProspectCampaign, prospectCampaignEtag, updateProspectCampaign } from "./prospect-campaigns.js";
...
async function main() {
    ...
    if (!configuredUrl) {
        throw new Error("Prospect Strategy MySQL test requires MYSQL_TEST_ADMIN_URL, DATABASE_URL or MYSQL_URL");
    }
    ...
    try {
        ...
        const nextOwner = firstStore.users.find((item) => item.id === "u_sales_mia");
        const manager = firstStore.users.find((item) => item.id === "u_manager_alex");
        assert.ok(owner);
        assert.ok(nextOwner);
        assert.ok(manager);
        ...
        const strategy = firstStore.prospectStrategies.find((item) => item.campaignId === campaignId);
        assert.ok(strategy);
        assert.match(strategy.id, /^ps_[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
        ...
        const reloadedCampaign = secondStore.prospectCampaigns.find((item) => item.id === campaignId);
        const reloadedStrategy = secondStore.prospectStrategies.find((item) => item.id === strategy.id);
        ...
        assert.equal(secondStore.prospectStrategyEvents.filter((item) => item.strategyId === strategy.id).length, 1);
        stage = "update strategy";
        const updated = await updateProspectStrategy({
            store: secondStore,
            user: publicUser(secondStore.users.find((item) => item.id === owner.id)),
        ...
        const persistedCampaign = thirdStore.prospectCampaigns.find((item) => item.id === campaignId);
        const persistedStrategy = thirdStore.prospectStrategies.find((item) => item.id === strategy.id);
        ...
        assert.equal(thirdStore.prospectStrategyEvents.filter((item) => item.strategyId === strategy.id).length, 3);
        const strategyRevisionBeforeTransfer = persistedStrategy.revision;
        stage = "transfer owner";
        await updateProspectCampaign({
            store: thirdStore,
            user: publicUser(thirdStore.users.find((item) => item.id === manager.id)),
            campaignId,
        ...
        const transferredStrategy = fourthStore.prospectStrategies.find((item) => item.id === strategy.id);
        ...
        assert.equal(fourthStore.prospectStrategyEvents.filter((item) => item.strategyId === strategy.id).length, 4);
        const originalFingerprint = transferredStrategy.queryFingerprint;
        transferredStrategy.queryFingerprint = "0".repeat(64);
        await assert.rejects(fourthStore.persist(), /获客搜索策略规范化内容或查询指纹不一致/);
        transferredStrategy.queryFingerprint = originalFingerprint;
        const persistedEvent = fourthStore.prospectStrategyEvents.find((item) => item.strategyId === strategy.id);
        const originalReason = persistedEvent.reason;
        persistedEvent.reason = "试图改写审计历史";
        await assert.rejects(fourthStore.persist(), /获客搜索策略审计事件不可变历史被删除或修改/);
        ...
        await assert.rejects(createProspectCampaign({
            store: fourthStore,
            user: publicUser(fourthStore.users.find((item) => item.id === owner.id)),
            body: {
                name: "必须整体回滚的策略项目",
                snapshot: fullSnapshot
            },
        ...
        assert.equal(fourthStore.prospectCampaigns.length, campaignCountBeforeRollback);
        assert.equal(fourthStore.prospectStrategies.length, strategyCountBeforeRollback);
        assert.equal(fourthStore.prospectCampaigns.some((item) => item.name === "必须整体回滚的策略项目"), false);
        fourthStore.prospectStrategies.find((item) => item.id === strategy.id).queryFingerprint = originalFingerprint;
        stage = "final valid persist";
        await fourthStore.persist();
        const [strategyRows] = await admin.query(`SELECT owner_id, status, revision_no, query_fingerprint
       FROM \`${databaseName}\`.prospect_strategies WHERE id = ?`, [strategy.id]);
        ...
        const [eventRows] = await admin.query(`SELECT COUNT(*) AS total
       FROM \`${databaseName}\`.prospect_strategy_events
       WHERE strategy_id = ?`, [strategy.id]);
        assert.equal(Number(eventRows[0]?.total), 4);
        console.log("Prospect strategy MySQL cold restart, activation, transfer, append-only and rollback tests passed");
        exitCode = 0;
    }
    catch (error) {
