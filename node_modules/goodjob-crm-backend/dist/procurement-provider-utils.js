...
export function procurementSearchTerms(query) {
...
export function firstCountryInEnglish(query) {
    const country = query.countries[0]?.trim() || "";
    return COUNTRY_ALIASES[country.toLocaleLowerCase()] || country;
}
export function isExcludedProcurement(value, query) {
    const normalized = plainText(value).toLocaleLowerCase();
    return query.excludeKeywords.some((keyword) => normalized.includes(plainText(keyword).toLocaleLowerCase()));
}
...
export function textLimit(value, length) {
    return plainText(value).slice(0, length);
}
export function positiveCursor(value, fallback, maximum = 10_000) {
    const parsed = Number.parseInt(value, 10);
    return Number.isFinite(parsed) && parsed >= 0
        ? Math.min(parsed, maximum)
        : fallback;
}
export function freeProcurementUsage(display) {
    return {
        requestCount: 1,
        quotaUsed: null,
        quotaRemaining: null,
...
export function procurementSchemaError(source, cause) {
