...
import { migrateProspectFollowUpTodos, recordProspectTouchpoint } from "./prospect-outreach.js";
import { getStore } from "./store.js";
const at = (day, hour = 9) => new Date(Date.UTC(2026, 6, day, hour)).toISOString();
function localDueText(value) {
...
function candidate(suffix, ownerId = "sales-owner", teamId = "team-a") {
    return {
        id: `candidate-${suffix}`,
...
async function testSuppressionCancelsActiveTodo() {
    for (const classification of ["rejected", "unsubscribed"]) {
        const store = isolatedStore();
...
async function testBounceMarksChannelInvalid() {
    ...
    assert.equal(store.todos[0]?.done, true);
    assert.equal(store.todos[0]?.cancellationReason, "联系方式退信，待补充有效联系方式");
}
async function testLeadMigrationReusesOutreachData() {
    ...
    assert.equal(store.prospectTouchpoints[0]?.leadId, "lead-migration-1");
...
async function testOwnerIsolation() {
    ...
    store.websiteOpportunities.push(prospect);
    await assert.rejects(recordProspectTouchpoint(store, {
        candidate: prospect,
