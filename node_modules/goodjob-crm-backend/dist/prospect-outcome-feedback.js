...
function latestCoverageEvent(store, scope, prospectId) {
    return store.prospectCoverageEvents
        .filter((item) => item.teamId === scope.teamId
...
function providerCodesForCoverageEvent(store, event) {
    if (!event?.sourceHitId)
        return [];
    const hit = store.prospectSourceRawHits.find((item) => item.id === event.sourceHitId
        && item.teamId === event.teamId
        && item.ownerId === event.ownerId);
    if (!hit)
        return [];
    const batch = store.prospectSourceRawBatches.find((item) => item.id === hit.batchId
        && item.teamId === event.teamId
        && item.ownerId === event.ownerId);
    return batch?.providerCode ? [batch.providerCode] : [];
}
function resolveAcquisitionLink(store, deal) {
    const recommendation = store.dealRecommendations.find((item) => item.teamId === deal.teamId
        && item.ownerId === deal.ownerId
        && item.linkedDealId === deal.id);
    const linkedCandidate = store.websiteOpportunities.find((item) => item.teamId === deal.teamId
        && item.ownerId === deal.ownerId
        && item.dealId === deal.id);
    const linkedProspect = store.tenantProspects.find((item) => item.teamId === deal.teamId
        && item.dealId === deal.id);
    const candidate = recommendation
        ? store.websiteOpportunities.find((item) => item.id === recommendation.prospectCandidateId
            && item.teamId === deal.teamId
            && item.ownerId === deal.ownerId)
        : linkedCandidate
            || store.websiteOpportunities.find((item) => item.teamId === deal.teamId
                && item.ownerId === deal.ownerId
                && item.tenantProspectId === linkedProspect?.id);
    const tenantProspect = linkedProspect
        || store.tenantProspects.find((item) => item.teamId === deal.teamId
            && item.id === (recommendation?.tenantProspectId || candidate?.tenantProspectId));
    if (!recommendation && !linkedCandidate && !linkedProspect)
        return undefined;
    return { recommendation, candidate, tenantProspect };
}
export function recordAcquisitionOutcomeFeedback(store, input) {
    const existing = store.acquisitionOutcomeFeedback.find((item) => item.teamId === input.deal.teamId
    ...
    if (!link)
        return { feedback: undefined, created: false };
    ...
    const coverage = prospectId
        ...
        : undefined;
    const run = coverage
        ? store.prospectSearchRuns.find((item) => item.id === coverage.runId
            && item.teamId === input.deal.teamId
            && item.ownerId === input.deal.ownerId)
        : undefined;
    const assessment = prospectId
        ? store.prospectIcpAssessmentSnapshots
            .filter((item) => item.teamId === input.deal.teamId
            && item.ownerId === input.deal.ownerId
            && item.prospectId === prospectId
...
function acquisitionDealIds(store, scope, candidates) {
    const ids = new Set();
    ...
    const prospectIds = new Set(candidates.map((item) => item.tenantProspectId).filter(Boolean));
    ...
    return new Set([...ids].filter((id) => store.deals.some((deal) => deal.id === id
        && deal.teamId === scope.teamId
        && deal.ownerId === scope.ownerId)));
}
function candidateCampaignContext(store, scope, candidate) {
    if (!candidate.tenantProspectId)
        return undefined;
    const event = latestCoverageEvent(store, scope, candidate.tenantProspectId);
    if (!event)
        return undefined;
    const run = store.prospectSearchRuns.find((item) => item.id === event.runId
...
export function prospectPerformance(store, scope) {
    const candidates = store.websiteOpportunities.filter((item) => item.teamId === scope.teamId && item.ownerId === scope.ownerId);
    const candidateIds = new Set(candidates.map((item) => item.id));
    const touchpoints = store.prospectTouchpoints.filter((item) => item.teamId === scope.teamId
        && item.ownerId === scope.ownerId
        && candidateIds.has(item.prospectCandidateId));
    const contactedIds = new Set(touchpoints
        .filter((item) => item.direction === "outbound")
        .map((item) => item.prospectCandidateId));
    const validReplyIds = new Set(touchpoints
    ...
    for (const item of store.tenantProspects) {
        if (item.teamId !== scope.teamId)
            continue;
        if (!candidates.some((candidate) => candidate.tenantProspectId === item.id))
            continue;
        if (item.leadId)
    ...
    const feedback = store.acquisitionOutcomeFeedback.filter((item) => item.teamId === scope.teamId && item.ownerId === scope.ownerId);
    const pendingSuggestions = store.prospectStrategySuggestions.filter((item) => item.teamId === scope.teamId
        && item.ownerId === scope.ownerId
    ...
    for (const candidate of candidates) {
        ...
        row.candidates += 1;
        if (contactedIds.has(candidate.id))
            row.contacted += 1;
        if (validReplyIds.has(candidate.id))
            row.validReplies += 1;
        campaignMap.set(key, row);
    }
    for (const item of feedback) {
        if (!item.campaignId || !item.strategyId)
            continue;
    ...
    for (const item of feedback) {
        for (const providerCode of item.providerCodes) {
            const providerFeedback = feedback.filter((entry) => entry.providerCodes.includes(providerCode));
    ...
    return {
        ...
        campaignBreakdown: [...campaignMap.values()].sort((left, right) => right.candidates - left.candidates
            || left.campaignId.localeCompare(right.campaignId)),
        providerBreakdown: [...providerMap.values()].sort((left, right) => right.outcomes - left.outcomes
            || left.providerCode.localeCompare(right.providerCode))
    };
}
function sampleRange(dates) {
...
function insertSuggestion(store, input, now) {
    const payloadHash = sha256(input);
    const existing = store.prospectStrategySuggestions.find((item) => item.teamId === input.teamId
...
function suggestionInput(scope, context, suggestionType, sampleMetrics, proposedAdjustments, rationale, reasonCodes, range) {
    return {
        ...scope,
        ...context,
        suggestionType,
...
export function generateProspectStrategySuggestions(store, scope, now = new Date().toISOString()) {
    ...
    const touchpoints = store.prospectTouchpoints.filter((item) => item.teamId === scope.teamId
        && item.ownerId === scope.ownerId
        && candidateIds.has(item.prospectCandidateId));
    const contactedIds = new Set(touchpoints.filter((item) => item.direction === "outbound")
        .map((item) => item.prospectCandidateId));
    const validReplyIds = new Set(touchpoints.filter((item) => item.direction === "inbound"
        && VALID_REPLY_CLASSIFICATIONS.has(item.replyClassification || "")).map((item) => item.prospectCandidateId));
    const created = [];
    for (const group of performance.campaignBreakdown) {
        if (group.candidates < 10 || group.contacted < 5)
            continue;
        ...
        const range = sampleRange([
            ...
            ...touchpoints
                .filter((item) => groupCandidates.some((candidate) => candidate.id === item.prospectCandidateId))
                .map((item) => item.occurredAt)
        ]);
        const suggestion = insertSuggestion(store, suggestionInput(scope, group, "refine_targeting_keywords", {
        ...
        }, "已有足够触达样本，但有效回复率偏低，建议人工检查关键词、客户类型和排除条件。", ["minimum_reply_sample_met", "valid_reply_rate_below_10_percent"], range), now);
        if (suggestion)
            created.push(suggestion);
    }
    const feedback = store.acquisitionOutcomeFeedback.filter((item) => item.teamId === scope.teamId
        && item.ownerId === scope.ownerId
        && item.campaignId
        && item.strategyId);
    ...
    for (const outcomes of outcomeGroups.values()) {
        ...
        for (const item of outcomes) {
            for (const providerCode of item.providerCodes) {
                providerRows.set(providerCode, [...(providerRows.get(providerCode) || []), item]);
            }
        }
        ...
        if (comparableProviders.length >= 2) {
            ...
            if (best.winRate - worst.winRate >= 0.25) {
                const commonMetrics = {
                    ...
                    winRateDifferencePoints: Math.round((best.winRate - worst.winRate) * 1000) / 10
                };
                const increase = insertSuggestion(store, suggestionInput(scope, context, "increase_provider_priority", commonMetrics, {
                    providerCode: best.providerCode,
                    direction: "increase",
                    keepManualApproval: true
                }, `${best.providerCode} 的成交率明显高于可比来源，建议人工评估提高其搜索优先级。`, ["provider_minimum_outcomes_met", "provider_win_rate_gap_at_least_25_points"], range), now);
                if (increase)
                    created.push(increase);
                const decrease = insertSuggestion(store, suggestionInput(scope, context, "decrease_provider_priority", commonMetrics, {
                    providerCode: worst.providerCode,
                    direction: "decrease",
                    keepManualApproval: true
                }, `${worst.providerCode} 的成交率明显低于可比来源，建议人工评估降低其搜索优先级。`, ["provider_minimum_outcomes_met", "provider_win_rate_gap_at_least_25_points"], range), now);
                if (decrease)
                    created.push(decrease);
            }
        }
        ...
        if (outcomes.length >= 3 && lost.length >= 2) {
            ...
            if (dominant && dominant[1] >= 2 && dominant[1] / lost.length >= 0.6) {
                const suggestion = insertSuggestion(store, suggestionInput(scope, context, "review_icp_exclusions", {
                    ...
                    keepManualApproval: true
                }, `丢单原因“${dominant[0]}”重复出现，建议人工检查是否应加入 ICP 排除条件。`, ["minimum_closed_sample_met", "dominant_loss_category_repeated"], range), now);
                if (suggestion)
                    created.push(suggestion);
            }
        }
        if (outcomes.length >= 3) {
            const assessed = outcomes.map((item) => ({
                ...
                assessment: store.prospectIcpAssessmentSnapshots.find((assessment) => assessment.id === item.icpAssessmentId
            ...
            if (wonScores.length && lostScores.length && assessed.length >= 3) {
                ...
                if (Math.abs(wonAverage - lostAverage) < 10) {
                    const suggestion = insertSuggestion(store, suggestionInput(scope, context, "review_icp_weights", {
                        ...
                        keepManualApproval: true
                    }, "现有 ICP 分数未能明显区分成交和丢单样本，建议人工复核各维度权重。", ["minimum_assessed_outcomes_met", "icp_score_separation_below_10_points"], range), now);
                    if (suggestion)
                        created.push(suggestion);
                }
            }
...
export function reviewProspectStrategySuggestion(store, input) {
    const suggestion = store.prospectStrategySuggestions.find((item) => item.id === input.suggestionId);
    if (!suggestion
