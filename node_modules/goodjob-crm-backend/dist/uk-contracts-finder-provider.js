...
import { freeProcurementUsage, isExcludedProcurement, procurementSchemaError, procurementSearchTerms, textLimit } from "./procurement-provider-utils.js";
...
function targetsUnitedKingdom(countries) {
    return !countries.length || countries.some((country) => UK_ALIASES.has(country.toLocaleLowerCase()));
}
...
export const UK_CONTRACTS_FINDER_PROVIDER = defineProvider({
    ...
    async search({ query }, _cred, tools) {
        ...
        const response = await tools.http.fetch(`${CONTRACTS_FINDER_BASE_URL}${CONTRACTS_FINDER_PATH}`, {
            method: "POST",
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "UK Contracts Finder");
        let data;
        try {
