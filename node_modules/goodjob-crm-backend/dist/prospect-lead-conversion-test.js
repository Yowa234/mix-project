...
import { convertProspectToLead, ProspectLeadConversionError } from "./prospect-lead-conversion.js";
import { PROSPECT_COVERAGE_MEMORY_CONTRACT, recordProspectCoverage, setTenantProspectDisposition } from "./prospect-coverage-memory.js";
...
const coverageSecret = "prospect-lead-conversion-coverage-test-secret-v1-".repeat(2);
const at = (minute) => new Date(Date.UTC(2026, 6, 15, 8, minute)).toISOString();
...
function addCoverageContext(store, input) {
...
function existingLead(id, teamId, ownerId) {
    return {
        id,
...
function createApprovedFixture(label) {
    sequence += 100;
...
function isConversionError(code) {
    return (error) => error instanceof ProspectLeadConversionError && error.code === code;
}
function testCreateAndIdempotency() {
    ...
    assert.equal(result.coverageEvent.reasonCode, "HUMAN_CONFIRMED_LEAD_CONVERSION");
    ...
    assert.equal(fixture.store.leadSourceEvents.length, 1);
    assert.equal(fixture.store.leadActivities.length, 1);
    assert.throws(() => convertProspectToLead(fixture.store, {
        ...fixture.input,
        remark: "同一个幂等键不能修改请求"
...
function testLinkExistingAndOwnerIsolation() {
    ...
    const lead = existingLead("lead-existing-current-owner", fixture.teamId, fixture.ownerId);
    ...
    inaccessible.store.leads.push(existingLead("lead-existing-other-owner", inaccessible.teamId, inaccessible.otherOwnerId));
    const before = {
    ...
    assert.throws(() => convertProspectToLead(inaccessible.store, {
        ...inaccessible.input,
        mode: "link_existing",
...
function testApprovalAndTenantGuards() {
    ...
    assert.throws(() => convertProspectToLead(missing.store, {
        ...missing.input,
...
console.log("Prospect human-confirmed lead conversion memory tests passed");
