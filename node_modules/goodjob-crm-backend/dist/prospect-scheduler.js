...
import { advanceProspectScheduleBeyond, scheduleOwnerSession, strategyNeedsRecurringCostApproval } from "./prospect-schedules.js";
...
export class ProspectScheduler {
    ...
    async runOnce() {
        ...
        try {
            ...
            for (const id of activeIds) {
                const schedule = this.store.prospectSchedules.find((item) => item.id === id && item.status === "active");
                if (!schedule)
                    continue;
    ...
    invalidReason(schedule) {
        const owner = scheduleOwnerSession(this.store, schedule);
        const campaign = this.store.prospectCampaigns.find((item) => item.id === schedule.campaignId && item.teamId === schedule.teamId);
        const anchorStrategy = this.store.prospectStrategies.find((item) => item.id === schedule.strategyId && item.teamId === schedule.teamId);
        if (!owner)
            return { code: "SCHEDULE_OWNER_INVALID", reason: "负责人账号不存在或已停用" };
    ...
    activeConnectionId(schedule, providerCode) {
        return this.store.providerConnections.find((item) => item.providerId === providerCode
            && item.teamId === schedule.teamId
            && item.ownerId === schedule.ownerId
            && item.scope === "personal"
            && item.status === "active")?.id || `builtin:${providerCode}`;
    }
    strategySourceExhausted(schedule, strategy) {
        if (!strategy.providerPlan.length)
            return true;
        return strategy.providerPlan.every((plan) => {
            const provider = plan.providerId === "ai_search"
                ? null
                : getProvider(plan.providerId);
            const catalog = this.store.providerCatalog.find((item) => item.code.toLocaleLowerCase("en-US") === plan.providerId);
            if (!catalog || (plan.providerId !== "ai_search" && !provider)) {
                return false;
            }
            const position = this.store.prospectStrategySourcePositions.find((item) => item.teamId === schedule.teamId
                && item.ownerId === schedule.ownerId
                ...
                && item.connectionId === this.activeConnectionId(schedule, plan.providerId)
                && item.endpointCode === "company-search"
                && item.adapterVersion === (provider?.adapterVersion || "ai-search-control-v1")
                && item.contractVersion === (provider?.contractVersion || "search_run_control_plane_v1")
                && item.catalogVersion === catalog.version);
            return position?.status === "exhausted";
        });
    }
    eligibleStrategies(schedule) {
    ...
    selectNextStrategy(schedule) {
        ...
        const lastStrategyId = this.store.prospectSearchRuns.find((item) => item.id === schedule.lastRunId
            && item.teamId === schedule.teamId
    ...
    dueReviewBatch(schedule, now) {
        ...
        const fingerprint = createHash("sha256")
            .update(prospects.map((item) => `${item.id}:${item.nextReviewAt}`).join("|"))
            .digest("hex")
            .slice(0, 24);
        return { prospects, fingerprint };
    }
    reviewBatchAlreadyStarted(schedule, fingerprint) {
        ...
        const runIds = new Set(this.store.prospectSearchRuns
            ...
            && item.campaignId === schedule.campaignId)
            .map((item) => item.id));
        return this.store.prospectRunEvents.some((item) => item.teamId === schedule.teamId
            && runIds.has(item.runId)
            && item.eventType === "created"
            && item.reason.includes(marker));
    }
    reviewRetryDeferred(schedule, now) {
    ...
    async updateSchedule(scheduleId, update) {
        return persistMutation(this.store, () => {
            const schedule = this.store.prospectSchedules.find((item) => item.id === scheduleId);
            if (!schedule) {
    ...
    async executeDue(scheduleId, now, reviewBatch) {
        const schedule = this.store.prospectSchedules.find((item) => item.id === scheduleId && item.status === "active");
        if (!schedule)
            return;
        ...
        if (!selected.strategy) {
            await this.updateSchedule(schedule.id, (current) => {
                ...
                current.nextRunAt = advanceProspectScheduleBeyond(plannedAt, current.frequency, current.timezone, now);
            });
            return;
        }
    ...
    async executeReviewDue(scheduleId, now, reviewBatch) {
        const schedule = this.store.prospectSchedules.find((item) => item.id === scheduleId && item.status === "active");
        if (!schedule)
            return;
        const invalid = this.invalidReason(schedule);
        ...
        try {
            const result = await createProspectRun({
                ...
                strategyId: strategy.id,
                ifMatch: prospectStrategyEtag(strategy),
                idempotencyKey: `prospect-review:${schedule.id}:${reviewBatch.fingerprint}`,
                body: {
