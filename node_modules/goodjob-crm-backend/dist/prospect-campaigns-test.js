...
const departingSales = testUser("campaign_departing_sales", "campaign_team_a", "sales");
store.users.push(salesA, salesA2, managerA, adminA, salesB, managerB, superAdmin, departingSales);
...
try {
    ...
    assert.deepEqual([...emptyActivation.json.missingFields].sort(), [
    ...
    assert.equal(versionCreated.json.currentVersion.contentHash.length, 64);
    ...
    const departingCampaignId = departingCampaignCreated.json.campaign.id;
    ...
    assert.equal(terminalOwnerTransferred.json.events.some((item) => item.eventType === "owner_transferred"
        && item.reason === "离职项目交接"), true);
    ...
    assert.equal(store.users.some((item) => item.id === departingSales.id), false);
    ...
    const activeCampaign = store.prospectCampaigns.find((item) => item.id === activeFixture.json.campaign.id);
    ...
    const concurrentId = concurrentFixture.json.campaign.id;
    ...
    assert.deepEqual(concurrentResults.map((item) => item.response.status).sort(), [200, 412]);
    ...
    assert.equal(store.prospectCampaigns.some((item) => item.name === "Must Roll Back"), false);
    ...
    assert.equal(campaignCreate.requestBody.content["application/json"].schema.required[0], "name");
    assert.ok(campaignVersion.parameters.some((item) => item.name === "If-Match" && item.required === true));
    ...
    store.prospectCampaignEvents.splice(0, store.prospectCampaignEvents.length, ...original.events);
