...
const COUNTRY_NAMES = Object.fromEntries(Object.entries(COUNTRY_CODES)
    .filter(([name]) => /^[a-z]/.test(name))
    .map(([name, code]) => [
    code,
    name.replace(/\b\w/g, (character) => character.toUpperCase())
]));
const localizedValueSchema = z.record(z.union([z.string(), z.array(z.string())]));
...
function sourceUrl(notice) {
    const links = notice.links?.html || notice.links?.htmlDirect;
    if (links)
        return links.ENG || links.eng || Object.values(links)[0] || "";
...
function expertQuery(terms, countryCode) {
...
function toLead(notice, query) {
    ...
    if (!buyer || isExcludedProcurement(`${buyer} ${title}`, query))
        return null;
...
export const EU_TED_PROVIDER = defineProvider({
    ...
    async search({ query, cursor }, _cred, tools) {
        ...
        const requestedCountries = query.countries.map((item) => COUNTRY_CODES[item.toLocaleLowerCase()]).filter(Boolean);
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "EU TED");
        let data;
        try {
