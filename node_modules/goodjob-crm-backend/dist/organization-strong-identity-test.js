...
import { PROSPECT_SOURCE_RAW_SCHEMA_VERSION, ProspectSourceRawError, appendProspectSourceRawBatch, prospectProviderRawArtifactHash } from "./prospect-source-raw.js";
...
const organizationIdentitySecret = "organization-strong-identity-secret-".repeat(2);
...
function snapshotArrays(names) {
    return Object.fromEntries(names.map((name) => [
        name,
        structuredClone(store[name])
    ]));
}
function restoreArrays(names, snapshot) {
...
function identityFacts(target = store) {
    return {
        organizations: structuredClone(target.organizations),
        claims: structuredClone(target.organizationIdentityClaims),
        identifiers: structuredClone(target.organizationAcceptedIdentifiers),
        resolutions: structuredClone(target.organizationIdentityResolutions),
...
function createRaw(teamId, ownerId, payload = {}, sourceOverrides = {}, providerCode = "identity.test-provider", endpointCode = "company-identity") {
    sequence += 1;
    const suffix = String(sequence).padStart(4, "0");
    const fetchedAt = new Date(Date.UTC(2026, 6, 14, 2, 0, sequence)).toISOString();
    const sourceRecords = [{
...
function registration(value, jurisdiction = "DE", scheme = "registry-a", subjectRef = "company") {
    return baseClaim({
        kind: "registration_number",
        value,
...
function vat(value, entityType, subjectRef = "company") {
    return baseClaim({
        kind: "vat",
        value,
        normalizedValue: value,
...
function legalName(value, subjectRef = "company") {
    return baseClaim({
        kind: "legal_name",
        value,
        jurisdiction: "DE",
...
function officialDomain(value, subjectRef = "company") {
    return baseClaim({
...
function resolve(raw, claims, overrides = {}, targetStore = store) {
...
function assertIdentityError(operation, code) {
    assert.throws(operation, (error) => error instanceof OrganizationStrongIdentityError
        && error.code === code);
}
const rawSnapshot = snapshotArrays(rawArrayNames);
const identitySnapshot = snapshotArrays(identityArrayNames);
const protectedBusinessSnapshot = snapshotArrays(protectedBusinessArrayNames);
try {
    restoreArrays(rawArrayNames, Object.fromEntries(rawArrayNames.map((name) => [name, []])));
    restoreArrays(identityArrayNames, Object.fromEntries(identityArrayNames.map((name) => [name, []])));
    ...
    const idempotentRaw = createRaw("identity-team-idempotent", "owner-a");
    ...
    assert.equal(firstResolution.createdIdentifiers[0]?.normalizedValue, "5493001KJTIIGC8Y1R12");
    ...
    assert.equal(idempotentReplay.resolution.claimHash, firstResolution.resolution.claimHash);
    assert.deepEqual(identityFacts(), factsAfterFirstResolution);
    const factsBeforeReplayConflict = identityFacts();
    assertIdentityError(() => resolve(idempotentRaw, [
        ...idempotentClaims,
    ...
    const secondOwnerRaw = createRaw("identity-team-idempotent", "owner-b");
    ...
    assert.equal(secondOwnerMatch.resolution.organizationId, firstResolution.resolution.organizationId);
    assert.equal(store.organizations.filter((item) => item.teamId === "identity-team-idempotent").length, 1);
    const crossTeamRaw = createRaw("identity-team-cross", "owner-a");
    const crossTeamResult = resolve(crossTeamRaw, [
        lei("5493001KJTIIGC8Y1R12")
    ]);
    assert.equal(crossTeamResult.resolution.result, "new_entity");
    assert.notEqual(crossTeamResult.resolution.organizationId, firstResolution.resolution.organizationId);
    ...
    assert.notEqual(domainEntityA.resolution.organizationId, domainEntityB.resolution.organizationId);
    const namespaceRawA = createRaw("identity-team-namespace", "owner-a");
    const namespaceRawB = createRaw("identity-team-namespace", "owner-b");
    const namespaceRawC = createRaw("identity-team-namespace", "owner-c");
    ...
    const ambiguousVatRaw = createRaw("identity-team-vat", "owner-a");
    const ambiguousVatResult = resolve(ambiguousVatRaw, [
        vat("DE123456789", "vat_group", "vat-group"),
        vat("DE987654321", "branch", "branch")
    ]);
    assert.equal(ambiguousVatResult.resolution.result, "insufficient_identity");
    assert.ok(ambiguousVatResult.claims.every((claim) => claim.classification === "strong_identifier_unverified"));
    const profileMismatchRaw = createRaw("identity-team-profile", "owner-a");
    ...
    assert.equal(profileMismatchResult.resolution.result, "insufficient_identity");
    assert.equal(profileMismatchResult.claims[0]?.classification, "strong_identifier_unverified");
    const invalidLeiRaw = createRaw("identity-team-invalid-lei", "owner-a");
    ...
    const multiSubjectRaw = createRaw("identity-team-multi-subject", "owner-a");
    ...
    assert.equal(multiSubjectResult.resolution.result, "insufficient_identity");
    assert.ok(multiSubjectResult.claims.every((claim) => claim.classification === "strong_identifier_unverified"));
    ...
    assert.equal(appendMatch.createdIdentifiers[0]?.kind, "registration_number");
    assert.equal(appendMatch.createdIdentifiers[0]?.organizationId, appendBase.resolution.organizationId);
    ...
    assert.deepEqual(store.organizations, splitFactsBefore.organizations);
    assert.deepEqual(store.organizationAcceptedIdentifiers, splitFactsBefore.identifiers);
    assert.deepEqual(store.organizationSourceBindings, splitFactsBefore.bindings);
    assert.notEqual(splitEntityA.resolution.organizationId, splitEntityB.resolution.organizationId);
    ...
    assert.equal(store.organizationSourceBindings.filter((item) => item.rawRecordId === parserRaw.recordId).length, 1);
    const moveTargetRaw = createRaw("identity-team-binding", "owner-target");
    const moveSourceRaw = createRaw("identity-team-binding", "owner-source");
    ...
    assert.equal(store.organizationSourceBindings.find((item) => item.id === moveSource.binding?.id)?.organizationId, moveSource.resolution.organizationId);
    assert.notEqual(moveSource.resolution.organizationId, moveTarget.resolution.organizationId);
    assert.deepEqual(store.organizations, moveFactsBefore.organizations);
    assert.deepEqual(store.organizationAcceptedIdentifiers, moveFactsBefore.identifiers);
    assert.deepEqual(store.organizationSourceBindings, moveFactsBefore.bindings);
    const scopeRaw = createRaw("identity-team-scope", "owner-a");
    const factsBeforeScopeFailure = identityFacts();
    assert.throws(() => resolve(scopeRaw, [registration("SCOPE-REG-100")], {
        ownerId: "owner-b"
    }), (error) => error instanceof ProspectSourceRawError
    ...
    const tamperRecord = store.prospectSourceRawRecords.find((item) => item.id === tamperRaw.recordId);
    const originalEnvelope = tamperRecord.encryptedEnvelope;
    tamperRecord.encryptedEnvelope = `${originalEnvelope.slice(0, -1)}${originalEnvelope.endsWith("A") ? "B" : "A"}`;
    assert.throws(() => resolve(tamperRaw, [registration("TAMPER-REG-100")]), (error) => error instanceof ProspectSourceRawError
    ...
    const collisionRawA = createRaw("identity-team-collision", "owner-a");
    const collisionRawB = createRaw("identity-team-collision", "owner-b");
    const collisionSeed = resolve(collisionRawA, [
        registration("COLLISION-REG-100")
    ]);
    const collisionIdentifier = store.organizationAcceptedIdentifiers.find((item) => item.organizationId === collisionSeed.resolution.organizationId);
    ...
    assertIdentityError(() => resolve(collisionRawB, [
        registration("COLLISION-REG-100")
    ...
    const rollbackRaw = createRaw("identity-team-rollback", "owner-a");
    const rollbackEvents = structuredClone(store.organizationIdentityEvents);
    ...
    const rollbackStore = {
        ...
        organizationIdentityClaims: structuredClone(store.organizationIdentityClaims),
        organizationAcceptedIdentifiers: structuredClone(store.organizationAcceptedIdentifiers),
        organizationIdentityResolutions: structuredClone(store.organizationIdentityResolutions),
        organizationSourceBindings: structuredClone(store.organizationSourceBindings),
        organizationIdentityConflicts: structuredClone(store.organizationIdentityConflicts),
        organizationIdentityEvents: rollbackEvents
    };
    const rollbackFactsBefore = identityFacts(rollbackStore);
    assert.throws(() => resolve(rollbackRaw, [registration("ROLLBACK-REG-100")], {}, rollbackStore), /forced organization identity commit failure/u);
    assert.deepEqual(identityFacts(rollbackStore), rollbackFactsBefore);
    assert.deepEqual(snapshotArrays(protectedBusinessArrayNames), protectedBusinessSnapshot);
