...
import { buildProspectVerificationReport, ensureProspectVerificationReport, prospectVerificationReferenceTime } from "./prospect-verification.js";
const createdAt = "2026-07-16T08:00:00.000Z";
function evidence(providerId, sourceLevel, officialWebsite = "") {
    return {
        providerId,
...
assert.equal(l0.checks.find((item) => item.code === "crawler_free_policy")?.status, "passed");
...
assert.equal(prospectVerificationReferenceTime(historical), "2026-07-14T08:00:00.000Z");
ensureProspectVerificationReport(historical);
assert.equal(historical.verificationReport?.generatedAt, "2026-07-14T08:00:00.000Z");
