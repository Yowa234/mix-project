...
if (!address || typeof address === "string")
    throw new Error("Cannot start trade observation list test server");
const baseUrl = `http://127.0.0.1:${address.port}`;
async function request(token, campaignId, query = {}) {
    const url = new URL(`/api/prospect-campaigns/${campaignId}/trade-observations`, baseUrl);
...
try {
    ...
    assert.deepEqual(firstPage.json.observations.map((item) => item.id), ["mto_page_204", "mto_page_203"]);
    const cursorPayload = JSON.parse(Buffer.from(firstPage.json.nextCursor.split(".")[0], "base64url").toString("utf8"));
    ...
    assert.deepEqual(secondPage.json.observations.map((item) => item.id), ["mto_page_202", "mto_page_201"]);
    ...
    const changedObservation = store.marketTradeObservations.find((item) => item.id === "mto_page_100");
    ...
    assert.equal(managerOwn.json.observations.some((item) => item.id.startsWith("mto_page_")), false);
    ...
    assert.deepEqual(monthlyRange.json.observations.map((item) => item.period), ["2024-01", "2023-12"]);
    ...
    const suppressed = allFilters.json.observations.find((item) => item.id === "mto_filter_suppressed");
    ...
    const unknown = allFilters.json.observations.find((item) => item.id === "mto_filter_unknown");
    ...
    assert.equal(unknownWithPeriodType.json.observations.some((item) => item.id === "mto_filter_unknown"), false);
    ...
    const operation = document.paths["/api/prospect-campaigns/{id}/trade-observations"]?.get;
    assert.ok(operation);
    ...
    assert.deepEqual(successSchema.properties.campaignContractMode.enum, ["compat_v1", "formal_v1"]);
    ...
    assert.match(successSchema.properties.observations.items.properties.statusFlags.description, /原样返回持久化的 Provider 状态标记/);
    ...
    assert.throws(() => validateTradeObservationCursorSecurity(), /生产环境必须配置至少 32 位/);
