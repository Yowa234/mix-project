import { z } from "zod";
import { defineProvider, providerHttpStatusError } from "./provider-contract.js";
import { freeProcurementUsage, isExcludedProcurement, positiveCursor, procurementSchemaError, procurementSearchTerms, textLimit } from "./procurement-provider-utils.js";
...
function targetsCountry(countries, aliases) {
    return !countries.length || countries.some((country) => aliases.has(country.toLocaleLowerCase()));
}
function skippedPage(message, label) {
    return {
        records: [],
        rawCount: 0,
        exhausted: true,
...
export const USASPENDING_AWARDS_PROVIDER = defineProvider({
    ...
    async search({ query, cursor }, _cred, tools) {
        ...
        if (!terms.length)
            return skippedPage("USAspending 需要产品或行业关键词", "USAspending 联邦采购");
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "USAspending");
        let data;
        try {
...
function readFindTenderCursor(value) {
    if (!value)
        return null;
    try {
...
export const UK_FIND_A_TENDER_PROVIDER = defineProvider({
    ...
    async search({ query, cursor }, _cred, tools) {
        ...
        const terms = procurementSearchTerms(query);
        if (!terms.length)
            return skippedPage("Find a Tender 需要产品或行业关键词", "英国 Find a Tender");
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "UK Find a Tender");
        let data;
        try {
...
export const SAM_OPPORTUNITIES_PROVIDER = defineProvider({
    ...
    async search({ query, cursor }, cred, tools) {
        ...
        const terms = procurementSearchTerms(query);
        if (!terms.length)
            return skippedPage("SAM.gov 需要产品或行业关键词", "SAM.gov 商机");
        const limit = Math.min(query.limit, 20);
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "SAM.gov");
        let data;
        try {
...
export const BRAZIL_PNCP_PROVIDER = defineProvider({
    ...
    async search({ query, cursor }, _cred, tools) {
        ...
        const terms = procurementSearchTerms(query);
        if (!terms.length)
            return skippedPage("PNCP 需要产品或行业关键词", "巴西 PNCP 采购");
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "Brazil PNCP");
        let data;
        try {
...
export const MEXICO_DENUE_PROVIDER = defineProvider({
    ...
    async search({ query, cursor }, cred, tools) {
        ...
        const terms = procurementSearchTerms(query);
        if (!terms.length)
            return skippedPage("DENUE 需要产品、行业或企业关键词", "墨西哥 DENUE");
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "Mexico DENUE");
        let data;
        try {
...
export const FMCSA_QCMOBILE_PROVIDER = defineProvider({
    ...
    async search({ query }, cred, tools) {
        ...
        const terms = procurementSearchTerms(query);
        if (!terms.length)
            return skippedPage("FMCSA 需要物流、承运商或企业关键词", "美国 FMCSA 承运商");
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "FMCSA QCMobile");
        let data;
        try {
...
export const SINGAPORE_GEBIZ_PROVIDER = defineProvider({
    ...
    async search({ query, cursor }, _cred, tools) {
        ...
        const terms = procurementSearchTerms(query);
        if (!terms.length)
            return skippedPage("GeBIZ 需要产品或行业关键词", "新加坡 GeBIZ");
        const limit = Math.min(query.limit, 20);
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "Singapore GeBIZ");
        let data;
        try {
...
export const PUBLIC_PROCUREMENT_PROVIDERS = [
