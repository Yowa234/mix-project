...
export const FR_COMPANY_SEARCH_PROVIDER = defineProvider({
    ...
    async search({ query, cursor }, _cred, tools) {
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "France Company Search");
        let data;
        try {
        ...
        const records = data.results
            .filter((item) => !isExcludedProcurement(`${item.nom_complet} ${item.activite_principale || ""} ${item.siege?.adresse || ""}`, query))
            .map((item) => {
...
export const ROR_PROVIDER = defineProvider({
    ...
    async search({ query, cursor }, _cred, tools) {
        const terms = procurementSearchTerms(query);
        if (!terms.length)
            return skippedPage("ROR 需要产品、行业或机构关键词", "ROR 全球机构库");
        const page = Math.max(1, positiveCursor(cursor, 1, 100));
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "ROR");
        let data;
        try {
...
export const NPPES_PROVIDER = defineProvider({
    ...
    async search({ query, cursor }, _cred, tools) {
        ...
        const terms = procurementSearchTerms(query);
        if (!terms.length)
            return skippedPage("NPPES 需要医疗产品、行业或机构关键词", "美国 NPPES 医疗机构");
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "NPPES");
        let data;
        try {
        ...
        const records = data.results
            .filter((item) => !isExcludedProcurement(`${item.basic.organization_name} ${item.taxonomies.map((taxonomy) => taxonomy.desc || "").join(" ")}`, query))
            .map((item) => {
...
export const OPENFDA_510K_PROVIDER = defineProvider({
    ...
    async search({ query, cursor }, _cred, tools) {
        const terms = procurementSearchTerms(query);
        if (!terms.length)
            return skippedPage("FDA 510(k) 需要医疗器械产品关键词", "openFDA 510(k)");
        const limit = Math.min(query.limit, 20);
        const skip = positiveCursor(cursor, 0, 1_000);
        ...
        if (!response.ok)
            throw providerHttpStatusError(response, "openFDA 510(k)");
        let data;
        try {
...
export const PUBLIC_COMPANY_PROVIDERS = [
