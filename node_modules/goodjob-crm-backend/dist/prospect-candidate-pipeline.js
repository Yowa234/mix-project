...
import { ORGANIZATION_STRONG_IDENTITY_CONTRACT, resolveOrganizationStrongIdentity } from "./organization-strong-identity.js";
import { PROSPECT_COVERAGE_MEMORY_CONTRACT, recordProspectCoverage } from "./prospect-coverage-memory.js";
import { ProspectSourceRawError, readProspectSourceRawRecord } from "./prospect-source-raw.js";
...
function providerRecord(payload) {
    if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
        throw new ProspectCandidatePipelineError("CANDIDATE_PAYLOAD_INVALID", "Provider 原始结果不是公司候选记录");
    }
    ...
    if (!text(record.company, 200)
        ...
        throw new ProspectCandidatePipelineError("CANDIDATE_PAYLOAD_INVALID", "Provider 原始结果缺少候选必需字段");
    }
    if (!Number.isFinite(new Date(record.fetchedAt).getTime())
        || !/^[a-f0-9]{64}$/iu.test(record.payloadHash)) {
        throw new ProspectCandidatePipelineError("CANDIDATE_PAYLOAD_INVALID", "Provider 原始结果时间或摘要无效");
    }
    return record;
}
function evidenceSnapshot(providerCode, record) {
    return {
        providerId: providerCode,
        providerRecordId: text(record.providerRecordId, 255),
        officialWebsite: normalizeWebsite(text(record.officialWebsite || record.website, 255)),
...
function hasManualState(opportunity) {
...
function candidateFromRecord(store, hit, providerCode, record) {
    const catalog = store.providerCatalog.find((item) => item.code === providerCode);
    const website = normalizeWebsite(text(record.officialWebsite || record.website, 255));
...
function mergeCandidate(store, incoming) {
    const existing = store.websiteOpportunities.find((item) => sameCandidate(item, incoming)
        || Boolean(incoming.organizationId
            && item.organizationId === incoming.organizationId
            && item.teamId === incoming.teamId
            && item.ownerId === incoming.ownerId));
    if (!existing) {
        if (store.websiteOpportunities.some((item) => item.id === incoming.id)) {
            throw new ProspectCandidatePipelineError("CANDIDATE_ID_CONFLICT", "候选确定性标识已被其它记录占用");
        }
        store.websiteOpportunities.unshift(incoming);
        return { candidate: incoming, created: true };
    }
    const sourceEvidence = mergeEvidence(existing.sourceEvidence, incoming.sourceEvidence);
...
function existingCandidate(store, incoming) {
    return store.websiteOpportunities.find((item) => sameCandidate(item, incoming)
        || Boolean(incoming.organizationId
            && item.organizationId === incoming.organizationId
            && item.teamId === incoming.teamId
            && item.ownerId === incoming.ownerId));
}
function applyCoverageState(candidate, coverage) {
    ...
    withProspectVerificationReport(candidate, coverage.prospect.updatedAt);
}
function failureCode(error) {
    ...
    if (typeof error === "object"
        && error !== null
        && "code" in error) {
        return String(error.code || "UNCLASSIFIED");
    }
    return "UNCLASSIFIED";
}
function candidateMutation(store, operation) {
    ...
    const rollback = () => {
        store.websiteOpportunities.splice(0, store.websiteOpportunities.length, ...structuredClone(previousCandidates));
        processingStates.splice(0, processingStates.length, ...structuredClone(previousProcessingStates));
    };
    try {
        return {
            value: operation(),
...
export class ProspectCandidatePipeline {
    ...
    async processPending(filter = {}) {
        const terminalHitIds = new Set((this.store.prospectCandidateProcessingStates || [])
            .map((item) => item.hitId));
        const hits = this.store.prospectSourceRawHits
            .filter((hit) => !terminalHitIds.has(hit.id)
        ...
        for (const hit of hits) {
            try {
                ...
                if (code === "PROSPECT_SOURCE_RAW_ENVELOPE_INVALID"
                    ...
                    try {
                        await this.persistCandidateTerminal(hit, "rejected", code, () => ({
                            created: false,
                            updated: false,
                            suppressed: false
                        }));
    ...
    async processHit(hit) {
        ...
        const incoming = candidateFromRecord(this.store, hit, providerCode, record);
        ...
        if (providerCode === "gleif"
            ...
            const identity = await this.resolveGleifIdentity(hit, raw.record.id, record);
    ...
    async persistCandidateMerge(hit, operation) {
        return this.persistCandidateTerminal(hit, "completed", "", operation);
    }
    async persistCandidateTerminal(hit, status, failure, operation) {
        const mutation = () => candidateMutation(this.store, () => {
            const processingStates = this.store.prospectCandidateProcessingStates ||= [];
            const existing = processingStates.find((item) => item.hitId === hit.id);
            if (existing) {
                if (existing.teamId !== hit.teamId
                    || existing.ownerId !== hit.ownerId
                    || existing.runId !== hit.runId
                    || existing.ledgerId !== hit.ledgerId) {
                    throw new ProspectCandidatePipelineError("CANDIDATE_PROCESSING_SCOPE_CONFLICT", "候选处理状态与原始命中隔离范围冲突");
                }
                return {
                    created: false,
                    updated: false,
    ...
    async resolveGleifIdentity(hit, rawRecordId, record) {
    ...
    async recordCoverage(hit, identity, record) {
