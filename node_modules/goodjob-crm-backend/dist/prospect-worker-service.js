...
import { ProspectQueueCoordinator } from "./prospect-queue-coordinator.js";
import { ProspectWorker } from "./prospect-worker.js";
const DEVELOPMENT_CLAIM_SECRET = randomBytes(48).toString("base64url");
function executionClaimSecret() {
    const configured = process.env.PROSPECT_EXECUTION_CLAIM_SECRET?.trim()
        || process.env.JWT_SECRET?.trim();
    if (configured)
        return configured;
    if (process.env.NODE_ENV === "production") {
        throw new Error("生产环境必须配置 PROSPECT_EXECUTION_CLAIM_SECRET 或 JWT_SECRET");
    }
...
export class ProspectWorkerService {
    ...
    constructor(options) {
        ...
        this.worker = new ProspectWorker({
            ...
            providerRawEnvelopeSecret: process.env.PROSPECT_SOURCE_RAW_ENVELOPE_SECRET,
            organizationIdentitySecret: process.env.ORGANIZATION_IDENTITY_MASTER_SECRET,
            prospectCoverageSecret: process.env.PROSPECT_COVERAGE_MASTER_SECRET
    ...
    async start() {
        ...
        if (this.queueRequired && !this.redisUrl) {
            throw new Error("PROSPECT_QUEUE_REQUIRED=true 时必须配置 REDIS_URL");
        }
        ...
        try {
            const coordinator = new ProspectQueueCoordinator({
                ...
                backend: new BullMqProspectQueueBackend({
                    ...
                    connectionTimeoutMs: Number(process.env.PROSPECT_REDIS_CONNECT_TIMEOUT_MS || 3_000)
                }),
                onWake: () => this.worker.wakeNow(),
                syncIntervalMs: Number(process.env.PROSPECT_QUEUE_SYNC_MS || 5_000)
            });
            await coordinator.start();
            this.coordinator = coordinator;
            console.log("GoodJob CRM prospect queue coordination enabled with BullMQ");
        }
        catch (error) {
            ...
            console.warn("GoodJob CRM Redis/BullMQ unavailable, "
