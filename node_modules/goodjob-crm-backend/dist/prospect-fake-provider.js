...
import { prospectProviderResponseComponentHashes, prospectProviderResponseHash, sha256CanonicalJson } from "./prospect-provider-request-ledger.js";
import { PROSPECT_SOURCE_RAW_SCHEMA_VERSION, normalizeProspectProviderSourceRecords, prospectProviderRawArtifactHash } from "./prospect-source-raw.js";
...
function resultFromScriptStep(step) {
    const { fakeFaultAfter: _fakeFaultAfter, fakeDelayMs: _fakeDelayMs, ...result } = step;
    ...
    const sourceRecords = normalizeProspectProviderSourceRecords(providerResult.sourceRecords, providerResult.rawCount);
...
function validateStep(step) {
    ...
    if (result.cost.amount !== null)
        normalizedNonnegative(result.cost.amount);
...
function validateDispatchRequest(request) {
    ...
    if (required.some((item) => !item.trim())) {
        throw new FakeProspectProviderError("FAKE_PROVIDER_REQUEST_INVALID", "Fake Provider 请求作用域不完整");
    }
    if (!/^[a-f0-9]{64}$/.test(request.requestHash)
        || !/^[a-f0-9]{64}$/.test(request.idempotencyKey)) {
        throw new FakeProspectProviderError("FAKE_PROVIDER_REQUEST_INVALID", "Fake Provider 请求哈希或幂等键无效");
    }
}
function responseStatus(step) {
    if (step.kind === "success")
        return 200;
    return step.retryable ? 503 : 400;
}
function fakeInterruptionCode(faultAfter) {
...
export class DeterministicFakeProspectProvider {
    scripts;
    legacyOffsets = new Map();
    dispatchOffsets = new Map();
    responsesByKey = new Map();
    responsesByExternalId = new Map();
    physicalCalls = [];
    invocationNo = 0;
    constructor(scripts) {
        ...
        if (!entries.length)
            throw new Error("Fake Provider 必须提供脚本");
    ...
    externalScopeKey(scope, externalRequestId) {
        return [
            scope.teamId,
            scope.ownerId,
            scope.providerCode,
    ...
    async dispatch(request) {
        ...
        if (!steps) {
            throw new FakeProspectProviderError("FAKE_PROVIDER_SCRIPT_NOT_FOUND", "Fake Provider 未配置该数据源脚本");
        }
        ...
        if (existing) {
            if (existing.request.requestHash !== request.requestHash) {
                ...
                throw new FakeProspectProviderError("IDEMPOTENCY_KEY_REQUEST_MISMATCH", "同一幂等键不能绑定不同请求");
            }
            this.recordCall({
                operation: "dispatch",
                teamId: request.teamId,
        ...
        if (!scripted) {
            throw new FakeProspectProviderError("FAKE_PROVIDER_SCRIPT_EXHAUSTED", "Fake Provider 脚本步骤已耗尽");
        }
        this.dispatchOffsets.set(scriptScopeKey, offset + 1);
        const step = resultFromScriptStep(scripted);
        const externalRequestId = `fake_req_${createHash("sha256")
            .update([
            requestScopeKey,
            request.requestHash
        ].join("\u001f"))
            .digest("hex")
            .slice(0, 32)}`;
        const { rawResponseHash, normalizedResultHash, accountingEvidenceHash } = prospectProviderResponseComponentHashes(step);
        ...
        this.responsesByExternalId.set(this.externalScopeKey(request, externalRequestId), stored);
        this.recordCall({
            operation: "dispatch",
            teamId: request.teamId,
            ownerId: request.ownerId,
        ...
        if (scripted.fakeDelayMs) {
            await new Promise((resolve) => setTimeout(resolve, scripted.fakeDelayMs));
        }
        if (scripted.fakeFaultAfter) {
            throw new FakeProspectProviderError(fakeInterruptionCode(scripted.fakeFaultAfter), "Fake Provider 已记录请求后模拟中断");
        }
    ...
    async getByIdempotencyKey(input) {
        ...
        if (!found) {
            throw new FakeProspectProviderError("FAKE_PROVIDER_REQUEST_NOT_FOUND", "Fake Provider 未找到该作用域内的请求");
        }
        return this.cloneResponse(stored, true);
    }
    async getByExternalRequestId(input) {
        const stored = this.responsesByExternalId.get(this.externalScopeKey(input, input.externalRequestId));
        this.recordCall({
    ...
    async search(request) {
        const steps = this.scripts.get(request.providerCode);
        if (!steps)
            throw new Error("Fake Provider 未配置该数据源脚本");
        const key = [
            request.runId,
            request.shardId,
            request.providerCode
        ...
        const scripted = steps[offset];
        if (!scripted)
            throw new Error("Fake Provider 脚本步骤已耗尽");
